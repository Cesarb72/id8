...
function normalizeIntent(input2) {
  if (!input2.primaryVibe) {
    throw new Error("Primary vibe is required before generating a plan.");
  }
  const crew = mapPersonaToCrew(input2.persona, input2.primaryVibe);
  const refinementModes = Array.from(new Set(input2.refinementModes ?? []));
  const secondaryAnchors = input2.secondaryVibe ? [input2.secondaryVibe] : void 0;
  const normalizedAnchor = input2.anchor?.venueId ? {
    venueId: input2.anchor.venueId,
    role: input2.anchor.role ?? "highlight"
  } : void 0;
  const discoveryPreferenceSource = [
    ...input2.discoveryPreferences ?? [],
  ...
  const autoHiddenGemPreference = refinementModes.includes("more-unique") || input2.primaryVibe === "adventurous-urban" || input2.mode === "surprise";
  const planningMode = normalizedAnchor?.venueId ? "user-led" : input2.planningMode ?? "engine-led";
  return {
    crew,
    persona: input2.persona,
    personaSource: input2.persona ? "explicit" : "derived",
    primaryAnchor: input2.primaryVibe,
    secondaryAnchors,
    city: input2.city.trim() || "San Jose",
    district: input2.district?.trim() || void 0,
    neighborhood: input2.neighborhood?.trim() || void 0,
    distanceMode: input2.distanceMode,
    budget: input2.budget,
    timeWindow: input2.timeWindow ?? input2.startTime,
    prefersHiddenGems: input2.prefersHiddenGems ?? autoHiddenGemPreference,
    refinementModes,
    mode: input2.mode ?? "build",
    planningMode,
    anchor: normalizedAnchor,
    discoveryPreferences,
    selectedDirectionContext: input2.selectedDirectionContext
  };
}
...
function resolveHospitalityContract(intent2) {
  const persona = intent2.personaSource === "explicit" ? intent2.persona : void 0;
  if (persona === "romantic") {
    return buildRomanticContract(intent2.primaryAnchor);
  }
  if (persona === "friends") {
    return buildFriendsContract(intent2.primaryAnchor);
  }
  if (persona === "family") {
    return buildFamilyContract(intent2.primaryAnchor);
  }
  return buildVibeOnlyContract(intent2.primaryAnchor);
}
...
var tasteModes = {
  "cozy-flow": {
    ...
    reasonBuilder: (intent2) => `Primary vibe is ${intent2.primaryAnchor}, so Taste keeps the night low-friction and close to conversation.`
  },
  "highlight-centered": {
    ...
    reasonBuilder: (intent2) => intent2.refinementModes?.includes("more-exciting") ? "Interpretation asked for more excitement, so Taste centers the route on a stronger peak moment." : `Primary vibe is ${intent2.primaryAnchor}, so Taste gives one standout stop more weight than the connectors.`
  },
  "activity-led": {
    id: "activity-led",
    label: "Activity-Led",
  ...
  "wander-explore": {
    ...
    reasonBuilder: (intent2) => intent2.mode === "surprise" ? "Surprise mode is active, so Taste opens the route to more local and underexposed options." : `Primary vibe is ${intent2.primaryAnchor}, so Taste pushes away from obvious defaults and toward local pockets.`
  },
  "social-night": {
    id: "social-night",
    label: "Social Night",
    ...
    reasonBuilder: (intent2) => `Crew and vibe read as more social, so Taste favors nights with stronger group energy and lively anchors.`
  },
  "scenic-outdoor": {
    id: "scenic-outdoor",
...
function hasRefinement(intent2, value) {
  return intent2.refinementModes?.includes(value) ?? false;
}
function getTasteModeId(intent2) {
  if (intent2.primaryAnchor === "adventurous-outdoor") {
    return "scenic-outdoor";
  }
  if (intent2.primaryAnchor === "playful") {
    return "activity-led";
  }
  if (intent2.primaryAnchor === "adventurous-urban") {
    return "wander-explore";
  }
  if (intent2.mode === "surprise" || hasRefinement(intent2, "more-unique")) {
    return intent2.primaryAnchor === "cozy" || intent2.primaryAnchor === "chill" ? "cozy-flow" : "wander-explore";
  }
  if (intent2.primaryAnchor === "cozy" || intent2.primaryAnchor === "chill") {
    return hasRefinement(intent2, "more-exciting") ? "highlight-centered" : "cozy-flow";
  }
  if (intent2.primaryAnchor === "cultured") {
    return hasRefinement(intent2, "more-exciting") ? "highlight-centered" : "highlight-centered";
  }
  if (intent2.primaryAnchor === "lively") {
    return intent2.crew === "socialite" ? "social-night" : "highlight-centered";
  }
  if (intent2.crew === "socialite") {
    return "social-night";
  }
  return "highlight-centered";
}
function selectTasteMode(intent2) {
  const blueprint = tasteModes[getTasteModeId(intent2)];
  return {
    ...blueprint,
    reason: blueprint.reasonBuilder(intent2)
  };
}
function unique2(values) {
  return [...new Set(values)];
}
function applyTasteModeToLens(lens2, tasteMode) {
  const nextLens = {
    ...lens2,
    tasteMode,
    energyBand: unique2([...lens2.energyBand, ...tasteMode.energyBand]),
    preferredCategories: unique2([...lens2.preferredCategories, ...tasteMode.favoredCategories]),
    discouragedCategories: unique2([
      ...lens2.discouragedCategories,
      ...tasteMode.discouragedCategories
    ]),
    preferredTags: unique2([...lens2.preferredTags, ...tasteMode.favoredTags]),
    discouragedTags: unique2([...lens2.discouragedTags, ...tasteMode.discouragedTags]),
    movementTolerance: tasteMode.movementTolerance,
    discoveryBias: tasteMode.discoveryBias,
    wildcardAggressiveness: Math.max(
      lens2.wildcardAggressiveness,
      tasteMode.wildcardAggressivenessFloor
    ),
    preferredStopShapes: { ...lens2.preferredStopShapes },
    windDownExpectation: {
      ...lens2.windDownExpectation,
      preferredCategories: unique2([
        ...lens2.windDownExpectation.preferredCategories,
        ...tasteMode.stopShapePatches.windDown?.preferredCategories ?? []
      ]),
      discouragedCategories: unique2([
        ...lens2.windDownExpectation.discouragedCategories,
...
function mergeStopShape(base, patch) {
  return {
    ...
    preferredTags: unique3([...base.preferredTags ?? [], ...patch.preferredTags ?? []]),
...
function applyVibeShaping(lens2, intent2) {
  const anchors = [intent2.primaryAnchor, ...intent2.secondaryAnchors ?? []];
  const next = {
    ...lens2,
    preferredCategories: [...lens2.preferredCategories],
    discouragedCategories: [...lens2.discouragedCategories],
    preferredTags: [...lens2.preferredTags],
    discouragedTags: [...lens2.discouragedTags],
    preferredStopShapes: { ...lens2.preferredStopShapes },
    windDownExpectation: { ...lens2.windDownExpectation }
  };
  for (const [index, anchor] of anchors.entries()) {
    const profile = getVibeProfile(anchor);
    next.preferredCategories = unique3([
      ...next.preferredCategories,
      ...profile.preferredCategories
    ]);
    next.discouragedCategories = unique3([
      ...next.discouragedCategories,
      ...profile.discouragedCategories
    ]);
    next.preferredTags = unique3([...next.preferredTags, ...profile.preferredTags]);
    next.discouragedTags = unique3([...next.discouragedTags, ...profile.discouragedTags]);
    next.energyBand = unique3([...next.energyBand, ...profile.energyBand]);
    ...
    next.windDownExpectation.preferredCategories = unique3([
      ...next.windDownExpectation.preferredCategories,
      ...profile.windDown.preferredCategories
    ]);
    next.windDownExpectation.discouragedCategories = unique3([
  ...
  if (intent2.primaryAnchor === "adventurous-outdoor") {
    ...
    next.preferredCategories = unique3([...next.preferredCategories, "park", "cafe", "dessert"]);
    next.discouragedCategories = unique3([...next.discouragedCategories, "museum", "live_music"]);
    next.preferredTags = unique3([
      ...next.preferredTags,
      "trail",
      "viewpoint",
      "garden",
  ...
  if (intent2.primaryAnchor === "adventurous-urban") {
    ...
    next.discouragedCategories = unique3([...next.discouragedCategories, "park"]);
    next.preferredTags = unique3([
      ...next.preferredTags,
      "district",
      "street-food",
    ...
    next.discouragedTags = unique3([...next.discouragedTags, "nature", "trail", "viewpoint"]);
...
function applyResolvedContractShaping(lens2, resolvedContract) {
  const next = {
    ...lens2,
    energyBand: [...lens2.energyBand],
    preferredCategories: [...lens2.preferredCategories],
    discouragedCategories: [...lens2.discouragedCategories],
    preferredTags: [...lens2.preferredTags],
    discouragedTags: [...lens2.discouragedTags],
  ...
  next.preferredTags = unique3([...next.preferredTags, ...resolvedContract.preferredTags]);
  next.discouragedTags = unique3([...next.discouragedTags, ...resolvedContract.discouragedTags]);
  next.energyBand = unique3(
...
function applyModeShaping(lens2, intent2) {
  const next = { ...lens2 };
  const refinements = new Set(intent2.refinementModes ?? []);
  if (refinements.has("more-relaxed")) {
    next.energyBand = unique3([...next.energyBand, "low"]);
    next.movementTolerance = "low";
  }
  if (refinements.has("more-exciting")) {
    next.energyBand = unique3([...next.energyBand, "high"]);
  ...
  if (refinements.has("little-fancier")) {
    ...
    next.preferredTags = unique3([...next.preferredTags, "elevated", "chef-led", "craft"]);
  }
  if (intent2.mode === "surprise") {
    return {
      ...next,
      discoveryBias: "high",
      wildcardAggressiveness: Math.max(next.wildcardAggressiveness, 0.78),
      repetitionTolerance: "medium",
      preferredTags: unique3([...next.preferredTags, "underexposed", "unexpected"])
    };
  }
  if (intent2.mode === "curate") {
    return {
      ...next,
...
function applyStarterPackShaping(lens2, starterPack) {
  ...
  if (preset.energyBand?.length) {
    next.energyBand = unique3([...preset.energyBand]);
  }
  ...
  if (preset.preferredTags?.length) {
    next.preferredTags = unique3([...next.preferredTags, ...preset.preferredTags]);
  }
  if (preset.discouragedTags?.length) {
    next.discouragedTags = unique3([...next.discouragedTags, ...preset.discouragedTags]);
  }
...
function buildPersonaEffectSummary(intent2) {
  if (!intent2.persona || intent2.personaSource !== "explicit") {
    return "No persona modifier applied.";
  }
  if (intent2.persona === "romantic") {
    return "Refined toward intimate pacing and calmer wind-down choices.";
  }
  if (intent2.persona === "friends") {
...
function applyStrictDebugShaping(lens2, intent2, starterPack) {
  const next = {
    ...lens2,
    energyBand: [...lens2.energyBand],
    preferredCategories: [...lens2.preferredCategories],
  ...
  if (intent2.personaSource === "explicit" && intent2.crew === "curator") {
    next.energyBand = unique3(next.energyBand.filter((band) => band !== "high"));
    next.movementTolerance = "low";
    next.discouragedCategories = unique3([
      ...next.discouragedCategories,
      "bar",
  ...
  if (intent2.personaSource === "explicit" && intent2.crew === "romantic") {
    next.tone = "intimate";
    next.movementTolerance = "low";
    next.energyBand = unique3(next.energyBand.filter((band) => band !== "high"));
  ...
  return {
    ...next,
    preferredCategories: unique3(next.preferredCategories),
    discouragedCategories: unique3(next.discouragedCategories),
    preferredTags: unique3(next.preferredTags),
    discouragedTags: unique3(next.discouragedTags),
    energyBand: unique3(next.energyBand.length > 0 ? next.energyBand : ["low", "medium"])
  };
}
function buildExperienceLens({
  ...
  const resolvedContractPackage = resolveHospitalityContract(intent2);
  const base = buildNeutralBaseline();
  const afterVibe = applyVibeShaping(base, intent2);
  const afterPersona = applyResolvedContractShaping(
    afterVibe,
    resolvedContractPackage.resolvedContract
  );
  const afterMode = applyModeShaping(afterPersona, intent2);
  const tasteMode = selectTasteMode(intent2);
  ...
  const strictShaped = strictShape ? applyStrictDebugShaping(afterPack, intent2, starterPack) : afterPack;
  return {
    ...strictShaped,
    tasteMode,
    ...
    interpretation: {
      primaryVibe: intent2.primaryAnchor,
      personaModifier: intent2.persona ?? void 0,
      personaSource: intent2.personaSource ?? "derived",
      personaEffectSummary: resolvedContractPackage.resolvedContract.persona ? resolvedContractPackage.resolvedContract.resolutionSummary : buildPersonaEffectSummary(intent2)
    },
    preferredCategories: unique3(strictShaped.preferredCategories),
    discouragedCategories: unique3(strictShaped.discouragedCategories),
    preferredTags: unique3(strictShaped.preferredTags),
    discouragedTags: unique3(strictShaped.discouragedTags),
    energyBand: unique3(
...
function mergeRule(base, patch) {
  return {
    ...
    requiredCategories: unique4([...base.requiredCategories ?? [], ...patch.requiredCategories ?? []]),
    preferredCategories: unique4([...base.preferredCategories ?? [], ...patch.preferredCategories ?? []]),
    discouragedCategories: unique4([...base.discouragedCategories ?? [], ...patch.discouragedCategories ?? []]),
    requiredTags: unique4([...base.requiredTags ?? [], ...patch.requiredTags ?? []]),
...
function getRoleContract({
  ...
  const merged = {
    sourceLabels: [intent2.crew, starterPack?.id].filter(Boolean),
    personaContract: resolvedContractPackage.personaContract,
    resolvedContract: resolvedContractPackage.resolvedContract,
    byRole: {
...
function scoreLensCompatibility(venue, intent2, lens2) {
  const categoryPreferred = lens2.preferredCategories.includes(venue.category) ? 1 : 0.42;
  const categoryPenalty = lens2.discouragedCategories.includes(venue.category) ? 0.24 : 0;
  const preferredTags = hasTagOverlap(venue.tags, lens2.preferredTags);
  const discouragedTags = hasTagOverlap(venue.tags, lens2.discouragedTags);
  const energyBand = mapEnergyLevelToBand(venue.energyLevel);
  const energyFit = lens2.energyBand.includes(energyBand) ? 1 : 0.45;
  const movementPressure = lens2.movementTolerance === "low" ? Math.max(0, (venue.driveMinutes - 12) / 20) : 0;
  const discoveryPreference = lens2.discoveryBias === "high" ? venue.underexposureScore * 0.12 + venue.distinctivenessScore * 0.08 : lens2.discoveryBias === "low" ? -venue.underexposureScore * 0.06 : 0;
  const neighborhoodBias = intent2.neighborhood && intent2.neighborhood.toLowerCase() === venue.neighborhood.toLowerCase() ? 0.08 : 0;
  return clamp01(
...
function scoreLensStopShapeCompatibility(venue, lens2, role) {
  const shape = lens2.preferredStopShapes[role];
...
function applyLensToVenue(venue, intent2, lens2) {
  return {
    venue,
    lensCompatibility: scoreLensCompatibility(venue, intent2, lens2)
  };
}
...
function getTimeWindowSignal(intent2) {
  const now = /* @__PURE__ */ new Date();
  const parsedHour = parseHourFromTimeWindow(intent2.timeWindow);
  ...
  return {
    ...
    minute,
    phase,
    label: parsedHour === void 0 ? `${phase} now` : intent2.timeWindow?.trim() || phase,
...
function getPersonaTerms(intent2) {
  if (intent2.crew === "romantic") {
...
function getVibeTerms(intent2) {
  const primary = intent2.primaryAnchor;
  if (primary === "cozy") {
...
function buildLocationLabel(intent2) {
  return intent2.neighborhood ? `${intent2.neighborhood}, ${intent2.city}` : intent2.city;
}
...
function buildLiveQueryPlan(intent2, starterPack) {
  const locationLabel = buildLocationLabel(intent2);
  const timeSignal = getTimeWindowSignal(intent2);
  const personaTerms = getPersonaTerms(intent2);
  const vibeTerms = getVibeTerms(intent2);
  const starterPackTerms = pickStarterPackTerms(starterPack);
  const dateOrSocialTerms = intent2.crew === "romantic" ? ["wine", "dessert"] : intent2.crew === "socialite" ? ["cocktail", "group friendly"] : ["coffee", "daytime"];
  const startKind = timeSignal.phase === "morning" || timeSignal.phase === "afternoon" ? "cafe" : "restaurant";
  const highlightKind = timeSignal.phase === "late-night" || intent2.primaryAnchor === "lively" || intent2.crew === "socialite" ? "bar" : "restaurant";
  const windDownKind = timeSignal.phase === "late-night" ? "bar" : "cafe";
  const cultureKind = intent2.primaryAnchor === "cultured" || intent2.crew === "curator" ? "museum" : "activity";
  const strollKind = intent2.primaryAnchor === "cozy" || intent2.primaryAnchor === "chill" ? "park" : "activity";
  const plan = [
    {
      kind: startKind,
      roleHint: "start",
...
function formatLocationLabel(intent2) {
  return intent2.neighborhood ? `${intent2.neighborhood}, ${intent2.city}` : intent2.city;
}
...
function normalizeRawPlaces(rawPlaces, intent2) {
  const timeWindowSignal = getTimeWindowSignal(intent2);
  const venues = [];
  let droppedCount = 0;
...
async function fetchLivePlaces(intent2, starterPack) {
  const config = getGooglePlacesConfig();
  const queryLocationLabel = formatLocationLabel(intent2);
  const baseQueryPlan = buildLiveQueryPlan(intent2, starterPack);
  const queryCenters = deriveQueryCenters(intent2.city, config.maxCenters, config.centerOffsetM);
  ...
  for (const result of settled) {
    ...
    places.forEach((place, index) => {
      const mapped = mapLivePlaceToRawPlaceWithDiagnostics(place, {
        city: intent2.city,
        neighborhood: intent2.neighborhood,
  ...
  const normalized = normalizeRawPlaces(rawPlaces, intent2);
...
function summarizeGeoSpread(venues) {
  ...
  const dominantAreaShare = total2 > 0 ? Number((dominantCount / total2).toFixed(3)) : 0;
...
function applyGeoDiversityShaping(venues) {
  ...
  const total2 = venues.length;
  const dominantCap = Math.max(6, Math.ceil(total2 * 0.55));
...
async function fetchHybridPortableVenues(city) {
  ...
  const liveResults = await Promise.all(liveIntents.map((intent2) => fetchLivePlaces(intent2)));
...
function isMeaningfulMomentStretchCandidate(candidate, intent2) {
  if (!isOutsideStrictNearbyButWithinBoundedStretch(
    candidate.venue.driveMinutes,
    intent2.distanceMode
  )) {
    return false;
  }
...
function isCandidateWithinActiveDistanceWindow(candidate, intent2, options = {}) {
  if (intent2.distanceMode !== "nearby") {
  ...
  if (status === "inside_bounded_stretch" && options.allowMeaningfulStretch) {
    return isMeaningfulMomentStretchCandidate(candidate, intent2);
  }
  return false;
}
...
async function retrieveVenues(intent2, lens2, options = {}) {
  const curatedVenues2 = options.seedVenues ? [...options.seedVenues, ...curatedVenues] : curatedVenues;
  const requestedSourceMode = options.requestedSourceMode ?? "curated";
  const cityQuery = sanitizeCity(intent2.city);
  const curatedCoverageForCity = hasCuratedCityCoverage(curatedVenues2, cityQuery);
  const normalizedNeighborhood = intent2.neighborhood ? sanitize(intent2.neighborhood) : void 0;
  const maxDriveMinutes = intent2.distanceMode === "nearby" ? BOUNDED_NEARBY_STRETCH_DRIVE_MINUTES : 28;
  const liveFetch = requestedSourceMode === "curated" ? {
    venues: [],
    diagnostics: {
      attempted: false,
      provider: "google-places",
      queryLocationLabel: intent2.neighborhood ? `${intent2.neighborhood}, ${intent2.city}` : intent2.city,
      queryCentersCount: 0,
      queryCentersUsed: [],
      queryRadiusM: 0,
  ...
  } : await fetchLivePlaces(intent2, options.starterPack);
  const hybridPortable = requestedSourceMode !== "curated" && cityQuery.length > 0 && !curatedCoverageForCity ? await fetchHybridPortableVenues(intent2.city) : void 0;
  ...
  const shapedCandidates = fallbackMatches.map((venue) => applyLensToVenue(venue, intent2, lens2)).sort((left, right) => right.lensCompatibility - left.lensCompatibility);
  const compatibilityThreshold = lens2.discoveryBias === "high" ? 0.36 : lens2.tone === "refined" ? 0.44 : 0.41;
  ...
  const localFirstLensShapedVenues = intent2.distanceMode === "nearby" ? [
    ...lensShapedVenues.filter(
      (venue) => isWithinStrictNearbyWindow(venue.driveMinutes, intent2.distanceMode)
    ),
    ...lensShapedVenues.filter(
  ...
  if (intent2.distanceMode === "nearby" && neighborhoodMatches.length > 0) {
    return buildResult(
...
function scoreAnchorCategoryPressure(venue, anchor) {
  ...
  return clamp019(categoryFit - categoryPenalty);
}
function scoreAnchorFit(venue, intent2) {
  const anchors = [intent2.primaryAnchor, ...intent2.secondaryAnchors ?? []];
  ...
  return clamp019(weightedSum / totalWeight);
}
...
function scoreCrewFit(venue, crewPolicy2) {
  if (crewPolicy2.blockedCategories.includes(venue.category)) {
    return 0;
  }
  const preferredScore = crewPolicy2.preferredCategories.includes(venue.category) ? 1 : 0.55;
  const discouragedPenalty = crewPolicy2.discouragedCategories.includes(venue.category) ? 0.28 : 0;
  const useCaseScore = venue.useCases.includes(crewPolicy2.crew) ? 1 : 0.6;
  const energyDistance = Math.abs(venue.energyLevel - crewPolicy2.targetEnergy);
  const energyScore = clamp0111(1 - energyDistance / 4);
  const vibeMatchScore = venue.vibeTags.some((tag) => crewPolicy2.preferredVibes?.includes(tag)) ? 1 : 0.58;
  return clamp0111(
...
function scoreHiddenGemFit(venue, fitScore, intent2) {
  ...
  const intentLift = intent2.prefersHiddenGems ? 0.04 : 0;
  return clamp0112(weighted - chainPenalty + localBonus + hiddenGemLift + intentLift);
}
...
function scoreProximityFit(venue, intent2) {
  const neighborhoodQuery = intent2.neighborhood?.trim().toLowerCase();
  const sameNeighborhood = neighborhoodQuery ? venue.neighborhood.toLowerCase() === neighborhoodQuery : false;
  const nearbyCap = 14;
  const shortDriveCap = 30;
  const cap = intent2.distanceMode === "nearby" ? nearbyCap : shortDriveCap;
  const drivePenalty = clamp0113(venue.driveMinutes / cap);
  const driveScore = clamp0113(1 - drivePenalty);
  const neighborhoodBonus = sameNeighborhood ? 1 : 0;
  const neighborhoodPenalty = neighborhoodQuery && !sameNeighborhood ? 0.18 : 0;
  if (intent2.distanceMode === "nearby") {
    const strictDriveScore = clamp0113(1 - venue.driveMinutes / nearbyCap);
    return clamp0113(
      strictDriveScore * 0.62 + neighborhoodBonus * 0.38 - neighborhoodPenalty
    );
  }
  return clamp0113(driveScore * 0.8 + neighborhoodBonus * 0.2 - neighborhoodPenalty * 0.5);
}
...
function scoreContextSpecificity({
  venue,
  intent: intent2,
  crewPolicy: crewPolicy2,
  lens: lens2,
  fitBreakdown,
  stopShapeFit
}) {
  const personaSignal = clamp0114(
    fitBreakdown.crewFit * 0.66 + (venue.useCases.includes(crewPolicy2.crew) ? 0.24 : 0) + (crewPolicy2.preferredCategories.includes(venue.category) ? 0.1 : 0)
  );
  const vibeSignal = clamp0114(
    fitBreakdown.anchorFit * 0.72 + (venueMatchesVibeTag(venue, intent2.primaryAnchor) ? 0.2 : 0) + (intent2.secondaryAnchors?.some((anchor) => venueMatchesVibeTag(venue, anchor)) ? 0.08 : 0)
  );
  const lensSignal = clamp0114(
    (lens2.preferredCategories.includes(venue.category) ? 0.52 : 0.18) + tagOverlapScore2(venue.tags, lens2.preferredTags) * 0.24 + (lens2.discouragedCategories.includes(venue.category) ? -0.22 : 0) + (tagOverlapScore2(venue.tags, lens2.discouragedTags) > 0 ? -0.14 : 0)
  );
  const start = clamp0114(
    personaSignal * 0.26 + vibeSignal * 0.22 + lensSignal * 0.18 + stopShapeFit.start * 0.2 + venue.roleAffinity.warmup * 0.14
  );
  const peak = clamp0114(
    personaSignal * 0.3 + vibeSignal * 0.3 + lensSignal * 0.16 + stopShapeFit.highlight * 0.16 + venue.roleAffinity.peak * 0.08
  );
  const wildcard = clamp0114(
...
function roleAffinityUniversality(venue) {
  ...
  const spreadPenalty = clamp0115(1 - (max - min));
  return clamp0115(mean * 0.58 + spreadPenalty * 0.42);
}
...
function scoreUniquenessFit(venue, intent2) {
  const baseline = venue.uniquenessScore * 0.6 + venue.distinctivenessScore * 0.4;
  const adventurousLift = intent2.primaryAnchor === "adventurous-urban" || intent2.secondaryAnchors?.includes("adventurous-urban") ? 0.06 : 0;
  const chainPenalty = venue.isChain ? 0.08 : 0;
...
function evaluateRoleContract(venue, rule) {
  ...
  const maxEnergyPenalty = typeof rule.maxEnergyLevel === "number" && venue.energyLevel > rule.maxEnergyLevel ? clamp0116((venue.energyLevel - rule.maxEnergyLevel) / 4) : 0;
  if (maxEnergyPenalty > 0) {
    violations.push("energy too high");
  }
  const score = clamp0116(
...
function getHighlightValidityRules(intent2, starterPack) {
  switch (starterPack?.id) {
    ...
    default:
      return {
        literalRequirementType: intent2.primaryAnchor === "adventurous-outdoor" ? "outdoor-anchor" : void 0,
        literalRequirementLabel: intent2.primaryAnchor === "adventurous-outdoor" ? "Highlight should read as an outdoor anchor for this outing." : void 0,
        literalRequirementStrength: intent2.primaryAnchor === "adventurous-outdoor" ? "strong" : void 0,
        dateCentered: intent2.crew === "romantic",
        musicPack: false,
        outdoorPack: intent2.primaryAnchor === "adventurous-outdoor"
      };
  }
}
...
function computeCandidateTier(venue, intent2, starterPack, tags) {
  ...
  if (venue.category === "museum") {
    return intent2.primaryAnchor === "cultured" || starterPack?.id === "coffee-books" || starterPack?.id === "museum-afternoon" || intent2.crew === "curator" ? "highlight-capable" : "support-only";
  }
  if (venue.category === "activity") {
    return intent2.primaryAnchor === "playful" || intent2.crew === "curator" || intent2.primaryAnchor === "adventurous-outdoor" ? "highlight-capable" : "support-only";
  }
  if (venue.category === "park") {
    return isOutdoorAnchorVenue(venue, tags) ? "highlight-capable" : "support-only";
  }
  if (venue.category === "dessert") {
    return starterPack?.id === "dessert-conversation" || starterPack?.id === "cozy-date-night" || starterPack?.id === "sunset-stroll" || intent2.crew === "romantic" && isQuietLingeringVenue(venue, tags) ? "highlight-capable" : "support-only";
  }
  if (venue.category === "cafe") {
    return starterPack?.id === "coffee-books" || starterPack?.id === "dessert-conversation" || intent2.primaryAnchor === "chill" && isThoughtfulVenue(venue, tags) ? "highlight-capable" : "support-only";
  }
...
function evaluateHighlightValidity({
  ...
  const rules = getHighlightValidityRules(intent2, starterPack);
  const tags = normalizedTags2(venue);
  const candidateTier = computeCandidateTier(venue, intent2, starterPack, tags);
...
function isRomanticPersonaContractActive(lens2) {
  return lens2.resolvedContract?.persona === "romantic";
}
function requiresRomanticPersonaMoment(lens2) {
  return Boolean(
    isRomanticPersonaContractActive(lens2) && lens2.resolvedContract?.highlight.requiresMomentPresence
  );
}
function scopeRomanticHighlightCandidatesByMomentTier(candidates, lens2) {
  if (!lens2 || !isRomanticPersonaContractActive(lens2) || candidates.length === 0) {
    return {
      candidates,
      appliedTier: "none",
      fallbackUsed: false
...
function assessRomanticPersonaHighlightQualification(candidate, lens2) {
  const archetype = candidate.taste.signals.primaryExperienceArchetype;
  ...
  const cozyRomanticHighlightMode = lens2?.resolvedContract?.persona === "romantic" && lens2.resolvedContract.primaryVibe === "cozy";
...
function satisfiesRomanticPersonaHighlightContract(candidate, lens2) {
  const assessment = assessRomanticPersonaHighlightQualification(candidate, lens2);
  return Boolean(
    isRomanticPersonaContractActive(lens2) && assessment.highlightType && lens2.resolvedContract?.highlight.preferredHighlightTypes.includes(
...
function deriveSeedCalibratedTasteProfile(venue) {
  ...
  const energy = clamp0117(venue.energyLevel / 5);
  const socialDensity = clamp0117(venue.socialDensity / 5);
  ...
  const conversationFriendliness = clamp0117(
...
function inferRuleProfile(venue, context) {
  ...
  profile.experientialFactor = clamp0118(
  ...
  profile.conversationFriendliness = clamp0118(
  ...
  if (venue.priceLevel === "$$$" || venue.priceLevel === "$$$$") {
    profile.destinationFactor = clamp0118(profile.destinationFactor + 0.08);
    profile.experientialFactor = clamp0118(profile.experientialFactor + 0.06);
  ...
  if (hasHighRatingCue) {
    profile.destinationFactor = clamp0118(profile.destinationFactor + 0.04);
    profile.experientialFactor = clamp0118(profile.experientialFactor + 0.04);
  }
  if (hasHighReviewCue) {
    profile.socialDensity = clamp0118(profile.socialDensity + 0.06);
    profile.destinationFactor = clamp0118(profile.destinationFactor + 0.04);
  }
...
function applyContextTonePressure(profile, venue, context) {
  ...
  const containedSocialCue = clamp0118(
  ...
  const broadSocialCue = clamp0118(
  ...
  if (livelyLike) {
    ...
    nextProfile.energy = clamp0118(nextProfile.energy + livelyLift);
    nextProfile.socialDensity = clamp0118(nextProfile.socialDensity + 0.07 + broadSocialCue * 0.07);
    nextProfile.experientialFactor = clamp0118(nextProfile.experientialFactor + 0.06);
    nextProfile.destinationFactor = clamp0118(
      nextProfile.destinationFactor + 0.03 + energeticCue * 0.03
    );
    if (persona === "romantic") {
      nextProfile.intimacy = clamp0118(nextProfile.intimacy - 0.01 + containedSocialCue * 0.05);
      nextProfile.conversationFriendliness = clamp0118(
      ...
      nextProfile.intimacy = clamp0118(nextProfile.intimacy - 0.06);
      nextProfile.conversationFriendliness = clamp0118(nextProfile.conversationFriendliness - 0.03);
      signals.push("tone:vibe-lively-friends-broad-social-lift");
    } else {
      nextProfile.intimacy = clamp0118(nextProfile.intimacy - 0.03);
      nextProfile.conversationFriendliness = clamp0118(nextProfile.conversationFriendliness - 0.02);
      signals.push("tone:vibe-lively-general-lift");
    }
  } else if (cozyLike) {
    const cozySoftening = 0.09 + calmCue * 0.06;
    nextProfile.energy = clamp0118(nextProfile.energy - cozySoftening);
    nextProfile.socialDensity = clamp0118(nextProfile.socialDensity - 0.08 - broadSocialCue * 0.04);
    nextProfile.intimacy = clamp0118(nextProfile.intimacy + 0.08 + calmCue * 0.06);
    nextProfile.conversationFriendliness = clamp0118(
      nextProfile.conversationFriendliness + 0.07 + calmCue * 0.04
    );
    nextProfile.lingerFactor = clamp0118(nextProfile.lingerFactor + 0.05);
    nextProfile.destinationFactor = clamp0118(nextProfile.destinationFactor - 0.02 + calmCue * 0.02);
    signals.push("tone:vibe-cozy-softening");
  } else if (vibe === "cultured") {
    nextProfile.destinationFactor = clamp0118(nextProfile.destinationFactor + 0.05);
    nextProfile.experientialFactor = clamp0118(nextProfile.experientialFactor + 0.05);
    nextProfile.conversationFriendliness = clamp0118(nextProfile.conversationFriendliness + 0.03);
    nextProfile.energy = clamp0118(nextProfile.energy + 0.01);
    signals.push("tone:vibe-cultured-curation-lift");
  } else if (vibe === "adventurous-outdoor" || vibe === "adventurous-urban") {
    nextProfile.energy = clamp0118(nextProfile.energy + 0.06);
    nextProfile.socialDensity = clamp0118(nextProfile.socialDensity + 0.03);
    nextProfile.experientialFactor = clamp0118(nextProfile.experientialFactor + 0.07);
    nextProfile.destinationFactor = clamp0118(nextProfile.destinationFactor + 0.03);
    nextProfile.lingerFactor = clamp0118(nextProfile.lingerFactor + 0.02);
...
function deriveRoleSuitability(venue, profile, durationEstimate, noveltyWeight, calibration, experienceStrengths, momentPotential, experienceArchetypes, momentEnrichment, hyperlocalActivation, context) {
  ...
  const centerpieceSignal = clamp0118(
  ...
  const startFrictionSignal = clamp0118(
  ...
  const nightlifePressure = clamp0118(
  ...
  if (highlight >= 0.72) {
    const dominance = clamp0118((highlight - 0.72) * 1.18 + centerpieceSignal * 0.28);
    start = clamp0118(start - 0.14 * dominance);
    windDown = clamp0118(windDown - 0.2 * dominance);
  }
  if (start >= 0.66 && centerpieceSignal >= 0.58) {
    start = clamp0118(start - 0.12);
  }
  if (windDown >= 0.62 && (profile.energy >= 0.66 || nightlifePressure >= 0.62 || centerpieceSignal >= 0.66)) {
    windDown = clamp0118(windDown - 0.16);
  }
  if (landingSignal >= 0.58) {
    highlight = clamp0118(highlight - 0.08 * landingSignal);
  }
  const vibe = context?.vibe ?? void 0;
  const persona = context?.persona ?? void 0;
  const livelyLike = vibe === "lively" || vibe === "playful";
  const cozyLike = vibe === "cozy" || vibe === "chill";
  const livelyPulseSignal = clamp0118(
  ...
  if (livelyLike) {
    ...
    start = clamp0118(start - livelyPulseSignal * 0.08);
    windDown = clamp0118(windDown - livelyPulseSignal * 0.06);
    if (persona === "romantic") {
      highlight = clamp0118(highlight + containedPulseSignal * 0.08 - broadChaosSignal * 0.1);
      if (softCenterSignal >= 0.62 && centerpieceSignal < 0.62) {
        highlight = clamp0118(highlight - 0.1);
      }
    } else if (persona === "friends") {
      highlight = clamp0118(highlight + livelyPulseSignal * 0.08);
      start = clamp0118(start + profile.socialDensity * 0.04);
    } else if (persona === "family") {
      highlight = clamp0118(highlight - broadChaosSignal * 0.08);
      windDown = clamp0118(windDown + profile.conversationFriendliness * 0.05);
    }
  } else if (cozyLike) {
    highlight = clamp0118(
      highlight - livelyPulseSignal * 0.14 + profile.intimacy * 0.08 + landingSignal * 0.06
    );
    start = clamp0118(start + profile.conversationFriendliness * 0.06 + profile.intimacy * 0.04);
    windDown = clamp0118(windDown + landingSignal * 0.08 + profile.conversationFriendliness * 0.06);
  }
  const surprise = clamp0118(
...
function deriveHighlightTier(venue, highlightSuitability, profile, anchorStrength, experienceStrengths, durationEstimate, context) {
  ...
  const vibeAdjustedStrength = clamp0118(
...
function deriveMomentIntensity(venue, profile, experienceStrengths, experienceArchetypes, momentPotential, momentIdentity, romanticSignals, noveltyWeight, calibration, anchorStrength, highlightTier, momentEnrichment, hyperlocalActivation) {
  ...
  const scenicStrength = clamp0118(
  ...
  const activityRichness = clamp0118(
  ...
  const culturalImmersion = clamp0118(
  ...
  const hyperlocalLift = clamp0118(
...
function deriveTasteCalibration(venue, profile, noveltyWeight) {
  ...
  const tagRichness = clamp0118(venue.tags.length / 5);
  const placeTypeRichness = clamp0118((venue.placeTypes?.length ?? 0) / 4);
  const categorySpecificity = clamp0118(
  ...
  const personalityStrength = clamp0118(
...
function deriveExperienceStrengths(venue, keywordSignals) {
  ...
  return {
    outdoor: clamp0118(outdoor),
    interactive: clamp0118(interactive),
    passiveHospitality: clamp0118(passiveHospitality)
  };
}
...
function deriveExperienceFamily(venue, profile, experienceStrengths, experienceArchetypes, romanticSignals, momentIntensity, calibration, momentEnrichment, hyperlocalActivation) {
  ...
  const activitySignal = clamp0118(
  ...
  const immersiveCultureSignal = clamp0118(
  ...
  const familyScores = [
    {
      ...
      score: clamp0118(scenicSignal * 0.66 + quietSignal * 0.12 + richnessSignal * 0.12)
    },
    {
      family: "quiet_activity",
      score: clamp0118(activitySignal * 0.5 + quietSignal * 0.22 + richnessSignal * 0.16)
    },
    {
      family: "cultural",
      score: clamp0118(cultureSignal * 0.62 + ambientSignal * 0.12 + richnessSignal * 0.16)
    },
    {
    ...
    },
    {
      ...
      score: clamp0118(ambientSignal * 0.58 + quietSignal * 0.14 + richnessSignal * 0.14)
    },
    {
      family: "intimate_dining",
      score: clamp0118(diningSignal * 0.62 + quietSignal * 0.14 + richnessSignal * 0.12)
    }
  ];
  const priority = [
    "outdoor_scenic",
...
function deriveExpandedExperienceFamily(venue, profile, momentPotential, momentIntensity, momentEnrichment, hyperlocalActivation, baseFamily) {
  ...
  const activationStrength = clamp0118(
...
function applyExpandedFamilyRoleAdjustment(roleSuitability, experienceFamily, baseExperienceFamily, hyperlocalActivation) {
  ...
  const activationScale = clamp0118(
  ...
  if (experienceFamily === "tasting_experience") {
    adjusted.highlight = clamp0118(adjusted.highlight + 0.026 * activationScale);
    adjusted.windDown = clamp0118(adjusted.windDown + 0.018 * activationScale);
  } else if (experienceFamily === "live_experience") {
    adjusted.highlight = clamp0118(adjusted.highlight + 0.028 * activationScale);
    adjusted.surprise = clamp0118(adjusted.surprise + 0.02 * activationScale);
  } else if (experienceFamily === "immersive_experience") {
    adjusted.highlight = clamp0118(adjusted.highlight + 0.024 * activationScale);
    adjusted.surprise = clamp0118(adjusted.surprise + 0.022 * activationScale);
  } else if (experienceFamily === "atmospheric_experience") {
    adjusted.highlight = clamp0118(adjusted.highlight + 0.022 * activationScale);
    adjusted.windDown = clamp0118(adjusted.windDown + 0.02 * activationScale);
  }
  return adjusted;
}
...
function deriveRomanticSignals(venue, profile, experienceStrengths, experienceArchetypes, momentEnrichment) {
  ...
  const sharedActivity = clamp0118(
  ...
  const ambientExperience = clamp0118(
...
function deriveHyperlocalActivation(venue, profile, experienceArchetypes, context) {
  const keywords = buildKeywordCorpus(venue);
  const archetypes = new Set(experienceArchetypes.archetypes);
  const livePerformance = clamp0118(
  ...
  const tastingActivation = clamp0118(
  ...
  const culturalActivation = clamp0118(
  ...
  const temporalRelevance = clamp0118(
  ...
  const intensityContribution = activationTypes.length === 0 ? 0 : clamp0118(
  ...
  return {
    ...
    interpretationImpact: {
      highlightSuitability: clamp0118(highlightSuitabilityImpact),
      momentPotential: clamp0118(momentPotentialImpact),
      novelty: clamp0118(noveltyImpact),
      momentIntensity: clamp0118(momentIntensityImpact),
      familyRefinements
    },
...
function deriveHyperlocalTemporalCompatibility(venue, keywords, activationTypes, temporalRelevance, temporalLabel, intensityContribution, context) {
  ...
  const eveningCueScore = clamp0118(
...
function deriveMomentEnrichment(venue, profile, experienceStrengths, experienceArchetypes, hyperlocalActivation) {
  ...
  const temporalEnergy = clamp0118(
  ...
  const ambientUniqueness = clamp0118(
  ...
  const highlightSurfaceBoost = clamp0118(
...
function normalizeCount(value, ceiling) {
  ...
  return clamp0118(value / ceiling);
}
function clampProfile(profile) {
  return {
    energy: clamp0118(profile.energy),
    socialDensity: clamp0118(profile.socialDensity),
    intimacy: clamp0118(profile.intimacy),
    lingerFactor: clamp0118(profile.lingerFactor),
    destinationFactor: clamp0118(profile.destinationFactor),
    experientialFactor: clamp0118(profile.experientialFactor),
    conversationFriendliness: clamp0118(profile.conversationFriendliness)
  };
}
...
function getHighlightVibeWeight(intent2, starterPack) {
  let weight = 0.28;
  if (intent2.crew === "romantic") {
    weight += 0.05;
  }
  if (intent2.primaryAnchor === "cozy" || intent2.primaryAnchor === "cultured" || intent2.primaryAnchor === "adventurous-outdoor" || intent2.primaryAnchor === "adventurous-urban") {
    weight += 0.07;
  }
  if (intent2.primaryAnchor === "lively" || intent2.primaryAnchor === "playful") {
    weight += 0.05;
  }
  if (intent2.primaryAnchor === "chill") {
    weight += 0.04;
  }
...
function getMusicSupportSource(venue, intent2, packPressure) {
  if (!isMusicForwardVenue(venue)) {
    return "not-applicable";
  }
  const profile = getVibeProfile(intent2.primaryAnchor);
...
function computeVibeAuthority(venue, intent2, lens2, starterPack) {
  const primary = scoreVibeTagAffinity(venue, intent2.primaryAnchor);
  const secondaryValues = (intent2.secondaryAnchors ?? []).map(
  ...
  const adventureSplitLift = intent2.primaryAnchor === "adventurous-outdoor" ? clamp0119(adventureRead.outdoor) * 0.22 - clamp0119(adventureRead.urban) * 0.18 : intent2.primaryAnchor === "adventurous-urban" ? clamp0119(adventureRead.urban) * 0.22 - clamp0119(adventureRead.outdoor) * 0.18 : 0;
  const byRole = {
    start: clamp0119(
      scoreRoleProfileFit(venue, "start", intent2.primaryAnchor) * 0.78 + secondary * 0.12 + adventureSplitLift * 0.65 + (lens2.preferredStopShapes.start.preferredCategories.includes(venue.category) ? 0.1 : 0)
    ),
    highlight: clamp0119(
      scoreRoleProfileFit(venue, "highlight", intent2.primaryAnchor) * (1 + getHighlightVibeWeight(intent2, starterPack)) + secondary * 0.18 + adventureSplitLift
    ),
    surprise: clamp0119(
      scoreRoleProfileFit(venue, "surprise", intent2.primaryAnchor) * 0.74 + secondary * 0.18 + adventureSplitLift * 0.5 + (lens2.discoveryBias === "high" ? venue.underexposureScore * 0.08 : 0)
    ),
    windDown: clamp0119(
      scoreRoleProfileFit(venue, "windDown", intent2.primaryAnchor) * 0.84 + secondary * 0.14 + adventureSplitLift * 0.56
    )
  };
  const overall = clamp0119(
  ...
  return {
    ...
    musicSupportSource: getMusicSupportSource(venue, intent2, packHighlightPressure),
...
function getIntentTimeWindow(intent2) {
  const value = normalizeForMatch(intent2.timeWindow);
  if (!value) {
    return void 0;
  }
...
function toMomentVenue(moment, venuePool) {
  ...
  return normalizeVenue({
    ...
    distinctivenessScore: clamp0121(moment.uniquenessScore + 0.06),
    underexposureScore: clamp0121(moment.uniquenessScore + (moment.sourceType === "curated" ? 0.04 : -0.04)),
    shareabilityScore: clamp0121(0.52 + moment.uniquenessScore * 0.38),
...
function getScoreWeights(intent2, lens2) {
  const weights = {
    anchor: 0.22,
    crew: 0.21,
    vibe: 0.18,
  ...
  if (intent2.mode === "curate") {
    ...
    weights.vibe += 0.07;
    weights.lens += 0.12;
    weights.hiddenGem += lens2.discoveryBias === "high" ? 0.06 : 0.02;
  }
  if (lens2.discoveryBias === "high") {
...
function moderateBandScore(value, target, spread) {
  return clamp0122(1 - Math.abs(value - target) / spread);
}
function getMomentRolePreference(momentIdentity, role) {
  ...
  return clamp0122(typeWeight + strengthAdjustment);
}
...
function isHyperlocalVariantContractPlausible(candidate, lens2) {
  const activation = candidate.taste.signals.hyperlocalActivation;
  ...
  if (!requiresRomanticPersonaMoment(lens2)) {
    return true;
  }
...
function shouldInjectHyperlocalVariant(candidate, lens2) {
  const activation = candidate.taste.signals.hyperlocalActivation;
  const impact = activation.interpretationImpact;
  ...
  return isHyperlocalVariantContractPlausible(candidate, lens2);
}
function createHyperlocalActivationVariant(candidate, lens2) {
  const activation = candidate.taste.signals.hyperlocalActivation;
  const activationType = activation.primaryActivationType;
  if (!activationType || !shouldInjectHyperlocalVariant(candidate, lens2)) {
    return void 0;
  }
  const variantLift = computeHyperlocalVariantLift(candidate);
  ...
  const variantMomentPotential = clamp0122(
    candidate.taste.signals.momentPotential.score + momentPotentialBoost
  );
  const variantMomentIntensity = clamp0122(
  ...
  const variantRomanticSignals = {
    ...
    intimacy: clamp0122(candidate.taste.signals.romanticSignals.intimacy + intimacyBoost),
    ambiance: clamp0122(candidate.taste.signals.romanticSignals.ambiance + ambianceBoost),
    ambientExperience: clamp0122(
  ...
  return {
    ...
    fitScore: clamp0122(candidate.fitScore + fitLift),
    hiddenGemScore: clamp0122(
      candidate.hiddenGemScore + activation.interpretationImpact.novelty * 0.04
    ),
    lensCompatibility: clamp0122(
      candidate.lensCompatibility + activation.interpretationImpact.highlightSuitability * 0.04
    ),
    stopShapeFit: {
      ...candidate.stopShapeFit,
      highlight: clamp0122(candidate.stopShapeFit.highlight + highlightLift * 0.9),
      surprise: clamp0122(candidate.stopShapeFit.surprise + wildcardLift * 0.7),
      start: clamp0122(candidate.stopShapeFit.start + warmupLift * 0.4),
      windDown: clamp0122(candidate.stopShapeFit.windDown + cooldownLift * 0.5)
    },
    roleScores: {
      warmup: clamp0122(candidate.roleScores.warmup + warmupLift),
      peak: clamp0122(candidate.roleScores.peak + highlightLift),
      wildcard: clamp0122(candidate.roleScores.wildcard + wildcardLift),
      cooldown: clamp0122(candidate.roleScores.cooldown + cooldownLift)
    },
    taste: {
      ...candidate.taste,
      ...
      fallbackPenalty: {
        ...
        signalScore: clamp0122(candidate.taste.fallbackPenalty.signalScore - fallbackRelief),
        appliedPenalty: 0,
        applied: false,
        strongerAlternativePresent: false,
...
function scoreVenueFit(venue, intent2, crewPolicy2, lens2, roleContracts2, starterPack) {
  const anchorFit = scoreAnchorFit(venue, intent2);
  const crewFit = scoreCrewFit(venue, crewPolicy2);
  const proximityFit = scoreProximityFit(venue, intent2);
  const budgetFit = scoreBudgetFit(venue, intent2.budget);
  const uniquenessFit = scoreUniquenessFit(venue, intent2);
  const lensCompatibility = scoreLensCompatibility(venue, intent2, lens2);
  ...
  const hiddenGemFit = scoreHiddenGemFit(venue, provisionalFit, intent2);
  const weights = getScoreWeights(intent2, lens2);
  const stopShapeFit = {
    start: scoreLensStopShapeCompatibility(venue, lens2, "start"),
    highlight: scoreLensStopShapeCompatibility(venue, lens2, "highlight"),
    surprise: scoreLensStopShapeCompatibility(venue, lens2, "surprise"),
    windDown: scoreLensStopShapeCompatibility(venue, lens2, "windDown")
  };
  ...
  const roleContract = {
    warmup: evaluateRoleContract(
      venue,
      roleContracts2?.byRole.start ?? defaultRoleContractRule("start")
    ),
    peak: evaluateRoleContract(
      venue,
      roleContracts2?.byRole.highlight ?? defaultRoleContractRule("highlight")
    ),
    wildcard: evaluateRoleContract(
      venue,
      roleContracts2?.byRole.surprise ?? defaultRoleContractRule("surprise")
    ),
    cooldown: evaluateRoleContract(
      venue,
      roleContracts2?.byRole.windDown ?? defaultRoleContractRule("windDown")
    )
  };
  ...
  const vibeAuthority = computeVibeAuthority(venue, intent2, lens2, starterPack);
  ...
  const tasteSignals = interpretVenueTaste(mapVenueToTasteInput(venue), {
    timeWindow: intent2.timeWindow,
    persona: intent2.persona ?? void 0,
    vibe: intent2.primaryAnchor ?? void 0
  });
  ...
  const protectedByUserConstraint = intent2.anchor?.venueId === venue.id || Boolean(intent2.discoveryPreferences?.some((preference) => preference.venueId === venue.id));
  const genericHospitalityFallbackSignal = getGenericHospitalityFallbackPenalty({
    ...
    tasteModeId: lens2.tasteMode?.id
  });
  const tasteModeAlignment = getTasteModeAlignment(venue, lens2.tasteMode, {
    protectedCandidate: protectedByUserConstraint
  });
  const tasteModeWeight = lens2.tasteMode?.alignmentWeight ?? 0;
  ...
  const startLightModeAlignmentBoost = lens2.tasteMode && (tasteModeAlignment.byRole.start >= 0.42 || tasteModeAlignment.overall >= 0.44 && tasteModeAlignment.tier === "supporting") ? tasteModeWeight * (tasteModeAlignment.tier === "primary" ? 0.06 : tasteModeAlignment.tier === "supporting" ? 0.038 : 0.024) : 0;
  const windDownSoftModeAlignmentBoost = lens2.tasteMode && (tasteModeAlignment.byRole.windDown >= 0.4 || tasteModeAlignment.overall >= 0.42 && tasteModeAlignment.tier === "supporting") ? tasteModeWeight * (tasteModeAlignment.tier === "primary" ? 0.05 : tasteModeAlignment.tier === "supporting" ? 0.034 : 0.022) : 0;
  ...
  const discoveryPreference = intent2.discoveryPreferences?.find(
  ...
  const hiddenGemScore = clamp0122(
    hiddenGemFit * (1 + crewPolicy2.hiddenGemBias) + (intent2.prefersHiddenGems ? 0.03 : 0)
  );
  ...
  const discoveryLift = lens2.discoveryBias === "high" ? 0.08 : lens2.discoveryBias === "medium" ? 0.04 : 0;
  ...
  const formalDiningPressure = clamp0122(
  ...
  const adultNightlifePressure = clamp0122(
    (adultNightlifeVenue ? 0.8 : 0) + (isNightlifeCoded ? 0.12 : 0) - (venue.settings.familyFriendly ? 0.42 : 0)
  );
  const casualExplorationContext = intent2.primaryAnchor === "adventurous-urban" || intent2.primaryAnchor === "adventurous-outdoor" || intent2.primaryAnchor === "playful" || intent2.primaryAnchor === "chill";
  const contextualAdultPressure = Math.max(formalDiningPressure, adultNightlifePressure);
  const startIntentionalityBonus = clamp0122(
    tasteSignals.roleSuitability.start * 0.55 + tasteSignals.conversationFriendliness * 0.25 + (1 - tasteSignals.energy) * 0.2
  ) * 0.05;
  const windDownIntentionalityBonus = clamp0122(
  ...
  const modeSpecificPassiveHighlightPenalty = (lens2.tasteMode?.id === "activity-led" || lens2.tasteMode?.id === "scenic-outdoor") && passiveHighlightArchetype && tasteSignals.momentPotential.score < 0.55 ? 0.1 : lens2.tasteMode?.id === "highlight-centered" && passiveHighlightArchetype && tasteSignals.momentPotential.score < 0.48 ? 0.05 : 0;
  ...
  const cozyRomanticHighlightMode = crewPolicy2.crew === "romantic" && intent2.primaryAnchor === "cozy";
  ...
  const romanticGenericPenalty = crewPolicy2.crew === "romantic" && genericCategory && !isRomanticTone ? 0.09 : 0;
  const familyNightlifePenalty = crewPolicy2.crew === "curator" && isNightlifeCoded ? 0.17 : 0;
  const isDateCoded = hasAnyTag4(venue, ["intimate", "date-night", "romantic", "chef-led"]) || venue.category === "dessert";
  const romanticPlayfulStartPenalty = crewPolicy2.crew === "romantic" && venue.category === "activity" && !refinements.has("more-exciting") ? 0.13 : 0;
  const romanticHighlightGuardrailPenalty = crewPolicy2.crew === "romantic" && genericCategory && !isRomanticTone ? 0.16 : 0;
  const familyHighlightNightlifePenalty = crewPolicy2.crew === "curator" && (isNightlifeCoded || venue.category === "bar") ? 0.22 : 0;
  const familyHighlightDatePenalty = crewPolicy2.crew === "curator" && isDateCoded ? 0.11 : 0;
  const familyGenericPenalty = crewPolicy2.crew === "curator" && genericCategory && !isFamilyFriendly ? 0.12 : 0;
  const familyFormalDiningPenalty = crewPolicy2.crew === "curator" ? formalDiningPressure * 0.16 : 0;
  const familyFormalHighlightPenalty = crewPolicy2.crew === "curator" ? formalDiningPressure * 0.3 : 0;
  const familyAdultNightlifePenalty = crewPolicy2.crew === "curator" ? adultNightlifePressure * 0.18 : 0;
  const familyAdultNightlifeHighlightPenalty = crewPolicy2.crew === "curator" ? adultNightlifePressure * 0.34 : 0;
  const familyFormalWildcardPenalty = crewPolicy2.crew === "curator" ? formalDiningPressure * 0.24 : 0;
  const familyAdultNightlifeWildcardPenalty = crewPolicy2.crew === "curator" ? adultNightlifePressure * 0.28 : 0;
  ...
  const familyContextBoost = crewPolicy2.crew === "curator" && isFamilyFriendly ? 0.08 : 0;
  const romanticContextBoost = crewPolicy2.crew === "romantic" && isRomanticTone ? 0.08 : 0;
  const romanticMomentModifierActive = requiresRomanticPersonaMoment(lens2);
  ...
  const friendsMovementBoost = crewPolicy2.crew === "socialite" && (isNightlifeCoded || venue.category === "activity") ? 0.05 : 0;
  const universalHighlightPenalty = dominanceControl.flaggedUniversal && contextSpecificity.byRole.peak < 0.7 ? 0.04 : 0;
  const cozyConversationBoost = intent2.primaryAnchor === "cozy" && (venue.category === "restaurant" || venue.category === "dessert" || venue.category === "cafe" || hasAnyTag4(venue, ["intimate", "cozy", "craft", "wine"])) ? 0.12 : 0;
  const cozyMusicPenalty = intent2.primaryAnchor === "cozy" && crewPolicy2.crew === "romantic" && (venue.category === "live_music" || venue.category === "event") ? vibeAuthority.musicSupportSource === "pack" || vibeAuthority.musicSupportSource === "both" ? 0.04 : 0.2 : 0;
  const adventurousOutdoorBoost = intent2.primaryAnchor === "adventurous-outdoor" && (venue.category === "park" || hasAnyTag4(venue, ["nature", "walkable", "scenic", "viewpoint"])) ? 0.1 : 0;
  const adventurousUrbanBoost = intent2.primaryAnchor === "adventurous-urban" && (hasAnyTag4(venue, ["underexposed", "street-food", "community", "local"]) || venue.category === "bar" || venue.category === "event") ? 0.1 : 0;
  const outdoorAdventureLift = intent2.primaryAnchor === "adventurous-outdoor" && vibeAuthority.adventureRead === "outdoor" ? 0.12 : 0;
  const outdoorAdventurePenalty = intent2.primaryAnchor === "adventurous-outdoor" && vibeAuthority.adventureRead === "urban" ? 0.18 : intent2.primaryAnchor === "adventurous-outdoor" && vibeAuthority.adventureRead === "balanced" ? 0.06 : 0;
  const urbanAdventureLift = intent2.primaryAnchor === "adventurous-urban" && vibeAuthority.adventureRead === "urban" ? 0.12 : 0;
  const urbanAdventurePenalty = intent2.primaryAnchor === "adventurous-urban" && vibeAuthority.adventureRead === "outdoor" ? 0.18 : intent2.primaryAnchor === "adventurous-urban" && vibeAuthority.adventureRead === "balanced" ? 0.06 : 0;
...
function scoreVenueCollection(venues, intent2, crewPolicy2, lens2, roleContracts2, starterPack) {
  const baseCandidates = venues.map(
    (venue) => scoreVenueFit(venue, intent2, crewPolicy2, lens2, roleContracts2, starterPack)
  );
  const momentCandidates = deriveMomentVenueRecords({
    intent: intent2,
    venuePool: venues
  }).map(({ moment, venue }) => {
    const candidate = scoreVenueFit(venue, intent2, crewPolicy2, lens2, roleContracts2, starterPack);
  ...
  const injectedVariants = baseCandidates.flatMap((candidate) => {
    const variant = createHyperlocalActivationVariant(candidate, lens2);
    return variant ? [variant] : [];
  });
  const scored2 = [...baseCandidates, ...momentCandidates, ...injectedVariants];
  const adjusted = scored2.map((candidate) => {
    const fallbackAssessment = assessGenericHospitalityFallbackPenalty(candidate, scored2);
    ...
    return {
      ...candidate,
      fitScore: clamp0122(candidate.fitScore - appliedPenalty * 0.16),
      roleScores: {
        ...candidate.roleScores,
        peak: clamp0122(candidate.roleScores.peak - appliedPenalty)
      },
      taste: {
        ...candidate.taste,
        fallbackPenalty: {
...
function detectTemporalMode(input2) {
  return normalizeTimeWindow(input2?.timeWindow) ? "explicit" : "unspecified";
}
...
function getPreferredDiscoveryVenueId(intent2, role) {
  const preferredRole = role === "warmup" ? "start" : role === "peak" ? "highlight" : role === "cooldown" ? "windDown" : void 0;
  if (!preferredRole) {
    return void 0;
  }
  return intent2?.discoveryPreferences?.find(
    (preference) => preference.role === preferredRole
  )?.venueId;
}
function getAnchorVenueId(intent2, role) {
  if (intent2?.planningMode !== "user-led" || !intent2.anchor?.venueId) {
    return void 0;
  }
  const anchorRole = intent2.anchor.role ?? "highlight";
  if (role === "warmup" && anchorRole === "start" || role === "peak" && anchorRole === "highlight" || role === "cooldown" && anchorRole === "windDown") {
    return intent2.anchor.venueId;
  }
  return void 0;
}
function markRoleCandidate(candidate, intent2, role) {
  return getAnchorVenueId(intent2, role) === getScoredVenueBaseVenueId(candidate) ? { ...candidate, isAnchor: true } : candidate;
}
function isBaseRoleCandidate(item, role, lensRole, crewPolicy2, minRoleScore, minLensCompatibility, minShapeFit, minHighlightVibeFit, cooldownMaxEnergy) {
  ...
  if (crewPolicy2.blockedCategories.includes(item.venue.category)) {
    return false;
  }
...
function computeRolePoolRankingBreakdown(candidate, role, lens2, intent2) {
  ...
  const momentContribution = getMomentRolePreference2(candidate.momentIdentity, role) * (role === "peak" ? 0.4 : role === "cooldown" ? 0.18 : role === "warmup" ? 0.16 : 0.1);
  ...
  const cooldownPreferenceContribution = role === "cooldown" && lens2.windDownExpectation.preferredCategories.includes(candidate.venue.category) ? 0.08 : 0;
  const discoveryPreference = intent2?.discoveryPreferences?.find(
  ...
  const modeSpecificPassiveHighlightPenalty = role === "peak" && (lens2.tasteMode?.id === "activity-led" || lens2.tasteMode?.id === "scenic-outdoor") && passivePeakArchetype && candidate.taste.signals.momentPotential.score < 0.55 ? 0.18 : 0;
...
function computeRolePoolRankingScore(candidate, role, lens2, intent2) {
  return computeRolePoolRankingBreakdown(candidate, role, lens2, intent2).score;
}
function isPreferredRoleCandidateFeasible(candidate, role, lensRole, crewPolicy2) {
...
function isPeakHighlightFeasibleCandidate(candidate, intent2) {
  const anchoredPeakVenueId = getAnchorVenueId(intent2, "peak");
  ...
  if (intent2 && !isCandidateWithinActiveDistanceWindow(candidate, intent2, { allowMeaningfulStretch: true })) {
    return false;
  }
  if (!intent2 && candidate.fitBreakdown.proximityFit < 0.48) {
    return false;
  }
...
function isFeasibleStrongPeakMomentCandidate(candidate, intent2) {
  return isPeakHighlightFeasibleCandidate(candidate, intent2) && candidate.momentIdentity.strength === "strong" && (candidate.momentIdentity.type === "anchor" || candidate.momentIdentity.type === "explore");
}
function isWarmupAnchorConflict(candidate, intent2) {
  if (intent2?.planningMode !== "user-led" || !intent2.anchor?.venueId) {
    return false;
  }
  const anchorRole = intent2.anchor.role ?? "highlight";
  return anchorRole === "start" && intent2.anchor.venueId !== getScoredVenueBaseVenueId(candidate);
}
function isFeasibleRomanticMomentPoolCandidate(candidate, intent2) {
  if (!candidate.taste.signals.isRomanticMomentCandidate) {
    return false;
  }
  if (intent2 && !isCandidateWithinActiveDistanceWindow(candidate, intent2, { allowMeaningfulStretch: true })) {
  ...
  const highlightFeasible = isPeakHighlightFeasibleCandidate(candidate, intent2);
  const warmupFeasible = !isWarmupAnchorConflict(candidate, intent2) && !isPreferredRoleHoursInfeasible(candidate, "warmup") && candidate.roleScores.warmup >= roleThresholds.warmup - 0.02 && candidate.stopShapeFit.start >= 0.3 && candidate.momentIdentity.type !== "close";
...
function isFeasibleRomanticHighlightPoolCandidate(candidate, lens2, intent2) {
  return satisfiesRomanticPersonaHighlightContract(candidate, lens2) && isPeakHighlightFeasibleCandidate(candidate, intent2);
}
...
function getCentralMomentFamilyAdjustment(candidate, intent2) {
  const selectedFamily = intent2?.selectedDirectionContext?.family;
  const selectedFamilyConfidence = intent2?.selectedDirectionContext?.familyConfidence ?? 0;
...
function isRecoverableCentralMomentHighlightCandidate(candidate, intent2) {
  const hardContractConflict = candidate.roleContract.peak.strength === "hard" && !candidate.roleContract.peak.satisfied;
  ...
  if (intent2 && !isCandidateWithinActiveDistanceWindow(candidate, intent2, {
    allowMeaningfulStretch: true
  })) {
    return false;
  }
  if (!intent2 && candidate.fitBreakdown.proximityFit < 0.4) {
    return false;
  }
...
function getPeakStrongMomentSelectionBias(candidate, intent2, hasFeasibleStrongPeakMoment) {
  if (isFeasibleStrongPeakMomentCandidate(candidate, intent2)) {
    const archetype2 = candidate.taste.signals.primaryExperienceArchetype;
    const cozyRomanticMode = intent2?.persona === "romantic" && intent2.primaryAnchor === "cozy";
  ...
  if (!hasFeasibleStrongPeakMoment || !isPeakHighlightFeasibleCandidate(candidate, intent2)) {
    return 0;
  }
  const archetype = candidate.taste.signals.primaryExperienceArchetype;
...
function getRomanticMomentSelectionBias(candidate, role, lens2, intent2, hasFeasibleRomanticMoment) {
  if (!requiresRomanticPersonaMoment(lens2) || !hasFeasibleRomanticMoment) {
    return 0;
  }
  const archetype = candidate.taste.signals.primaryExperienceArchetype;
  const candidateIsRomantic = role === "peak" ? isFeasibleRomanticHighlightPoolCandidate(candidate, lens2, intent2) : isFeasibleRomanticMomentPoolCandidate(candidate, intent2);
  ...
  if (!isPeakHighlightFeasibleCandidate(candidate, intent2)) {
    return 0;
  }
  if (candidate.taste.signals.isRomanticMomentCandidate) {
    return 0;
...
function getLocalStretchSelectionBias(candidate, role, intent2, localSupplySufficient, strictNearbyFailed) {
  if (role !== "peak" || !intent2 || intent2.distanceMode !== "nearby" || !isOutsideStrictNearbyButWithinBoundedStretch(candidate.venue.driveMinutes, intent2.distanceMode)) {
    return 0;
  }
  if (!isMeaningfulMomentStretchCandidate(candidate, intent2)) {
    return -0.24;
  }
...
function getPeakMomentPriority(candidate, lens2) {
  if (lens2.tasteMode?.id !== "activity-led" && lens2.tasteMode?.id !== "scenic-outdoor" && lens2.tasteMode?.id !== "highlight-centered") {
    return 0;
  }
  const archetype = candidate.taste.signals.primaryExperienceArchetype;
...
function isAnchorCandidateForRole(candidate, role, intent2) {
  return getAnchorVenueId(intent2, role) === getScoredVenueBaseVenueId(candidate);
}
function shouldRelaxAnchorHoursRejection(candidate, role, intent2) {
  if (!intent2 || intent2.planningMode !== "user-led" || detectTemporalMode(intent2) === "explicit" || !isAnchorCandidateForRole(candidate, role, intent2)) {
    return false;
  }
  if (candidate.venue.source.businessStatus === "temporarily-closed" || candidate.venue.source.businessStatus === "closed-permanently") {
    return false;
...
function isPreferredRoleContextIncompatible(candidate, role, crewPolicy2) {
  if (crewPolicy2.blockedCategories.includes(candidate.venue.category)) {
    return true;
  }
...
function evaluatePreferredRoleAdmission(candidate, role, lensRole, crewPolicy2, intent2) {
  ...
  if (isPreferredRoleHoursInfeasible(candidate, role)) {
    if (shouldRelaxAnchorHoursRejection(candidate, role, intent2)) {
      return {
        preferredVenueId,
  ...
  if (isPreferredRoleContextIncompatible(candidate, role, crewPolicy2)) {
  ...
  if (!isPreferredRoleCandidateFeasible(candidate, role, lensRole, crewPolicy2) || isPreferredRoleSeverelyIncompatible(candidate, role)) {
...
function pickRoleCandidates(scoredVenues, role, crewPolicy2, lens2, intent2, roleContracts2, strictShapeEnabled = false, contractConstraints, experienceContract) {
  const lensRole = roleToLensStop(role);
  const cooldownBoosted = role === "cooldown";
  const refinementTightening = intent2?.refinementModes?.includes("closer-by") ? 0.03 : 0;
  const roleContract = roleContracts2?.byRole[lensRole] ?? defaultRoleContractRule2(lensRole);
  const enforceContract = strengthRank2(roleContract.strength) >= strengthRank2("strong");
  ...
  const preferredVenueId = getAnchorVenueId(intent2, role) ?? getPreferredDiscoveryVenueId(intent2, role);
  ...
  const hasFeasibleStrongPeakMoment = scoredVenues.some(
    (item) => isFeasibleStrongPeakMomentCandidate(item, intent2)
  );
  const hasFeasibleRomanticMoment = scoredVenues.some(
    (item) => isFeasibleRomanticMomentPoolCandidate(item, intent2)
  );
  const feasibleRomanticHighlights = scoredVenues.filter(
    (item) => isFeasibleRomanticHighlightPoolCandidate(item, lens2, intent2)
  );
  const personaContractRequiresRomanticHighlight = role === "peak" && requiresRomanticPersonaMoment(lens2) && feasibleRomanticHighlights.length > 0;
  const warmupNeedsRomanticSupportRelaxation = role === "warmup" && isRomanticPersonaContractActive(lens2) && feasibleRomanticHighlights.length > 0 && !contractAwareExpandedCandidates.some(
  ...
  if (personaContractRequiresRomanticHighlight) {
    const romanticHighlightCandidates = [
      ...new Map(
        roleCandidates.filter((item) => isFeasibleRomanticHighlightPoolCandidate(item, lens2, intent2)).map((item) => [getScoredVenueCandidateId(item), item])
      ).values()
    ];
  ...
  const localSupplySufficient = intent2 ? scoredVenues.some(
    (item) => isPeakHighlightFeasibleCandidate(item, intent2) && isWithinStrictNearbyWindow(item.venue.driveMinutes, intent2.distanceMode) && item.momentIdentity.strength === "strong" && item.roleScores.peak >= 0.64 && item.stopShapeFit.highlight >= 0.4 && item.taste.signals.momentPotential.score >= 0.66
  ) : false;
  const strictNearbyFailed = !localSupplySufficient && (intent2 ? scoredVenues.some((item) => isMeaningfulMomentStretchCandidate(item, intent2)) : false);
  ...
  if (role === "peak" && roleCandidates.length === 0) {
    const recoveredCentralMomentCandidates = scoredVenues.filter((candidate) => isRecoverableCentralMomentHighlightCandidate(candidate, intent2)).map((candidate) => {
      const familyAdjustment = getCentralMomentFamilyAdjustment(candidate, intent2);
  ...
  ) || isAnchorCandidateForRole(preferredAdmission.admittedCandidate, role, intent2);
  ...
  const selectionScoreByCandidateId = new Map(
    scoredRoleCandidates.map((candidate) => [
      ...
      computeRoleSelectionScore({
        ...
        localSupplySufficient,
        strictNearbyFailed
      }) + (contractPressureByCandidateId.get(getScoredVenueCandidateId(candidate))?.scoreAdjustment ?? 0) + (role === "peak" && candidate.recoveredCentralMomentHighlight ? CENTRAL_MOMENT_RECOVERY_BOOST + getCentralMomentFamilyAdjustment(candidate, intent2) : 0)
    ])
  );
  ...
  const rankedWithPreference = allowPreferredAdmission && preferredAdmission.admittedCandidate ? [
    markRoleCandidate(preferredAdmission.admittedCandidate, intent2, role),
    ...ranked.filter(
...
function buildRolePools(scoredVenues, crewPolicy2, lens2, intent2, roleContracts2, strictShapeEnabled = false, contractConstraints, experienceContract) {
...
function buildGenericDisplayName(rawDistrict, city) {
  const cleanDistrict = toTitleCase2(rawDistrict);
...
function createCityFallback(city, citySlug) {
  return {
...
function toReadableDistrictLabel(districtLabel, city) {
...
function resolveDistrictAnchor({
  ...
  if (promotedDistrict) {
    return {
      ...
      confidence: roundToHundredths(clamp3(0.75 + strongestCluster.clusterShare * 0.15, 0.75, 0.9)),
  ...
  if (strongestCluster.clusterShare >= 0.5) {
    ...
    return {
      ...
      confidence: roundToHundredths(clamp3(0.65 + strongestCluster.clusterShare * 0.2, 0.65, 0.85)),
      reason: "inferred from strongest cluster"
    };
  }
  return createCityFallback(trimmedCity, citySlug);
...
function buildHighlights(signals) {
  ...
  for (const candidate of candidates) {
    if (!unique7.includes(candidate)) {
      unique7.push(candidate);
    }
    if (unique7.length >= desiredCount) {
      break;
    }
  }
  return unique7.slice(0, 3);
}
...
function hasCategory2(mix, category, threshold = 0.2) {
...
function buildWhatStandsOut(signals) {
  ...
  if (hasCategory2(mix, "bar", 0.18) && (hasCategory2(mix, "activity", 0.14) || hasCategory2(mix, "museum", 0.12) || hasCategory2(mix, "liveMusic", 0.12) || hasCategory2(mix, "event", 0.12))) {
    return "Mix of bars and creative spots in one tight pocket.";
  }
  if (hasCategory2(mix, "restaurant", 0.2) && (hasCategory2(mix, "park", 0.12) || hasCategory2(mix, "cafe", 0.14) || hasCategory2(mix, "dessert", 0.12))) {
    return "Strong dinner-and-stroll rhythm without needing long moves.";
  }
  if ((hasCategory2(mix, "museum", 0.1) || hasCategory2(mix, "event", 0.1)) && (hasCategory2(mix, "restaurant", 0.14) || hasCategory2(mix, "cafe", 0.14)) && (hasCategory2(mix, "park", 0.1) || hasCategory2(mix, "activity", 0.1))) {
...
function getVenueClusterId(venue) {
  const normalizedCity = normalizeCity3(venue.city);
...
function normalizeWeights(weights) {
  ...
  const total2 = floored.start + floored.highlight + floored.winddown;
  if (total2 <= 0) {
    return { start: 0.33, highlight: 0.34, winddown: 0.33 };
  }
  return {
    start: floored.start / total2,
    highlight: floored.highlight / total2,
    winddown: floored.winddown / total2
  };
}
function getRoleWeights(intent2) {
  let weights;
  switch (intent2.primaryAnchor) {
    case "cozy":
...
function getViableCandidates(rolePools2, scoredVenues) {
...
function getDistrictGroups(viableCandidates, rolePools2, intent2) {
  const warmupIds = new Set(rolePools2.warmup.map((candidate) => candidate.venue.id));
  const peakIds = new Set(rolePools2.peak.map((candidate) => candidate.venue.id));
  const cooldownIds = new Set(rolePools2.cooldown.map((candidate) => candidate.venue.id));
...
function getDensityScore(candidateCount, maxCandidateCount) {
  ...
  return clamp0123(candidateCount / maxCandidateCount);
}
function getAdjustedDensityScore(rawDensity) {
  return clamp0123(Math.pow(rawDensity, 0.72));
}
function getRelevanceScore(candidates) {
  if (candidates.length === 0) {
    return 0;
  }
  const total2 = candidates.reduce((sum, candidate) => {
  ...
  return clamp0123(total2 / candidates.length);
}
function getDiversityScore(group) {
  ...
  const categoryCoverage = clamp0123(group.categories.size / 4);
  return clamp0123(coreRoleCoverage * 0.65 + categoryCoverage * 0.35);
}
...
function getDominanceScore(values) {
  ...
  return clamp0123(maxCount / values.length);
}
function getCoherenceScore(group) {
  ...
  return clamp0123(neighborhoodDominance * 0.58 + experienceFamilyDominance * 0.42);
}
function getSequenceSupportScore(group) {
  ...
  return clamp0123(sequenceMean * 0.68 + coverage * 0.22 + sequenceBalance * 0.1);
}
function getTasteAverages(candidates) {
  ...
  return {
    intimacy: clamp0123(totals.intimacy / candidates.length),
    linger: clamp0123(totals.linger / candidates.length),
    conversation: clamp0123(totals.conversation / candidates.length),
    calm: clamp0123(totals.calm / candidates.length),
    social: clamp0123(totals.social / candidates.length),
    culturalDepth: clamp0123(totals.culturalDepth / candidates.length),
    discovery: clamp0123(totals.discovery / candidates.length),
    experiential: clamp0123(totals.experiential / candidates.length),
    outdoor: clamp0123(totals.outdoor / candidates.length),
    playful: clamp0123(totals.playful / candidates.length)
  };
}
function getCategoryMix(candidates) {
  ...
  return {
    restaurant: clamp0123(totals.restaurant / candidates.length),
    bar: clamp0123(totals.bar / candidates.length),
    cafe: clamp0123(totals.cafe / candidates.length),
    dessert: clamp0123(totals.dessert / candidates.length),
    liveMusic: clamp0123(totals.liveMusic / candidates.length),
    activity: clamp0123(totals.activity / candidates.length),
    park: clamp0123(totals.park / candidates.length),
    museum: clamp0123(totals.museum / candidates.length),
    event: clamp0123(totals.event / candidates.length)
  };
}
...
function getWalkabilitySignal(candidates) {
  ...
  return clamp0123(matched / candidates.length);
}
...
function getRoleStrengths(group) {
  ...
  const chaosPenalty = clamp0123(Math.max(0, social - 0.58) * 0.9 + Math.max(0, energy - 0.62) * 0.8);
  const overCalmPenalty = clamp0123(Math.max(0, taste.calm - 0.72) * 0.9);
  ...
  const highlightCategorySupport = clamp0123(
  ...
  const winddownCategorySupport = clamp0123(
  ...
  const highlightStrength = clamp0123(
  ...
  const winddownStrength = clamp0123(
...
function getVibeAffinity(intent2, taste, sequenceSupport) {
  switch (intent2.primaryAnchor) {
    case "cozy":
      return clamp0123(
...
function getCrewAffinity(intent2, taste, sequenceSupport, signature) {
  if (intent2.crew === "romantic") {
    return clamp0123(
...
function getAffinityScores(group, intent2, sequenceSupport, signature) {
  const taste = getTasteAverages(group.candidates);
  const vibeAffinity = getVibeAffinity(intent2, taste, sequenceSupport);
  const crewAffinity = getCrewAffinity(intent2, taste, sequenceSupport, signature);
  return {
    affinity: clamp0123(vibeAffinity * 0.58 + crewAffinity * 0.42),
    vibeAffinity: clamp0123(vibeAffinity),
    crewAffinity: clamp0123(crewAffinity)
  };
}
function getAlignmentLift(density, affinity, signature, coherence, relevance, diversity) {
  const contextStrength = clamp0123(relevance * 0.55 + diversity * 0.45);
  return clamp0123(
...
function recommendDistricts({
  scoredVenues,
  rolePools: rolePools2,
  intent: intent2,
  limit = 5
}) {
  const viableCandidates = getViableCandidates(rolePools2, scoredVenues);
  const districtGroups2 = getDistrictGroups(viableCandidates, rolePools2, intent2);
  const maxCandidateCount = Math.max(
    ...districtGroups2.map((group) => group.candidates.length),
    0
  );
  const roleWeights = getRoleWeights(intent2);
  return districtGroups2.map((group) => {
    ...
    const affinityScores = getAffinityScores(group, intent2, sequenceSupport, signature);
    ...
    const momentPotential = roundToHundredths2(
      average(group.candidates.map((candidate) => candidate.taste.signals.momentPotential.score))
    );
    ...
    const score = roundToHundredths2(
      clamp0123(baseScore + alignmentLift - breadthPenalty - sparsityPenalty)
    );
    const signals = {
      density,
      relevance,
    ...
    const districtExplanation = generateDistrictExplanation(
      ...
      {
        persona: intent2.persona,
        crew: intent2.crew,
        vibe: intent2.primaryAnchor
      }
    );
...
function toFixed(value) {
  return Number(clamp4(value, 0, 1).toFixed(3));
}
...
function getHoursAvailabilityScore(hoursStatus, hoursConfidence) {
  ...
  const confidence = clamp4(hoursConfidence ?? 0.5, 0, 1);
  if (status === "open") {
...
function getIntentFit(input2) {
  const { tasteSignals, context } = input2;
  ...
  if (persona.includes("romantic")) {
    personaFit = clamp4(
      tasteSignals.romanticScore * 0.46 + tasteSignals.intimacy * 0.22 + tasteSignals.conversationFriendliness * 0.16 + tasteSignals.lingerFactor * 0.16,
      0,
      1
    );
  ...
  return toFixed(clamp4(base * 0.2 + vibeFit * 0.45 + personaFit * 0.35, 0, 1));
}
function getTimingRelevance(input2) {
  const signals = input2.tasteSignals;
  const sourceFlags = input2.sourceFlags;
  const windowLabel = parseWindowLabel(input2.context?.timeWindow);
  const hoursAvailability = getHoursAvailabilityScore(
    input2.hoursStatus,
    input2.hoursConfidence
  );
  ...
  return toFixed(
    clamp4(
      signals.hyperlocalActivation.temporalRelevance * 0.42 + hoursAvailability * 0.26 + (hasEvent ? 0.14 : 0) + (hasPerformance ? 0.1 : 0) + happyHourBonus + temporalWindowBonus,
      0,
      1
    )
  );
}
function getSourceType(momentType, input2) {
  const eventLike = Boolean(
    input2.sourceFlags?.eventCapable || input2.sourceFlags?.musicCapable || input2.sourceFlags?.performanceCapable || input2.sourceFlags?.hasHappyHour
  );
...
function buildDetectedMoment(input2, momentType, strength, timingRelevance, intentFit, reason) {
  const windowLabel = parseWindowLabel(input2.context?.timeWindow);
  const hasEvent = Boolean(
    input2.sourceFlags?.eventCapable || input2.tasteSignals.hyperlocalActivation.activationTypes.includes("live_performance")
  );
  const hasPerformance = Boolean(
    input2.sourceFlags?.performanceCapable || input2.sourceFlags?.musicCapable
  );
  return {
    id: `moment:${input2.venueId}:${momentType}`,
    title: input2.venueName,
    venueId: input2.venueId,
    districtId: input2.districtId,
    sourceType: getSourceType(momentType, input2),
    momentType,
    strength: toFixed(strength),
    timingRelevance: toFixed(timingRelevance),
    intentFit: toFixed(intentFit),
    roleFit: {
      start: toFixed(input2.tasteSignals.roleSuitability.start),
      highlight: toFixed(input2.tasteSignals.roleSuitability.highlight),
      windDown: toFixed(input2.tasteSignals.roleSuitability.windDown)
    },
    reason,
    descriptors: getDescriptors(input2.tasteSignals),
    liveContext: {
      hasEvent,
      hasHappyHour: Boolean(input2.sourceFlags?.hasHappyHour),
      hasPerformance,
      timeWindowLabel: windowLabel
    }
  };
}
function detectAnchorMoment(input2, timingRelevance, intentFit) {
  const signals = input2.tasteSignals;
  const tierScore = signals.highlightTier === 1 ? 1 : signals.highlightTier === 2 ? 0.72 : 0.44;
  const anchorStrength = clamp4(
    signals.roleSuitability.highlight * 0.34 + tierScore * 0.2 + signals.anchorStrength * 0.18 + signals.momentIntensity.score * 0.16 + signals.momentPotential.score * 0.08 + intentFit * 0.04,
    0,
    1
  );
...
function detectSupportingMoment(input2, timingRelevance, intentFit) {
  const signals = input2.tasteSignals;
  const supportStrength = clamp4(
...
function detectTemporalMoment(input2, timingRelevance, intentFit) {
  const signals = input2.tasteSignals;
  const temporalSignalsPresent = Boolean(
    input2.sourceFlags?.eventCapable || input2.sourceFlags?.musicCapable || input2.sourceFlags?.performanceCapable || input2.sourceFlags?.hasHappyHour || signals.hyperlocalActivation.temporalLabel !== "background"
  );
  if (!temporalSignalsPresent) {
    return null;
  }
  const strength = clamp4(
    timingRelevance * 0.56 + Math.max(signals.momentPotential.score, signals.momentIntensity.score) * 0.2 + signals.roleSuitability.highlight * 0.12 + intentFit * 0.12,
    0,
    1
  );
  if (strength < 0.58) {
...
function detectDiscoveryMoment(input2, timingRelevance, intentFit) {
  const signals = input2.tasteSignals;
  const discoveryStrength = clamp4(
    signals.noveltyWeight * 0.34 + signals.categorySpecificity * 0.24 + signals.personalityStrength * 0.18 + signals.momentPotential.score * 0.14 + signals.experientialFactor * 0.1,
    0,
    1
  );
...
function detectCommunityMoment(input2, timingRelevance, intentFit) {
  const signals = input2.tasteSignals;
  ...
  const communityStrength = clamp4(
    signals.momentEnrichment.culturalDepth * 0.33 + signals.conversationFriendliness * 0.2 + signals.lingerFactor * 0.16 + signals.experientialFactor * 0.11 + hintScore + activationScore + intentFit * 0.1,
    0,
    1
  );
...
function detectMomentsFromTaste(input2) {
  const intentFit = getIntentFit(input2);
  const timingRelevance = getTimingRelevance(input2);
  const candidates = [
    detectAnchorMoment(input2, timingRelevance, intentFit),
    detectSupportingMoment(input2, timingRelevance, intentFit),
    detectTemporalMoment(input2, timingRelevance, intentFit),
    detectDiscoveryMoment(input2, timingRelevance, intentFit),
    detectCommunityMoment(input2, timingRelevance, intentFit)
...
function getFitWeight(input2) {
  return clamp5((input2.fitScore ?? 0.5) * 0.5 + 0.5, 0.4, 1);
}
...
function buildRoleReason(role, input2, score) {
  const signals = input2.tasteSignals;
...
function scoreRoleCandidate(role, input2) {
  const signals = input2.tasteSignals;
  const fitWeight = getFitWeight(input2);
  if (role === "start") {
    return clamp5(
      signals.roleSuitability.start * 0.56 + signals.conversationFriendliness * 0.12 + signals.intimacy * 0.11 + (1 - signals.energy) * 0.09 + signals.lingerFactor * 0.07 + fitWeight * 0.05,
      0,
      1
    );
  }
  if (role === "highlight") {
    return clamp5(
      signals.roleSuitability.highlight * 0.42 + getHighlightTierScore(signals.highlightTier) * 0.14 + signals.anchorStrength * 0.12 + signals.momentIntensity.score * 0.11 + signals.momentPotential.score * 0.09 + signals.destinationFactor * 0.06 + signals.experientialFactor * 0.04 + fitWeight * 0.02,
      0,
      1
    );
  }
  return clamp5(
    signals.roleSuitability.windDown * 0.5 + signals.intimacy * 0.14 + signals.conversationFriendliness * 0.12 + (1 - signals.energy) * 0.1 + signals.lingerFactor * 0.09 + fitWeight * 0.05,
    0,
    1
  );
}
...
function toRoleCandidate(role, input2) {
  const score = scoreRoleCandidate(role, input2);
  return {
    venueId: input2.venueId,
    venueName: input2.venueName,
    reason: buildRoleReason(role, input2, score),
    score: toFixed2(score)
  };
}
function getRoleCandidates(role, inputs, limit) {
  return inputs.map((input2) => toRoleCandidate(role, input2)).sort((left, right) => right.score - left.score || left.venueName.localeCompare(right.venueName)).slice(0, limit);
}
function getDominantEnergy(inputs) {
  const value = weightedAverage(
    inputs.map((input2) => ({ value: input2.tasteSignals.energy, weight: getFitWeight(input2) }))
  );
  if (value <= 0.42) {
    return "calm";
  }
...
function getDominantSocialDensity(inputs) {
  const socialDensity = weightedAverage(
    inputs.map((input2) => ({ value: input2.tasteSignals.socialDensity, weight: getFitWeight(input2) }))
  );
  const intimacy = weightedAverage(
    inputs.map((input2) => ({ value: input2.tasteSignals.intimacy, weight: getFitWeight(input2) }))
  );
...
function getMovementProfile(inputs) {
  const withCoordinates = inputs.filter(
    (input2) => typeof input2.lat === "number" && typeof input2.lng === "number"
  );
  ...
  const avgConversation = average2(inputs.map((input2) => input2.tasteSignals.conversationFriendliness));
  const avgIntimacy = average2(inputs.map((input2) => input2.tasteSignals.intimacy));
  const avgEnergy = average2(inputs.map((input2) => input2.tasteSignals.energy));
  const avgSocialDensity = average2(inputs.map((input2) => input2.tasteSignals.socialDensity));
...
function getDiscoveryBalance(inputs) {
  const noveltySignal = weightedAverage(
    inputs.map((input2) => ({
      value: input2.tasteSignals.noveltyWeight * 0.55 + input2.tasteSignals.momentPotential.score * 0.2 + input2.tasteSignals.experientialFactor * 0.15 + input2.tasteSignals.momentIntensity.score * 0.1,
      weight: getFitWeight(input2)
    }))
  );
...
function getRoleSignature(role, input2) {
  const signals = input2.tasteSignals;
  if (role === "start") {
...
function buildAggregatedMoments(inputs) {
  const allMoments = inputs.flatMap(
    (input2) => detectMomentsFromTaste({
      venueId: input2.venueId,
      venueName: input2.venueName,
      districtId: input2.districtId,
      districtLabel: input2.districtLabel,
      tasteSignals: input2.tasteSignals,
      context: input2.context,
      sourceFlags: input2.sourceFlags,
      hoursStatus: input2.hoursStatus,
      hoursConfidence: input2.hoursConfidence
    })
  );
...
function aggregateTasteOpportunityFromVenues(inputs) {
  const cleanInputs = inputs.filter((input2) => Boolean(input2?.tasteSignals));
  ...
  const inputById = new Map(cleanInputs.map((input2) => [input2.venueId, input2]));
  const archetypes = topKeys(
    cleanInputs.map((input2) => input2.tasteSignals.primaryExperienceArchetype),
    5
  );
  const momentIdentities = topKeys(
    cleanInputs.map(
      (input2) => `${input2.tasteSignals.momentIdentity.type}:${input2.tasteSignals.momentIdentity.strength}`
    ),
    5
  );
  const familyDistribution = countByKey(
    cleanInputs.map((input2) => input2.tasteSignals.experienceFamily || "unknown")
  );
  const momentDistribution = countByKey(
    cleanInputs.map((input2) => input2.tasteSignals.momentIdentity.type)
  );
  return {
    summary: {
    ...
    diagnostics: {
      ...
      roleCoverage: {
        start: cleanInputs.filter((input2) => input2.tasteSignals.roleSuitability.start >= 0.56).length,
        highlight: cleanInputs.filter(
          (input2) => input2.tasteSignals.roleSuitability.highlight >= 0.6
        ).length,
        windDown: cleanInputs.filter(
          (input2) => input2.tasteSignals.roleSuitability.windDown >= 0.56
        ).length
      },
      familyDistribution,
      momentDistribution
...
var intent = normalizeIntent({ ...input, district: void 0 });
var lens = buildExperienceLens({ intent });
var crewPolicy = getCrewPolicy(intent.crew);
var roleContracts = getRoleContract({ intent });
var retrieval = await retrieveVenues(intent, lens, { requestedSourceMode: "curated", sourceModeOverrideApplied: true });
var scored = scoreVenueCollection(retrieval.venues, intent, crewPolicy, lens, roleContracts);
var rolePools = buildRolePools(scored, crewPolicy, lens, intent, roleContracts);
var recommendations = recommendDistricts({ scoredVenues: scored, rolePools, intent, limit: 3 });
var clusterByVenue = /* @__PURE__ */ new Map();
...
var districtGroups = /* @__PURE__ */ new Map();
for (const sv of scored) {
...
var sortedDistrictCounts = [...districtCounts.entries()].sort((a, b) => b[1] - a[1]);
var topDistrictShare = total > 0 ? Number(((sortedDistrictCounts[0]?.[1] ?? 0) / total).toFixed(3)) : 0;
var top3DistrictShare = total > 0 ? Number((sortedDistrictCounts.slice(0, 3).reduce((s, [, c]) => s + c, 0) / total).toFixed(3)) : 0;
var strongHighlightCandidates = scored.map((sv) => {
  const s = sv.taste.signals;
  const highlightSignal = s.roleSuitability.highlight * 0.4 + (s.highlightTier === 1 ? 1 : s.highlightTier === 2 ? 0.72 : 0.44) * 0.2 + s.anchorStrength * 0.2 + s.momentIntensity.score * 0.1 + s.momentPotential.score * 0.1;
  return { sv, highlightSignal };
}).filter(({ highlightSignal }) => highlightSignal >= 0.66).sort((a, b) => b.highlightSignal - a.highlightSignal);
var strongHighlightByDistrict = /* @__PURE__ */ new Map();
...
var allDetectedMoments = scored.flatMap((sv) => {
  const district = clusterByVenue.get(sv.venue.id);
...
var recommendedWithAgg = recommendations.map((r) => ({ ...r, tasteAggregation: aggregationByDistrict.get(r.districtId) }));
var aggregatedMomentsAll = recommendedWithAgg.flatMap((r) => [
  ...r.tasteAggregation?.moments.primary ?? [],
  ...r.tasteAggregation?.moments.secondary ?? []
]);
var aggregatedAnchorMoments = aggregatedMomentsAll.filter((m) => m.momentType === "anchor");
var anchorVenueSet = new Set(
  recommendedWithAgg.map((r) => r.tasteAggregation?.anchors.strongestHighlight?.venueName).filter(Boolean)
);
var archetypeSet = new Set(
  recommendedWithAgg.flatMap((r) => r.tasteAggregation?.signatures.archetypes ?? [])
);
var momentVenueFrequency = /* @__PURE__ */ new Map();
for (const m of aggregatedMomentsAll) {
  const key = m.title;
  momentVenueFrequency.set(key, (momentVenueFrequency.get(key) ?? 0) + 1);
}
var topFrequency = [...momentVenueFrequency.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
var runTopAnchors = [];
for (let i = 0; i < 5; i += 1) {
