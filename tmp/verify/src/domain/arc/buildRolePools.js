...
import { isRomanticPersonaContractActive, requiresRomanticPersonaMoment, scopeRomanticHighlightCandidatesByMomentTier, satisfiesRomanticPersonaHighlightContract, } from '../contracts/romanticPersonaContract';
import { isCandidateWithinActiveDistanceWindow, isMeaningfulMomentStretchCandidate, isOutsideStrictNearbyButWithinBoundedStretch, isWithinStrictNearbyWindow, } from '../constraints/localStretchPolicy';
...
import { getHighlightArchetypeLift, getMomentIntensityTierBoost, } from '../taste/experienceSignals';
import { getScoredVenueBaseVenueId, getScoredVenueCandidateId, } from '../candidates/candidateIdentity';
...
export function roleToLensStop(role) {
    if (role === 'warmup') {
        return 'start';
    }
    if (role === 'peak') {
...
function getPreferredDiscoveryVenueId(intent, role) {
    const preferredRole = role === 'warmup'
        ? 'start'
        : role === 'peak'
            ? 'highlight'
            : role === 'cooldown'
    ...
    return intent?.discoveryPreferences?.find((preference) => preference.role === preferredRole)?.venueId;
}
function getAnchorVenueId(intent, role) {
    ...
    if ((role === 'warmup' && anchorRole === 'start') ||
...
function isBaseRoleCandidate(item, role, lensRole, crewPolicy, minRoleScore, minLensCompatibility, minShapeFit, minHighlightVibeFit, cooldownMaxEnergy) {
    const peakMomentException = role === 'peak' &&
...
export function getRolePoolForRole(role, pools) {
    if (role === 'warmup') {
        return pools.warmup;
    }
    if (role === 'peak') {
...
export function computeRolePoolRankingBreakdown(candidate, role, lens, intent) {
    ...
    const momentContribution = getMomentRolePreference(candidate.momentIdentity, role) *
        (role === 'peak' ? 0.4 : role === 'cooldown' ? 0.18 : role === 'warmup' ? 0.16 : 0.1);
    const highlightValidityContribution = role === 'peak'
        ? candidate.highlightValidity.validityLevel === 'valid'
            ...
            : candidate.highlightValidity.validityLevel === 'fallback'
                ...
                : -0.5
        : 0;
    const cooldownPreferenceContribution = role === 'cooldown' &&
        lens.windDownExpectation.preferredCategories.includes(candidate.venue.category)
        ? 0.08
        : 0;
    const discoveryPreference = intent?.discoveryPreferences?.find((preference) => preference.venueId === getScoredVenueBaseVenueId(candidate));
    ...
    const highlightMomentContribution = role === 'peak'
        ...
        : role === 'wildcard'
            ...
            : 0;
    const highlightArchetypeContribution = role === 'peak'
        ? getHighlightArchetypeLift(candidate.taste.signals, lens.tasteMode?.id) * 1.8
        : 0;
    const genericFallbackPenaltyContribution = role === 'peak'
        ? candidate.taste.fallbackPenalty.appliedPenalty * 1.6
        : 0;
    const passivePeakArchetype = role === 'peak' &&
    ...
    const modeSpecificPassiveHighlightPenalty = role === 'peak' &&
    ...
    const highlightPlausibilityContribution = role === 'peak'
    ...
    const score = roleFitContribution +
        tasteContribution +
...
export function computeRolePoolRankingScore(candidate, role, lens, intent) {
...
function isPreferredRoleCandidateFeasible(candidate, role, lensRole, crewPolicy) {
    if (!isBaseRoleCandidate(candidate, role, lensRole, crewPolicy, roleThresholds[role] - 0.13, 0.24, 0.24, 0.28, 4)) {
        return false;
    }
...
function isPeakHighlightFeasibleCandidate(candidate, intent) {
    ...
    if (candidate.highlightValidity.personaVetoes.length > 0 ||
...
function isFeasibleStrongPeakMomentCandidate(candidate, intent) {
    return (isPeakHighlightFeasibleCandidate(candidate, intent) &&
...
function isFeasibleRomanticMomentPoolCandidate(candidate, intent) {
    ...
    const wildcardFeasible = !isPreferredRoleHoursInfeasible(candidate, 'wildcard') &&
...
function getRomanticHighlightSupportSelectionBias(candidate, role, feasibleRomanticHighlights) {
    ...
    const maximumPeakEnergy = Math.max(...feasibleRomanticHighlights.map((romanticHighlight) => romanticHighlight.venue.energyLevel));
    const sameNeighborhood = feasibleRomanticHighlights.some((romanticHighlight) => romanticHighlight.venue.neighborhood === candidate.venue.neighborhood);
    const nearestDriveGap = Math.min(...feasibleRomanticHighlights.map((romanticHighlight) => Math.abs(romanticHighlight.venue.driveMinutes - candidate.venue.driveMinutes)));
    const archetype = candidate.taste.signals.primaryExperienceArchetype;
    const softSupportArchetype = archetype === 'drinks' ||
        archetype === 'sweet' ||
        archetype === 'culture' ||
        archetype === 'social' ||
        candidate.venue.category === 'cafe' ||
...
function isRecoverableCentralMomentHighlightCandidate(candidate, intent) {
    ...
    if (candidate.taste.signals.momentIntensity.score < CENTRAL_MOMENT_MIN_INTENSITY ||
        candidate.fitScore < CENTRAL_MOMENT_MIN_QUALITY) {
        return false;
    }
    if (candidate.roleScores.peak < roleThresholds.peak - 0.11 ||
        candidate.stopShapeFit.highlight < 0.28) {
        return false;
    }
    const weakGenericCandidate = isGenericHospitalityHighlightCandidate(candidate) &&
    ...
    const coherentIdentity = candidate.taste.signals.anchorStrength >= 0.52 ||
...
function computeRomanticCenterpieceConviction(candidate) {
    ...
    return Math.max(0, Math.min(1, destinationFeel * 0.28 +
...
function computeRoleSelectionScore(params) {
    const peakBias = params.role === 'peak'
        ? getPeakMomentPriority(params.candidate, params.lens) +
            getPeakStrongMomentSelectionBias(params.candidate, params.intent, params.hasFeasibleStrongPeakMoment)
        : 0;
    const energyBias = params.role === 'warmup'
        ...
        : params.role === 'cooldown'
            ...
            : 0;
    const romanticBias = getRomanticMomentSelectionBias(params.candidate, params.role, params.lens, params.intent, params.hasFeasibleRomanticMoment);
    const stretchBias = getLocalStretchSelectionBias(params.candidate, params.role, params.intent, params.localSupplySufficient, params.strictNearbyFailed);
    const romanticSupportBias = getRomanticHighlightSupportSelectionBias(params.candidate, params.role, params.feasibleRomanticHighlights);
    return (computeRolePoolRankingScore(params.candidate, params.role, params.lens, params.intent) +
        peakBias +
        energyBias +
        romanticBias +
        romanticSupportBias +
...
function selectPeakCandidatesWithFamilyPreservation(rankedCandidates, selectionScoreByCandidateId, limit) {
    ...
    const availableFamilies = new Set(rankedCandidates.slice(0, Math.min(rankedCandidates.length, limit + 8)).map(getExperienceFamily));
    if (availableFamilies.size <= 1) {
        return initial;
    }
    const rankIndexByCandidateId = new Map(rankedCandidates.map((candidate, index) => [getScoredVenueCandidateId(candidate), index]));
    ...
    const alternateFamilyLeaders = [...familyLeaders.values()]
        .filter((candidate) => {
        if (initial.some((selected) => getScoredVenueCandidateId(selected) === getScoredVenueCandidateId(candidate))) {
            return false;
        }
        if (familyCounts.has(getExperienceFamily(candidate))) {
            return false;
        }
        const scoreGap = bestScore -
            (selectionScoreByCandidateId.get(getScoredVenueCandidateId(candidate)) ?? 0);
        return (scoreGap <= 0.18 &&
        ...
        .sort((left, right) => {
        return ((selectionScoreByCandidateId.get(getScoredVenueCandidateId(right)) ?? 0) -
    ...
    return selected.sort((left, right) => {
        return ((rankIndexByCandidateId.get(getScoredVenueCandidateId(left)) ?? 0) -
            (rankIndexByCandidateId.get(getScoredVenueCandidateId(right)) ?? 0));
    });
}
function getPeakStrongMomentSelectionBias(candidate, intent, hasFeasibleStrongPeakMoment) {
    if (isFeasibleStrongPeakMomentCandidate(candidate, intent)) {
        ...
        const experientialArchetypeBoost = cozyRomanticMode
    ...
    const weakMomentPenalty = candidate.momentIdentity.strength === 'medium'
        ? candidate.momentIdentity.type === 'anchor' ||
            candidate.momentIdentity.type === 'explore'
            ? 0.04
            : 0.08
        : 0.12;
    const passiveFallbackPenalty = passiveHospitalityFallback
        ? candidate.taste.signals.momentPotential.score < 0.62
            ? 0.14
            : 0.08
        : 0;
    const lowIntensityPenalty = candidate.taste.signals.momentIntensity.tier === 'standard'
        ? 0.05
...
function getRomanticMomentSelectionBias(candidate, role, lens, intent, hasFeasibleRomanticMoment) {
    ...
    const archetype = candidate.taste.signals.primaryExperienceArchetype;
    const candidateIsRomantic = role === 'peak'
                ...
                : archetype === 'activity' || archetype === 'scenic'
                    ...
                    : archetype === 'outdoor' || archetype === 'culture'
                        ...
                        : archetype === 'social'
                            ? 0.02
                            : 0;
            const convictionBias = cozyRomanticMode
                ? conviction >= 0.66
    ...
    const lowMomentPenalty = candidate.taste.signals.momentPotential.score < 0.64
        ? 0.05
    ...
    const lowIntensityPenalty = candidate.taste.signals.momentIntensity.tier === 'standard'
        ...
        : candidate.taste.signals.momentIntensity.tier === 'strong'
            ? 0.02
            : 0;
    const weakPeakMomentPenalty = candidate.momentIdentity.type === 'anchor' || candidate.momentIdentity.type === 'explore'
        ? candidate.momentIdentity.strength === 'strong'
            ? 0
            : 0.03
        : 0.06;
...
function getWarmupEnergySelectionBias(candidate, hasFeasibleStrongPeakMoment) {
    ...
    const archetype = candidate.taste.signals.primaryExperienceArchetype;
    const softClosingMoment = (momentIdentity.type === 'close' || momentIdentity.type === 'linger') &&
        momentIdentity.strength !== 'strong';
    const arrivalExploreMoment = momentIdentity.type === 'arrival' || momentIdentity.type === 'explore';
    const interactiveLight = archetype === 'activity' &&
        candidate.venue.energyLevel <= 3 &&
        momentIdentity.strength !== 'strong';
    const socialEntry = archetype === 'social' &&
        (momentIdentity.type === 'arrival' ||
            momentIdentity.type === 'explore' ||
            momentIdentity.type === 'transition');
    const casualActivity = (candidate.venue.category === 'activity' || archetype === 'activity') &&
        candidate.venue.energyLevel <= 3;
    return ((arrivalExploreMoment ? 0.13 : 0) +
...
function getCooldownEnergySelectionBias(candidate) {
    const momentIdentity = candidate.momentIdentity;
    const closeLingerBoost = momentIdentity.type === 'close'
        ? 0.11
        : momentIdentity.type === 'linger'
            ? 0.08
            : 0;
    const secondPeakPenalty = (momentIdentity.type === 'anchor' || momentIdentity.type === 'explore') &&
        momentIdentity.strength === 'strong'
        ? 0.16
...
function getPeakMomentPriority(candidate, lens) {
    ...
    const archetype = candidate.taste.signals.primaryExperienceArchetype;
    const experientialArchetypeBoost = archetype === 'activity' || archetype === 'scenic'
        ? 0.24
        : archetype === 'outdoor' || archetype === 'culture'
            ? 0.2
            : archetype === 'social'
                ? 0.12
                : 0;
    const passiveHospitalityPenalty = archetype === 'dining' || archetype === 'drinks' || archetype === 'sweet'
        ? 0.18
        : 0;
    return (candidate.taste.signals.momentPotential.score * 0.58 +
                        ...
                        : momentIdentity.type === 'anchor'
                            ...
                            : momentIdentity.type === 'linger'
                                ...
                                : momentIdentity.type === 'arrival'
                                    ? 0.48
                                    : 0.34;
    const strengthAdjustment = role === 'warmup'
...
function isPreferredRoleHoursInfeasible(candidate, role) {
    if (candidate.venue.source.businessStatus === 'temporarily-closed' ||
    ...
    if (!candidate.venue.source.likelyOpenForCurrentWindow &&
...
function isPreferredRoleContextIncompatible(candidate, role, crewPolicy) {
    ...
    if (vetoReason.includes('closed for the current planning window') ||
        vetoReason.includes('temporarily-closed') ||
        vetoReason.includes('closed-permanently')) {
        return false;
    }
    return (candidate.highlightValidity.personaVetoes.length > 0 ||
        candidate.highlightValidity.contextVetoes.length > 0);
}
...
function isPreferredRoleSeverelyIncompatible(candidate, role) {
    ...
    if (role !== 'peak') {
        return (candidate.roleContract[role].strength === 'hard' &&
...
function evaluatePreferredRoleAdmission(candidate, role, lensRole, crewPolicy, intent) {
    ...
    if (!isPreferredRoleCandidateFeasible(candidate, role, lensRole, crewPolicy) ||
...
function computeContractRolePressure(params) {
    ...
    const nightlifeLike = energy * 0.45 +
        socialDensity * 0.4 +
        ((category === 'bar' || category === 'live_music' || tags.has('late-night')) ? 0.15 : 0);
    const calmness = (1 - energy) * 0.35 +
        (1 - socialDensity) * 0.2 +
        intimacy * 0.2 +
        linger * 0.15 +
        signals.conversationFriendliness * 0.1;
    const quickStopLeaning = signals.durationEstimate === 'quick' ||
    ...
    if (role === 'warmup') {
        ...
        if (experienceContract.persona === 'romantic' &&
...
function pickRoleCandidates(scoredVenues, role, crewPolicy, lens, intent, roleContracts, strictShapeEnabled = false, contractConstraints, experienceContract) {
    ...
    const strictCandidates = scoredVenues.filter((item) => isBaseRoleCandidate(item, role, lensRole, crewPolicy, roleThresholds[role] - refinementTightening, 0.36, 0.4, 0.45, 3));
    const strictValidHighlightCandidates = role === 'peak'
        ? strictCandidates.filter((item) => item.highlightValidity.validityLevel === 'valid')
        : [];
    const strictFallbackHighlightCandidates = role === 'peak'
        ? strictCandidates.filter((item) => item.highlightValidity.validityLevel === 'fallback')
        : [];
    const strictInvalidHighlightCandidates = role === 'peak'
    ...
    const contractStrictCandidates = validityScopedStrictCandidates.filter((item) => item.roleContract[role].satisfied);
    let fallbackReason;
    ...
    const relaxedCandidates = workingStrictCandidates.length >= 6
        ? workingStrictCandidates
        : scoredVenues.filter((item) => isBaseRoleCandidate(item, role, lensRole, crewPolicy, roleThresholds[role] - 0.08, 0.3, 0.32, 0.36, 4));
    const validityScopedRelaxedCandidates = role === 'peak'
        ? (() => {
            ...
            const nonInvalidCandidates = relaxedCandidates.filter((item) => item.highlightValidity.validityLevel !== 'invalid');
            return nonInvalidCandidates.length > 0 ? nonInvalidCandidates : relaxedCandidates;
        })()
        : relaxedCandidates;
    const contractAwareRelaxedCandidates = enforceContract && !contractRelaxed
        ? validityScopedRelaxedCandidates.filter((item) => item.roleContract[role].satisfied)
        : validityScopedRelaxedCandidates;
    const expandedCandidates = contractAwareRelaxedCandidates.length >= 4
        ? contractAwareRelaxedCandidates
        : scoredVenues.filter((item) => isBaseRoleCandidate(item, role, lensRole, crewPolicy, roleThresholds[role] - 0.13, 0.24, 0.24, 0.28, 4));
    const validityScopedExpandedCandidates = role === 'peak'
        ? (() => {
            ...
            const nonInvalidCandidates = expandedCandidates.filter((item) => item.highlightValidity.validityLevel !== 'invalid');
            return nonInvalidCandidates.length > 0 ? nonInvalidCandidates : expandedCandidates;
        })()
        : expandedCandidates;
    const contractAwareExpandedCandidates = enforceContract && !contractRelaxed
    ...
    const preferredCandidate = preferredVenueId
        ...
        : undefined;
    const preferredAdmission = evaluatePreferredRoleAdmission(preferredCandidate, role, lensRole, crewPolicy, intent);
    const hasFeasibleStrongPeakMoment = scoredVenues.some((item) => isFeasibleStrongPeakMomentCandidate(item, intent));
    const hasFeasibleRomanticMoment = scoredVenues.some((item) => isFeasibleRomanticMomentPoolCandidate(item, intent));
    const feasibleRomanticHighlights = scoredVenues.filter((item) => isFeasibleRomanticHighlightPoolCandidate(item, lens, intent));
    const personaContractRequiresRomanticHighlight = role === 'peak' &&
        requiresRomanticPersonaMoment(lens) &&
        feasibleRomanticHighlights.length > 0;
    const warmupNeedsRomanticSupportRelaxation = role === 'warmup' &&
        isRomanticPersonaContractActive(lens) &&
        feasibleRomanticHighlights.length > 0 &&
        !contractAwareExpandedCandidates.some((item) => item.venue.energyLevel <=
    ...
    let roleCandidates = warmupNeedsRomanticSupportRelaxation
        ? [
            ...new Map([...contractAwareExpandedCandidates, ...validityScopedExpandedCandidates].map((item) => [
    ...
    if (personaContractRequiresRomanticHighlight) {
        const romanticHighlightCandidates = [
            ...new Map(roleCandidates
                .filter((item) => isFeasibleRomanticHighlightPoolCandidate(item, lens, intent))
                .map((item) => [getScoredVenueCandidateId(item), item])).values(),
        ];
    ...
    if (role === 'peak') {
        const scopedByMomentTier = scopeRomanticHighlightCandidatesByMomentTier(roleCandidates, lens);
    ...
    const localSupplySufficient = intent
        ? scoredVenues.some((item) => isPeakHighlightFeasibleCandidate(item, intent) &&
        ...
        : false;
    const strictNearbyFailed = !localSupplySufficient &&
        ...
        const recoveredCentralMomentCandidates = scoredVenues
            ...
            return {
                candidate: {
                    ...
                    centralMomentRecoveryReason: familyAdjustment > 0
            ...
            return getScoredVenueCandidateId(left.candidate).localeCompare(getScoredVenueCandidateId(right.candidate));
        })
    ...
    const scopedPeakCandidateIds = role === 'peak'
        ? new Set(roleCandidates.map((candidate) => getScoredVenueCandidateId(candidate)))
        : undefined;
    const allowPreferredAdmission = !preferredAdmission.admittedCandidate ||
        role !== 'peak' ||
        !scopedPeakCandidateIds ||
        scopedPeakCandidateIds.has(getScoredVenueCandidateId(preferredAdmission.admittedCandidate)) ||
        isAnchorCandidateForRole(preferredAdmission.admittedCandidate, role, intent);
    const scoredRoleCandidates = allowPreferredAdmission &&
        preferredAdmission.admittedCandidate &&
        !roleCandidates.some((candidate) => getScoredVenueCandidateId(candidate) ===
            getScoredVenueCandidateId(preferredAdmission.admittedCandidate))
        ? [...roleCandidates, preferredAdmission.admittedCandidate]
        : roleCandidates;
    const contractPressureByCandidateId = new Map(scoredRoleCandidates.map((candidate) => [
    ...
    const hardRejectedCandidates = roleCandidates.filter((candidate) => contractPressureByCandidateId.get(getScoredVenueCandidateId(candidate))?.hardReject);
    if (hardRejectedCandidates.length > 0) {
        const contractEligibleCandidates = roleCandidates.filter((candidate) => !contractPressureByCandidateId.get(getScoredVenueCandidateId(candidate))?.hardReject);
    ...
    const selectionScoreByCandidateId = new Map(scoredRoleCandidates.map((candidate) => [
        getScoredVenueCandidateId(candidate),
    ...
    const rankedWithPreference = allowPreferredAdmission && preferredAdmission.admittedCandidate
        ? [
            ...
            ...ranked.filter((item) => getScoredVenueCandidateId(item) !==
                getScoredVenueCandidateId(preferredAdmission.admittedCandidate)),
        ]
        : ranked;
    const limitedRanked = role === 'peak'
        ? selectPeakCandidatesWithFamilyPreservation(rankedWithPreference, selectionScoreByCandidateId, cooldownBoosted ? 16 : 14)
    ...
    const bestContractCandidateId = [...contractStrictCandidates].sort((left, right) => right.roleScores[role] - left.roleScores[role])[0]?.venue.id;
    const bestValidHighlightCandidateId = role === 'peak'
        ? [...expandedCandidates]
        ...
        : undefined;
    const bestValidHighlightChallengerId = role === 'peak'
        ? [...expandedCandidates]
            .filter((item) => item.highlightValidity.validityLevel === 'valid')
    ...
    return {
        ...
        status: {
            ...
            contractSatisfied: roleContract.strength === 'none'
                ? true
            ...
            recoveredCentralMomentHighlight: role === 'peak' ? usedRecoveredCentralMomentHighlight : undefined,
            recoveredHighlightCandidatesCount: role === 'peak' ? recoveredHighlightCandidatesCount : undefined,
            centralMomentRecoveryReason: role === 'peak' ? centralMomentRecoveryReason : undefined,
        },
    };
}
export function buildRolePools(scoredVenues, crewPolicy, lens, intent, roleContracts, strictShapeEnabled = false, contractConstraints, experienceContract) {
    const warmup = pickRoleCandidates(scoredVenues, 'warmup', crewPolicy, lens, intent, roleContracts, strictShapeEnabled, contractConstraints, experienceContract);
    const peak = pickRoleCandidates(scoredVenues, 'peak', crewPolicy, lens, intent, roleContracts, strictShapeEnabled, contractConstraints, experienceContract);
    const wildcard = pickRoleCandidates(scoredVenues, 'wildcard', crewPolicy, lens, intent, roleContracts, strictShapeEnabled, contractConstraints, experienceContract);
    const cooldown = pickRoleCandidates(scoredVenues, 'cooldown', crewPolicy, lens, intent, roleContracts, strictShapeEnabled, contractConstraints, experienceContract);
    return {
