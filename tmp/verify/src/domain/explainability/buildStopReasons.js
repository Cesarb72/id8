...
import { getArcStopBaseVenueId, getScoredVenueBaseVenueId, getScoredVenueCandidateId, } from '../candidates/candidateIdentity';
...
function roleCandidateScore(candidate, role) {
    const lensStop = roleToLensStop(role);
    const highlightValidityLift = role === 'peak'
...
function getFallbackLabel(confidence, fallbackUsed, rolePoolSize, strongCandidateCount) {
    if (fallbackUsed) {
...
function getTopConfidenceBand(selectedConfidence, rolePoolSize, strongCandidateCount) {
...
function getWeakPoolReason(rolePoolSize, strongCandidateCount, fallbackUsed, topConfidenceBand) {
    if (fallbackUsed) {
...
function getPreferredCandidateStillLostReason(role, preferredVenueId, preferredCandidateSurvivedToArc, arcCandidates, selectedArc) {
    if (!preferredVenueId) {
        return undefined;
    }
    ...
    const selectedUsesPreferred = selectedArc.stops.some((stop) => stop.role === role && stop.scoredVenue.venue.id === preferredVenueId);
    if (selectedUsesPreferred || preferredCandidateSurvivedToArc !== true) {
        return undefined;
    }
    const preferredArc = [...arcCandidates]
        .filter((candidate) => candidate.stops.some((stop) => stop.role === role && stop.scoredVenue.venue.id === preferredVenueId))
    ...
    const pacingDelta = (selectedArc.scoreBreakdown.durationPacingScore ?? 0) -
        (preferredArc.scoreBreakdown.durationPacingScore ?? 0);
    const transitionDelta = (selectedArc.scoreBreakdown.transitionSmoothnessScore ?? 0) -
    ...
    const categoryPenaltyDelta = (preferredArc.scoreBreakdown.categoryDiversityPenalty ?? 0) -
    ...
    const highlightValidityDelta = (selectedArc.scoreBreakdown.highlightValidityScore ?? 0) -
...
function getStarterPackImpact(starterPack, role, venue) {
    if (!starterPack) {
        return [];
    }
    const impact = [];
    ...
    if (preset) {
        ...
        if (preset.preferredTags?.some((tag) => normalizedTags.has(tag.toLowerCase()))) {
            impact.push('Starter pack matched preferred local tags');
        }
        if (roleShape?.preferredCategories?.includes(venue.venue.category)) {
            impact.push(`Starter pack preferred ${formatRoleTitle(role).toLowerCase()} ${formatCategory(venue.venue.category)}`);
        }
...
function buildSelectedBecause(fallbackUsed, roleFit, vibeMatch, personaMatch, proximityMatch, lensCompatibility, starterPackImpact, refinementImpact) {
    if (fallbackUsed) {
...
function getMaterialRefinementImpact(role, venue, modes, stopDelta) {
...
export function buildStopReasons({ selectedArc, arcCandidates, scoredVenues, rolePools, intent, lens, starterPack, baselineArc, previousStopExplainability, refinementPathContext, }) {
    const rolePoolDiagnostics = {};
    const stopExplainability = {};
    const starterPackInfluenceSummary = new Set();
    ...
    for (const [index, stop] of selectedArc.stops.entries()) {
        ...
        const rankedForRole = [...scoredVenues].sort((left, right) => roleCandidateScore(right, role) - roleCandidateScore(left, role));
        const selectedScore = roleCandidateScore(selected, role);
        const selectedRank = rankedForRole.findIndex((item) => getScoredVenueCandidateId(item) === getScoredVenueCandidateId(selected)) + 1;
        const runnerUp = rankedForRole.find((item) => getScoredVenueCandidateId(item) !== getScoredVenueCandidateId(selected));
        const runnerUpScore = runnerUp ? roleCandidateScore(runnerUp, role) : undefined;
        const rolePoolSize = rolePool.length;
        const strongCandidateCount = rolePool.filter((item) => item.roleScores[role] >= 0.72 && item.lensCompatibility >= 0.52).length;
        const categoryDistribution = rolePool.reduce((acc, item) => {
            const category = item.venue.category;
            acc[category] = (acc[category] ?? 0) + 1;
            return acc;
        }, {});
        const familyDistribution = rolePool.reduce((acc, item) => {
            const family = getExperienceFamily(item);
            acc[family] = (acc[family] ?? 0) + 1;
            return acc;
        }, {});
        const categoryDiversityCount = Object.keys(categoryDistribution).length;
        const familyEntries = Object.entries(familyDistribution).sort((left, right) => (right[1] ?? 0) - (left[1] ?? 0) || left[0].localeCompare(right[0]));
        const dominantFamilyEntry = familyEntries[0];
        const familyDiversityCount = familyEntries.length;
        const fallbackUsed = !rolePool.some((item) => getScoredVenueCandidateId(item) === getScoredVenueCandidateId(selected));
        ...
        const weakPoolReason = getWeakPoolReason(rolePoolSize, strongCandidateCount, fallbackUsed, topConfidenceBand);
        ...
        const preferredCandidateSurvivedToArc = contractPool.preferredDiscoveryVenueAdmitted && contractPool.preferredDiscoveryVenueId
            ? arcCandidates.some((candidate) => candidate.stops.some((arcStop) => arcStop.role === role &&
                getArcStopBaseVenueId(arcStop) === contractPool.preferredDiscoveryVenueId))
            : undefined;
        const preferredCandidateDroppedPreAssembly = typeof preferredCandidateSurvivedToArc === 'boolean'
            ? !preferredCandidateSurvivedToArc
            : undefined;
        const preferredContractOverrideApplied = contractPool.preferredDiscoveryVenueAdmitted === true &&
            preferredCandidateSurvivedToArc === true;
        const selectedContractOverrideApplied = preferredContractOverrideApplied &&
        ...
        const selectedCandidateStillLostReason = getPreferredCandidateStillLostReason(role, contractPool.preferredDiscoveryVenueId, preferredCandidateSurvivedToArc, arcCandidates, selectedArc);
        ...
        const selectedViolatesContract = selectedContractOverrideApplied
            ? false
        ...
        const contractFallbackReason = selectedContractOverrideApplied
            ? undefined
            : contractPool.fallbackReason ??
                (selectedViolatesContract
                    ...
                    : undefined);
        const highlightFallbackReason = role === 'peak' && contractPool.fallbackUsedBecauseNoValidHighlight
        ...
        const rejectedAlternatives = rankedForRole
            .filter((item) => getScoredVenueCandidateId(item) !== getScoredVenueCandidateId(selected))
            .slice(0, 6)
            .map((candidate) => ({
        ...
        rolePoolDiagnostics[userRole] = {
            ...
            dominantFamilyShare: dominantFamilyEntry && rolePoolSize > 0
            ...
            runnerUpScore: typeof runnerUpScore === 'number'
            ...
            selectedHighlightValidityLevel: role === 'peak' ? selected.highlightValidity.validityLevel : undefined,
            selectedHighlightValidForIntent: role === 'peak' ? selected.highlightValidity.validForIntent : undefined,
            selectedHighlightIsFallback: role === 'peak' ? selected.highlightValidity.validityLevel === 'fallback' : undefined,
            selectedHighlightViolatesIntent: role === 'peak' ? selected.highlightValidity.validityLevel === 'invalid' : undefined,
            selectedHighlightVetoReason: role === 'peak' ? selected.highlightValidity.vetoReason : undefined,
            packLiteralRequirementSatisfied: role === 'peak' ? selected.highlightValidity.packLiteralRequirementSatisfied : undefined,
        };
        stopExplainability[userRole] = {
            ...
            selectedBecause: buildSelectedBecause(fallbackUsed, selected.roleScores[role], selected.fitBreakdown.anchorFit, selected.fitBreakdown.crewFit, selected.fitBreakdown.proximityFit, selected.lensCompatibility, starterPackImpact, []),
            rejectionReasons,
            refinementImpact: [],
            starterPackImpact,
            ...
            tasteRoleSuitabilityContribution: Number((tasteInfluence.roleSuitabilityContribution * 100).toFixed(1)),
            tasteHighlightPlausibilityBonus: role === 'peak'
            ...
            highlightValidForIntent: role === 'peak' ? selected.highlightValidity.validForIntent : undefined,
            highlightValidityLevel: role === 'peak' ? selected.highlightValidity.validityLevel : undefined,
            highlightCandidateTier: role === 'peak' ? selected.highlightValidity.candidateTier : undefined,
            highlightVetoReason: role === 'peak' ? selected.highlightValidity.vetoReason : undefined,
            packLiteralRequirementSatisfied: role === 'peak' ? selected.highlightValidity.packLiteralRequirementSatisfied : undefined,
            fallbackUsedBecauseNoValidHighlight: role === 'peak' ? contractPool.fallbackUsedBecauseNoValidHighlight : undefined,
            bestValidHighlightChallenger: role === 'peak' ? contractPool.bestValidHighlightChallengerId : undefined,
            selectedHighlightViolatesIntent: role === 'peak' ? selected.highlightValidity.validityLevel === 'invalid' : undefined,
            selectedHighlightIsFallback: role === 'peak' ? selected.highlightValidity.validityLevel === 'fallback' : undefined,
            highlightMatchedSignals: role === 'peak' ? selected.highlightValidity.matchedSignals : undefined,
            highlightViolationReasons: role === 'peak'
                ? [
                ...
                : undefined,
            supportStopVibeFit: role === 'warmup' || role === 'cooldown'
                ? Number(((supportFit?.score ?? 0) * 100).toFixed(1))
                : role === 'peak'
                    ? Number((supportStopVibeFit.overall * 100).toFixed(1))
                    : undefined,
            supportStopVibeEffect: role === 'warmup' || role === 'cooldown'
                ? supportFit?.effect
                : role === 'peak'
                    ? supportStopVibeFit.effect
                    : undefined,
            supportStopVibeNotes: role === 'warmup' || role === 'cooldown'
                ? supportFit?.notes
                : role === 'peak'
                    ? supportStopVibeFit.notes
                    : undefined,
            routeShapeBiasApplied: role === 'peak' ? routeShapeBias.appliedLabel : undefined,
            outdoorVsUrbanRead: role === 'peak' ? selected.vibeAuthority.adventureRead : undefined,
            outdoorVsUrbanNotes: role === 'peak' ? selected.vibeAuthority.adventureNotes : undefined,
        };
    }
    ...
    if (refinementOutcome) {
        for (const delta of refinementOutcome.stopDeltas) {
            ...
            const refinementImpact = getMaterialRefinementImpact(internalRole, selectedStop.scoredVenue, refinementOutcome.requestedModes, delta);
            ...
            explainability.selectedBecause = buildSelectedBecause(explainability.fallbackUsed, selectedStop.scoredVenue.roleScores[internalRole], selectedStop.scoredVenue.fitBreakdown.anchorFit, selectedStop.scoredVenue.fitBreakdown.crewFit, selectedStop.scoredVenue.fitBreakdown.proximityFit, selectedStop.scoredVenue.lensCompatibility, explainability.starterPackImpact, refinementImpact);
