...
function collectNormalizedTokens(venue) {
    const summary = `${venue.shortDescription} ${venue.narrativeFlavor} ${venue.subcategory}`
        .toLowerCase()
        .replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, ' ');
    return new Set([...venue.tags, ...venue.vibeTags, ...venue.source.sourceTypes, ...summary.split(/\s+/)]
        .map(normalizeToken)
        .filter(Boolean));
}
...
export function deriveVenueHappeningsSignals(venue) {
    const tokens = collectNormalizedTokens(venue);
    const eventLikeSignal = (venue.settings.eventCapable ? 0.14 : 0) +
    ...
    const currentOpenSignal = venue.source.openNow
        ...
        : venue.source.likelyOpenForCurrentWindow
            ? 0.08
            : 0;
    const highQualitySignal = venue.signature.signatureScore * 0.08 +
        venue.source.qualityScore * 0.06 +
        venue.source.sourceConfidence * 0.05;
    const hotspotStrength = toFixed01(getCategoryBase(venue, {
        bar: 0.68,
        live_music: 0.72,
        event: 0.66,
        restaurant: 0.56,
        ...
        highQualitySignal);
    const eventPotential = toFixed01(getCategoryBase(venue, {
        event: 0.82,
        live_music: 0.72,
        activity: 0.5,
        museum: 0.42,
    ...
    const performancePotential = toFixed01(getCategoryBase(venue, {
        live_music: 0.84,
        event: 0.64,
        museum: 0.46,
        activity: 0.44,
        ...
        (hasAnyToken(tokens, [
            ...
            ? 0.12
            : 0));
    const liveNightlifePotential = toFixed01(getCategoryBase(venue, {
        bar: 0.84,
        live_music: 0.78,
        event: 0.58,
        restaurant: 0.36,
        ...
        (hasAnyToken(tokens, [
            ...
            : 0) +
        currentOpenSignal);
    const culturalAnchorPotential = toFixed01(getCategoryBase(venue, {
        museum: 0.82,
        event: 0.58,
        activity: 0.56,
        live_music: 0.5,
        ...
        (hasAnyToken(tokens, [
            ...
            ? 0.16
            : 0));
    const lateNightPotential = toFixed01(getCategoryBase(venue, {
        bar: 0.74,
        live_music: 0.66,
        event: 0.58,
        restaurant: 0.38,
    ...
    const currentRelevance = toFixed01(0.22 +
    ...
    const hiddenGemStrength = toFixed01((venue.isHiddenGem ? 0.48 : 0.16) +
        venue.underexposureScore * 0.22 +
        venue.distinctivenessScore * 0.14 +
        venue.localSignals.localFavoriteScore * 0.1);
    const majorVenueStrength = toFixed01(getCategoryBase(venue, {
        museum: 0.46,
        event: 0.44,
        live_music: 0.4,
    }, 0.22) +
