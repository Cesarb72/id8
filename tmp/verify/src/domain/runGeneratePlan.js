...
function scoreFallbackSupportReadability(support, peak, role) {
    const roleScore = role === 'warmup' ? support.roleScores.warmup : support.roleScores.cooldown;
    const shapeScore = role === 'warmup' ? support.stopShapeFit.start : support.stopShapeFit.windDown;
    const categoryContrast = support.venue.category === peak.venue.category ? -0.08 : 0.1;
    const archetypeContrast = support.taste.signals.primaryExperienceArchetype ===
        peak.taste.signals.primaryExperienceArchetype
        ? -0.07
        : 0.09;
    const energyContrast = role === 'warmup'
        ? Math.max(0, (peak.venue.energyLevel - support.venue.energyLevel + 1) / 4)
        : Math.max(0, (support.venue.energyLevel <= peak.venue.energyLevel
            ? peak.venue.energyLevel - support.venue.energyLevel + 1
            : 0) / 4);
    return (roleScore * 0.48 +
        shapeScore * 0.22 +
        energyContrast * 0.14 +
        categoryContrast +
        archetypeContrast);
...
function buildFallbackArcTieKey(stops) {
    return stops
        .map((stop) => `${stop.role}:${stop.scoredVenue.venue.id}`)
        .join('|');
}
function compareFallbackArcOptions(left, right) {
...
function chooseFallbackSupports(scoredVenues, peak, role, limit = FALLBACK_SUPPORT_CANDIDATE_LIMIT) {
    return scoredVenues
        .filter((candidate) => candidate.venue.id !== peak.venue.id)
        .sort((left, right) => {
        const scoreDelta = scoreFallbackSupportReadability(right, peak, role) -
...
function buildFallbackCandidate(scoredVenues, rolePools, intent, crewPolicy, lens) {
    const poolPeakCandidates = rolePools.peak.slice(0, FALLBACK_PEAK_CANDIDATE_LIMIT);
    const poolRecoveryCount = rolePools.contractPoolStatus.peak.recoveredHighlightCandidatesCount ?? 0;
    const poolUsedRecovery = Boolean(rolePools.contractPoolStatus.peak.recoveredCentralMomentHighlight);
    const poolRecoveryReason = rolePools.contractPoolStatus.peak.centralMomentRecoveryReason;
    const peaks = poolPeakCandidates.length > 0
        ? poolPeakCandidates
        : [...scoredVenues]
            .sort((left, right) => {
    ...
    if (peaks.length === 0) {
        const fallbackFailureReason = poolRecoveryCount > 0
    ...
    const addOption = (stops, fallbackType, partialReason) => {
        ...
        const supportingStopsCount = stops.filter((stop) => stop.role === 'warmup' || stop.role === 'cooldown').length;
        const viabilitySatisfied = stops.length === 1
        ...
        const supportReadability = fallbackType === 'single'
            ? 0
            : stops.reduce((acc, stop) => {
                if (stop.role === 'warmup' || stop.role === 'cooldown') {
                    return (acc +
                        scoreFallbackSupportReadability(stop.scoredVenue, highlight.scoredVenue, stop.role));
                }
                return acc;
            }, 0);
        options.push({
    ...
    for (const peak of peaks) {
        ...
        for (const cooldown of cooldowns) {
            ...
            addOption([
                { role: 'peak', scoredVenue: peak },
                { role: 'cooldown', scoredVenue: cooldown },
            ], 'partial', 'highlight_plus_wind_down');
        }
    ...
    const fallbackTrace = {
        ...
        usedRecoveredCentralMomentHighlight: poolUsedRecovery || peaks.some((candidate) => Boolean(candidate.recoveredCentralMomentHighlight)),
        recoveredHighlightCandidatesCount: poolRecoveryCount > 0
            ? poolRecoveryCount
    ...
    if (!bestFull && !bestPartial && !bestSingle) {
        const fallbackFailureReason = fallbackTrace.usedRecoveredCentralMomentHighlight &&
    ...
    const fullArcIsLikelyPadded = (bestFull?.score.scoreBreakdown.fakeCompletenessPenalty ?? 0) > 0 &&
        Boolean(bestPartial) &&
        (bestPartial?.score.totalScore ?? 0) >=
            (bestFull?.score.totalScore ?? 0) - PARTIAL_ARC_SELECTION_DELTA;
    const selected = !bestFull ? bestPartial ?? bestSingle ?? options.sort(compareFallbackArcOptions)[0] :
    ...
    return {
        candidate: {
            ...
            scoreBreakdown: {
                ...
                fakeCompletenessAvoided: selected.stops.length === 2 &&
                ...
                fullArcUnavailable,
                recoveredCentralMomentHighlight: Boolean(selectedHighlight?.recoveredCentralMomentHighlight),
                usedRecoveredCentralMomentHighlight: Boolean(fallbackTrace.usedRecoveredCentralMomentHighlight ||
                    selectedHighlight?.recoveredCentralMomentHighlight),
                recoveredHighlightCandidatesCount: fallbackTrace.recoveredHighlightCandidatesCount,
                centralMomentRecoveryReason: selectedHighlight?.centralMomentRecoveryReason ??
                    fallbackTrace.centralMomentRecoveryReason,
                partialArcChosenReason: selected.fallbackType === 'partial'
                    ? fullArcUnavailable
...
function resolveAnchorVenueIdFromRetrieval(intent, retrieval, seedVenues) {
    ...
    const dedupeResolvedVenueId = retrieval.sourceMode.dedupeLosses.find((loss) => loss.removedVenueId === anchorVenueId)?.keptVenueId;
    ...
    return retrieval.venues.find((venue) => {
        if (selectedAnchorVenue.source.providerRecordId &&
            venue.source.providerRecordId === selectedAnchorVenue.source.providerRecordId) {
            return true;
        }
        return (venue.category === selectedAnchorVenue.category &&
...
function resolvePlanningIntent(intent, retrieval, seedVenues) {
    const rawAnchorVenueId = intent.planningMode === 'user-led' ? intent.anchor?.venueId : undefined;
    const resolvedAnchorVenueId = resolveAnchorVenueIdFromRetrieval(intent, retrieval, seedVenues);
    if (!rawAnchorVenueId) {
        return {
            intent,
            rawAnchorVenueId,
    ...
    return {
        intent: {
            ...
            discoveryPreferences: intent.discoveryPreferences?.map((preference) => preference.venueId === rawAnchorVenueId
...
function countChangedStops(nextArc, baselineArc) {
...
function rehydrateBaselineArc(baselineArc, scoredVenues, intent, crewPolicy, lens) {
    const venueById = new Map(scoredVenues.map((item) => [item.venue.id, item]));
    const nextStops = [];
...
function countStarterPackMatches(scoredVenues, starterPack) {
...
function countRefinementMatches(scoredVenues, mode) {
    if (!mode) {
        return 0;
    }
    if (mode === 'more-unique') {
        return scoredVenues.filter((item) => item.hiddenGemScore >= 0.64 || item.venue.distinctivenessScore >= 0.62).length;
    }
    if (mode === 'closer-by') {
...
function countRoleShapeEligible(scoredVenues) {
    return scoredVenues.filter((item) => item.stopShapeFit.start >= 0.4 ||
        item.stopShapeFit.highlight >= 0.4 ||
        item.stopShapeFit.surprise >= 0.4 ||
        item.stopShapeFit.windDown >= 0.4).length;
}
...
function buildConstraintTrace(params) {
    ...
    trace.push({
        ...
        details: params.anchorAdmissionFailureReason === 'rejected_hours'
    ...
    trace.push({
        ...
        details: params.geographyViolationCount > 0
    ...
    if (params.anchorApplied) {
        const anchorConflict = params.temporalMode === 'explicit' &&
            params.anchorAdmissionFailureReason === 'rejected_hours';
        trace.push({
            type: 'anchor',
            priority: 'user',
            decision: anchorConflict
                ...
                : params.anchorSurvivedToArc === true
                    ...
                    : params.anchorAdmissionFailureReason || params.anchorDroppedReason || params.finalAnchorArcLossReason
                        ? 'failed'
                        : 'skipped',
            reason: params.finalAnchorArcLossReason ??
...
function arcSignature(candidate) {
    return candidate.stops
        .map((stop) => `${stop.role}:${stop.scoredVenue.venue.id}`)
        .join('|');
}
function categoryComposition(candidate) {
    return [...new Set(candidate.stops.map((stop) => stop.scoredVenue.venue.category))];
}
function buildArcSnapshot(candidate) {
    const stopIdsByRole = {};
    const stopCategoriesByRole = {};
...
function computeTopCandidateOverlapPct(candidates, limit = 8) {
...
function computeRoleWinnerFrequency(rankedCandidates, sampleSize = 20) {
    const sample = rankedCandidates.slice(0, sampleSize);
    const counts = {};
    for (const candidate of sample) {
        for (const stop of candidate.stops) {
            const role = roleProjection[stop.role];
            const roleMap = counts[role] ?? new Map();
...
function compareTasteDiagnostics(left, right) {
    return (left.highlightTier - right.highlightTier ||
...
function buildTasteDiagnostics(scoredVenues) {
    const interpreted = scoredVenues.map((item) => {
        const sourceType = item.venue.source.normalizedFromRawType === 'seed' ||
        ...
        return {
            ...
            conversationFriendliness: roundToHundredths(taste.conversationFriendliness),
            roleSuitability: {
...
function buildSurpriseSelectionReason(selectedArc, surpriseDiagnostics) {
    if (selectedArc.hasWildcard) {
        return (selectedArc.surpriseInjection?.selectionReason ??
    ...
    if (surpriseDiagnostics.rejectedBySpatialCount === 0 &&
    ...
    if (surpriseDiagnostics.rejectedBySpatialCount >=
    ...
    if (surpriseDiagnostics.rejectedByProbabilityCount >=
        surpriseDiagnostics.rejectedByScoreCount) {
        return `Strong surprise candidates existed, but the deterministic ${Math.round(surpriseDiagnostics.gateProbability * 100)}% injection gate held the plan at 3 stops.`;
    }
    if (typeof surpriseDiagnostics.bestRejectedScoreDelta === 'number' &&
...
function getBoundaryContributionLevel(changedWinner, changedOrderCount, averageRankDelta, candidateArcCount) {
...
async function simulateOverlapScenario(config, context) {
    ...
    const scenarioScored = scoreVenueCollection(scenarioRetrieval.venues, scenarioIntent, scenarioCrewPolicy, scenarioLens, scenarioRoleContracts, starterPack);
    const scenarioPools = buildRolePools(scenarioScored, scenarioCrewPolicy, scenarioLens, scenarioIntent, scenarioRoleContracts, context.strictShapeEnabled);
    const scenarioArcAssembly = assembleArcCandidates(scenarioScored, scenarioIntent, scenarioCrewPolicy, scenarioLens, scenarioPools);
...
export async function runGeneratePlan(input, options = {}) {
    // Boundary guardrail: interpretation and bearings artifacts should be passed as a canonical pair.
    console.assert((Boolean(options.experienceContract) && Boolean(options.contractConstraints)) ||
        (!options.experienceContract && !options.contractConstraints), '[ARC-BOUNDARY] runGeneratePlan expects experienceContract + contractConstraints together.');
    const intent = normalizeIntent(input);
    console.assert(!intent.selectedDirectionContext || Boolean(intent.selectedDirectionContext.directionId), '[ARC-BOUNDARY] selectedDirectionContext should carry a canonical directionId when present.');
    ...
    const anchorCanonicalization = resolvePlanningIntent(intent, retrieval, options.seedVenues);
    const planningIntent = anchorCanonicalization.intent;
    const anchorInjectionSource = anchorCanonicalization.rawAnchorVenueId
        ? options.seedVenues?.some((venue) => venue.id === anchorCanonicalization.rawAnchorVenueId)
            ? 'search'
            : options.seedVenues && options.seedVenues.length > 0
                ? 'session'
                : 'unknown'
        : undefined;
    const anchorInjectedToInventory = Boolean(anchorCanonicalization.rawAnchorVenueId &&
        anchorCanonicalization.canonicalAnchorVenueId);
    const anchorInjectionFailureReason = anchorCanonicalization.rawAnchorVenueId && !anchorInjectedToInventory
        ? 'anchor_not_present_after_inventory_assembly'
        : undefined;
    const rawScoredVenues = scoreVenueCollection(retrieval.venues, planningIntent, crewPolicy, lens, roleContracts, options.starterPack);
    ...
    const rolePools = buildRolePools(scoredVenues, crewPolicy, lens, planningIntent, roleContracts, strictShapeEnabled, options.contractConstraints, options.experienceContract);
    ...
    const anchorApplied = planningIntent.planningMode === 'user-led' && Boolean(planningIntent.anchor?.venueId);
    ...
    const anchorInternalRole = anchorRole === 'start' || anchorRole === 'highlight' || anchorRole === 'windDown'
        ? toInternalRole(anchorRole)
        : undefined;
    const anchorSurvivedToArc = anchorApplied && planningIntent.anchor?.venueId && anchorInternalRole
        ? arcCandidates.some((candidate) => candidate.stops.some((stop) => stop.role === anchorInternalRole &&
            stop.scoredVenue.venue.id === planningIntent.anchor.venueId))
        : undefined;
    const anchorRolePool = anchorInternalRole === 'warmup'
        ? rolePools.warmup
    ...
    const anchorRolePoolIndex = anchorApplied && planningIntent.anchor?.venueId && anchorRolePoolVenueIds
        ? anchorRolePoolVenueIds.indexOf(planningIntent.anchor.venueId)
        : undefined;
    const anchorRolePoolStatus = anchorApplied && anchorInternalRole
    ...
    const anchorHoursRelaxationReason = anchorRolePoolStatus?.preferredDiscoveryVenueHoursRelaxationReason;
    const anchorAdmittedToRolePool = anchorRolePoolStatus?.preferredDiscoveryVenueAdmitted;
    const anchorAdmissionFailureReason = anchorRolePoolStatus?.preferredDiscoveryVenueRejectedReason ??
        (anchorRolePoolStatus?.preferredDiscoveryVenueAdmitted === false
            ? 'anchor_not_scored'
            : undefined);
    const anchorDroppedReason = anchorApplied &&
        ...
        ? anchorRolePoolStatus?.preferredDiscoveryVenueRejectedReason ??
            (!(anchorInternalRole === 'warmup'
                ...
                : anchorInternalRole === 'peak'
                    ? rolePools.peak
                    : rolePools.cooldown).some((candidate) => candidate.venue.id === planningIntent.anchor.venueId)
                ? 'anchor_missing_from_role_pool'
                : 'anchor_trimmed_before_arc_assembly')
        : undefined;
    const survivingAnchorArcCount = anchorApplied && planningIntent.anchor?.venueId && anchorInternalRole
        ? arcCandidates.filter((candidate) => candidate.stops.some((stop) => stop.role === anchorInternalRole &&
            stop.scoredVenue.venue.id === planningIntent.anchor.venueId)).length
        : 0;
    const shouldApplyUserLedFinalRoleLock = planningIntent.planningMode === 'user-led' &&
        anchorRole === 'highlight' &&
        anchorInternalRole === 'peak' &&
        anchorSurvivedToArc === true;
    const anchorLockedArcCandidates = shouldApplyUserLedFinalRoleLock && planningIntent.anchor?.venueId
        ? arcCandidates.filter((candidate) => candidate.stops.some((stop) => stop.role === 'peak' &&
            stop.scoredVenue.venue.id === planningIntent.anchor.venueId))
        : arcCandidates;
    const finalArcFilteredToAnchorRole = shouldApplyUserLedFinalRoleLock && anchorLockedArcCandidates.length > 0;
    const userLedFinalRoleLockApplied = finalArcFilteredToAnchorRole;
    const userLedFinalRoleLockFallbackReason = shouldApplyUserLedFinalRoleLock && anchorLockedArcCandidates.length === 0
    ...
    const finalAnchorCandidates = anchorApplied && planningIntent.anchor?.venueId && anchorInternalRole
        ? rankedCandidates.filter((candidate) => candidate.stops.some((stop) => stop.role === anchorInternalRole &&
            stop.scoredVenue.venue.id === planningIntent.anchor.venueId))
        : [];
    const bestFinalAnchorArc = finalAnchorCandidates[0];
    const bestNonAnchorArc = anchorApplied && planningIntent.anchor?.venueId && anchorInternalRole
        ? rankedCandidates.find((candidate) => !candidate.stops.some((stop) => stop.role === anchorInternalRole &&
            stop.scoredVenue.venue.id === planningIntent.anchor.venueId))
        : undefined;
    const finalAnchorArcLossReason = anchorApplied && finalAnchorCandidates.length === 0
    ...
    const preBoundaryIndex = new Map(preBoundaryOrder.map((id, index) => [id, index]));
    ...
    const averageRankDelta = postBoundaryOrder.length > 0 ? roundToTenths(rankDeltaSum / postBoundaryOrder.length) : 0;
    const winnerBeforeBoundary = preBoundaryOrder[0];
    const winnerAfterBoundary = postBoundaryOrder[0];
    const changedWinner = Boolean(winnerBeforeBoundary && winnerAfterBoundary && winnerBeforeBoundary !== winnerAfterBoundary);
    const preBoundarySnapshot = boundaryCandidates
        .slice(0, 8)
        .map((candidate) => buildArcSnapshot(candidate));
    const postBoundarySnapshot = ranking.ranked
        .slice(0, 8)
        ...
        return {
            ...
            boundaryRefinementTokenDeltas: Object.fromEntries(Object.entries(rankedEntry.refinementTokenDeltas).map(([token, delta]) => [
                token,
    ...
    let fallbackTrace;
    const buildFallbackFailureError = (reason, trace) => new Error(`Fallback arc recovery failed (${reason}). full=${trace.fullArcCandidatesCount} partial=${trace.partialArcCandidatesCount} highlightOnly=${trace.highlightOnlyCandidatesCount}`);
    const selectFallbackArc = (params) => {
        const fallback = buildFallbackCandidate(scoredVenues, rolePools, planningIntent, crewPolicy, lens);
        fallbackTrace = {
            ...fallback.trace,
    ...
    let itinerary;
    try {
        itinerary = projectItinerary(selectedArc, planningIntent, lens, stopReasons.stopExplainability);
    }
    catch (error) {
        if (fallbackTrace) {
            fallbackTrace = {
            ...
            throw buildFallbackFailureError('final_plan_construction_rejected_arc', fallbackTrace);
        }
        throw error;
    }
    if (fallbackTrace && (fallbackTrace.selectedFallbackType === 'partial' || fallbackTrace.selectedFallbackType === 'single')) {
        ...
        if (droppedRole) {
            ...
            throw buildFallbackFailureError('partial_arc_selected_but_dropped_downstream', fallbackTrace);
        }
    }
    ...
    const overPruned = retrieval.stageCounts.finalRetrieved < 12 ||
    ...
    const surpriseInjection = {
        ...
        surpriseCandidateTierBreakdown: arcAssembly.surpriseDiagnostics.surpriseCandidateTierBreakdown,
        generatedSurpriseArcCount: arcAssembly.surpriseDiagnostics.generatedSurpriseArcCount,
        surpriseComparedTierBreakdown: arcAssembly.surpriseDiagnostics.comparedSurpriseArcTierBreakdown,
        surpriseGateProbability: arcAssembly.surpriseDiagnostics.gateProbability,
        surpriseSelectionReason: buildSurpriseSelectionReason(selectedArc, arcAssembly.surpriseDiagnostics),
        surpriseRejectedBySpatialCount: arcAssembly.surpriseDiagnostics.rejectedBySpatialCount,
        surpriseRejectedBySpatialTier2Count: arcAssembly.surpriseDiagnostics.rejectedBySpatialNearStrongCount,
        scoreDeltaVsBaseArc: selectedArc.surpriseInjection?.scoreDeltaFromBase ??
            arcAssembly.surpriseDiagnostics.bestRejectedScoreDelta,
        tradeoffThreshold: selectedArc.surpriseInjection?.allowedTradeoff ??
            arcAssembly.surpriseDiagnostics.bestRejectedAllowedTradeoff,
        tradeoffThresholdBlocked: !selectedSurpriseStop && arcAssembly.surpriseDiagnostics.rejectedByScoreCount > 0,
        rejectedByScoreCount: arcAssembly.surpriseDiagnostics.rejectedByScoreCount,
        surpriseAcceptanceBonusApplied: (selectedArc.surpriseInjection?.acceptanceBonusApplied ??
            ((arcAssembly.surpriseDiagnostics.bestRejectedAcceptanceBonusValue ?? 0) > 0)),
        surpriseAcceptanceBonusValue: selectedArc.surpriseInjection?.acceptanceBonusValue ??
        ...
        surpriseTasteSignals: selectedSurpriseStop
            ? {
                ...
                experientialFactor: roundToHundredths(selectedSurpriseStop.scoredVenue.taste.signals.experientialFactor),
                noveltyWeight: roundToHundredths(selectedSurpriseStop.scoredVenue.taste.signals.noveltyWeight),
                roleSuitability: roundToHundredths(selectedSurpriseStop.scoredVenue.taste.signals.roleSuitability.surprise),
                supportingSignals: selectedArc.surpriseInjection?.tasteSignals.supportingSignals ??
    ...
    const categoryDiversity = {
        ...
        categoryDiversityPenaltyApplied: (selectedArc.scoreBreakdown.categoryDiversityPenalty ?? 0) > 0,
        categoryDiversityPenalty: roundToHundredths(selectedArc.scoreBreakdown.categoryDiversityPenalty ?? 0),
    ...
    const boundaryDiagnostics = {
        ...
        finalProjectedMatchesPostBoundaryWinner: Boolean(winnerAfterBoundary && selectedArc.id === winnerAfterBoundary),
        changedWinner,
        ...
        refinementNudgeTrace: {
            ...
            greatStopQualityContext: ranking.qualityContext
                ? {
                    ...
                    suppressionRecommended: ranking.qualityContext.greatStopSuppressionRecommended,
                    penaltyHint: ranking.qualityContext.greatStopPenaltyHint,
                    reasonCodes: [...ranking.qualityContext.reasonCodes],
                }
                : undefined,
        },
        boundaryContributionLevel: getBoundaryContributionLevel(changedWinner, changedOrderCount, averageRankDelta, boundaryCandidates.length),
        preBoundarySnapshot,
    ...
    const faultIsolationNotes = [];
    if (fallbackTrace) {
        faultIsolationNotes.push(`Fallback trace: full=${fallbackTrace.fullArcCandidatesCount}, partial=${fallbackTrace.partialArcCandidatesCount}, highlight-only=${fallbackTrace.highlightOnlyCandidatesCount}, selected=${fallbackTrace.selectedFallbackType ?? 'none'}.`);
        if (fallbackTrace.primaryPathFailureReason) {
            faultIsolationNotes.push(`Fallback trigger: ${fallbackTrace.primaryPathFailureReason} at ${fallbackTrace.triggerStage ?? 'unknown_stage'}.`);
        }
        if (typeof fallbackTrace.selectedFallbackScore === 'number') {
            faultIsolationNotes.push(`Fallback selection quality: score=${fallbackTrace.selectedFallbackScore}, stops=${fallbackTrace.selectedFallbackStopCount ?? 'n/a'}.`);
        }
        if (fallbackTrace.usedRecoveredCentralMomentHighlight) {
            faultIsolationNotes.push(`Recovered central-moment highlights considered: ${fallbackTrace.recoveredHighlightCandidatesCount ?? 0}${fallbackTrace.centralMomentRecoveryReason ? ` (${fallbackTrace.centralMomentRecoveryReason})` : ''}.`);
        }
    ...
    if (userLedFinalRoleLockApplied) {
        faultIsolationNotes.push(`User-led final role lock restricted winner comparison to ${survivingAnchorArcCount} anchor-preserving highlight arcs.`);
    }
    if (userLedFinalRoleLockFallbackReason) {
        faultIsolationNotes.push(`User-led final role lock could not be applied: ${userLedFinalRoleLockFallbackReason}.`);
    }
    if (anchorApplied && anchorAdmittedToRolePool === false) {
        faultIsolationNotes.push(`Anchor admission failed before arc assembly: ${anchorAdmissionFailureReason ?? 'anchor_not_scored'}.`);
    }
    ...
    if (anchorHoursRelaxed) {
        faultIsolationNotes.push(`Anchor hours rejection was softened because ${anchorHoursRelaxationReason}.`);
    }
    if (temporalMode === 'explicit' && anchorAdmissionFailureReason === 'rejected_hours') {
        faultIsolationNotes.push('Anchor conflicted with an explicit time window, so hours remained a hard constraint.');
    }
    if (anchorGeographyRelaxed) {
        faultIsolationNotes.push(`Anchor-containing arcs kept geography as a soft penalty across ${geographyViolationCount} generated combinations.`);
    }
    if (anchorCanonicalization.rawAnchorVenueId && anchorInjectedToInventory) {
        faultIsolationNotes.push(`Anchor injected into planning inventory via ${anchorInjectionSource ?? 'unknown'} source.`);
    }
    ...
    if (anchorCanonicalization.anchorCanonicalized) {
        faultIsolationNotes.push(`Anchor canonicalized from ${anchorCanonicalization.rawAnchorVenueId} to ${anchorCanonicalization.canonicalAnchorVenueId}.`);
    }
    if (anchorCanonicalization.anchorCanonicalizationFailureReason) {
        faultIsolationNotes.push(`Anchor canonicalization failed: ${anchorCanonicalization.anchorCanonicalizationFailureReason}.`);
    }
    ...
    if (surpriseInjection.surpriseInjected) {
        faultIsolationNotes.push(`Surprise stop injected: ${surpriseInjection.surpriseSelectionReason}`);
    }
    ...
    if (categoryDiversity.categoryDiversityPenaltyApplied) {
        faultIsolationNotes.push(`Category diversity penalty applied: ${categoryDiversity.notes.join('; ')}.`);
    }
    if (primaryVibeMatchCount < 6) {
        faultIsolationNotes.push(`Sparse inventory for primary vibe "${intent.primaryAnchor}".`);
    }
    if (retrieval.stageCounts.qualitySuppressed > 0) {
        faultIsolationNotes.push(`${retrieval.stageCounts.qualitySuppressed} venues were excluded before retrieval by the quality gate.`);
    }
    ...
    if (retrieval.sourceMode.fallbackToCurated) {
        faultIsolationNotes.push(`Live source fallback used: ${retrieval.sourceMode.failureReason ?? 'live inventory did not stay strong enough after normalization and retrieval.'}`);
    }
    ...
    if (retrieval.sourceMode.liveHoursSuppressedCount > 0) {
        faultIsolationNotes.push(`${retrieval.sourceMode.liveHoursSuppressedCount} live venues were suppressed by hours/open-now pressure.`);
    }
    if (retrieval.sourceMode.liveHoursDemotedCount > 0) {
        faultIsolationNotes.push(`${retrieval.sourceMode.liveHoursDemotedCount} live venues were demoted by hours uncertainty or likely closure.`);
    }
    const highlightLeader = roleWinnerFrequency.highlight?.[0];
    if (highlightLeader && highlightLeader.winShare >= 55) {
        faultIsolationNotes.push(`Highlight role shows dominant winner tendency (${highlightLeader.winShare}% in top candidates).`);
    }
    ...
    if (highlightExplain && highlightExplain.dominancePenalty > 0) {
        faultIsolationNotes.push(`Generic winner penalty applied on Highlight (${highlightExplain.dominancePenalty}).`);
    }
    if (highlightExplain && highlightExplain.contextSpecificity >= 65) {
        faultIsolationNotes.push(`Context-specificity bonus influenced Highlight (${highlightExplain.contextSpecificity}).`);
    }
    if (highlightExplain?.highlightPressureSource) {
        faultIsolationNotes.push(`Highlight pressure source: ${highlightExplain.highlightPressureSource}.`);
    }
    ...
    if (highlightExplain?.highlightValidityLevel) {
        faultIsolationNotes.push(`Highlight validity level: ${highlightExplain.highlightValidityLevel}.`);
    }
    ...
    if (highlightExplain?.bestValidHighlightChallenger) {
        faultIsolationNotes.push(`Best valid Highlight challenger: ${highlightExplain.bestValidHighlightChallenger}.`);
    }
    if (highlightExplain?.musicSupportSource &&
        highlightExplain.musicSupportSource !== 'not-applicable') {
        faultIsolationNotes.push(`Music/performance support came from ${highlightExplain.musicSupportSource}.`);
    }
    ...
    if (selectedArc.pacing.pacingPenaltyApplied) {
        faultIsolationNotes.push(`Route pacing penalty applied: ${selectedArc.pacing.pacingPenaltyReasons.join('; ')}.`);
    }
    if (selectedArc.pacing.smoothProgressionRewardApplied) {
        faultIsolationNotes.push(`Smooth progression rewarded: ${selectedArc.pacing.smoothProgressionRewardReasons.join('; ')}.`);
    }
    if (selectedArc.pacing.totalRouteFriction >= 8) {
        faultIsolationNotes.push(`Route friction is high (${selectedArc.pacing.totalRouteFriction}) and may feel more effortful.`);
    }
    if (selectedArc.spatial.jumpUsed) {
        faultIsolationNotes.push(`Spatial model used one allowed destination jump in ${selectedArc.spatial.mode} mode.`);
    }
    ...
    if (selectedArc.spatial.score <= 0.55) {
        faultIsolationNotes.push(`Spatial coherence was weak (${selectedArc.spatial.score}) relative to the selected outing mode.`);
    }
    ...
    const retrievalDiagnostics = {
        ...
        liveSource: {
            ...
            selectedStopSources: Object.fromEntries(selectedArc.stops.map((stop) => [
        ...
        vibeInfluence: {
            ...
            selectedHighlightVibeFit: selectedArc.stops.find((stop) => stop.role === 'peak')
                ? roundToTenths((selectedArc.stops.find((stop) => stop.role === 'peak').scoredVenue.vibeAuthority.byRole.highlight ?? 0) *
                    100)
                : undefined,
            selectedHighlightPackPressure: selectedArc.stops.find((stop) => stop.role === 'peak')
                ? roundToTenths((selectedArc.stops.find((stop) => stop.role === 'peak').scoredVenue.vibeAuthority.packPressure.highlight ?? 0) *
                    100)
                : undefined,
    ...
    if (liveCompetitiveness.dedupeNoveltyLoss.liveDedupedAgainstCuratedCount > 0) {
        faultIsolationNotes.push(`${liveCompetitiveness.dedupeNoveltyLoss.liveDedupedAgainstCuratedCount} live venues were deduped against curated seeds before final competition.`);
    }
    const internalRoles = selectedArc.stops.map((stop) => stop.role);
    const alternativeCounts = {};
    const nearbyAlternativeCounts = {};
    ...
    if (options.debugMode) {
        ...
        const scenarioById = new Map(overlapScenarios.map((scenario) => [scenario.scenarioId, scenario]));
    ...
    const diagnostics = {
        ...
        anchorCanonicalizationFailureReason: anchorCanonicalization.anchorCanonicalizationFailureReason,
        anchorHoursRelaxed,
        anchorHoursRelaxationReason,
        anchorAdmittedToRolePool,
        anchorAdmissionFailureReason,
        anchorRolePoolSize: anchorRolePool?.length,
        anchorRolePoolIndex: typeof anchorRolePoolIndex === 'number' && anchorRolePoolIndex >= 0
            ? anchorRolePoolIndex
            : undefined,
        ...
        anchorSurvivedToArc,
        anchorDroppedReason,
        preValidationAnchorArcCount: arcAssembly.anchorTrace?.preValidationAnchorArcCount ?? 0,
        postValidationAnchorArcCount: arcAssembly.anchorTrace?.postValidationAnchorArcCount ?? 0,
        postPruneAnchorArcCount: arcAssembly.anchorTrace?.postPruneAnchorArcCount ?? 0,
        finalAnchorArcCount: finalAnchorCandidates.length,
        invalidatedAnchorArcCount: arcAssembly.anchorTrace?.invalidatedAnchorArcCount ?? 0,
        invalidatedAnchorArcReasons: arcAssembly.anchorTrace?.invalidatedAnchorArcReasons ?? {},
        anchorArcTrimmedByTopN: arcAssembly.anchorTrace?.anchorArcTrimmedByTopN ?? false,
        anchorArcTrimmedByWhichStage: arcAssembly.anchorTrace?.anchorArcTrimmedByWhichStage,
        bestPreValidationAnchorArcId: arcAssembly.anchorTrace?.bestPreValidationAnchorArcId,
        bestPreValidationAnchorArcScore: arcAssembly.anchorTrace?.bestPreValidationAnchorArcScore,
        bestValidatedAnchorArcId: arcAssembly.anchorTrace?.bestValidatedAnchorArcId,
        bestValidatedAnchorArcScore: arcAssembly.anchorTrace?.bestValidatedAnchorArcScore,
        bestPrunedAnchorArcId: arcAssembly.anchorTrace?.bestPrunedAnchorArcId,
        bestPrunedAnchorArcScore: arcAssembly.anchorTrace?.bestPrunedAnchorArcScore,
        bestFinalAnchorArcId: bestFinalAnchorArc?.id,
        bestFinalAnchorArcScore: typeof bestFinalAnchorArc?.totalScore === 'number'
            ? roundToHundredths(bestFinalAnchorArc.totalScore)
            : undefined,
        bestNonAnchorArcId: bestNonAnchorArc?.id,
        bestNonAnchorArcScore: typeof bestNonAnchorArc?.totalScore === 'number'
        ...
        preRankingAnchorRoleLockTrace: {
            ...
            lockedVenueId: shouldApplyUserLedFinalRoleLock && planningIntent.anchor?.venueId
                ? planningIntent.anchor.venueId
                : undefined,
            anchorApplied,
            anchorSurvivedToArc,
        ...
        routePacing: {
            transitions: selectedArc.pacing.transitions.map((transition) => ({
                fromRole: roleProjection[transition.fromRoleKey],
                toRole: roleProjection[transition.toRoleKey],
