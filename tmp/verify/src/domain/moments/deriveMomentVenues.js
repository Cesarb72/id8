...
const MOMENT_TYPE_BASE_CATEGORY = {
...
const MOMENT_TYPE_ROLE_AFFINITY = {
...
function computeTimeWindowFit(momentWindow, intentWindow) {
    ...
    if ((momentWindow === 'evening' && intentWindow === 'late') ||
...
function getDistrictDriveFallback(district, venues) {
    ...
    const matches = venues.filter((venue) => normalizeForMatch(venue.neighborhood) === districtKey ||
        normalizeForMatch(venue.neighborhood).includes(districtKey) ||
        districtKey.includes(normalizeForMatch(venue.neighborhood)));
    if (matches.length === 0) {
        return fallback;
    }
    const driveMinutes = Math.round(matches.reduce((sum, venue) => sum + venue.driveMinutes, 0) / matches.length);
    return {
...
function toMomentVenue(moment, venuePool) {
    ...
    const socialDensity = clamp01(moment.momentType === 'social_ritual' || moment.momentType === 'seasonal_activation'
        ? 0.58 + moment.energy * 0.36
        : 0.34 + moment.energy * 0.32);
    const roleAffinity = {
        warmup: clamp01(baseRoleAffinity.warmup +
            (moment.timeWindow === 'day' ? 0.08 : 0) +
            (moment.timeWindow === 'late' ? -0.06 : 0)),
        peak: clamp01(baseRoleAffinity.peak +
            moment.uniquenessScore * 0.08 +
            moment.romanticPotential * 0.05),
        wildcard: clamp01(baseRoleAffinity.wildcard +
            (moment.momentType === 'seasonal_activation' ? 0.05 : 0) +
            (moment.uniquenessScore - 0.6) * 0.06),
        cooldown: clamp01(baseRoleAffinity.cooldown +
...
function selectMomentsForIntent(params) {
    ...
    const ranked = curatedMoments
        ...
        const neighborhoodMatch = neighborhoodKey.length > 0 &&
        ...
        const distanceFit = params.intent.distanceMode === 'nearby'
            ...
            : driveMinutes <= 22
                ? 0.9
                : 0.62;
        const score = timeWindowFit * 0.34 +
            distanceFit * 0.24 +
...
export function deriveMomentVenueRecords(params) {
