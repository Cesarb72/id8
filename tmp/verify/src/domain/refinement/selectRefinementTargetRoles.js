...
export function selectRefinementTargetRoles({ directive, baselineArc, }) {
    ...
    const includeVisibleOnly = (roles) => uniqueRoles(roles).filter((role) => visibleRoles.includes(role));
    if (directive.mode === 'more-unique') {
        const hasSurprise = visibleRoles.includes('surprise');
        const preferred = hasSurprise
    ...
    if (directive.mode === 'more-exciting') {
        const hasSurprise = visibleRoles.includes('surprise');
        const roles = includeVisibleOnly(hasSurprise
            ? ['highlight', 'surprise', 'start', 'windDown']
            : ['highlight', 'start', 'windDown']).slice(0, 4);
        return {
            roles,
            primaryTargetRole: roles[0],
            targetRoleExistedInVisiblePlan: hasSurprise,
