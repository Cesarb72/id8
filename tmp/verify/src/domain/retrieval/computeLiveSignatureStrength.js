...
export function computeLiveSignatureStrength(raw, category) {
    ...
    const categorySpecificSignals = category === 'bar'
    ...
    const reviewSignal = (rating >= 4.6 ? 0.12 : rating >= 4.4 ? 0.08 : rating >= 4.2 ? 0.04 : 0) +
        (ratingCount >= 800 ? 0.1 : ratingCount >= 250 ? 0.07 : ratingCount >= 80 ? 0.04 : 0);
    const summarySignal = raw.shortDescription && raw.shortDescription.trim().length >= 36
        ? 0.06
