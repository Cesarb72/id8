...
export function scoreAnchorFit(venue, intent) {
    ...
    const scores = anchors.map((anchor, index) => {
        ...
        return clamp01((directAffinity * 0.62 + categoryPressure * 0.24 + exactVibeMatch * 0.14) *
            anchorStrength);
    });
    const weightedSum = scores.reduce((sum, score, index) => sum + score * (index === 0 ? 0.82 : 0.18), 0);
