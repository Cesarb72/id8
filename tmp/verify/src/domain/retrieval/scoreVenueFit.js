...
import { assessGenericHospitalityFallbackPenalty, getGenericHospitalityFallbackPenalty, getHighlightArchetypeLift, getMomentIntensityTierBoost, } from '../taste/experienceSignals';
...
function normalizeWeightedScore(scores, weights) {
    ...
    return ((scores.anchor * weights.anchor +
...
function computeRomanticCenterpieceConvictionScore(venue, signals) {
    const atmosphericDepth = Math.max(signals.romanticSignals.ambiance, signals.romanticSignals.ambientExperience, signals.momentEnrichment.ambientUniqueness);
    ...
    const memorability = Math.max(venue.uniquenessScore, venue.distinctivenessScore, signals.momentIntensity.score);
    const hiddenGemPull = Math.max(venue.underexposureScore, venue.localSignals.localFavoriteScore);
    const chefLedSignal = venue.category === 'restaurant' &&
    ...
    return clamp01(destinationFeel * 0.28 +
                        ...
                        : momentIdentity.type === 'anchor'
                            ...
                            : momentIdentity.type === 'linger'
                                ...
                                : momentIdentity.type === 'arrival'
                                    ? 0.5
                                    : 0.38;
    const strengthAdjustment = role === 'start'
...
function deriveTasteRolePoolInfluence(tasteSignals) {
    ...
    const moderateStartSocial = moderateBandScore(tasteSignals.socialDensity, 0.5, 0.36);
    ...
    const warmupMomentContribution = getMomentRolePreference(tasteSignals.momentIdentity, 'start') * 0.018;
    const peakMomentContribution = getMomentRolePreference(tasteSignals.momentIdentity, 'highlight') * 0.024;
    const wildcardMomentContribution = getMomentRolePreference(tasteSignals.momentIdentity, 'surprise') * 0.018;
    const cooldownMomentContribution = getMomentRolePreference(tasteSignals.momentIdentity, 'windDown') * 0.02;
    ...
    return {
        warmup: {
            tasteBonus: roundToThousandths(tasteSignals.conversationFriendliness * 0.03 +
            ...
            roleSuitabilityContribution: roundToThousandths(warmupRoleSuitabilityContribution),
        ...
        peak: {
            tasteBonus: roundToThousandths(tasteSignals.destinationFactor * 0.022 +
            ...
            roleSuitabilityContribution: roundToThousandths(peakRoleSuitabilityContribution),
            momentContribution: roundToThousandths(peakMomentContribution),
            highlightPlausibilityBonus: roundToThousandths((tasteSignals.highlightTier === 1
                ? 0.022
        ...
        wildcard: {
            tasteBonus: roundToThousandths(tasteSignals.experientialFactor * 0.028 +
                tasteSignals.noveltyWeight * 0.028 +
                tasteSignals.momentPotential.score * 0.014 +
                intensityScore * 0.012 +
                wildcardMomentContribution +
                wildcardRoleSuitabilityContribution),
            roleSuitabilityContribution: roundToThousandths(wildcardRoleSuitabilityContribution),
        ...
        cooldown: {
            tasteBonus: roundToThousandths(tasteSignals.intimacy * 0.024 +
            ...
            roleSuitabilityContribution: roundToThousandths(cooldownRoleSuitabilityContribution),
...
function computeHyperlocalVariantLift(candidate) {
    ...
    return clamp01(activation.intensityContribution * 0.42 +
...
function isHyperlocalVariantContractPlausible(candidate, lens) {
    ...
    if (activation.primaryActivationType === 'social_ritual' &&
        signals.socialDensity >= 0.84 &&
        signals.intimacy < 0.5) {
        return false;
    }
    if (activation.primaryActivationType === 'live_performance' &&
        signals.energy >= 0.86 &&
    ...
    return (hints.has('romantic_ambient') ||
...
function shouldInjectHyperlocalVariant(candidate, lens) {
    ...
    const strongImpact = impact.highlightSuitability >= 0.04 ||
        impact.momentIntensity >= 0.04 ||
        impact.momentPotential >= 0.04;
    const highlightReady = candidate.roleScores.peak >= 0.54 &&
        candidate.stopShapeFit.highlight >= 0.24 &&
        candidate.highlightValidity.validityLevel !== 'invalid';
    const lowSignalGeneric = candidate.taste.fallbackPenalty.signalScore >= 0.18 &&
...
function createHyperlocalActivationVariant(candidate, lens) {
    ...
    const variantLift = computeHyperlocalVariantLift(candidate);
    const highlightLift = Math.min(0.11, 0.028 +
    ...
    const fitLift = Math.min(0.055, 0.012 +
        activation.interpretationImpact.highlightSuitability * 0.06 +
        activation.interpretationImpact.momentPotential * 0.04);
    const warmupLift = activationType === 'seasonal_market' || activationType === 'social_ritual'
        ...
        : activationType === 'ambient_activation' || activationType === 'tasting_activation'
            ...
            : 0;
    const wildcardLift = activationType === 'live_performance' ||
        ...
        : 0;
    const cooldownLift = activationType === 'ambient_activation' || activationType === 'tasting_activation'
        ? Math.min(0.04, 0.01 + activation.interpretationImpact.highlightSuitability * 0.05)
        : 0;
    const fallbackRelief = Math.min(0.08, activation.intensityContribution * 0.12 +
    ...
    const ambianceBoost = activationType === 'ambient_activation'
        ? 0.12
        : activationType === 'tasting_activation'
            ...
            : activationType === 'live_performance'
                ...
                : activationType === 'cultural_activation'
                    ? 0.05
                    : 0.02;
    const intimacyBoost = activationType === 'tasting_activation'
        ? 0.09
        : activationType === 'ambient_activation'
            ...
            : activationType === 'live_performance'
                ? 0.04
                : 0.02;
    const ambientExperienceBoost = activationType === 'ambient_activation' || activationType === 'live_performance'
        ? 0.08
        : activationType === 'tasting_activation' || activationType === 'cultural_activation'
            ? 0.06
            : 0.03;
    const ambientUniquenessBoost = activationType === 'ambient_activation' || activationType === 'tasting_activation'
        ? 0.12
        : activationType === 'live_performance'
            ? 0.08
            : 0.04;
    const culturalDepthBoost = activationType === 'cultural_activation'
        ? 0.1
        : activationType === 'live_performance'
            ? 0.05
            : 0.02;
    const momentPotentialBoost = Math.min(0.08, activation.interpretationImpact.momentPotential * 0.34 +
        activation.intensityContribution * 0.04);
    const momentIntensityBoost = Math.min(0.06, activation.interpretationImpact.momentIntensity * 0.24 +
        activation.intensityContribution * 0.03);
    const variantMomentPotential = clamp01(candidate.taste.signals.momentPotential.score + momentPotentialBoost);
    const variantMomentIntensity = clamp01(candidate.taste.signals.momentIntensity.score + momentIntensityBoost);
    const variantMomentIntensityTier = variantMomentIntensity >= 0.9
        ? 'signature'
    ...
    const variantRomanticSignals = {
        ...
        ambientExperience: clamp01(candidate.taste.signals.romanticSignals.ambientExperience + ambientExperienceBoost),
    };
    const variantMomentEnrichment = {
        ...candidate.taste.signals.momentEnrichment,
        ambientUniqueness: clamp01(candidate.taste.signals.momentEnrichment.ambientUniqueness + ambientUniquenessBoost),
        culturalDepth: clamp01(candidate.taste.signals.momentEnrichment.culturalDepth + culturalDepthBoost),
        signals: [
            ...new Set([
    ...
    const variantRomanticScore = clamp01(candidate.taste.signals.romanticScore +
        ambianceBoost * 0.42 +
        intimacyBoost * 0.32 +
        ambientExperienceBoost * 0.26);
    const variantMomentElevationPotential = clamp01(candidate.taste.signals.momentElevationPotential +
        activation.interpretationImpact.highlightSuitability * 0.14 +
        activation.interpretationImpact.momentIntensity * 0.12 +
        activation.interpretationImpact.momentPotential * 0.1 +
        activation.intensityContribution * 0.08);
    const variantElevatedMomentCandidate = candidate.taste.signals.isElevatedMomentCandidate ||
    ...
    const variantSignals = {
        ...
        isRomanticMomentCandidate: candidate.taste.signals.isRomanticMomentCandidate ||
    ...
    return {
        ...
        hiddenGemScore: clamp01(candidate.hiddenGemScore + activation.interpretationImpact.novelty * 0.04),
        lensCompatibility: clamp01(candidate.lensCompatibility +
...
export function scoreVenueFit(venue, intent, crewPolicy, lens, roleContracts, starterPack) {
    ...
    const provisionalFit = anchorFit * 0.25 +
        crewFit * 0.23 +
        proximityFit * 0.2 +
        budgetFit * 0.11 +
        uniquenessFit * 0.1 +
    ...
    const roleContract = {
        warmup: evaluateRoleContract(venue, roleContracts?.byRole.start ?? defaultRoleContractRule('start')),
        peak: evaluateRoleContract(venue, roleContracts?.byRole.highlight ?? defaultRoleContractRule('highlight')),
        wildcard: evaluateRoleContract(venue, roleContracts?.byRole.surprise ?? defaultRoleContractRule('surprise')),
        cooldown: evaluateRoleContract(venue, roleContracts?.byRole.windDown ?? defaultRoleContractRule('windDown')),
    };
    ...
    const highlightMomentRoleFit = getMomentRolePreference(tasteSignals.momentIdentity, 'highlight');
    const surpriseMomentRoleFit = getMomentRolePreference(tasteSignals.momentIdentity, 'surprise');
    const windDownMomentRoleFit = getMomentRolePreference(tasteSignals.momentIdentity, 'windDown');
    const tasteRolePoolInfluence = deriveTasteRolePoolInfluence(tasteSignals);
    const protectedByUserConstraint = intent.anchor?.venueId === venue.id ||
    ...
    const strongAlignmentFitBonus = tasteModeAlignment.tier === 'primary'
    ...
    const primaryAlignmentMultiplier = tasteModeAlignment.tier === 'primary'
        ? 1.42
        : tasteModeAlignment.tier === 'supporting'
            ? 1.12
            : 1;
    const modeAlignmentRoleInfluence = {
        warmup: roundToThousandths(tasteModeAlignment.byRole.start * 1.22 * primaryAlignmentMultiplier +
            ...
            (tasteModeAlignment.tier === 'primary'
                ...
                : tasteModeAlignment.tier === 'supporting'
                    ? 0.026
                    : 0)),
        peak: roundToThousandths(tasteModeAlignment.byRole.highlight * 1.42 * primaryAlignmentMultiplier +
            ...
            (tasteModeAlignment.tier === 'primary'
                ...
                : tasteModeAlignment.tier === 'supporting'
                    ? 0.045
                    : 0)),
        wildcard: roundToThousandths(tasteModeAlignment.byRole.surprise * 1.14 * primaryAlignmentMultiplier +
            tasteModeAlignment.lanePriorityScore * tasteModeWeight * 0.13),
        cooldown: roundToThousandths(tasteModeAlignment.byRole.windDown * 1.14 * primaryAlignmentMultiplier +
    ...
    const startLightModeAlignmentBoost = lens.tasteMode &&
        ...
        ? tasteModeWeight *
            (tasteModeAlignment.tier === 'primary'
                ...
                : tasteModeAlignment.tier === 'supporting'
                    ...
                    : 0.024)
        : 0;
    const windDownSoftModeAlignmentBoost = lens.tasteMode &&
    ...
    const discoveryPreference = intent.discoveryPreferences?.find((preference) => preference.venueId === venue.id);
    ...
    const fitScore = clamp01(normalizeWeightedScore({
        anchor: anchorFit,
        crew: crewFit,
    ...
    const hiddenGemScore = clamp01(hiddenGemFit * (1 + crewPolicy.hiddenGemBias) + (intent.prefersHiddenGems ? 0.03 : 0));
    ...
    const fancyLift = refinements.has('little-fancier') && venue.priceTier !== '$'
        ? 0.06
        : refinements.has('little-fancier')
            ? -0.04
            : 0;
    const discoveryLift = lens.discoveryBias === 'high' ? 0.08 : lens.discoveryBias === 'medium' ? 0.04 : 0;
    const isNightlifeCoded = venue.category === 'bar' || venue.category === 'live_music' || hasAnyTag(venue, ['late-night', 'party']);
    const isRomanticTone = venue.category === 'dessert' ||
        venue.category === 'cafe' ||
        venue.category === 'live_music' ||
        venue.category === 'park' ||
        hasAnyTag(venue, ['cozy', 'intimate', 'design-forward', 'listening', 'calm']);
    const isFamilyFriendly = (venue.category === 'park' ||
    ...
    const genericHighlightCategory = venue.category === 'restaurant' ||
        venue.category === 'cafe' ||
        venue.category === 'dessert';
    const formalDiningVenue = venue.category === 'restaurant' &&
        (hasAnyTag(venue, [
            'chef-led',
            'reservation',
            'tasting-menu',
    ...
    const startIntentionalityBonus = clamp01(tasteSignals.roleSuitability.start * 0.55 +
        tasteSignals.conversationFriendliness * 0.25 +
        (1 - tasteSignals.energy) * 0.2) * 0.05;
    const windDownIntentionalityBonus = clamp01(tasteSignals.roleSuitability.windDown * 0.5 +
        tasteSignals.intimacy * 0.25 +
        (1 - tasteSignals.energy) * 0.25) * 0.05;
    const softClosingStartMoment = (tasteSignals.momentIdentity.type === 'close' ||
        tasteSignals.momentIdentity.type === 'linger') &&
        tasteSignals.momentIdentity.strength !== 'strong';
    const arrivalExploreStartMoment = tasteSignals.momentIdentity.type === 'arrival' ||
        tasteSignals.momentIdentity.type === 'explore';
    const interactiveLightStart = tasteSignals.primaryExperienceArchetype === 'activity' &&
        venue.energyLevel <= 3 &&
        tasteSignals.momentIdentity.strength !== 'strong';
    const socialEntryStart = tasteSignals.primaryExperienceArchetype === 'social' &&
        (tasteSignals.momentIdentity.type === 'arrival' ||
            tasteSignals.momentIdentity.type === 'explore' ||
            tasteSignals.momentIdentity.type === 'transition');
    const casualActivityStart = (venue.category === 'activity' ||
        tasteSignals.primaryExperienceArchetype === 'activity') &&
        venue.energyLevel <= 3;
    const startEnergyEntryLift = (arrivalExploreStartMoment ? 0.05 : 0) +
    ...
    const windDownCloseLingerBoost = tasteSignals.momentIdentity.type === 'close'
        ? 0.06
        : tasteSignals.momentIdentity.type === 'linger'
            ? 0.045
            : 0;
    const windDownSecondPeakPenalty = (tasteSignals.momentIdentity.type === 'anchor' ||
        tasteSignals.momentIdentity.type === 'explore') &&
        tasteSignals.momentIdentity.strength === 'strong'
        ? 0.11
    ...
    const momentIntensityLift = tasteSignals.momentIntensity.score * 0.18 +
        getMomentIntensityTierBoost(tasteSignals.momentIntensity) * 0.9;
    const passiveHighlightArchetype = tasteSignals.primaryExperienceArchetype === 'dining' ||
        tasteSignals.primaryExperienceArchetype === 'drinks' ||
        tasteSignals.primaryExperienceArchetype === 'sweet';
    const modeSpecificPassiveHighlightPenalty = (lens.tasteMode?.id === 'activity-led' || lens.tasteMode?.id === 'scenic-outdoor') &&
    ...
    const strongHighlightMomentLift = tasteSignals.momentIdentity.strength === 'strong' &&
        (tasteSignals.momentIdentity.type === 'anchor' ||
            tasteSignals.momentIdentity.type === 'explore')
        ? 0.16
        : 0;
    const highlightArchetypeLift = getHighlightArchetypeLift(tasteSignals, lens.tasteMode?.id) * 1.1;
    const supportRoleCollisionPenalty = Math.max(0, tasteSignals.roleSuitability.start - tasteSignals.roleSuitability.highlight) *
        0.1 +
        Math.max(0, tasteSignals.roleSuitability.windDown - tasteSignals.roleSuitability.highlight) *
            0.1;
    const genericHighlightPenalty = (genericHighlightCategory ? 0.12 : 0.05) *
    ...
    const romanticCenterpieceConviction = computeRomanticCenterpieceConvictionScore(venue, tasteSignals);
    const romanticAmbientRichness = Math.max(tasteSignals.romanticSignals.ambiance, tasteSignals.romanticSignals.ambientExperience, tasteSignals.momentEnrichment.ambientUniqueness);
    const hospitalityHighlightArchetype = tasteSignals.primaryExperienceArchetype === 'dining' ||
        tasteSignals.primaryExperienceArchetype === 'drinks' ||
        tasteSignals.primaryExperienceArchetype === 'sweet';
    const intimateDiningHighlightSignal = hospitalityHighlightArchetype &&
        tasteSignals.romanticSignals.intimacy >= 0.6 &&
        romanticAmbientRichness >= 0.58 &&
        tasteSignals.experientialFactor >= 0.62;
    const viewBackedDiningHighlightSignal = hospitalityHighlightArchetype &&
        tasteSignals.romanticSignals.scenic >= 0.5 &&
        tasteSignals.romanticSignals.intimacy >= 0.56 &&
        romanticAmbientRichness >= 0.54;
    const lowChaosHighIntimacySignal = tasteSignals.energy <= 0.68 &&
        tasteSignals.socialDensity <= 0.72 &&
        tasteSignals.romanticSignals.intimacy >= 0.64 &&
        romanticAmbientRichness >= 0.52;
    const romanticAtmosphericHighlightBoost = cozyRomanticHighlightMode &&
        ...
        : 0;
    const scenicPrimaryHighlightCandidate = tasteSignals.primaryExperienceArchetype === 'outdoor' ||
        tasteSignals.primaryExperienceArchetype === 'scenic' ||
        tasteSignals.experienceFamily === 'outdoor_scenic';
    const scenicDepthLowForCozyRomantic = scenicPrimaryHighlightCandidate &&
    ...
    const cozyRomanticScenicModeration = cozyRomanticHighlightMode && scenicPrimaryHighlightCandidate
        ? 0.04 +
            (scenicDepthLowForCozyRomantic
                ...
                : 0)
        : 0;
    const genericDiningWithoutDateSignal = cozyRomanticHighlightMode &&
    ...
    const romanticCenterpieceHighlightBoost = cozyRomanticHighlightMode &&
        ...
        : 0;
    const romanticLowConvictionPenalty = cozyRomanticHighlightMode &&
        romanticCenterpieceConviction < 0.62 &&
        hospitalityHighlightArchetype &&
        !viewBackedDiningHighlightSignal
        ? 0.08 + Math.max(0, 0.62 - romanticCenterpieceConviction) * 0.2
        : 0;
    const discoveryWarmupBoost = discoveryPreference?.role === 'start'
        ? 0.24
        : discoveryPreference?.role === 'highlight'
            ? -0.08
            : discoveryPreference?.role === 'windDown'
                ? -0.05
                : 0;
    const discoveryPeakBoost = discoveryPreference?.role === 'highlight'
        ? 0.34
        : discoveryPreference?.role === 'start' || discoveryPreference?.role === 'windDown'
            ? -0.08
            : 0;
    const discoveryCooldownBoost = discoveryPreference?.role === 'windDown'
        ? 0.24
        : discoveryPreference?.role === 'highlight'
            ? -0.08
    ...
    const isDateCoded = hasAnyTag(venue, ['intimate', 'date-night', 'romantic', 'chef-led']) ||
        venue.category === 'dessert';
    const romanticPlayfulStartPenalty = crewPolicy.crew === 'romantic' &&
        venue.category === 'activity' &&
    ...
    const casualExplorationWildcardPenalty = casualExplorationContext
    ...
    const romanticMomentStartLift = romanticMomentCandidate &&
        (tasteSignals.momentIdentity.type === 'arrival' ||
            tasteSignals.momentIdentity.type === 'explore' ||
            tasteSignals.momentIdentity.type === 'transition')
        ? 0.05
        : romanticMomentCandidate
            ? 0.02
            : 0;
    const romanticMomentHighlightLift = romanticMomentCandidate
        ? 0.12 +
            (tasteSignals.momentIdentity.type === 'anchor' ||
                tasteSignals.momentIdentity.type === 'explore'
                ? 0.05
    ...
    const romanticFallbackHighlightPenalty = romanticMomentModifierActive &&
    ...
    const cozyConversationBoost = intent.primaryAnchor === 'cozy' &&
        ...
        ? 0.12
        : 0;
    const cozyMusicPenalty = intent.primaryAnchor === 'cozy' &&
        ...
        ? vibeAuthority.musicSupportSource === 'pack' || vibeAuthority.musicSupportSource === 'both'
            ...
            : 0.2
        : 0;
    const adventurousOutdoorBoost = intent.primaryAnchor === 'adventurous-outdoor' &&
        (venue.category === 'park' || hasAnyTag(venue, ['nature', 'walkable', 'scenic', 'viewpoint']))
        ? 0.1
        : 0;
    const adventurousUrbanBoost = intent.primaryAnchor === 'adventurous-urban' &&
        (hasAnyTag(venue, ['underexposed', 'street-food', 'community', 'local']) ||
            venue.category === 'bar' ||
            venue.category === 'event')
        ? 0.1
        : 0;
    const outdoorAdventureLift = intent.primaryAnchor === 'adventurous-outdoor' && vibeAuthority.adventureRead === 'outdoor'
        ? 0.12
        : 0;
    const outdoorAdventurePenalty = intent.primaryAnchor === 'adventurous-outdoor' && vibeAuthority.adventureRead === 'urban'
        ? 0.18
        : intent.primaryAnchor === 'adventurous-outdoor' && vibeAuthority.adventureRead === 'balanced'
            ? 0.06
            : 0;
    const urbanAdventureLift = intent.primaryAnchor === 'adventurous-urban' && vibeAuthority.adventureRead === 'urban'
        ? 0.12
        : 0;
    const urbanAdventurePenalty = intent.primaryAnchor === 'adventurous-urban' && vibeAuthority.adventureRead === 'outdoor'
        ? 0.18
    ...
    const highlightValidityLift = highlightValidity.validityLevel === 'valid'
        ? 0.16
        : highlightValidity.validityLevel === 'fallback'
            ? -0.05
            : -0.42;
    const highlightTierAdjustment = highlightValidity.candidateTier === 'highlight-capable'
        ? 0.04
        : highlightValidity.candidateTier === 'support-only'
            ? -0.03
            : -0.14;
    const qualityDemotionPenalty = venue.source.qualityGateStatus === 'demoted'
    ...
    const livePeakTimePenalty = weakLiveWindow
        ? 0.12
        : isLiveSource && !venue.source.hoursKnown && venue.category !== 'restaurant'
            ? liveFairness.supportRecoveryEligible ? 0.012 : 0.03
            : 0;
    const packLiteralPenalty = highlightValidity.packLiteralRequirementLabel && !highlightValidity.packLiteralRequirementSatisfied
        ? highlightValidity.validityLevel === 'invalid'
            ? 0.24
            : 0.08
        : 0;
    const warmup = clamp01(fitScore * 0.41 +
    ...
    const peak = clamp01(fitScore * 0.42 +
        ...
        relaxedPenalty * 0.3);
    const wildcard = clamp01(fitScore * 0.3 +
        ...
        relaxedPenalty * 0.2);
    const cooldown = clamp01(fitScore * 0.33 +
...
export function scoreVenueCollection(venues, intent, crewPolicy, lens, roleContracts, starterPack) {
    const baseCandidates = venues.map((venue) => scoreVenueFit(venue, intent, crewPolicy, lens, roleContracts, starterPack));
