...
function scoreRoleCandidate(role, input) {
    ...
    if (role === 'start') {
        return clamp(signals.roleSuitability.start * 0.56 +
    ...
    if (role === 'highlight') {
        return clamp(signals.roleSuitability.highlight * 0.42 +
    ...
    return clamp(signals.roleSuitability.windDown * 0.5 +
...
function getDominantEnergy(inputs) {
    const value = weightedAverage(inputs.map((input) => ({ value: input.tasteSignals.energy, weight: getFitWeight(input) })));
    if (value <= 0.42) {
        return 'calm';
    }
    if (value >= 0.64) {
...
function getDominantSocialDensity(inputs) {
    const socialDensity = weightedAverage(inputs.map((input) => ({ value: input.tasteSignals.socialDensity, weight: getFitWeight(input) })));
    const intimacy = weightedAverage(inputs.map((input) => ({ value: input.tasteSignals.intimacy, weight: getFitWeight(input) })));
...
function getMovementProfile(inputs) {
    const withCoordinates = inputs.filter((input) => typeof input.lat === 'number' && typeof input.lng === 'number');
    if (withCoordinates.length >= 3) {
        ...
        for (let index = 0; index < withCoordinates.length; index += 1) {
            for (let cursor = index + 1; cursor < withCoordinates.length; cursor += 1) {
                maxDistance = Math.max(maxDistance, distanceKm(withCoordinates[index], withCoordinates[cursor]));
            }
        }
        if (maxDistance <= 0.9) {
            return 'tight';
...
function getDiscoveryBalance(inputs) {
    const noveltySignal = weightedAverage(inputs.map((input) => ({
        value: input.tasteSignals.noveltyWeight * 0.55 +
...
function getMomentPriority(momentType) {
    if (momentType === 'anchor') {
        return 5;
    }
    if (momentType === 'temporal') {
...
function buildAggregatedMoments(inputs) {
    const allMoments = inputs.flatMap((input) => detectMomentsFromTaste({
        venueId: input.venueId,
        venueName: input.venueName,
    ...
    const ranked = allMoments
        .slice()
        .sort((left, right) => {
        const leftPrimaryScore = left.strength * 0.7 +
            left.intentFit * 0.2 +
            left.timingRelevance * 0.1 +
            getMomentTypePrimaryBoost(left.momentType);
        const rightPrimaryScore = right.strength * 0.7 +
            right.intentFit * 0.2 +
    ...
    for (const moment of ranked) {
        const primaryEligible = (moment.momentType === 'anchor' ||
...
export function aggregateTasteOpportunityFromVenues(inputs) {
    ...
    const archetypes = topKeys(cleanInputs.map((input) => input.tasteSignals.primaryExperienceArchetype), 5);
    const momentIdentities = topKeys(cleanInputs.map((input) => `${input.tasteSignals.momentIdentity.type}:${input.tasteSignals.momentIdentity.strength}`), 5);
    const familyDistribution = countByKey(cleanInputs.map((input) => input.tasteSignals.experienceFamily || 'unknown'));
    const momentDistribution = countByKey(cleanInputs.map((input) => input.tasteSignals.momentIdentity.type));
    return {
        summary: {
        ...
        signatures: {
            ...
            highlightTypes: topKeys(highlightCandidates.map((candidate) => getRoleSignature('highlight', inputById.get(candidate.venueId))), 4),
            windDownTypes: topKeys(windDownCandidates.map((candidate) => getRoleSignature('windDown', inputById.get(candidate.venueId))), 4),
            archetypes,
            momentIdentities,
        },
        anchors: {
        ...
        diagnostics: {
            ...
            roleCoverage: {
                start: cleanInputs.filter((input) => input.tasteSignals.roleSuitability.start >= 0.56)
                    .length,
                highlight: cleanInputs.filter((input) => input.tasteSignals.roleSuitability.highlight >= 0.6).length,
                windDown: cleanInputs.filter((input) => input.tasteSignals.roleSuitability.windDown >= 0.56).length,
            },
            familyDistribution,
            momentDistribution,
        },
