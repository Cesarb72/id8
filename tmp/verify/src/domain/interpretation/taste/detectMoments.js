...
function parseWindowLabel(timeWindow) {
    ...
    if (token.includes('late') ||
        token.includes('night') ||
        token.includes('after')) {
        return 'late';
    }
    if (token.includes('morning') ||
...
function getIntentFit(input) {
    ...
    const base = clamp(tasteSignals.experientialFactor * 0.22 +
...
function getTimingRelevance(input) {
    ...
    const hoursAvailability = getHoursAvailabilityScore(input.hoursStatus, input.hoursConfidence);
    const hasEvent = Boolean(sourceFlags?.eventCapable ||
    ...
    const hasPerformance = Boolean(sourceFlags?.performanceCapable ||
        sourceFlags?.musicCapable ||
        signals.hyperlocalActivation.activationTypes.includes('cultural_activation'));
    const happyHourBonus = sourceFlags?.hasHappyHour && (windowLabel === 'evening' || windowLabel === 'late')
        ? 0.08
        : 0;
    const temporalWindowBonus = windowLabel === 'late'
        ...
        : windowLabel === 'day'
            ...
            : 0.04;
    return toFixed(clamp(signals.hyperlocalActivation.temporalRelevance * 0.42 +
        ...
        happyHourBonus +
        temporalWindowBonus, 0, 1));
}
function getSourceType(momentType, input) {
    const eventLike = Boolean(input.sourceFlags?.eventCapable ||
...
function getDescriptors(tasteSignals) {
    const energy = tasteSignals.energy >= 0.68
        ? 'lively'
        : tasteSignals.energy <= 0.42
            ? 'calm'
            : 'balanced';
    const tone = tasteSignals.intimacy >= 0.62
        ? 'intimate'
        : tasteSignals.socialDensity >= 0.64
            ? 'social'
            : 'mixed';
    const pacing = tasteSignals.lingerFactor >= 0.62
        ? 'linger'
...
function buildDetectedMoment(input, momentType, strength, timingRelevance, intentFit, reason) {
    const windowLabel = parseWindowLabel(input.context?.timeWindow);
    const hasEvent = Boolean(input.sourceFlags?.eventCapable ||
        input.tasteSignals.hyperlocalActivation.activationTypes.includes('live_performance'));
    const hasPerformance = Boolean(input.sourceFlags?.performanceCapable || input.sourceFlags?.musicCapable);
    return {
...
function detectAnchorMoment(input, timingRelevance, intentFit) {
    ...
    const anchorStrength = clamp(signals.roleSuitability.highlight * 0.34 +
        tierScore * 0.2 +
    ...
    return buildDetectedMoment(input, 'anchor', anchorStrength, timingRelevance, intentFit, 'Strong central fit for tonight\'s route shape');
}
function detectSupportingMoment(input, timingRelevance, intentFit) {
    const signals = input.tasteSignals;
    const supportStrength = clamp(Math.max(signals.roleSuitability.start, signals.roleSuitability.windDown) * 0.44 +
    ...
    const reason = signals.roleSuitability.start >= signals.roleSuitability.windDown
        ? 'Good early stop with a softer pace'
        : 'Strong closer fit that supports a calm landing';
    return buildDetectedMoment(input, 'supporting', supportStrength, timingRelevance, intentFit, reason);
}
function detectTemporalMoment(input, timingRelevance, intentFit) {
    const signals = input.tasteSignals;
    const happenings = input.happenings;
    const temporalSignalsPresent = Boolean(input.sourceFlags?.eventCapable ||
        input.sourceFlags?.musicCapable ||
        input.sourceFlags?.performanceCapable ||
        input.sourceFlags?.hasHappyHour ||
    ...
    const strength = clamp(timingRelevance * 0.56 +
    ...
    return buildDetectedMoment(input, 'temporal', strength, timingRelevance, intentFit, 'More relevant in the current time window');
}
function detectDiscoveryMoment(input, timingRelevance, intentFit) {
    const signals = input.tasteSignals;
    const happenings = input.happenings;
    const discoveryStrength = clamp(signals.noveltyWeight * 0.34 +
        ...
        getHappeningsSignal(happenings, 'hiddenGemStrength') * 0.04, 0, 1);
    const roleUsable = Math.max(signals.roleSuitability.start, signals.roleSuitability.highlight, signals.roleSuitability.windDown) >= 0.45;
    if (discoveryStrength < 0.6 || !roleUsable) {
        return null;
    }
    return buildDetectedMoment(input, 'discovery', discoveryStrength, timingRelevance, intentFit, 'High-discovery option with usable role fit');
}
function detectCommunityMoment(input, timingRelevance, intentFit) {
    ...
    const activationScore = signals.hyperlocalActivation.activationTypes.includes('social_ritual') ||
        signals.hyperlocalActivation.activationTypes.includes('cultural_activation')
        ? 0.12
        : 0;
    const communityStrength = clamp(signals.momentEnrichment.culturalDepth * 0.33 +
    ...
    return buildDetectedMoment(input, 'community', communityStrength, timingRelevance, intentFit, 'Community-forward signal with strong local context');
}
...
export function detectMomentsFromTaste(input) {
