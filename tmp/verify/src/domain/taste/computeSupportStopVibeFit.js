...
function hasAnyTag(source, target) {
    return overlapScore(source, target) > 0;
}
function effectFromScore(score) {
    if (score >= 0.7) {
        return 'reinforces';
    }
    if (score >= 0.5) {
...
function computeContextualAdjustment(role, stop, highlight, intent) {
    const notes = [];
    ...
    if (intent.primaryAnchor === 'adventurous-outdoor') {
        const outdoorAdjacent = stop.scoredVenue.venue.category === 'park' ||
            hasAnyTag(tags, [
                'walkable',
                'scenic',
                'trail',
        ...
        if (stop.scoredVenue.venue.category === 'museum' ||
    ...
    if (intent.primaryAnchor === 'adventurous-urban') {
        const urbanAdjacent = hasAnyTag(tags, [
            'district',
            'street-food',
            'community',
            'local',
        ...
        if (stop.scoredVenue.venue.category === 'park' ||
    ...
    if (intent.primaryAnchor === 'lively') {
        if (hasAnyTag(tags, ['social', 'cocktails', 'community']) ||
...
export function computeSupportStopVibeFit(stops, intent) {
    ...
    for (const stop of supportStops) {
        ...
        const score = clamp01(baseRoleFit * 0.44 +
            categoryFit * 0.18 +
