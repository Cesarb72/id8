function getMovementMode(sameNeighborhood, driveGap) {
...
function getNeighborhoodContinuity(sameNeighborhood, driveGap) {
    if (sameNeighborhood) {
...
function getEnergyMismatchPenalty(fromStop, toStop) {
...
export function computeTransitionFriction({ fromStop, toStop, fromDuration, toDuration, priorDistinctNeighborhoods, }) {
    ...
    const estimatedTravelMinutes = movementMode === 'walkable'
        ? Math.max(5, 5 + Math.round(driveGap / 2))
        : movementMode === 'short-drive'
            ? 8 + Math.round(driveGap * 0.75)
            : 11 + Math.round(driveGap * 0.85);
    const transitionBufferMinutes = movementMode === 'walkable' ? 4 : movementMode === 'short-drive' ? 6 : 8;
    let frictionScore = sameNeighborhood && driveGap <= 2
        ? 0
