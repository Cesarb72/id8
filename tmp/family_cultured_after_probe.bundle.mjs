...
function normalizeCity3(value) {
  const normalized = normalizeToken2(value).replace(/\./g, "");
  const [head] = normalized.split(",");
  return (head ?? normalized).trim();
}
...
function uniqueLowerTokens(venue) {
  ...
  return new Set(
    normalizeToken2(seed).split(" ").map((token) => token.trim()).filter(Boolean)
  );
}
function hasAnyToken2(tokens, values) {
  return values.some((value) => tokens.has(normalizeToken2(value)));
}
function hasAnyPhrase(value, phrases) {
  const normalized = normalizeToken2(value);
  return phrases.some((phrase) => normalized.includes(normalizeToken2(phrase)));
}
...
function asRecordByStopType(board2) {
  const next = {};
  Object.keys(board2).forEach((stopType) => {
    next[stopType] = board2[stopType].slice().sort((left, right) => {
...
function mapCategoryToVenueTypeLabel(category, subcategory) {
  const normalizedSubcategory = normalizeToken3(subcategory ?? "");
...
function deriveVenueFeatures(candidate) {
  const tokens = new Set(
    [...candidate.venueTags ?? [], ...candidate.sourceTypes ?? []].map((entry) => normalizeToken3(entry)).filter(Boolean)
  );
  const features = [];
...
function getStopQuality(stop) {
  ...
  return clamp0125(stop.authorityScore * 0.68 + stop.currentRelevance * 0.24 + hiddenGemLift);
}
function getRoleFitScore(stop) {
  const roleFitKey = ROLE_FIT_KEY_BY_POSITION[stop.position];
  return clamp0125(stop.roleFit[roleFitKey] ?? 0);
}
function getRoleFitThreshold(scenarioFamily, position) {
  let threshold = 0.34;
...
function getRoleDiscipline(scenarioFamily, stops) {
  if (stops.length === 0) {
    return 0;
  }
  const values = stops.map((stop) => {
    ...
    return clamp0125(score >= threshold ? 0.7 + score * 0.3 : score * 0.8);
  });
  return clamp0125(values.reduce((sum, value) => sum + value, 0) / values.length);
}
function getDistrictPlausibility(stops) {
  const districts = stops.map((stop) => normalizeToken3(stop.district ?? "")).filter(Boolean);
  ...
  const dominantShare = Math.max(...Object.values(districts.reduce((acc, district) => {
    acc[district] = (acc[district] ?? 0) + 1;
    return acc;
  }, {}))) / districts.length;
  const dominantPenalty = dominantShare < 0.4 ? 0.12 : dominantShare < 0.5 ? 0.06 : 0;
  return clamp0125(0.94 - transitionPenalty - spreadPenalty - dominantPenalty);
}
function getScenarioCoherence(scenarioFamily, stops) {
  const highlight = stops[Math.min(2, stops.length - 1)];
  const start = stops[0];
  const closer = stops[stops.length - 1];
  const reasonCorpus = normalizeToken3(
...
function getCandidateNightScore(scenarioFamily, stops) {
  ...
  const landingQuality = clamp0125(
  ...
  const hiddenGemPresence = clamp0125(
    stops.filter((stop) => stop.isHiddenGem).length / Math.max(1, stops.length)
  );
  const currentRelevance = clamp0125(
  ...
  const score = clamp0125(
    anchorAuthority * 0.22 + scenarioCoherence * 0.18 + startQuality * 0.1 + landingQuality * 0.12 + hiddenGemPresence * 0.05 + currentRelevance * 0.08 + districtPlausibility * 0.16 + roleDiscipline * 0.16 - syntheticPenalty
  );
  return {
    stops,
    score,
...
function buildStopTypePools(board2, requiredStopTypes) {
  const pools = {};
  for (const stopType of requiredStopTypes) {
    pools[stopType] = dedupeByVenueId(board2.candidatesByStopType[stopType] ?? []).slice(0, 5);
  }
  return pools;
}
...
function buildCandidateNightCombinations(params) {
  ...
  function rankCandidateCoherence(candidate, selectedStops, index) {
    const district = normalizeToken3(candidate.district ?? "");
    if (!district || selectedStops.length === 0) {
      return 0;
    }
    const selectedDistricts = selectedStops.map((stop) => normalizeToken3(stop.district ?? "")).filter(Boolean);
    const previousDistrict = selectedDistricts[selectedDistricts.length - 1];
    const districtCounts = selectedDistricts.reduce((acc, entry) => {
      acc[entry] = (acc[entry] ?? 0) + 1;
      return acc;
    }, {});
    const dominantDistrict = Object.entries(districtCounts).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))[0]?.[0];
    let boost = 0;
  ...
  function walk(index, selectedStops, usedVenueIds) {
    ...
    const orderedPool = (nonDuplicateFirst.length > 0 ? nonDuplicateFirst : fallbackDuplicates).slice().sort((left, right) => {
      const rightScore = rankCandidateCoherence(right, selectedStops, index) + (right.roleFit[position === "highlight" ? "highlight" : position === "start" || position === "mid" ? "start" : "windDown"] ?? 0) * 0.16;
      const leftScore = rankCandidateCoherence(left, selectedStops, index) + (left.roleFit[position === "highlight" ? "highlight" : position === "start" || position === "mid" ? "start" : "windDown"] ?? 0) * 0.16;
      if (rightScore !== leftScore) {
        return rightScore - leftScore;
      }
      return left.venueId.localeCompare(right.venueId);
    });
    const strictRoleCandidates = orderedPool.filter((candidate) => {
      const stop = toBuiltStop(candidate, position);
      return getRoleFitScore(stop) >= getRoleFitThreshold(scenarioFamily, position);
    });
    const walkPool = strictRoleCandidates.length > 0 ? strictRoleCandidates : orderedPool;
...
function overlapScore2(left, right) {
  ...
  const sharedDistricts = new Set(left.stops.map((stop) => normalizeToken3(stop.district ?? ""))).size > 0 && new Set(right.stops.map((stop) => normalizeToken3(stop.district ?? ""))).size > 0 ? left.stops.filter((stop, index) => {
    const rightStop = right.stops[index];
    return normalizeToken3(stop.district ?? "") === normalizeToken3(rightStop?.district ?? "");
  }).length : 0;
  const baseline = Math.max(1, Math.max(left.stops.length, right.stops.length));
  return clamp0125(sharedVenues / baseline * 0.78 + sharedDistricts / baseline * 0.22);
}
  ...
  while (selected.length < targetCount) {
    ...
    const scoredRemaining = remaining.map((candidate) => {
      const maxOverlap = selected.reduce(
        (max, existing) => Math.max(max, overlapScore2(candidate, existing)),
        0
      );
...
function getIntentRight(scenarioFamily, stop) {
  const reasonCorpus = normalizeToken3([stop.name, stop.whyThisStop, ...stop.reasons].join(" "));
  if (scenarioFamily === "romantic_cozy") {
...
function getExpectedPositionForStop(scenarioFamily, stopType) {
...
function getPlaceRight(stop, stops) {
  const districts = stops.map((entry) => normalizeToken3(entry.district ?? "")).filter(Boolean);
  const district = normalizeToken3(stop.district ?? "");
  if (!district) {
    return false;
  }
...
function buildScenarioNightsFromCandidateBoard(board2, options = {}) {
  const minNights = Math.max(2, options.minNights ?? 3);
  const maxNights = Math.max(minNights, Math.min(4, options.maxNights ?? 4));
  const requiredStopTypes = board2.requiredStopTypes;
  const pools = buildStopTypePools(board2, requiredStopTypes);
  const missingStopTypes = hasAnyMissingPool(requiredStopTypes, pools);
  if (missingStopTypes.length > 0) {
    return [
      {
        id: `built_${board2.scenarioFamily}_incomplete`,
        city: board2.city,
        persona: board2.persona,
        vibe: board2.vibe,
        scenarioFamily: board2.scenarioFamily,
        title: `${SCENARIO_FLAVOR_LINE[board2.scenarioFamily]} (incomplete)`,
        flavorLine: SCENARIO_FLAVOR_LINE[board2.scenarioFamily],
        stops: [],
  ...
  const rankingPool = coherentCandidates.length >= Math.max(2, minNights) ? coherentCandidates : candidateNights;
  const rankedCandidates = rankingPool.slice().sort((left, right) => right.score - left.score || getHighlightStopName(left).localeCompare(getHighlightStopName(right)));
  ...
  return selected.map((night, index) => {
    const stopsWithContract = withPreviewContract(board2.scenarioFamily, night.stops);
    ...
    return {
      id: `built_${board2.scenarioFamily}_${index + 1}`,
      city: board2.city,
      persona: board2.persona,
      vibe: board2.vibe,
      scenarioFamily: board2.scenarioFamily,
      title: pickTitle(board2.scenarioFamily, highlight?.name ?? "local highlight"),
      flavorLine: SCENARIO_FLAVOR_LINE[board2.scenarioFamily],
...
// tmp/family_cultured_after_probe.ts
var board = await buildStopTypeCandidateBoardFromIntent({ city: "San Jose", persona: "family", vibe: "cultured" });
if (!board) {
  console.log("unsupported");
  process.exit(0);
}
var nights = buildScenarioNightsFromCandidateBoard(board, { minNights: 3, maxNights: 4 });
