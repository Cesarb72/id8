...
const clusterByVenue = new Map<string, { districtId: string; districtLabel: string }>()
...
const districtGroups = new Map<string, { districtLabel: string; venues: typeof scored }>()
for (const sv of scored) {
  const district = clusterByVenue.get(sv.venue.id)
  if (!district) continue
  const current = districtGroups.get(district.districtId) ?? { districtLabel: district.districtLabel, venues: [] as typeof scored }
...
const strongHighlightByDistrict = new Map<string, number>()
...
const momentTypeCounts = new Map<string, number>()
for (const m of allDetectedMoments) {
  momentTypeCounts.set(m.momentType, (momentTypeCounts.get(m.momentType) ?? 0) + 1)
}

const aggregationByDistrict = new Map<string, ReturnType<typeof aggregateTasteOpportunityFromVenues>>()
...
const momentVenueFrequency = new Map<string, number>()
