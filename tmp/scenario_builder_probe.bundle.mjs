...
var STOP_TYPE_POSITION_EXPECTATION = {
...
function isSyntheticName(value) {
  ...
  return normalized.includes("microcrawl") || normalized.includes("sketchbook") || normalized.includes("drop") || normalized.includes("proto") || normalized.includes("testbed") || normalized.includes("simulator");
}
function pickTitle(scenarioFamily, highlightName) {
...
function getWhyThisWorks(stops) {
  const start = stops[0];
...
function mapCategoryToVenueTypeLabel(category, subcategory) {
...
function deriveVenueFeatures(candidate) {
  const tokens = new Set(
    [...candidate.venueTags ?? [], ...candidate.sourceTypes ?? []].map((entry) => normalizeToken(entry)).filter(Boolean)
  );
  const features = [];
  ...
  return features.length > 0 ? features.slice(0, 3) : void 0;
}
function deriveServiceOptions(candidate) {
  const tokens = new Set(
    [...candidate.venueTags ?? [], ...candidate.sourceTypes ?? []].map((entry) => normalizeToken(entry)).filter(Boolean)
  );
  const options = [];
  ...
  return options.length > 0 ? options.slice(0, 3) : void 0;
}
...
function buildMomentLabel(scenarioFamily, stop) {
  if (scenarioFamily === "romantic_cozy") {
...
function buildSignalReason(stop) {
...
function buildWhyTonight(scenarioFamily, stop) {
  if (stop.currentRelevance >= 0.66) {
    return "Current relevance is strong for tonight.";
  }
  if (scenarioFamily === "romantic_lively" && Math.max(stop.eventPotential ?? 0, stop.performancePotential ?? 0, stop.liveNightlifePotential ?? 0) >= 0.62) {
    return "Live/nightlife signals are active tonight.";
  }
  if ((scenarioFamily === "romantic_cultured" || scenarioFamily === "friends_cultured" || scenarioFamily === "family_cultured") && Math.max(stop.culturalAnchorPotential ?? 0, stop.performancePotential ?? 0) >= 0.62) {
    return "Cultural programming signal is active tonight.";
  }
  if ((scenarioFamily === "friends_lively" || scenarioFamily === "family_lively") && Math.max(stop.eventPotential ?? 0, stop.liveNightlifePotential ?? 0, stop.performancePotential ?? 0) >= 0.62) {
    return "High-energy event signal is active tonight.";
  }
  if ((scenarioFamily === "friends_cozy" || scenarioFamily === "family_cozy") && stop.authorityScore >= 0.68 && stop.currentRelevance >= 0.5) {
...
function withPreviewContract(scenarioFamily, stops) {
...
function toBuiltStop(candidate, position) {
  const venueTypeLabel = mapCategoryToVenueTypeLabel(candidate.venueCategory, candidate.venueSubcategory);
  return {
    position,
...
function getDistrictPlausibility(stops) {
  ...
  const spreadPenalty = uniqueCount >= 5 ? 0.14 : uniqueCount === 4 ? 0.08 : 0;
  return clamp01(0.92 - transitionPenalty - spreadPenalty);
}
function getScenarioCoherence(scenarioFamily, stops) {
  const highlight = stops[Math.min(2, stops.length - 1)];
  const start = stops[0];
  const closer = stops[stops.length - 1];
  const reasonCorpus = normalizeToken(
    stops.flatMap((stop) => [stop.name, ...stop.reasons]).join(" ")
  );
  if (scenarioFamily === "romantic_cozy") {
    const cozySignal = ["cozy", "intimate", "quiet", "garden", "wine", "tea", "nightcap"].some(
      (token) => reasonCorpus.includes(token)
    ) ? 1 : 0;
    return clamp01(
      highlight.authorityScore * 0.28 + getStopQuality(closer) * 0.28 + getStopQuality(start) * 0.18 + cozySignal * 0.16 + (highlight.isHiddenGem ? 0.1 : 0)
    );
  }
  if (scenarioFamily === "romantic_lively") {
    const livelySignal = ["performance", "nightlife", "cocktail", "late", "event"].some(
      (token) => reasonCorpus.includes(token)
    ) ? 1 : 0;
    return clamp01(
      highlight.authorityScore * 0.34 + highlight.currentRelevance * 0.2 + getStopQuality(stops[3] ?? closer) * 0.16 + getStopQuality(closer) * 0.14 + livelySignal * 0.16
    );
  }
  if (scenarioFamily === "romantic_cultured" || scenarioFamily === "friends_cultured" || scenarioFamily === "family_cultured") {
    const culturalSignal = ["cultural", "museum", "gallery", "institution", "performance", "reflective"].some(
      (token) => reasonCorpus.includes(token)
    ) ? 1 : 0;
    return clamp01(
      highlight.authorityScore * 0.3 + getStopQuality(stops[0]) * 0.2 + getStopQuality(stops[1] ?? stops[0]) * 0.14 + getStopQuality(closer) * 0.14 + culturalSignal * 0.22
    );
  }
  if (scenarioFamily === "friends_lively" || scenarioFamily === "family_lively") {
    const energySignal = ["event", "activity", "late", "night", "cocktail", "live"].some(
      (token) => reasonCorpus.includes(token)
    ) ? 1 : 0;
    return clamp01(
      highlight.authorityScore * 0.28 + getStopQuality(stops[0]) * 0.16 + getStopQuality(stops[1] ?? stops[0]) * 0.14 + getStopQuality(closer) * 0.2 + energySignal * 0.22
    );
  }
  const socialSignal = ["group", "casual", "family", "neighborhood", "conversation", "easy"].some(
    (token) => reasonCorpus.includes(token)
  ) ? 1 : 0;
  return clamp01(
    highlight.authorityScore * 0.24 + getStopQuality(stops[0]) * 0.18 + getStopQuality(stops[1] ?? stops[0]) * 0.16 + getStopQuality(closer) * 0.2 + socialSignal * 0.22
  );
}
function getCandidateNightScore(scenarioFamily, stops) {
  ...
  const landingQuality = clamp01(
    landingStops.reduce((sum, stop) => sum + getStopQuality(stop), 0) / Math.max(1, landingStops.length)
  );
  ...
  const syntheticPenalty = stops.filter((stop) => isSyntheticName(stop.name)).length > 0 ? 0.08 : 0;
  const score = clamp01(
    anchorAuthority * 0.26 + scenarioCoherence * 0.22 + startQuality * 0.14 + landingQuality * 0.16 + hiddenGemPresence * 0.08 + currentRelevance * 0.08 + districtPlausibility * 0.06 - syntheticPenalty
  );
  return {
    stops,
    score,
...
function dedupeByVenueId(candidates) {
...
function buildStopTypePools(board, requiredStopTypes) {
  const pools = {};
...
function hasAnyMissingPool(requiredStopTypes, pools) {
...
function buildCandidateNightCombinations(params) {
  ...
  function walk(index, selectedStops, usedVenueIds) {
    if (index >= stopCount) {
...
function overlapScore(left, right) {
  ...
  const sharedDistricts = new Set(left.stops.map((stop) => normalizeToken(stop.district ?? ""))).size > 0 && new Set(right.stops.map((stop) => normalizeToken(stop.district ?? ""))).size > 0 ? left.stops.filter((stop, index) => {
...
function sharedVenueCount(left, right) {
  const leftVenues = new Set(left.stops.map((stop) => stop.venueId));
  const rightVenues = new Set(right.stops.map((stop) => stop.venueId));
  return [...leftVenues].filter((venueId) => rightVenues.has(venueId)).length;
}
function selectDistinctNights(candidates, targetCount) {
  if (candidates.length <= targetCount) {
    return candidates;
  }
  const sorted = candidates.slice().sort((left, right) => right.score - left.score || getHighlightStopName(left).localeCompare(getHighlightStopName(right)));
  const selected = [];
  if (sorted.length > 0) {
...
function ensureHiddenGemVariant(selected, allCandidates) {
  ...
  const hiddenGemCandidate = allCandidates.filter((night) => night.hiddenGemPresence >= 0.45).sort((left, right) => right.score - left.score || getHighlightStopName(left).localeCompare(getHighlightStopName(right)))[0];
  if (!hiddenGemCandidate) {
    return selected;
  }
  const weakestIndex = selected.map((night, index) => ({ index, score: night.score })).sort((left, right) => left.score - right.score)[0]?.index;
...
function getIntentRight(scenarioFamily, stop) {
  const reasonCorpus = normalizeToken([stop.name, stop.whyThisStop, ...stop.reasons].join(" "));
  if (scenarioFamily === "romantic_cozy") {
    return stop.isHiddenGem === true || stop.authorityScore >= 0.56 || reasonCorpus.includes("cozy") || reasonCorpus.includes("intimate") || reasonCorpus.includes("atmospheric");
  }
  if (scenarioFamily === "romantic_lively") {
    return Math.max(stop.liveNightlifePotential ?? 0, stop.eventPotential ?? 0, stop.performancePotential ?? 0) >= 0.5 || stop.currentRelevance >= 0.55 || reasonCorpus.includes("nightlife") || reasonCorpus.includes("performance") || reasonCorpus.includes("cocktail");
  }
  if (scenarioFamily === "friends_lively") {
    return Math.max(stop.liveNightlifePotential ?? 0, stop.eventPotential ?? 0, stop.performancePotential ?? 0) >= 0.48 || stop.currentRelevance >= 0.52 || reasonCorpus.includes("group") || reasonCorpus.includes("activity") || reasonCorpus.includes("energy");
  }
  if (scenarioFamily === "friends_cozy" || scenarioFamily === "family_cozy") {
    return stop.authorityScore >= 0.5 || stop.currentRelevance >= 0.44 || reasonCorpus.includes("casual") || reasonCorpus.includes("easy") || reasonCorpus.includes("neighborhood") || reasonCorpus.includes("family");
  }
  if (scenarioFamily === "family_lively") {
    return Math.max(stop.eventPotential ?? 0, stop.performancePotential ?? 0) >= 0.46 || stop.authorityScore >= 0.52 || reasonCorpus.includes("active") || reasonCorpus.includes("outdoor") || reasonCorpus.includes("group");
  }
  return Math.max(stop.culturalAnchorPotential ?? 0, stop.performancePotential ?? 0) >= 0.48 || stop.venueCategory === "museum" || reasonCorpus.includes("cultural") || reasonCorpus.includes("museum") || reasonCorpus.includes("institution");
}
function getPlaceRight(stop, stops) {
...
function getMomentRight(stop) {
  if (stop.position === "highlight") {
    return stop.authorityScore >= 0.6 || stop.currentRelevance >= 0.56;
  }
  if (stop.position === "windDown" || stop.position === "closer") {
    return stop.currentRelevance >= 0.46 || stop.isHiddenGem === true || Math.max(stop.lateNightPotential ?? 0, stop.liveNightlifePotential ?? 0) >= 0.46;
  }
  return stop.authorityScore >= 0.44 || stop.currentRelevance >= 0.4;
}
function evaluateBuiltScenarioStop(params) {
  ...
  const expectedPosition = STOP_TYPE_POSITION_EXPECTATION[stop.stopType];
  const roleFitKey = ROLE_FIT_KEY_BY_POSITION[stop.position];
  const roleFitScore = stop.roleFit[roleFitKey] ?? 0;
  const isRoleRight = expectedPosition === stop.position && roleFitScore >= 0.35;
  ...
  const notes = failedCriteria.length > 0 ? [`failed ${failedCriteria.join(", ")}`] : void 0;
  return {
    isReal,
    isRoleRight,
    isIntentRight,
...
function evaluateBuiltScenarioNight(params) {
  ...
  const failedStops = stopEvaluations.filter((entry) => entry.evaluation.failedCriteria.length > 0).map((entry) => entry.venueId);
  return {
    stopEvaluations,
    passesGreatStopStandard: failedStops.length === 0,
    failedStops,
    notes: failedStops.length > 0 ? [`${failedStops.length}/${params.stops.length} stops failed at least one Great Stop criterion.`] : ["All stops pass Great Stop criteria."]
  };
}
function buildScenarioNightsFromCandidateBoard(board, options = {}) {
  ...
  const rankedCandidates = candidateNights.slice().sort((left, right) => right.score - left.score || getHighlightStopName(left).localeCompare(getHighlightStopName(right)));
  const desiredCount = rankedCandidates.length >= maxNights ? maxNights : rankedCandidates.length >= minNights ? minNights : Math.max(1, rankedCandidates.length);
...
// src/domain/retrieval/fieldPolicy.ts
var DEFAULT_FIELD_POLICY = {
  authorityTargets: [],
  sourceMode: {
...
function resolveFieldRetrievalSourceMode(params) {
  const policy = resolveFieldCityRetrievalPolicy(params.cityQuery);
  if (params.requestedSourceMode === "curated" && !params.sourceModeOverrideApplied && policy.sourceMode.autoUpgradeCuratedToHybrid) {
    return "hybrid";
  }
  return params.requestedSourceMode;
}
function resolveAllowCuratedFallback(params) {
  const policy = resolveFieldCityRetrievalPolicy(params.cityQuery);
  return params.curatedCoverageForCity || params.cityQuery.length === 0 || policy.sourceMode.allowCuratedFallbackWithoutCoverage;
}
function resolveDefaultCityFallback(params) {
  const policy = resolveFieldCityRetrievalPolicy(params.cityQuery);
...
function getFieldAuthorityTargets(cityQuery) {
...
function computeLiveQualityFairness(venue) {
  ...
  const signatureFairness = clamp013(
...
function applyQualityGate(venue) {
  ...
  const effectiveGenericScore = clamp014(venue.signature.genericScore - liveFairness.genericRelief);
  const effectiveSourceConfidence = clamp014(
  ...
  const completenessScore = clamp014(1 - completenessPenalty);
  const qualityScore = clamp014(
...
function toFixed01(value) {
  return Number(clamp015(value).toFixed(3));
}
...
function collectNormalizedTokens(venue) {
  ...
  return new Set(
    [...venue.tags, ...venue.vibeTags, ...venue.source.sourceTypes, ...summary.split(/\s+/)].map(normalizeToken2).filter(Boolean)
  );
}
...
function inferHoursPressure({
  ...
  const softenedConfidence = clamp016(
...
function normalizeRawPlace(raw, options = {}) {
  ...
  const completenessScore = clamp018(1 - missingFields.length * 0.1);
...
function normalizeRawEvent(raw, options = {}) {
  ...
  const completenessScore = clamp019(1 - missingFields.length * 0.11);
  const sourceConfidence = clamp019(
...
function summarizeGeoSpread(venues) {
  ...
  const bucketComponent = clamp0110((bucketCount - 1) / 4);
  const balanceComponent = clamp0110(1 - dominantAreaShare);
...
function resolveRequiredInventoryVenues(availableVenues, seedVenues, dedupeLosses) {
  ...
  return seedVenues.map((seedVenue) => {
    ...
    return availableVenues.find((venue) => {
      ...
      return venue.category === seedVenue.category && sanitize2(venue.name) === sanitize2(seedVenue.name) && sanitize2(venue.city) === sanitize2(seedVenue.city) && sanitize2(venue.neighborhood) === sanitize2(seedVenue.neighborhood);
    });
...
async function retrieveVenues(intent, lens, options = {}) {
  ...
  const normalizedNeighborhood = intent.neighborhood ? sanitize2(intent.neighborhood) : void 0;
  ...
  const fallbackMatches = cityMatches.length > 0 ? cityMatches : defaultFallbackCity ? qualityEligibleVenues.filter(
    (venue) => sanitizeCity(venue.city) === defaultFallbackCity && venue.driveMinutes <= maxDriveMinutes
  ) : [];
  ...
  const neighborhoodMatches = localFirstLensShapedVenues.filter(
    (venue) => sanitize2(venue.neighborhood) === normalizedNeighborhood
  );
...
function overlapScore2(venue, tags) {
  if (tags.length === 0) {
    return 0;
  }
...
function evaluateRoleContract(venue, rule) {
  ...
  const requiredTagScore = overlapScore2(venue, rule.requiredTags);
  ...
  const preferredTagScore = overlapScore2(venue, rule.preferredTags);
  ...
  const discouragedTagPenalty = overlapScore2(venue, rule.discouragedTags) * 0.22;
...
function resolveScenarioFamily(input) {
  const city = normalizeCity3(input.city);
  const persona = parsePersona(input.persona);
  const vibe = parseVibe(input.vibe);
  if (city !== "san jose" || !persona || !vibe) {
    return null;
  }
  const vibeBucket = vibe === "cozy" || vibe === "chill" ? "cozy" : vibe === "lively" || vibe === "playful" ? "lively" : vibe === "cultured" || vibe === "adventurous-outdoor" || vibe === "adventurous-urban" ? "cultured" : null;
  if (!vibeBucket) {
    return null;
  }
  if (persona === "romantic") {
...
function getVenueSignals(scoredVenue) {
  ...
  const roleStart = clamp0125(scoredVenue.roleScores.warmup);
  const roleHighlight = clamp0125(scoredVenue.roleScores.peak);
  const roleWindDown = clamp0125(scoredVenue.roleScores.cooldown);
  const authorityScore = clamp0125(
  ...
  return {
    authorityScore,
    hiddenGemScore: clamp0125(
      scoredVenue.hiddenGemScore * 0.55 + scoredVenue.venue.underexposureScore * 0.2 + (happenings?.hiddenGemStrength ?? 0) * 0.25
    ),
    currentRelevance: clamp0125(happenings?.currentRelevance ?? 0),
    eventPotential: clamp0125(happenings?.eventPotential ?? 0),
    performancePotential: clamp0125(happenings?.performancePotential ?? 0),
    liveNightlifePotential: clamp0125(happenings?.liveNightlifePotential ?? 0),
    culturalAnchorPotential: clamp0125(happenings?.culturalAnchorPotential ?? 0),
    lateNightPotential: clamp0125(happenings?.lateNightPotential ?? 0),
    majorVenueStrength: clamp0125(happenings?.majorVenueStrength ?? 0),
    roleFit: {
      start: roleStart,
...
function getFamilyAlignment(scenarioFamily, venueSignals, scoredVenue) {
  const socialConversation = scoredVenue.taste.signals.socialSignals?.conversationFriendliness ?? 0;
  ...
  if (scenarioFamily === "friends_cozy") {
    return clamp0125(
      venueSignals.roleFit.start * 0.2 + venueSignals.roleFit.windDown * 0.24 + socialConversation * 0.24 + venueSignals.hiddenGemScore * 0.14 + venueSignals.authorityScore * 0.1 + (1 - scoredVenue.venue.energyLevel / 5) * 0.08
    );
  }
  if (scenarioFamily === "friends_lively") {
    return clamp0125(
      venueSignals.liveNightlifePotential * 0.26 + venueSignals.eventPotential * 0.18 + venueSignals.performancePotential * 0.16 + venueSignals.currentRelevance * 0.14 + venueSignals.roleFit.highlight * 0.14 + socialActivity * 0.12
    );
  }
  if (scenarioFamily === "friends_cultured") {
    return clamp0125(
      venueSignals.culturalAnchorPotential * 0.24 + venueSignals.performancePotential * 0.14 + socialConversation * 0.18 + scoredVenue.taste.signals.momentEnrichment.culturalDepth * 0.16 + venueSignals.roleFit.highlight * 0.14 + venueSignals.authorityScore * 0.14
    );
  }
  if (scenarioFamily === "family_cozy") {
    return clamp0125(
      venueSignals.roleFit.start * 0.22 + socialConversation * 0.16 + venueSignals.currentRelevance * 0.14 + venueSignals.authorityScore * 0.14 + (1 - scoredVenue.venue.energyLevel / 5) * 0.18 + venueSignals.culturalAnchorPotential * 0.16
    );
  }
  if (scenarioFamily === "family_lively") {
    return clamp0125(
      venueSignals.eventPotential * 0.2 + venueSignals.performancePotential * 0.16 + venueSignals.currentRelevance * 0.16 + venueSignals.roleFit.highlight * 0.18 + socialActivity * 0.2 + venueSignals.authorityScore * 0.1
    );
  }
  return clamp0125(
    venueSignals.culturalAnchorPotential * 0.26 + venueSignals.performancePotential * 0.14 + scoredVenue.taste.signals.momentEnrichment.culturalDepth * 0.2 + venueSignals.currentRelevance * 0.12 + venueSignals.roleFit.highlight * 0.14 + socialConversation * 0.14
  );
}
function getStopTypeFit(scoredVenue, stopType, scenarioFamily) {
  ...
  switch (stopType) {
    ...
    case "casual_group_food": {
      fit = clamp0125(
        (isRestaurantLike ? 0.3 : 0) + (venue.category === "bar" ? 0.08 : 0) + (hasAnyToken2(tokens, ["group", "sharing", "casual", "pizza", "taco", "brewery"]) ? 0.24 : 0) + signals.roleFit.start * 0.14 + signals.currentRelevance * 0.12 + signals.authorityScore * 0.12
      );
      if (isRestaurantLike) reasons.push("group-friendly food base");
      if (hasAnyToken2(tokens, ["sharing", "casual", "group"])) reasons.push("casual group table signal");
      break;
    }
    case "low_key_experience": {
      fit = clamp0125(
        (venue.category === "activity" ? 0.2 : 0) + (venue.category === "park" ? 0.2 : 0) + signals.roleFit.windDown * 0.18 + (hasAnyToken2(tokens, ["arcade", "board", "lounge", "quiet", "low key", "hang"]) ? 0.24 : 0) + (signals.hiddenGemScore + signals.currentRelevance) * 0.09
      );
      if (hasAnyToken2(tokens, ["quiet", "hang", "lounge"])) reasons.push("low-key social reset");
      if (signals.roleFit.windDown >= 0.56) reasons.push("late-sequence comfort fit");
      break;
    }
    case "neighborhood_dive_or_pub": {
      fit = clamp0125(
        (venue.category === "bar" ? 0.34 : 0) + signals.roleFit.windDown * 0.2 + signals.liveNightlifePotential * 0.14 + (hasAnyToken2(tokens, ["pub", "dive", "tap", "local", "tavern"]) ? 0.24 : 0) + signals.hiddenGemScore * 0.08
      );
      if (venue.category === "bar") reasons.push("pub/dive-ready lane");
      if (hasAnyToken2(tokens, ["pub", "dive", "tavern"])) reasons.push("neighborhood pub signal");
      break;
    }
    case "group_gathering_point": {
      fit = clamp0125(
        (venue.category === "bar" ? 0.2 : 0) + (venue.category === "restaurant" ? 0.14 : 0) + (venue.category === "event" ? 0.14 : 0) + signals.roleFit.start * 0.2 + (hasAnyToken2(tokens, ["gather", "meet", "plaza", "market", "social"]) ? 0.2 : 0) + signals.currentRelevance * 0.12
      );
      if (signals.roleFit.start >= 0.58) reasons.push("group launch fit");
      if (hasAnyToken2(tokens, ["market", "social", "meet"])) reasons.push("gathering point signal");
      break;
    }
    case "group_activity_anchor": {
      fit = clamp0125(
        (venue.category === "activity" ? 0.28 : 0) + (venue.category === "event" ? 0.2 : 0) + (isPerformanceLike ? 0.12 : 0) + signals.roleFit.highlight * 0.18 + signals.eventPotential * 0.12 + (hasAnyToken2(tokens, ["game", "activity", "bowling", "karaoke", "live"]) ? 0.1 : 0)
      );
    ...
    case "late_energy_venue": {
      fit = clamp0125(
        (venue.category === "bar" ? 0.24 : 0) + (venue.category === "event" ? 0.14 : 0) + signals.liveNightlifePotential * 0.2 + signals.lateNightPotential * 0.18 + signals.roleFit.windDown * 0.14 + (hasAnyToken2(tokens, ["late", "dance", "dj", "music", "energy"]) ? 0.1 : 0)
      );
    ...
    case "wine_or_craft_debrief": {
      fit = clamp0125(
        (venue.category === "bar" ? 0.2 : 0) + (venue.category === "restaurant" ? 0.14 : 0) + signals.roleFit.windDown * 0.18 + signals.authorityScore * 0.12 + (hasAnyToken2(tokens, ["wine", "craft", "brew", "conversation", "debrief"]) ? 0.24 : 0) + socialConversation * 0.12
      );
      if (hasAnyToken2(tokens, ["wine", "craft", "conversation"])) reasons.push("debrief-friendly setting");
      break;
    }
    case "group_dinner": {
      fit = clamp0125(
        (venue.category === "restaurant" ? 0.34 : 0) + signals.roleFit.highlight * 0.16 + signals.roleFit.windDown * 0.12 + signals.authorityScore * 0.14 + (hasAnyToken2(tokens, ["group", "dinner", "table", "family style", "shared"]) ? 0.16 : 0) + signals.currentRelevance * 0.08
      );
    ...
    case "cultured_closer": {
      fit = clamp0125(
        (venue.category === "museum" ? 0.16 : 0) + (venue.category === "bar" ? 0.16 : 0) + (venue.category === "live_music" ? 0.16 : 0) + signals.roleFit.windDown * 0.2 + signals.culturalAnchorPotential * 0.14 + (hasAnyToken2(tokens, ["gallery", "jazz", "nightcap", "closing", "reflective"]) ? 0.18 : 0)
      );
    ...
    case "family_wander": {
      fit = clamp0125(
        (venue.category === "park" ? 0.34 : 0) + (venue.category === "museum" ? 0.12 : 0) + signals.roleFit.start * 0.2 + (hasAnyToken2(tokens, ["walk", "garden", "trail", "outdoor", "family"]) ? 0.24 : 0) + (1 - venue.energyLevel / 5) * 0.1
      );
    ...
    case "family_lunch": {
      fit = clamp0125(
        (isRestaurantLike ? 0.3 : 0) + (venue.category === "cafe" ? 0.14 : 0) + signals.roleFit.start * 0.16 + (hasAnyToken2(tokens, ["lunch", "family", "kid", "casual"]) ? 0.22 : 0) + signals.authorityScore * 0.1 + (1 - venue.energyLevel / 5) * 0.08
      );
      if (hasAnyToken2(tokens, ["lunch", "family", "kid"])) reasons.push("family lunch fit");
      break;
    }
    case "gentle_experience": {
      fit = clamp0125(
        (venue.category === "museum" ? 0.2 : 0) + (venue.category === "park" ? 0.18 : 0) + (venue.category === "activity" ? 0.14 : 0) + signals.roleFit.highlight * 0.16 + signals.culturalAnchorPotential * 0.1 + (hasAnyToken2(tokens, ["gentle", "interactive", "family", "learning"]) ? 0.22 : 0)
      );
      if (hasAnyToken2(tokens, ["family", "learning", "interactive"])) reasons.push("gentle family experience");
      break;
    }
    case "easy_dinner": {
      fit = clamp0125(
        (venue.category === "restaurant" ? 0.34 : 0) + signals.roleFit.windDown * 0.18 + (hasAnyToken2(tokens, ["easy", "family", "dinner", "casual"]) ? 0.22 : 0) + signals.currentRelevance * 0.1 + (1 - venue.energyLevel / 5) * 0.12
      );
    ...
    case "high_energy_anchor": {
      fit = clamp0125(
        (venue.category === "activity" ? 0.24 : 0) + (venue.category === "event" ? 0.2 : 0) + signals.eventPotential * 0.14 + signals.performancePotential * 0.1 + signals.roleFit.highlight * 0.18 + (hasAnyToken2(tokens, ["play", "interactive", "active", "sports", "arcade"]) ? 0.14 : 0)
      );
    ...
    case "outdoor_reset": {
      fit = clamp0125(
        (venue.category === "park" ? 0.3 : 0) + (scenicLike ? 0.18 : 0) + signals.roleFit.windDown * 0.18 + (hasAnyToken2(tokens, ["outdoor", "garden", "reset", "walk"]) ? 0.22 : 0) + (1 - venue.energyLevel / 5) * 0.12
      );
    ...
    case "casual_group_lunch": {
      fit = clamp0125(
        (isRestaurantLike ? 0.3 : 0) + (venue.category === "cafe" ? 0.1 : 0) + signals.roleFit.start * 0.16 + (hasAnyToken2(tokens, ["lunch", "group", "casual", "family"]) ? 0.24 : 0) + signals.currentRelevance * 0.1 + signals.authorityScore * 0.1
      );
      if (hasAnyToken2(tokens, ["lunch", "group", "casual"])) reasons.push("casual group lunch signal");
      break;
    }
    case "afternoon_neighborhood_or_cultural": {
      fit = clamp0125(
        (venue.category === "museum" ? 0.2 : 0) + (venue.category === "park" ? 0.16 : 0) + signals.culturalAnchorPotential * 0.18 + signals.roleFit.windDown * 0.16 + (hasAnyToken2(tokens, ["neighborhood", "cultural", "afternoon", "market"]) ? 0.2 : 0) + signals.hiddenGemScore * 0.1
      );
    ...
    case "primary_cultural_institution": {
      fit = clamp0125(
        (venue.category === "museum" ? 0.4 : 0) + signals.culturalAnchorPotential * 0.24 + signals.roleFit.start * 0.14 + (hasAnyToken2(tokens, ["museum", "institution", "gallery", "cultural"]) ? 0.16 : 0) + signals.authorityScore * 0.06
      );
    ...
    case "debrief_stop": {
      fit = clamp0125(
        (venue.category === "cafe" ? 0.24 : 0) + (venue.category === "bar" ? 0.12 : 0) + signals.roleFit.windDown * 0.16 + (hasAnyToken2(tokens, ["debrief", "coffee", "conversation", "pause"]) ? 0.26 : 0) + socialConversation * 0.14 + signals.currentRelevance * 0.08
      );
      if (hasAnyToken2(tokens, ["coffee", "conversation", "pause"])) reasons.push("debrief pacing support");
      break;
    }
    case "secondary_cultural_stop": {
      fit = clamp0125(
        (venue.category === "museum" ? 0.28 : 0) + (venue.category === "activity" ? 0.12 : 0) + signals.culturalAnchorPotential * 0.22 + signals.roleFit.highlight * 0.16 + (hasAnyToken2(tokens, ["cultural", "history", "gallery", "learning"]) ? 0.16 : 0) + signals.hiddenGemScore * 0.06
      );
    ...
    case "thematic_lunch": {
      fit = clamp0125(
        (isRestaurantLike ? 0.28 : 0) + signals.roleFit.start * 0.14 + signals.authorityScore * 0.12 + (hasAnyToken2(tokens, ["thematic", "regional", "chef", "lunch"]) ? 0.24 : 0) + signals.culturalAnchorPotential * 0.12 + signals.currentRelevance * 0.1
      );
      if (hasAnyToken2(tokens, ["regional", "thematic", "lunch"])) reasons.push("thematic lunch support");
      break;
    }
    case "adult_payoff_dinner": {
      fit = clamp0125(
        (venue.category === "restaurant" ? 0.34 : 0) + signals.roleFit.windDown * 0.16 + signals.authorityScore * 0.16 + signals.currentRelevance * 0.08 + (hasAnyToken2(tokens, ["chef", "tasting", "signature", "dinner", "payoff"]) ? 0.18 : 0) + (venue.priceTier === "$$$" || venue.priceTier === "$$$$" ? 0.08 : 0)
      );
      if (hasAnyToken2(tokens, ["chef", "tasting", "signature"])) reasons.push("adult payoff dinner signal");
      break;
    }
    default:
      fit = 0;
  ...
  const scenarioAdjustedFit = clamp0125(fit * 0.72 + familyAlignment * 0.28);
...
function buildStopTypeCandidateBoard(input) {
  ...
  for (const scoredVenue of scored) {
    ...
    for (const stopType of requiredStopTypes) {
      ...
      const scenarioRelevance = clamp0125(
...
=== SCENARIO BUILDER PROBE ${scenario.city} / ${scenario.persona} / ${scenario.vibe} ===`);
  ...
  nights.forEach((night, index) => {
    ...
    console.log(
      `     great-stop: pass=${night.evaluation?.passesGreatStopStandard ?? false} | failedStops=${(night.evaluation?.failedStops ?? []).join(", ") || "none"}`
    );
    night.stops.forEach((stop, stopIndex) => {
      console.log(
        `     - ${stopIndex + 1} ${stop.position} [${stop.stopType}] ${stop.name} (${stop.district ?? "n/a"}) | authority ${format(stop.authorityScore)} | current ${format(stop.currentRelevance)}${stop.isHiddenGem ? " | hidden-gem" : ""}`
      );
      ...
      if (stop.venueTypeLabel || stop.factualSummary) {
        console.log(
          `       trust: ${stop.venueTypeLabel ?? "n/a"} | ${stop.factualSummary ?? "n/a"} | source=${stop.sourceType ?? "n/a"}`
        );
      }
      if (stop.venueFeatures?.length || stop.serviceOptions?.length) {
        console.log(
          `       facts: features=[${(stop.venueFeatures ?? []).join(", ")}] services=[${(stop.serviceOptions ?? []).join(", ")}]`
        );
      }
      if (stop.evaluation) {
        console.log(
          `       eval: real=${stop.evaluation.isReal} role=${stop.evaluation.isRoleRight} intent=${stop.evaluation.isIntentRight} place=${stop.evaluation.isPlaceRight} moment=${stop.evaluation.isMomentRight} fails=${stop.evaluation.failedCriteria.join(", ") || "none"}`
        );
      }
    });
  });
