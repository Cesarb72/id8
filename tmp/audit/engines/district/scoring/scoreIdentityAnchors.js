...
function buildAnchorScore(entity, pocketEntities, primaryMicroPocket) {
    ...
    const identityAlignment = (primaryMicroPocket.dominantCategories.includes(category) ? 0.62 : 0.26) +
    ...
    const activationLift = primaryMicroPocket.activationStrength * 0.58 +
        primaryMicroPocket.environmentalInfluencePotential * 0.42;
    const genericPenalty = !primaryMicroPocket.dominantCategories.includes(category) &&
    ...
    const score = clamp(proximityScore * 0.33 +
...
export function scoreIdentityAnchors({ pocket, microPockets, }) {
