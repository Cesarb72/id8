...
function computeAnchorCandidateIds(entities, dominantCategories, dominantLanes, centroid, radiusM) {
    ...
    return entities
        ...
        const experienceForwardSignal = ['activity', 'event', 'program', 'hub'].includes(entity.type) ||
            entity.categories.some((value) => ['bar', 'restaurant', 'cafe', 'museum', 'event', 'activity'].includes(value.toLowerCase()))
            ? 1
            : 0.42;
        const score = proximityScore * 0.34 +
...
function buildMicroPocket(pocketId, entities, index) {
    ...
    const coherenceScore = clamp(compactness * 0.34 +
        (1 - categoryDominance) * 0.16 +
        (1 - laneDominance) * 0.15 +
        categoryDiversity * 0.18 +
        laneDiversity * 0.17, 0, 1);
    const densitySignal = clamp((entities.length / Math.max(1, Math.pow(Math.max(55, radiusM), 2))) * 4200, 0, 1);
    const experienceForwardCount = entities.filter((entity) => ['activity', 'event', 'program', 'hub'].includes(entity.type) ||
        entity.categories.some((category) => ['bar', 'restaurant', 'cafe', 'museum', 'event', 'activity'].includes(category.toLowerCase()))).length;
    const experienceForwardSignal = clamp(experienceForwardCount / Math.max(1, entities.length), 0, 1);
    const identityStrength = clamp(coherenceScore * 0.3 +
        categoryDiversity * 0.18 +
        laneDiversity * 0.14 +
        (experienceForwardSignal >= 0.55 ? 0.2 : 0.09) +
        (categoryDiversity >= 0.6 && laneDiversity >= 0.5 ? 0.18 : 0.08), 0, 1);
    const activationStrength = clamp(densitySignal * 0.42 +
        categoryDiversity * 0.2 +
        laneDiversity * 0.14 +
        compactness * 0.12 +
        experienceForwardSignal * 0.12, 0, 1);
    const categoryMixAdaptability = clamp(categoryCounts.filter((entry) => entry.count >= 1).length / 6, 0, 1);
    const environmentalInfluencePotential = clamp(activationStrength * 0.48 +
        categoryMixAdaptability * 0.24 +
        categoryDiversity * 0.16 +
        densitySignal * 0.12, 0, 1);
    const reasonSignals = [];
    ...
    return {
        ...
        anchorCandidateIds: computeAnchorCandidateIds(entities, dominantCategories, dominantLanes, centroid, radiusM),
        reasonSignals,
    };
}
...
export function resolveMicroPockets(pocket) {
