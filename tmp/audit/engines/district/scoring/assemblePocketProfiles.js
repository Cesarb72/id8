...
function buildWhyHereSignals(input) {
    const signals = Array.from(new Set([...input.primaryReasons, ...input.secondaryReasons, ...input.anchorReasons])).slice(0, 5);
    if (signals.length === 0) {
...
function buildHyperlocal(pocket) {
    ...
    const localSpecificityScore = toFixed(clamp(primaryMicroPocket.identityStrength * 0.34 +
        primaryMicroPocket.activationStrength * 0.24 +
        primaryMicroPocket.environmentalInfluencePotential * 0.22 +
        (anchorScores.primaryAnchor?.score ?? 0) * 0.2, 0, 1));
    return {
        primaryMicroPocket,
...
export function assemblePocketProfiles(identifiedPockets, context = {}) {
    ...
    return identifiedPockets.map((pocket) => {
        ...
        const appSignals = vertical === 'generic'
            ? undefined
            : {
