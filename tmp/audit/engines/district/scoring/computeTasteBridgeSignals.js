...
function computeHospitalityMix(pocket) {
    ...
    for (const [category, count] of Object.entries(pocket.categoryCounts)) {
        const mapping = CATEGORY_TO_MIX_WEIGHTS[category] ??
...
export function computeTasteBridgeSignals(pocket) {
    ...
    const compactness = clamp(1 - (Math.max(1, pocket.geometry.elongationRatio) - 1) / 3, 0, 1);
    const spreadTightness = clamp(1 - pocket.geometry.maxDistanceFromCentroidM / 460, 0, 1);
    const energyScore = clamp(density * 0.45 + hospitalityMix.activity * 0.35 + hospitalityMix.drinks * 0.2, 0, 1);
    const noiseScore = clamp(energyScore * 0.55 + hospitalityMix.drinks * 0.25 + hospitalityMix.activity * 0.2, 0, 1);
    const intimacyScore = clamp(compactness * 0.35 +
        spreadTightness * 0.25 +
        (1 - noiseScore) * 0.2 +
        (hospitalityMix.cafe + hospitalityMix.dining) * 0.2, 0, 1);
    const compositionRichness = clamp(Object.values(hospitalityMix).filter((weight) => weight >= 0.12).length / 5, 0, 1);
    const momentPotential = toFixed(diversity * 0.35 + density * 0.25 + compositionRichness * 0.4);
    return {
        experientialTags: deriveExperientialTags(hospitalityMix, energyScore, noiseScore, intimacyScore),
        hospitalityMix,
        ambianceProfile: {
