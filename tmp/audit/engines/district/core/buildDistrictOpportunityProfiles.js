...
function promoteBestReject(viability) {
    ...
    return {
        evaluated: viability.evaluated.map((pocket) => pocket.id === promoted.id ? promoted : pocket),
        accepted: [promoted],
        rejected: viability.rejected.filter((pocket) => pocket.id !== promoted.id),
    };
}
export async function buildDistrictOpportunityProfiles(input, context = {}) {
    ...
    if (location.source === 'unresolved_query') {
        stageNotes.push(location.meta.unresolvedReason ??
    ...
    if (rawPockets.length === 0 && entities.length >= 3) {
        ...
        const synthetic = buildRawPocketFromEntities(entities.slice(0, Math.min(8, entities.length)), FALLBACK_CLUSTERING, {
