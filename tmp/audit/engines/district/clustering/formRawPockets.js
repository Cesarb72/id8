...
import { centroidOf, computeBoundingBoxMetrics, haversineDistanceM, } from './geoDistance';
const ZERO_GEOMETRY = {
...
function computeGeometryMetrics(entities) {
    ...
    const avgDistanceFromCentroidM = distancesFromCentroidM.length === 0
        ? 0
...
export function buildRawPocketFromEntities(entities, clustering, options) {
...
export function formRawPockets(input) {
    ...
    return clusteringResult.clusters
        .map((cluster) => buildRawPocketFromEntities(cluster.points, input.clustering, {
