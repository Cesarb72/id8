...
export function getCanonicalDistanceStatus(driveMinutes) {
...
export function isWithinStrictNearbyWindow(driveMinutes, distanceMode) {
    return distanceMode === 'nearby' && getCanonicalDistanceStatus(driveMinutes) === 'inside_strict_nearby';
}
export function isOutsideStrictNearbyButWithinBoundedStretch(driveMinutes, distanceMode) {
    return distanceMode === 'nearby' && getCanonicalDistanceStatus(driveMinutes) === 'inside_bounded_stretch';
}
export function getStretchDistanceStatus(candidate, intent) {
...
export function isMeaningfulMomentStretchCandidate(candidate, intent) {
    if (!isOutsideStrictNearbyButWithinBoundedStretch(candidate.venue.driveMinutes, intent.distanceMode)) {
        return false;
    }
    const archetype = candidate.taste.signals.primaryExperienceArchetype;
    const genericHospitalityFallback = isHospitalityArchetype(archetype) &&
        candidate.taste.signals.momentPotential.score < 0.72 &&
        candidate.venue.signature.genericScore >= 0.45;
    const contractRelevant = candidate.taste.modeAlignment.score >= 0.56 ||
    ...
    return (candidate.momentIdentity.strength === 'strong' &&
...
export function isCandidateWithinActiveDistanceWindow(candidate, intent, options = {}) {
...
export function isCandidateUsedByStretch(candidate, intent) {
    return (intent.distanceMode === 'nearby' &&
