...
function computeOutingLengthScore(estimatedTotalMinutes, intent) {
...
function computePacingBalance(stops, transitions) {
    const reasons = [];
    const rewards = [];
    let score = 0.58;
    let penalty = 0;
    ...
    if (transitions.every((transition) => transition.frictionScore <= 2) &&
...
export function computeRouteDuration(stops, intent) {
    const stopDurations = stops.map((stop) => estimateStopDuration(stop));
    const transitions = stops.slice(0, -1).map((stop, index) => {
        const priorDistinctNeighborhoods = new Set(stops
            .slice(0, index + 1)
            .map((item) => item.scoredVenue.venue.neighborhood)).size;
    ...
    const estimatedStopMinutes = stopDurations.reduce((sum, stop) => sum + stop.estimatedDurationMinutes, 0);
    ...
    const transitionSmoothnessScore = clamp01(1 - totalRouteFriction / Math.max(4, transitions.length * 4));
