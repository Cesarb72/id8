const tasteModes = {
    ...
    'highlight-centered': {
        ...
        reasonBuilder: (intent) => intent.refinementModes?.includes('more-exciting')
    ...
    'wander-explore': {
        ...
        reasonBuilder: (intent) => intent.mode === 'surprise'
...
export function selectTasteMode(intent) {
...
export function applyTasteModeToLens(lens, tasteMode) {
    const nextLens = {
        ...lens,
        tasteMode,
        ...
        wildcardAggressiveness: Math.max(lens.wildcardAggressiveness, tasteMode.wildcardAggressivenessFloor),
...
export function getTasteExperienceLane(venue) {
...
export function getTasteModeAlignment(venue, tasteMode, options = {}) {
    ...
    const lanePriorityScore = lanePriorityIndex === 0
        ? 1
        : lanePriorityIndex === 1
            ? 0.82
            : lanePriorityIndex === 2
    ...
    const modeSignalStrength = getModeSignalStrength(venue, tasteMode, normalizedTags, textSignals);
    const hospitalityLane = lane === 'dining' || lane === 'drinks' || lane === 'sweet';
    const genericFallbackPenalty = protectedCandidate
        ? 0
        : (tasteMode.id === 'scenic-outdoor' || tasteMode.id === 'activity-led') &&
            ...
            : hospitalityLane &&
                ...
                : hospitalityLane &&
                    ...
                    ? 0.58
                    : 0;
    const overall = Math.max(0, Math.min(1, lanePriorityScore * 0.42 +
        categoryMatch * 0.22 +
    ...
    const penalty = protectedCandidate
        ? 0
        : Math.max(0, categoryPenalty * tasteMode.penaltyWeight * 0.34 +
...
function getModeSignalStrength(venue, tasteMode, normalizedTags, textSignals) {
    if (tasteMode.id === 'scenic-outdoor') {
        const strong = (venue.category === 'park' ? 0.84 : 0) +
    ...
    if (tasteMode.id === 'activity-led') {
        const strong = (venue.category === 'activity' ? 0.78 : venue.category === 'event' ? 0.28 : 0) +
            (hasAnyTag(normalizedTags, [
                'arcade',
                'board-games',
                'games',
    ...
    const genericStrong = (tasteMode.favoredCategories.includes(venue.category) ? 0.38 : 0) +
        tagOverlap(venue.tags, tasteMode.favoredTags) * 0.42;
    const genericLight = genericStrong * 0.72 +
