...
function isDistrictStyleGenericName(name) {
...
function getHighlightValidityScore(candidate) {
...
function getRoleSpecificLift(candidate, role) {
    if (role === 'peak') {
        return (candidate.taste.signals.momentIntensity.score * 0.16 +
    ...
    if (role === 'warmup') {
        const durationBonus = candidate.taste.signals.durationEstimate === 'quick' ||
            candidate.taste.signals.durationEstimate === 'moderate'
            ? 0.04
            : 0;
        return (candidate.taste.signals.roleSuitability.start * 0.08 +
            candidate.stopShapeFit.start * 0.06 +
            durationBonus);
    }
    if (role === 'cooldown') {
        const durationBonus = candidate.taste.signals.durationEstimate === 'moderate' ||
            candidate.taste.signals.durationEstimate === 'extended'
            ? 0.03
            : 0;
        return (candidate.taste.signals.roleSuitability.windDown * 0.08 +
    ...
    return (candidate.taste.signals.roleSuitability.surprise * 0.06 +
        candidate.stopShapeFit.surprise * 0.05);
}
function getGenericPenalty(candidate, role) {
    let penalty = candidate.taste.fallbackPenalty.signalScore * 0.18 +
...
export function scoreAnchoredRoleFit(candidate, role) {
    const base = candidate.roleScores[role] * 0.34 +
