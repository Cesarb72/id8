...
import { isRomanticPersonaContractActive, satisfiesRomanticPersonaHighlightContract, } from '../contracts/romanticPersonaContract';
import { getArcStopBaseVenueId, getArcStopCandidateId, getScoredVenueBaseVenueId, getScoredVenueCandidateId, } from '../candidates/candidateIdentity';
import { isCandidateWithinActiveDistanceWindow } from '../constraints/localStretchPolicy';
import { buildRolePools } from './buildRolePools';
import { getInvalidArcCombinationReasons, hasArcGeographyViolation, isValidArcCombination, } from './isValidArcCombination';
import { isArcViable, scoreArcAssembly } from './scoreArcAssembly';
function buildCoreStops(warmup, peak, cooldown) {
    return [
...
function withWildcard(stops, wildcard) {
    return [
        stops[0],
        stops[1],
...
function scoreSupportReadability(support, peak, role) {
    ...
    const energyContrast = role === 'warmup'
        ...
        : Math.max(0, (support.venue.energyLevel <= peak.venue.energyLevel
            ...
            : 0) / 4);
    const intensityContrast = role === 'warmup'
        ? Math.max(0, peak.taste.signals.momentIntensity.score -
            support.taste.signals.momentIntensity.score)
        : Math.max(0, peak.taste.signals.momentIntensity.score -
            support.taste.signals.momentIntensity.score +
            0.04);
    return (roleScore * 0.46 +
        shapeScore * 0.24 +
        energyContrast * 0.14 +
...
function getBestSupportStop(supports, peak, role) {
    const candidates = supports
        .filter((support) => support.venue.id !== peak.venue.id)
        .sort((left, right) => {
        const scoreDelta = scoreSupportReadability(right, peak, role) -
            scoreSupportReadability(left, peak, role);
        if (scoreDelta !== 0) {
            return scoreDelta;
        }
        return getScoredVenueCandidateId(left).localeCompare(getScoredVenueCandidateId(right));
    });
    return candidates[0];
}
function buildPartialFallbackCandidates(pools, intent, crewPolicy, lens) {
    const candidates = [];
    ...
    for (const peak of peaks) {
        ...
        for (const stops of twoStopOptions) {
            ...
            const invalidReasons = getInvalidArcCombinationReasons(stops, intent, crewPolicy, lens);
        ...
        if (highlightIntensity >= 0.8) {
            const highlightOnlyStops = [{ role: 'peak', scoredVenue: peak }];
            const invalidReasons = getInvalidArcCombinationReasons(highlightOnlyStops, intent, crewPolicy, lens);
            if (invalidReasons.length === 0) {
                const key = buildDiagnosticArcId(highlightOnlyStops);
                if (!candidateKeys.has(key)) {
                    const score = scoreArcAssembly(highlightOnlyStops, intent, crewPolicy, lens, pools);
                    candidateKeys.add(key);
                    candidates.push({
...
function buildDiagnosticArcId(stops) {
    return stops.map((stop) => `${stop.role}:${getArcStopCandidateId(stop)}`).join('|');
}
const preferredDiscoveryRoleOrder = ['peak', 'warmup', 'cooldown'];
...
function getSurpriseGateProbability(lens) {
...
function getSurpriseComparisonGateProbability(candidateTier, lens) {
...
function getAllowedSurpriseScoreTradeoff(lens) {
    if (lens.discoveryBias === 'high') {
        return 0.025;
    }
    if (lens.discoveryBias === 'medium') {
...
function getSurpriseAcceptanceBonus(candidate) {
...
function getSurpriseCandidateTier(candidate, lens) {
    const taste = candidate.taste.signals;
    const minimumStrongRoleScore = lens.discoveryBias === 'high' ? 0.6 : 0.64;
    const isStrong = candidate.roleScores.wildcard >= minimumStrongRoleScore &&
    ...
    const isNearStrong = candidate.roleScores.wildcard >= 0.55 &&
...
function computeHighlightPromotionAlignmentScore(candidate) {
...
function isGenericHospitalityFallbackHighlight(candidate) {
...
function getRomanticPromotionAdvantage(wildcard, currentHighlight, lens) {
    if (!isRomanticPersonaContractActive(lens) ||
        !satisfiesRomanticPersonaHighlightContract(wildcard, lens)) {
        return 0;
    }
    let advantage = 0.04;
    if (wildcard.momentIdentity.type === 'anchor' ||
...
function isHighlightPromotionHoursOk(candidate) {
...
function getHighlightPromotionBlockReason(candidate, intent) {
    if (intent.planningMode === 'user-led' &&
    ...
    if (!isCandidateWithinActiveDistanceWindow(candidate, intent, {
...
function shouldKeepDemotedHighlightAsSurprise(baseStops, promotedHighlight) {
    const demotedHighlight = baseStops[1]?.scoredVenue;
    if (!demotedHighlight) {
        return false;
    }
    if (demotedHighlight.roleScores.wildcard < 0.55 ||
    ...
    const repeatedWithPromoted = demotedHighlight.venue.category === promotedHighlight.venue.category &&
        demotedArchetype === promotedArchetype;
    const repeatedWithSupport = [baseStops[0], baseStops[2]].some((stop) => stop.scoredVenue.venue.category === demotedHighlight.venue.category &&
        stop.scoredVenue.taste.signals.primaryExperienceArchetype === demotedArchetype);
    return !((repeatedWithPromoted || repeatedWithSupport) &&
        demotedHighlight.taste.signals.momentPotential.score < 0.68);
}
function buildPromotedSurpriseStops(baseStops, promotedHighlight, demotedHighlightDisposition) {
...
function evaluateSurprisePromotion(params) {
    ...
    const romanticPromotionAdvantage = getRomanticPromotionAdvantage(params.wildcard, currentHighlight, params.lens);
    if (computeHighlightPromotionAlignmentScore(params.wildcard) + romanticPromotionAdvantage <=
        computeHighlightPromotionAlignmentScore(currentHighlight) + 0.03) {
        return { outcome: 'held_role_invalid' };
    }
    const dispositions = shouldKeepDemotedHighlightAsSurprise(params.baseStops, params.wildcard)
        ? ['surprise', 'removed']
        : ['removed'];
    const promotedCandidates = dispositions
        .map((demotedHighlightDisposition) => {
        const promotedStops = buildPromotedSurpriseStops(params.baseStops, params.wildcard, demotedHighlightDisposition);
        if (getInvalidArcCombinationReasons(promotedStops, params.intent, params.crewPolicy, params.lens).length > 0) {
            return null;
        }
        const promotedScore = scoreArcAssembly(promotedStops, params.intent, params.crewPolicy, params.lens, params.pools);
        return {
            promotedStops,
            promotedScore,
...
function isSurpriseSpatiallyFeasible(wildcardStops, wildcardCandidate, candidateTier) {
    const wildcardVenueId = wildcardStops[2]?.scoredVenue.venue.id;
    const relatedTransitions = wildcardCandidate.spatial.transitions.filter((transition) => transition.fromVenueId === wildcardVenueId ||
        transition.toVenueId === wildcardVenueId);
    if (relatedTransitions.length === 0) {
        return false;
    }
    if (wildcardCandidate.spatial.mode === 'walkable') {
        const relatedLongTransitions = relatedTransitions.filter((transition) => transition.longTransition);
        const strictWalkableFeasibility = wildcardCandidate.spatial.score >= 0.72 &&
        ...
        return (wildcardCandidate.spatial.score >= 0.7 &&
            wildcardCandidate.spatial.repeatedClusterEscapeCount === 0 &&
    ...
    return (wildcardCandidate.spatial.score >= 0.68 &&
...
function buildSurpriseInjectionMetadata(wildcard, candidateTier, gateProbability, gateScore, wildcardScore, scoreDeltaFromBase, allowedTradeoff, acceptanceBonusValue, promotion) {
    ...
    return {
        ...
        selectionReason: `${wildcard.venue.name} cleared the surprise gate with strong experiential/novelty read, ` +
...
function candidateMatchesPreferredRole(candidate, role, venueId) {
    return candidate.stops.some((stop) => stop.role === role && getArcStopBaseVenueId(stop) === venueId);
}
function stopsMatchRole(stops, role, venueId) {
    return stops.some((stop) => stop.role === role && getArcStopBaseVenueId(stop) === venueId);
}
function updateBestAnchorDiagnostic(current, candidateId, score) {
...
function preservePreferredArcCandidates(rankedCandidates, pools, intent, limit) {
    if (rankedCandidates.length <= limit) {
        return rankedCandidates;
    }
    const preferredArcCandidates = [];
    const preferredArcIds = new Set();
    if (intent.planningMode === 'user-led' && intent.anchor?.venueId) {
        ...
        if (anchorStatus.preferredDiscoveryVenueAdmitted &&
            anchorStatus.preferredDiscoveryVenueId === intent.anchor.venueId) {
            const anchorArc = rankedCandidates.find((candidate) => candidateMatchesPreferredRole(candidate, anchorRole, intent.anchor.venueId));
            if (anchorArc) {
    ...
    for (const role of preferredDiscoveryRoleOrder) {
        ...
        const preferredArc = rankedCandidates.find((candidate) => candidateMatchesPreferredRole(candidate, role, status.preferredDiscoveryVenueId));
    ...
    const missingPreferredArcCandidates = preferredArcCandidates.filter((candidate) => !topCandidateIds.has(candidate.id));
    if (missingPreferredArcCandidates.length === 0) {
        return topCandidates;
    }
    const missingPreferredArcIds = new Set(missingPreferredArcCandidates.map((candidate) => candidate.id));
    ...
    const rankedIndex = new Map(rankedCandidates.map((candidate, index) => [candidate.id, index]));
    return finalCandidates.sort((left, right) => (rankedIndex.get(left.id) ?? 0) - (rankedIndex.get(right.id) ?? 0));
}
export function assembleArcCandidates(scoredVenues, intent, crewPolicy, lens, prebuiltPools) {
    const pools = prebuiltPools ?? buildRolePools(scoredVenues, crewPolicy, lens);
    const anchorRole = intent.planningMode === 'user-led' && intent.anchor?.venueId
        ? toInternalRole(intent.anchor.role ?? 'highlight')
        : undefined;
    const anchorVenueId = intent.planningMode === 'user-led' ? intent.anchor?.venueId : undefined;
    ...
    const invalidatedAnchorArcReasons = {};
    let bestPreValidationAnchorArc;
    let bestValidatedAnchorArc;
    const strongSurpriseCandidates = pools.wildcard.filter((candidate) => getSurpriseCandidateTier(candidate, lens) === 'strong');
    const nearStrongSurpriseCandidates = pools.wildcard.filter((candidate) => getSurpriseCandidateTier(candidate, lens) === 'nearStrong');
    const surpriseCandidates = [...strongSurpriseCandidates, ...nearStrongSurpriseCandidates];
    const strongSurpriseIds = new Set(strongSurpriseCandidates.map((candidate) => getScoredVenueCandidateId(candidate)));
    const gateProbability = getSurpriseGateProbability(lens);
    const surpriseDiagnostics = {
    ...
    for (const warmup of pools.warmup) {
        for (const peak of pools.peak) {
            for (const cooldown of pools.cooldown) {
                ...
                const baseIncludesAnchor = anchorRole && anchorVenueId
                ...
                if (baseIncludesAnchor) {
                    ...
                    bestPreValidationAnchorArc = updateBestAnchorDiagnostic(bestPreValidationAnchorArc, buildDiagnosticArcId(baseStops), diagnosticScore.totalScore);
                }
                const baseInvalidationReasons = getInvalidArcCombinationReasons(baseStops, intent, crewPolicy, lens);
                ...
                if (baseIncludesAnchor) {
                    ...
                    bestValidatedAnchorArc = updateBestAnchorDiagnostic(bestValidatedAnchorArc, baseCandidate.id, baseCandidate.totalScore);
                }
                for (const wildcard of surpriseCandidates) {
                    ...
                    const wildcardIncludesAnchor = anchorRole && anchorVenueId
                        ? stopsMatchRole(wildcardStops, anchorRole, anchorVenueId)
                        : false;
                    const promotedIncludesAnchor = anchorRole &&
                        anchorVenueId &&
                    ...
                    if (wildcardIncludesAnchor) {
                        ...
                        bestPreValidationAnchorArc = updateBestAnchorDiagnostic(bestPreValidationAnchorArc, buildDiagnosticArcId(wildcardStops), diagnosticScore.totalScore);
                    }
                    let heldWildcardCandidate;
                    let heldWildcardComparableScore = baseScore.totalScore;
                    const wildcardInvalidationReasons = getInvalidArcCombinationReasons(wildcardStops, intent, crewPolicy, lens);
                    if (wildcardInvalidationReasons.length > 0) {
                        ...
                        if (!isSurpriseSpatiallyFeasible(wildcardStops, wildcardScore, candidateTier)) {
                            ...
                            const appliedGateProbability = getSurpriseComparisonGateProbability(candidateTier, lens);
                            if (gateScore > appliedGateProbability) {
                                ...
                                const allowedTradeoff = getAllowedSurpriseScoreTradeoff(lens) + acceptanceBonusValue;
                                ...
                                if (!surpriseWorthAdding) {
                                    ...
                                    heldWildcardCandidate = {
                                        ...
                                        hasWildcard: true,
                                        surpriseInjection: buildSurpriseInjectionMetadata(wildcard, candidateTier, appliedGateProbability, gateScore, wildcardScore, scoreDeltaFromBase, allowedTradeoff, acceptanceBonusValue),
                                    };
                                }
                            }
                        }
                    }
                    const promotedSurpriseWins = Boolean(promotionAssessment.promotedStops && promotionAssessment.promotedScore) &&
                        promotionAssessment.promotedScore.totalScore >=
                            heldWildcardComparableScore - 0.002;
                    if (promotionAssessment.promotedStops &&
                        ...
                        const promotedCandidate = {
                            ...
                            hasWildcard: promotionAssessment.promotedStops.some((stop) => stop.role === 'wildcard'),
                            surpriseInjection: buildSurpriseInjectionMetadata(wildcard, candidateTier, 1, 0, promotionAssessment.promotedScore, promotionAssessment.promotedScore.totalScore - baseScore.totalScore, 0, 0, {
                                ...
                                demotedHighlightDisposition: promotionAssessment.demotedHighlightDisposition,
                            }),
                        };
                        candidates.push(promotedCandidate);
                        if (promotedIncludesAnchor) {
                            postValidationAnchorArcCount += 1;
                            bestValidatedAnchorArc = updateBestAnchorDiagnostic(bestValidatedAnchorArc, promotedCandidate.id, promotedCandidate.totalScore);
                        }
                        continue;
                    }
                    if (heldWildcardCandidate) {
                        ...
                        const finalizedHeldWildcardCandidate = {
                            ...heldWildcardCandidate,
                            surpriseInjection: {
                                ...existingSurpriseInjection,
                                promotionOutcome: promotionAssessment.promotedStops && promotionAssessment.promotedScore
                                    ? 'held_score'
                                    : promotionAssessment.outcome,
                                promotionNote: promotionAssessment.promotedStops && promotionAssessment.promotedScore
                        ...
                        if (wildcardIncludesAnchor) {
                            postValidationAnchorArcCount += 1;
                            bestValidatedAnchorArc = updateBestAnchorDiagnostic(bestValidatedAnchorArc, finalizedHeldWildcardCandidate.id, finalizedHeldWildcardCandidate.totalScore);
                        }
                    }
                }
            }
    ...
    if (candidates.length === 0) {
        const partialFallbackCandidates = buildPartialFallbackCandidates(pools, intent, crewPolicy, lens);
    ...
    const rankedCandidates = candidates.sort((left, right) => {
        ...
        const leftPromotion = left.surpriseInjection?.promotionOutcome === 'promoted_to_highlight' ? 1 : 0;
        const rightPromotion = right.surpriseInjection?.promotionOutcome === 'promoted_to_highlight' ? 1 : 0;
        if (rightPromotion !== leftPromotion) {
            return rightPromotion - leftPromotion;
        }
        const leftTier = left.surpriseInjection?.candidateTier === 'strong'
            ? 2
            : left.surpriseInjection?.candidateTier === 'nearStrong'
                ? 1
                : 0;
        const rightTier = right.surpriseInjection?.candidateTier === 'strong'
            ? 2
    ...
    const prunedCandidates = preservePreferredArcCandidates(rankedCandidates, pools, intent, 40);
    const postPruneAnchorCandidates = anchorRole && anchorVenueId
        ? prunedCandidates.filter((candidate) => candidateMatchesPreferredRole(candidate, anchorRole, anchorVenueId))
        : [];
    const bestPrunedAnchorArc = postPruneAnchorCandidates[0];
    const anchorTrace = anchorRole && anchorVenueId
        ? {
            anchorRole,
            anchorVenueId,
            ...
            anchorArcTrimmedByTopN: postValidationAnchorArcCount > postPruneAnchorCandidates.length,
            anchorArcTrimmedByWhichStage: postValidationAnchorArcCount > postPruneAnchorCandidates.length
                ? 'top_n'
                : undefined,
            ...
            bestPrunedAnchorArcScore: typeof bestPrunedAnchorArc?.totalScore === 'number'
