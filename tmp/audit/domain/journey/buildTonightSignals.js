...
function inferEnergyFromStop(stop, tags) {
    let energy = 0.56;
...
function inferSocialDensityFromStop(stop, tags) {
    let social = 0.54;
...
function inferMomentIntensityFromStop(role, stop, tags) {
...
function inferTemporalLabel(tags) {
...
function getActivityState(socialDensity, energy, temporalLabel) {
...
function getClusterDensity(localTravelMinutes, nearbyOptionsCount) {
...
function getRoleFitBand(roleFit) {
...
function getTimingLine(role, activityState) {
    if (role === 'start') {
...
function getEnergyLine(role, energyBand, intensityBand, roleFitBand) {
    if (role === 'highlight') {
...
function getContextLine(role, activityState, clusterDensity, localTravelMinutes) {
...
export function buildTonightSignals({ stop, scoredVenue, roleTravelWindowMinutes, nearbySummary: _nearbySummary, nearbyOptionsCount, }) {
    const tags = toTagSet(stop.tags);
    const signals = scoredVenue?.taste.signals;
    const roleKey = inverseRoleProjection[stop.role];
    const localTravelMinutes = typeof roleTravelWindowMinutes === 'number' && roleTravelWindowMinutes > 0
    ...
    const momentIntensity = signals?.momentIntensity.score ?? inferMomentIntensityFromStop(stop.role, stop, tags);
