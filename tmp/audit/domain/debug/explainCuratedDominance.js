function formatSelectedStopSources(selectedStopSources) {
    return Object.entries(selectedStopSources)
        .map(([role, source]) => `${role}:${source}`)
        .join(', ');
}
export function explainCuratedDominance({ attritionTrace, dedupeNoveltyLoss, roleCompetitionByRole, selectedStopSources, }) {
    const comparisons = Object.values(roleCompetitionByRole).filter((comparison) => Boolean(comparison));
    ...
    const stageFrequency = curatedWins.reduce((acc, comparison) => {
        const stage = comparison.strongestLiveLostAtStage ?? 'unknown';
        acc[stage] = (acc[stage] ?? 0) + 1;
        return acc;
    }, {});
    const repeatedLossFactors = curatedWins.reduce((acc, comparison) => {
