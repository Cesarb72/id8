...
function formatEstimatedTotalLabel(minutes) {
...
function isSoftExtensionStop(result, index) {
...
function isCompactEligibleStop(result, index) {
    const stop = result.itinerary.stops[index];
    const arcStop = result.selectedArc.stops[index]?.scoredVenue.venue;
    if (!stop || !arcStop) {
        return false;
...
function isCompactLandingStop(result, index) {
    const stop = result.itinerary.stops[index];
    const arcStop = result.selectedArc.stops[index]?.scoredVenue.venue;
    if (!stop || !arcStop) {
        return false;
    }
    return (stop.role === 'windDown' &&
        (stop.category === 'park' ||
            stop.category === 'dessert' ||
            stop.category === 'cafe' ||
            (stop.category === 'bar' && arcStop.energyLevel <= 2)));
}
function canUseCompactPair(result, firstIndex, secondIndex) {
...
function pickCompactStopIndices(result, maxStops) {
...
function buildCompactStory(plan, stops) {
    const lead = stops[0];
    ...
    if (plan.transition.continuationMode === 'CONTINUATION_FRAGMENT') {
        return {
            ...
            subtitle: plan.transition.vibeShift === 'LAND'
...
function buildCompactItinerary(plan, maxStops) {
    const indices = pickCompactStopIndices(plan.arc2, maxStops);
    const stops = indices
        .map((index) => plan.arc2.itinerary.stops[index])
        .filter(Boolean);
    const transitions = indices.length === 2 && indices[1] === indices[0] + 1
    ...
    const estimatedTransitionMinutes = transitions.reduce((sum, transition) => sum + transition.estimatedTransitionMinutes, 0);
    ...
    const averageTransitionFriction = transitions.length > 0 ? Number((totalRouteFriction / transitions.length).toFixed(2)) : 0;
    const neighborhoods = [...new Set(stops.map((stop) => stop.neighborhood))];
    const itineraryWithoutSummary = {
        id: createId('itinerary'),
        title: plan.transition.continuationMode === 'CONTINUATION_FRAGMENT'
            ? 'Optional Next Move'
        ...
        routeFeelLabel: transitions.length === 0
...
export function buildContinuationDisplayItinerary(plan) {
    if (plan.transition.continuationMode === 'CONTINUATION_FRAGMENT') {
