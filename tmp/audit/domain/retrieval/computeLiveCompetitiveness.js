...
function getRoleKeyCounts(liveScored, role) {
    return liveScored.filter((item) => computeRoleAwareHoursPressure(item.venue, role).penalty > 0).length;
}
export function computeLiveCompetitiveness({ retrieval, scoredVenues, selectedArc, arcCandidates, rolePools, lens, effectiveSourceMode, }) {
    const roleKeys = ['warmup', 'peak', 'wildcard', 'cooldown'];
    ...
    const strongLiveCandidates = liveScored.filter((item) => {
        ...
        return ((liveLift.strongLiveCandidate || item.fitScore >= 0.67) &&
            item.venue.source.qualityGateStatus !== 'suppressed' &&
            item.venue.source.timeConfidence >= 0.48);
    });
    const liveCompetitive = strongLiveCandidates.filter((item) => item.fitScore >= 0.64 || computeHybridLiveLift(item.venue).liftApplied);
    const liveHighlightCandidates = liveScored.filter((item) => {
        const liveLift = computeHybridLiveLift(item.venue);
        return ((liveLift.strongHighlightCandidate || item.roleScores.peak >= 0.69) &&
    ...
    const rolePoolVenueIds = new Set([...rolePools.warmup, ...rolePools.peak, ...rolePools.wildcard, ...rolePools.cooldown].map((item) => item.venue.id));
    const liveRejectedByHighlightValidityCount = liveScored.filter((item) => item.roleScores.peak >= 0.58 && item.highlightValidity.validityLevel === 'invalid').length;
    const liveRejectedByRolePoolCount = liveScored.filter((item) => !rolePoolVenueIds.has(item.venue.id)).length;
    const strongLiveCandidatesFilteredCount = strongLiveCandidates.filter((item) => !rolePoolVenueIds.has(item.venue.id)).length;
    const liveTimeConfidenceAdjustedCount = liveScored.filter((item) => roleKeys.some((role) => computeRoleAwareHoursPressure(item.venue, role).adjusted)).length;
    const liveTimeConfidencePenaltyByRole = {
    ...
    const selectedStopSources = Object.fromEntries(selectedArc.stops.map((stop) => [roleProjection[stop.role], stop.scoredVenue.venue.source.sourceOrigin]));
    ...
    const liveLostToCuratedReason = new Set();
    const curatedVsLiveWinnerNotes = [];
    for (const [role, comparison] of Object.entries(roleComparison.roleCompetitionByRole)) {
        if (!comparison) {
            continue;
        }
        ...
        curatedVsLiveWinnerNotes.push(`${formatRole(role)} stayed curated over live challenger ${comparison.strongestLive.venueName}; lost at ${comparison.strongestLiveLostAtStage ?? 'unknown'}${topDrivers.length > 0 ? ` on ${topDrivers.join(' + ')}` : ''}.`);
    }
    const selectedLiveStops = selectedArc.stops.filter((stop) => stop.scoredVenue.venue.source.sourceOrigin === 'live').length;
    const sourceBalanceNotes = [];
    if (effectiveSourceMode === 'hybrid') {
        ...
        if (strongLiveCandidatesFilteredCount > 0) {
            sourceBalanceNotes.push(`${strongLiveCandidatesFilteredCount} strong live candidate${strongLiveCandidatesFilteredCount === 1 ? '' : 's'} fell out before final role-pool competition.`);
        }
    }
