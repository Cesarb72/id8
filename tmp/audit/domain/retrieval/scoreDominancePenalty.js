...
export function scoreDominancePenalty({ venue, contextSpecificityByRole, }) {
    const universalityScore = roleAffinityUniversality(venue);
    const genericWeight = genericCategoryWeight(venue.category);
    const warmupPenalty = clamp01(universalityScore * (1 - contextSpecificityByRole.warmup) * 0.06 + genericWeight * 0.01);
    const peakPenalty = clamp01(universalityScore * (1 - contextSpecificityByRole.peak) * 0.2 + genericWeight * 0.07);
    const wildcardPenalty = clamp01(universalityScore * (1 - contextSpecificityByRole.wildcard) * 0.12 + genericWeight * 0.03);
    const cooldownPenalty = clamp01(universalityScore * (1 - contextSpecificityByRole.cooldown) * 0.08 + genericWeight * 0.02);
    return {
        universalityScore,
        flaggedUniversal: universalityScore >= 0.72 &&
