...
function getNightlifePressure(candidate) {
    const tags = getTagSet(candidate);
    const signals = candidate.taste.signals;
    const lateSignal = hasAnyTag(tags, [
        'late-night',
        'nightlife',
        'night-owl',
        'dance',
...
function getMovementPenalty(driveMinutes, movementTolerance) {
...
function getContractInfluenceSummary(experienceContract, constraints) {
    const retrievalMode = experienceContract.persona === 'romantic' &&
    ...
    const highlightMode = constraints.highlightPressure === 'strong'
...
function evaluateContractGlobalPressure(candidate, experienceContract, constraints) {
    const signals = candidate.taste.signals;
    ...
    if (experienceContract.persona === 'romantic' &&
        ...
        if (constraints.kidEngagementRequired &&
            nightlife > 0.88 &&
            signals.energy > 0.82 &&
...
export function applyContractRetrievalPressure(params) {
    ...
    const evaluated = scoredVenues.map((candidate) => {
        const pressure = evaluateContractGlobalPressure(candidate, experienceContract, contractConstraints);
        const fitMultiplier = pressure.multiplier;
        const roleMultiplierStart = clampRange(fitMultiplier + (pressure.multiplier >= 1 ? 0.04 : -0.02), 0.66, 1.3);
        const roleMultiplierHighlight = clampRange(fitMultiplier +
            (contractConstraints.highlightPressure === 'strong' ? 0.08 : 0.04) -
            (contractConstraints.highlightPressure === 'distributed' ? 0.02 : 0), 0.62, 1.34);
        const roleMultiplierWindDown = clampRange(fitMultiplier +
            (contractConstraints.windDownStrictness === 'soft_required' ? 0.07 : 0.03), 0.66, 1.3);
        return {
            candidate: {
                ...candidate,
    ...
    return {
        ...
        retrievedVenueIds: dedupeIds(shapedCandidates
            .filter((candidate) => candidate.candidateIdentity.kind !== 'moment')
            .map((candidate) => candidate.venue.id)),
        contractInfluenceSummary: getContractInfluenceSummary(experienceContract, contractConstraints),
    };
}
