...
export function computeHybridLiveLift(venue) {
    ...
    const strongHours = venue.source.likelyOpenForCurrentWindow &&
        venue.source.timeConfidence >= 0.68 &&
        !venue.source.hoursSuppressionApplied;
    const softHours = venue.source.likelyOpenForCurrentWindow &&
        venue.source.timeConfidence >= 0.5 &&
        !venue.source.hoursSuppressionApplied;
    const strongSource = venue.source.sourceConfidence >= 0.72 &&
        venue.source.completenessScore >= 0.58 &&
        venue.source.qualityScore >= 0.72;
    const fairSupportSource = venue.source.sourceConfidence >= 0.6 &&
    ...
    const highlightReady = venue.highlightCapable &&
        venue.settings.highlightCapabilityTier === 'highlight-capable' &&
        !venue.settings.connectiveOnly;
    const supportReady = !venue.settings.connectiveOnly &&
        (venue.category === 'cafe' ||
    ...
    const strongLiveCandidate = (approved && strongSource && (strongHours || softHours)) ||
    ...
    const roleLiftByRole = {
        warmup: strongLiveCandidate && supportReady
    ...
    const rolePoolLift = (strongHighlightCandidate ? 0.018 : 0) +
        (strongLiveCandidate && !strongHighlightCandidate ? 0.01 : 0) +
        (freshnessLiftApplied ? 0.008 : 0);
    const dedupePriorityScore = clamp(venue.source.qualityScore * 0.34 +
        ...
        (strongHours ? 0.08 : softHours ? 0.03 : 0), 0, 1);
    if (strongHours) {
    ...
    const liftApplied = fitLift > 0.001 ||
        rolePoolLift > 0.001 ||
