...
export function computeRoleAwareLiveLift(item, role) {
    ...
    const highConfidence = item.venue.source.sourceConfidence >= (liveFairness.supportRecoveryEligible ? 0.66 : 0.72) &&
        item.venue.source.completenessScore >= 0.58;
    const roleHours = computeRoleAwareHoursPressure(item.venue, role);
    const strongHours = item.venue.source.likelyOpenForCurrentWindow &&
    ...
    if (role === 'warmup') {
        const qualifies = baseLift.strongLiveCandidate &&
            strongHours &&
            highConfidence &&
    ...
    if (role === 'peak') {
        const qualifies = baseLift.strongHighlightCandidate &&
            strongHours &&
            highConfidence &&
    ...
    if (baseLift.freshnessLiftApplied &&
