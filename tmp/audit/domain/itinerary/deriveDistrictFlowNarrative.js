...
function buildStructuralAnchors(itinerary) {
    const roles = ['start', 'highlight', 'windDown'];
    const anchors = roles
        .map((role) => {
...
function classifySingleTransition(transition) {
    if (transition.neighborhoodContinuity === 'same-neighborhood') {
        return 'stay_local';
    }
    if (transition.neighborhoodContinuity === 'adjacent-neighborhoods' ||
        transition.estimatedTravelMinutes <= 12) {
        return 'short_move';
    }
    return 'district_shift';
}
function classifyTransitionRange(itinerary, fromStopIndex, toStopIndex) {
...
function buildReason(role, steps, index) {
    if (steps.length === 1) {
...
export function deriveDistrictFlowNarrative(itinerary) {
    if (itinerary.stops.length === 0) {
        return [];
    }
    const districts = new Set(itinerary.stops.map((stop) => stop.neighborhood || itinerary.neighborhood || itinerary.city));
    if (districts.size <= 1) {
