...
function matchDistrictAlias(citySlug, value) {
    ...
    const match = getCityAliases(citySlug).find((alias) => alias.aliases.some((entry) => normalizeLabel(entry) === normalized));
    if (!match) {
        return undefined;
    }
    return {
...
function pickStrongestCluster(spatial) {
    ...
    const neighborhoodsByCluster = new Map();
...
export function resolveDistrictAnchor({ city, district, neighborhood, spatial, }) {
    ...
    const promotedDistrict = matchDistrictAlias(citySlug, strongestCluster.clusterId) ??
