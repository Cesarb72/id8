import { normalizeExperienceContractVibe, } from '../interpretation/buildCanonicalInterpretationBundle';
...
const CONTRACT_DISTRICT_PRESSURE_V0_1_MATRIX = {
    romantic: {
        cozy: {
...
function computeContractAwareDistrictFeatures(profile) {
    ...
    const hasLateSignal = hasTokenSignal(tags, ['late', 'night', 'music', 'pulse', 'cocktail']) ||
        hasTokenSignal(categories, ['bar', 'night', 'music', 'club']);
    const hasCulturalSignal = hasTokenSignal(tags, ['arts', 'curated', 'gallery', 'museum', 'historic']) ||
        hasTokenSignal(categories, ['museum', 'gallery', 'theater', 'arts']);
    const hasFamilySignal = hasTokenSignal(tags, ['family', 'kid', 'play', 'stroller', 'friendly']) ||
    ...
    const socialDensity = clampUnit((mix.drinks + mix.activity) * 0.52 + energy * 0.18 + noise * 0.15 + density * 0.15);
    const calmness = clampUnit((1 - energy) * 0.28 + (1 - noise) * 0.34 + intimacy * 0.2 + walkability * 0.18);
    const nightlife = clampUnit((mix.drinks + mix.activity) * 0.5 +
        energy * 0.22 +
        noise * 0.14 +
        (hasLateSignal ? 0.14 : 0));
    const culturalAnchor = clampUnit(mix.culture * 0.34 +
        (hasCulturalSignal ? 0.18 : 0) +
        localSpecificity * 0.16 +
        momentPotential * 0.14 +
        (profile.meta.identityKind === 'known_neighborhood' ? 0.1 : 0));
    const basecamp = clampUnit((mix.cafe + mix.dining) * 0.36 +
        walkability * 0.24 +
        (1 - radiusNorm) * 0.2 +
        intimacy * 0.1 +
        (hasHangSignal ? 0.1 : 0));
    const continuity = clampUnit((1 - radiusNorm) * 0.4 + walkability * 0.35 + intimacy * 0.1 + momentPotential * 0.15);
    const exploration = clampUnit(categoryDiversity * 0.34 +
        localSpecificity * 0.28 +
        mix.culture * 0.14 +
        density * 0.12 +
        radiusNorm * 0.12);
    const momentum = clampUnit(socialDensity * 0.42 + nightlife * 0.33 + exploration * 0.25);
    const highlightPotential = clampUnit(momentPotential * 0.3 +
        mix.dining * 0.2 +
        mix.culture * 0.18 +
        mix.activity * 0.12 +
        localSpecificity * 0.1 +
        intimacy * 0.1);
    const distributedPotential = clampUnit(categoryDiversity * 0.42 + mixEvenness * 0.28 + exploration * 0.15 + walkability * 0.15);
    const familyFriendly = clampUnit(calmness * 0.34 +
        walkability * 0.24 +
        (1 - radiusNorm) * 0.17 +
        (mix.activity * 0.08 + mix.cafe * 0.08 + mix.dining * 0.09) +
        (hasFamilySignal ? 0.12 : 0) -
        nightlife * 0.2);
    const movementContainedFit = clampUnit((1 - radiusNorm) * 0.48 + walkability * 0.32 + calmness * 0.2);
    const movementCompressedFit = clampUnit((1 - radiusNorm) * 0.42 + walkability * 0.33 + basecamp * 0.25);
    const movementModerateFit = clampUnit(clampUnit(1 - Math.abs(radiusNorm - 0.45) * 1.8) * 0.5 + walkability * 0.25 + exploration * 0.25);
    const movementExploratoryFit = clampUnit(exploration * 0.5 + radiusNorm * 0.2 + density * 0.1 + localSpecificity * 0.2);
    return {
        socialDensity,
        calmness,
        nightlife,
...
function rankDistrictProfilesWithContract(params) {
    ...
    const evaluated = ranked.map((entry) => {
        ...
        register((socialBandFit - 0.5) * 0.3, `density aligned:${contractConstraints.socialDensityBand}`, `density mismatch:${contractConstraints.socialDensityBand}`);
        const movementFit = getMovementFitScore(features, contractConstraints.movementTolerance);
        register((movementFit - 0.5) * 0.28, `movement fit:${contractConstraints.movementTolerance}`, `movement stretch:${contractConstraints.movementTolerance}`);
        ...
        if (pressureTemplate.preferCulturalAnchor) {
            register((features.culturalAnchor - 0.5) * 0.2, 'cultural anchor support', 'cultural anchor signal thin');
        }
        ...
        if (pressureTemplate.preferFamilyFriendly) {
            register((features.familyFriendly - 0.5) * 0.22, 'family-friendly recovery support', 'family-friendly recovery thin');
        }
        if (pressureTemplate.preferExploration) {
            register((features.exploration - 0.5) * 0.14, 'exploration density support', 'exploration density thin');
        }
        ...
        if (contractConstraints.multiAnchorAllowed) {
            register((features.distributedPotential - 0.5) * 0.08, 'multi-anchor friendly', 'single-anchor leaning');
        }
        if (contractConstraints.highlightPressure === 'strong') {
            ...
            register((features.distributedPotential - 0.5) * 0.16, 'distributed highlight support', 'distributed highlight thin');
        }
        else {
        ...
        if (contractConstraints.peakCountModel === 'single') {
            ...
            register((features.distributedPotential - 0.5) * 0.12, 'distributed-peak viability', 'distributed-peak thin');
        }
        else {
            register(((features.culturalAnchor + features.continuity) / 2 - 0.5) * 0.12, 'cumulative progression viability', 'cumulative progression thin');
        }
        if (contractConstraints.maxEnergyDropTolerance === 'low') {
            register(((features.momentum + features.continuity) / 2 - 0.5) * 0.08, 'low-drop continuity support', 'energy-drop risk');
        }
        else if (contractConstraints.maxEnergyDropTolerance === 'high') {
            register((features.distributedPotential - 0.5) * 0.06, 'high-drop tolerance support', 'variance tolerance thin');
        }
...
function getContractGateSignals(entry) {
    ...
    const nightlifeAdjacency = clamp(mix.drinks * 0.46 +
        mix.activity * 0.2 +
        (hasLateSeed(profile.tasteSignals.momentSeeds) ? 0.22 : 0) +
        (hasFlowSeed(profile.tasteSignals.momentSeeds) ? 0.12 : 0), 0, 1);
    const multiPeakCapability = clamp(momentPotential * 0.5 + (mix.activity + mix.drinks) * 0.28 + categoryDiversity * 0.22, 0, 1);
    const experientialRichness = clamp(momentPotential * 0.36 +
        categoryDiversity * 0.26 +
        laneDiversity * 0.18 +
        clamp(tags.size / 7, 0, 1) * 0.2, 0, 1);
    const containedMovement = clamp(features.compactness * 0.58 + features.walkability * 0.42, 0, 1);
    const softLandingCompatibility = clamp(features.calmness * 0.5 + (1 - energy) * 0.2 + (mix.cafe + mix.dining) * 0.3, 0, 1);
    const scenicDestinationIntimacy = clamp(features.scenicPotential * 0.62 + features.intimacy * 0.38, 0, 1);
    const thematicProgression = clamp((hasFlowSeed(profile.tasteSignals.momentSeeds) ? 0.3 : 0) +
        features.culturalPotential * 0.4 +
        categoryDiversity * 0.3, 0, 1);
    const groupThroughput = clamp(features.socialDensity * 0.52 + features.distributedPotential * 0.28 + features.walkability * 0.2, 0, 1);
    const lateCapability = clamp(nightlifeAdjacency * 0.66 + (hasLateSeed(profile.tasteSignals.momentSeeds) ? 0.34 : 0), 0, 1);
    const easyHangComfort = clamp(features.calmness * 0.34 +
        features.intimacy * 0.2 +
        (mix.dining + mix.cafe) * 0.32 +
        containedMovement * 0.14, 0, 1);
    const moderateDensity = clamp(1 - Math.abs(features.socialDensity - 0.56) * 1.9, 0, 1);
    const conversationalAnchorStrength = clamp(features.culturalPotential * 0.38 + features.intimacy * 0.28 + features.calmness * 0.34, 0, 1);
    const exploratoryVariety = clamp(features.explorationPotential * 0.48 + categoryDiversity * 0.26 + laneDiversity * 0.26, 0, 1);
    const multiAnchorEnrichment = clamp(features.culturalPotential * 0.45 + exploratoryVariety * 0.35 + momentPotential * 0.2, 0, 1);
    const familyCompatibility = clamp(features.familyPotential * 0.5 + features.calmness * 0.22 + containedMovement * 0.28, 0, 1);
    const resetRecoverySupport = clamp(features.calmness * 0.52 + containedMovement * 0.28 + familyCompatibility * 0.2, 0, 1);
    const activeEngagement = clamp(features.familyPotential * 0.42 + features.socialDensity * 0.24 + momentPotential * 0.34, 0, 1);
    const boundedEnergy = clamp((1 - Math.abs(energy - 0.58) * 1.8) * 0.58 + containedMovement * 0.42, 0, 1);
    ...
    const lowMeaningFiller = clamp((1 - momentPotential) * 0.38 + (1 - features.culturalPotential) * 0.36 + (1 - categoryDiversity) * 0.26, 0, 1);
    return {
...
function evaluateContractGateDecision(params) {
    ...
    const gateAdjustedScore = toFixed(clamp(entry.score +
...
function applyContractGate(params) {
    ...
    if (!gateProfile) {
        return {
            ...
            suppressedPreview: [],
            ranked,
            decisionByPocketId: new Map(ranked.map((entry) => [
    ...
    if (admitted.length < minimumSurvivors && fallbackRatioGate) {
        ...
        const suppressedNeeded = Math.min(CONTRACT_GATE_MAX_FALLBACK_ADMITS, minimumSurvivors - admitted.length);
    ...
    if (admitted.length < TARGET_DIRECTION_CANDIDATES) {
        ...
        const needed = Math.min(CONTRACT_GATE_MAX_FALLBACK_ADMITS, TARGET_DIRECTION_CANDIDATES - admitted.length);
    ...
    const fallbackAdmittedCount = decorated.filter(({ decision }) => decision.status === 'fallback_admitted').length;
...
export function buildContractGateWorld(input) {
