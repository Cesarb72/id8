...
const DIRECTION_STRATEGY_LIBRARY = {
    romantic_lively: [
        {
            id: 'contained_pulse',
...
const STRATEGY_CONTRACT_SHAPES = {
    anchored_pulse: {
        requiredSignals: {
...
function getStrategyContractSignals(entry, features) {
    ...
    const clearCenterOfGravity = clamp(features.anchorPotential * 0.66 +
        features.compactness * 0.24 +
        momentPotential * 0.1 -
        features.distributedPotential * 0.24, 0, 1);
    const tightRadiusCluster = clamp(features.compactness * 0.56 + features.walkability * 0.24 + density * 0.2, 0, 1);
    const broadMovementSpread = clamp((1 - features.compactness) * 0.52 + features.explorationPotential * 0.48, 0, 1);
    ...
    return {
        ...
        evenDensityDistribution: clamp(features.distributedPotential * 0.72 + (1 - clearCenterOfGravity) * 0.28, 0, 1),
        ...
        samePocketViability: clamp(density * 0.34 +
            features.walkability * 0.26 +
            features.compactness * 0.2 +
            features.anchorPotential * 0.2, 0, 1),
        wideRadiusSpread: clamp((1 - features.compactness) * 0.72 + normalizedRadius * 0.28, 0, 1),
        crossPocketDrift: clamp((1 - features.compactness) * 0.46 +
            (1 - features.walkability) * 0.24 +
            features.explorationPotential * 0.3, 0, 1),
        contrastDiversity,
        laneDiversity,
        broadMovementSpread,
        multiPocketPotential: clamp(broadMovementSpread * 0.58 + contrastDiversity * 0.24 + laneDiversity * 0.18, 0, 1),
        overlyTightClustering: tightRadiusCluster,
        singleAnchorDominance: clamp(features.anchorPotential * 0.62 + (1 - features.distributedPotential) * 0.38, 0, 1),
    };
}
...
function evaluateStrategyHardGuard(strategyId, signals, momentPotential) {
    if (strategyId === 'anchored_pulse') {
        const passed = signals.highAnchorStrength >= 0.74 ||
    ...
    if (strategyId === 'contained_pulse') {
        const passed = signals.tightRadiusCluster >= 0.72 ||
    ...
    if (strategyId === 'exploratory_pulse') {
        const passedSignals = Number(signals.contrastDiversity >= 0.62) +
...
export function resolveDirectionStrategiesForFamily(family) {
    if (family === 'adaptive') {
        return [
            {
                id: 'adaptive_balance',
...
function buildStrategyWorld(strategy, ranked) {
    ...
    const draft = seedPool.map((entry) => {
        ...
        const requiredFailedSignals = getSignalsBelowThresholds(signals, contractShape?.requiredSignals);
        const suppressedTriggeredSignals = getSignalsMeetingThresholds(signals, contractShape?.suppressedSignals);
        const hardGuard = evaluateStrategyHardGuard(strategy.id, signals, entry.profile.tasteSignals.momentPotential);
        const exploratoryPartialRequiredMatch = strategy.id === 'exploratory_pulse' &&
            !required.pass &&
            required.score >= EXPLORATORY_REQUIRED_DEGRADE_SCORE_FLOOR &&
            hardGuard.passed;
        const suppressedRejectScoreFloor = strategy.id === 'exploratory_pulse'
        ...
        const suppressedPenalty = suppressed.triggered
            ...
            : 0;
        const shapeScore = toFixed(clamp(entry.score * 0.7 +
            preferred.score * 0.3 -
            suppressedPenalty -
            (required.pass ? 0 : STRATEGY_REQUIRED_FAIL_PENALTY), 0, 1));
        let status = 'admitted';
    ...
    const decisionByPocketId = draft.reduce((accumulator, decision) => {
    ...
    const hardRequirementResults = draft.map((decision) => ({
        ...
        reasons: decision.requiredPass && decision.hardGuardPassed
            ? ['pass']
            : [
    ...
    const decisionLog = draft.map((decision) => ({
        pocketId: decision.pocketId,
        status: decision.status === 'fallback_admitted'
            ? 'admitted'
            : decision.status,
    ...
    const sampleDecisions = decisionLog
        .slice(0, 5)
        .map((decision) => ({
        pocketId: decision.pocketId,
        status: decision.status,
...
export function buildStrategyAdmissibleWorlds(input) {
