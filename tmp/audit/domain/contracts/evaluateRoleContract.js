...
export function evaluateRoleContract(venue, rule) {
    ...
    const preferredCategoryScore = rule.preferredCategories.length === 0
        ? 0.7
    ...
    const maxEnergyPenalty = typeof rule.maxEnergyLevel === 'number' && venue.energyLevel > rule.maxEnergyLevel
        ? clamp01((venue.energyLevel - rule.maxEnergyLevel) / 4)
        : 0;
    if (maxEnergyPenalty > 0) {
        violations.push('energy too high');
    }
    const score = clamp01(preferredCategoryScore * 0.42 +
    ...
    const satisfied = rule.strength === 'hard'
        ? !hardFail && score >= 0.62
