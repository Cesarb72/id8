...
function getTagSet(candidate) {
    return new Set(candidate.derivedFrom.experientialTags
        .map((tag) => tag.trim().toLowerCase())
        .filter((tag) => tag.length > 0));
}
function normalizeMomentSeed(seed) {
    return seed.trim().toLowerCase().replace(/\s*->\s*/g, ' then ');
}
function getSeedSet(candidate) {
    return new Set(candidate.derivedFrom.momentSeeds
        .map((seed) => normalizeMomentSeed(seed))
        .filter((seed) => seed.length > 0));
}
function getMixSimilarity(left, right) {
    const leftMix = left.derivedFrom.hospitalityMix;
    const rightMix = right.derivedFrom.hospitalityMix;
    const distance = Math.abs(leftMix.drinks - rightMix.drinks) +
...
function getLaneIdentitySimilarity(leftIdentity, rightIdentity) {
    ...
    if ((leftIdentity === 'district_core' && rightIdentity === 'lively_core') ||
...
function getMomentumProfileSimilarity(leftProfile, rightProfile) {
    if (leftProfile === rightProfile) {
        return 1;
    }
    if ((leftProfile === 'fast_build' && rightProfile === 'steady_build') ||
    ...
    if ((leftProfile === 'soft_start' && rightProfile === 'focused_flow') ||
...
function getCandidateSimilarity(left, right) {
    ...
    const laneIdentitySimilarity = getLaneIdentitySimilarity(left.contrastProfile.laneIdentity, right.contrastProfile.laneIdentity);
    const momentumSimilarity = getMomentumProfileSimilarity(left.contrastProfile.momentumProfile, right.contrastProfile.momentumProfile);
    return clamp(mixSimilarity * 0.31 +
        ambianceSimilarity * 0.17 +
        tagSimilarity * 0.12 +
        seedSimilarity * 0.08 +
        laneIdentitySimilarity * 0.2 +
        momentumSimilarity * 0.08 +
        clusterSimilarity * 0.04, 0, 1);
}
function getMaxSimilarityToSelection(candidate, selected) {
...
function getLivelyLaneAlignment(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const ambiance = candidate.derivedFrom.ambianceProfile;
    const momentumScore = candidate.derivedFrom.momentPotential * 0.55 + (hasFlowSeed(candidate) ? 0.45 : 0);
    const energyScore = toLevelScore(ambiance.energy);
    const activityPresence = mix.activity * 0.56 + mix.drinks * 0.44;
    const culturalCalmPenalty = ambiance.noise === 'low' && mix.culture > mix.activity && mix.activity < 0.26 ? 0.12 : 0;
    return clamp(activityPresence * 0.44 + momentumScore * 0.31 + energyScore * 0.25 - culturalCalmPenalty, 0, 1);
}
function getCozyLaneAlignment(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const ambiance = candidate.derivedFrom.ambianceProfile;
    ...
    const frictionPenalty = mix.activity > 0.4 && ambiance.energy === 'high'
        ? 0.2
        : mix.activity > 0.34
            ? 0.1
            : 0;
    return clamp(intimacyScore * 0.27 +
        lowNoiseScore * 0.24 +
        lowEnergyScore * 0.19 +
        comfortMix * 0.3 -
        frictionPenalty, 0, 1);
}
function getCulturedLaneAlignment(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const ambiance = candidate.derivedFrom.ambianceProfile;
    const curatedSignal = Number(hasTag(candidate, 'arts-adjacent')) +
    ...
    const curiositySignal = candidate.derivedFrom.momentSeeds.some((seed) => /gallery|museum|historic|architecture|wander|detour/i.test(seed))
        ? 1
        : 0.45;
    const lowChaos = ambiance.noise === 'high'
        ? 0.2
        : ambiance.noise === 'medium'
            ...
            : ambiance.energy === 'high'
                ? 0.56
                : 1;
    return clamp(mix.culture * 0.42 + intentionality * 0.24 + curiositySignal * 0.18 + lowChaos * 0.16, 0, 1);
}
function getLaneAlignmentScore(candidate, vibe) {
    if (vibe === 'lively') {
...
function getSignatureStrength(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    ...
    const distinctiveTagSignal = ['arts-adjacent', 'mixed-program', 'intentional', 'curated', 'offbeat'].filter((tag) => tags.has(tag)).length / 5;
    return clamp(mixShape * 0.24 + mixBreadth * 0.24 + tagBreadth * 0.18 + distinctiveTagSignal * 0.34, 0, 1);
}
function getMomentSupportStrength(candidate) {
    ...
    return clamp(candidate.derivedFrom.momentPotential * 0.6 +
        seedSupport * 0.2 +
        (hasFlowSeed(candidate) ? 0.2 : 0), 0, 1);
}
function getRouteSupportConfidence(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const laneCount = [mix.drinks, mix.dining, mix.culture, mix.cafe, mix.activity].filter((value) => value >= 0.16).length;
    const laneSupport = clamp(laneCount / 4, 0, 1);
    const ambiance = candidate.derivedFrom.ambianceProfile;
    const pacingSupport = ambiance.noise === 'high' && ambiance.energy === 'high'
        ? 0.36
        : ambiance.noise === 'high'
            ? 0.52
            : 0.78;
...
function getFrictionMismatch(candidate, vibe) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const ambiance = candidate.derivedFrom.ambianceProfile;
    if (vibe === 'lively') {
        return clamp((ambiance.energy === 'low' ? 0.42 : 0) +
            (mix.activity + mix.drinks < 0.36 ? 0.32 : 0) +
            (ambiance.noise === 'low' && mix.culture > mix.activity ? 0.24 : 0), 0, 1);
    }
    if (vibe === 'cozy') {
        return clamp((ambiance.noise === 'high' ? 0.4 : 0) +
            (ambiance.energy === 'high' ? 0.32 : 0) +
            (mix.activity > 0.42 ? 0.24 : 0), 0, 1);
    }
    if (vibe === 'cultured') {
        return clamp((mix.culture < 0.16 ? 0.34 : 0) +
            (ambiance.noise === 'high' ? 0.3 : 0) +
            (mix.activity > 0.45 ? 0.2 : 0), 0, 1);
    }
    return 0;
}
function getWinnerStrengthBonus(candidate, vibe) {
    ...
    const coherentActivation = laneAlignment >= 0.58 &&
        ...
        ? 1
        : 0;
    const rawBonus = laneAlignment * 0.028 +
...
function buildSlot1Decision(candidate, mode, winnerStrengthBonus) {
    return {
...
function buildFallbackDecision(candidate, selected, selectionRank) {
...
function buildSlot2Decision(candidate, winner, vibe) {
    ...
    const laneIdentityDifferent = candidate.contrastProfile.laneIdentity !== winner.contrastProfile.laneIdentity;
    const momentumProfileDifferent = candidate.contrastProfile.momentumProfile !== winner.contrastProfile.momentumProfile;
    const districtIdentityDelta = Math.abs(candidate.contrastProfile.districtIdentityStrength -
        winner.contrastProfile.districtIdentityStrength);
    const adjacencyLift = safeWindow * 0.042;
    const diversityLift = (1 - similarityToWinner) * 0.014 +
        (laneIdentityDifferent ? 0.018 : 0) +
        (momentumProfileDifferent ? 0.013 : 0) +
        districtIdentityDelta * 0.022;
    const similarityPenalty = (similarityToWinner > 0.84 ? (similarityToWinner - 0.84) * 0.22 : 0) +
    ...
    const adjustedScore = candidate.finalScore + adjacencyLift + diversityLift + laneAlignment * 0.015 - similarityPenalty;
    return {
        pocketId: candidate.pocketId,
        cluster: candidate.cluster,
        macroLane: candidate.contrastProfile.macroLane,
        ...
        sameLaneAsWinner: candidate.contrastProfile.macroLane === winner.contrastProfile.macroLane,
...
function buildSlot3Decision(candidate, winner, slot2, vibe) {
    const similarityToWinner = getCandidateSimilarity(candidate, winner);
    const similarityToSlot2 = slot2 ? getCandidateSimilarity(candidate, slot2) : 0;
    const laneAlignment = getLaneAlignmentScore(candidate, vibe);
    const laneIdentityDifferentFromWinner = candidate.contrastProfile.laneIdentity !== winner.contrastProfile.laneIdentity;
    const laneIdentityDifferentFromSlot2 = slot2
        ? candidate.contrastProfile.laneIdentity !== slot2.contrastProfile.laneIdentity
        : true;
    const momentumDifferentFromWinner = candidate.contrastProfile.momentumProfile !== winner.contrastProfile.momentumProfile;
    ...
    const districtIdentityStrengthBoost = candidate.contrastProfile.districtIdentityStrength * 0.028;
    const laneDifferenceBoost = (laneIdentityDifferentFromWinner ? 0.03 : 0) +
        (laneIdentityDifferentFromSlot2 ? 0.018 : 0);
    const momentumDifferenceBoost = (momentumDifferentFromWinner ? 0.018 : 0) +
        (momentumDifferentFromSlot2 ? 0.012 : 0);
    const contrastLift = (1 - similarityToWinner) * 0.05 +
        (1 - similarityToSlot2) * 0.03 +
        contrastEligibleBoost +
        districtIdentityStrengthBoost +
        laneDifferenceBoost +
        momentumDifferenceBoost;
    const similarityPenalty = (similarityToWinner > 0.66 ? (similarityToWinner - 0.66) * 0.28 : 0) +
    ...
    const viabilityPenalty = (scoreGap > 0.34 ? (scoreGap - 0.34) * 0.22 + 0.015 : 0) +
        (candidate.confidence < 0.72 ? (0.72 - candidate.confidence) * 0.08 : 0);
    const adjustedScore = candidate.finalScore + contrastLift + laneAlignment * 0.02 - similarityPenalty - viabilityPenalty;
...
function pickByDecision(candidates, decisionBuilder) {
...
function pickByDecisionWithFamilyDiversity(params) {
    ...
    const bestDifferentFamily = scored.find((entry) => !selectedFamilies.has(entry.candidate.experienceFamily));
    ...
    const withinBand = bestOverall.decision.adjustedScore - bestDifferentFamily.decision.adjustedScore <=
        viabilityBand;
    if (withinBand) {
        return {
...
function detectLaneCollapseRisk(winner, slot2, remainingCandidates) {
    if (!winner || !slot2) {
        return false;
    }
    ...
    const topRemaining = remainingCandidates
        ...
        .slice(0, 3);
    const sameLaneCount = topRemaining.filter((candidate) => candidate.contrastProfile.macroLane === winnerLane).length;
...
function pickSlot3WithLaneSeparation(params) {
    const bestOverallPick = pickByDecisionWithFamilyDiversity({
        ...
        decisionBuilder: (candidate) => params.slot2
    ...
    if (!params.laneCollapseRisk) {
        return {
            ...
            laneSeparatedSlot3: bestOverallPick.pick.candidate.contrastProfile.macroLane !== params.winner.contrastProfile.macroLane,
    ...
    const differentLaneCandidates = params.remaining.filter((candidate) => candidate.contrastProfile.macroLane !== params.winner.contrastProfile.macroLane);
    ...
    const viableBandSatisfied = bestDifferentLanePick.pick.candidate.confidence >= 0.68 &&
    ...
    if (viableBandSatisfied) {
        return {
            ...
            familyDiversityApplied: bestDifferentLanePick.familyDiversityApplied || bestOverallPick.familyDiversityApplied,
...
export function selectBestDistinctDirections({ candidates, preShapeCandidates, requestedVibe, finalLimit = DEFAULT_FINAL_LIMIT, }) {
    ...
    const remainingByPocketId = new Map(rankedCandidates.map((candidate) => [candidate.pocketId, candidate]));
    ...
    if (slot1 && resolvedVibe && rankedCandidates.length > 1) {
        ...
        if (lanePreferred) {
            ...
            if (scoreMargin <= SLOT1_GUARDRAIL_MAX_SCORE_MARGIN &&
    ...
    if (slot1) {
        ...
        decisions.push(buildSlot1Decision(slot1, slot1GuardrailApplied ? 'guardrail_lane_alignment' : 'winner_strength', slot1WinnerStrengthBonus));
    ...
    if (selected.length < effectiveFinalLimit) {
        ...
        const slot2Pick = pickByDecisionWithFamilyDiversity({
            ...
            selectedFamilies,
            decisionBuilder: (candidate) => buildSlot2Decision(candidate, selected[0], resolvedVibe),
        });
        if (slot2Pick.pick) {
    ...
    const laneCollapseRisk = detectLaneCollapseRisk(selected[0], selected[1], [...remainingByPocketId.values()]);
    let laneSeparatedSlot3 = false;
    let laneSeparationReason;
    if (selected.length < effectiveFinalLimit) {
        const remaining = [...remainingByPocketId.values()];
        const slot3Pick = selected.length >= 2
            ...
            : pickByDecisionWithFamilyDiversity({
                candidates: remaining,
                selectedFamilies,
                decisionBuilder: (candidate) => buildFallbackDecision(candidate, selected, selected.length + 1),
            });
        if (slot3Pick) {
    ...
    const elevatedPocketIds = postShapeTop3PocketIds.filter((pocketId) => !preShapeTop3PocketIds.includes(pocketId));
    const droppedPocketIds = preShapeTop3PocketIds.filter((pocketId) => !postShapeTop3PocketIds.includes(pocketId));
    const rawSecond = rankedCandidates[1];
    const adjustedSecond = withWinnerStrength.find((entry) => entry.candidate.pocketId !== selected[0]?.pocketId);
    return {
        finalists: selected,
        debug: {
            ...
            strongestShapedPreserved: strongestShaped !== undefined && selected[0]?.pocketId === strongestShaped.pocketId,
            ...
            top1RawSeparation: toFixed(strongestShaped && rawSecond ? strongestShaped.finalScore - rawSecond.finalScore : 0),
            top1AdjustedSeparation: toFixed(selected[0] && adjustedSecond
