...
function computeEnvironmentQuality(candidate) {
    ...
    const intimacyNoisePair = ambiance.intimacy === 'high' && ambiance.noise === 'low'
        ? 0.18
        : ambiance.intimacy === 'high' || ambiance.noise === 'low'
            ? 0.08
            : 0;
    const curatedSignal = ['arts-adjacent', 'curated', 'intentional', 'intimate', 'cultural'].some((tag) => tags.has(tag))
        ? 0.12
        : 0;
...
function computeSequenceQuality(candidate) {
    ...
    const diversityWithoutActivityDominance = mix.activity <= 0.45 && diversityCount >= 3 ? 0.12 : mix.activity <= 0.5 && diversityCount >= 2 ? 0.06 : 0;
    return clamp(seedMultiplicity + flowSignal + diversityWithoutActivityDominance, 0, 1);
}
function computeMixDiversityScore(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
    const diversityCount = Object.values(mix).filter((value) => value >= 0.14).length;
    return clamp(diversityCount / 5, 0, 1);
}
function computeLaneDiversityProxy(candidate) {
    const mix = candidate.derivedFrom.hospitalityMix;
...
function softenRomanticSubtitle(subtitle) {
...
function softenFriendsSubtitle(subtitle) {
...
export function applyPersonaShaping(candidates, persona) {
    const shaped = candidates.map((candidate) => {
        if (persona === 'family') {
            ...
            const balancedEnergyBoost = ambiance.energy === 'medium' &&
                ...
                : ambiance.energy === 'high' && recoverySupport >= 0.55
                    ? 0.012
                    : 0;
            const moderateAmbientBoost = (ambiance.energy === 'medium' || ambiance.energy === 'low') &&
                (ambiance.noise === 'medium' || ambiance.noise === 'low')
                ? 0.012
                : 0;
            const adultPayoffBoost = mix.culture >= 0.18 &&
            ...
            const momentPotentialPacedBoost = candidate.derivedFrom.momentPotential * (lowFrictionEnvironment ? 0.028 : 0.012) +
                (sequenceQuality >= 0.45 ? 0.012 : 0);
            const cultureContextBoost = (lowFrictionEnvironment ? mix.culture * 0.065 : mix.culture * 0.02) +
                (tags.has('arts-adjacent') && lowFrictionEnvironment ? 0.02 : 0) +
                (hasLearningSeed && lowFrictionEnvironment ? 0.015 : 0);
            const moderateDiversityBoost = mixDiversity >= 0.5 && mixDiversity <= 0.8
                ? 0.03
                : laneDiversity >= 0.4 && laneDiversity <= 0.75
                    ? 0.015
                    : 0;
            const intimacySoftBoost = ambiance.intimacy === 'high' ? 0.006 : ambiance.intimacy === 'medium' ? 0.003 : 0;
            const highEnergyPenalty = ambiance.energy === 'high' && recoverySupport < 0.55
                ? 0.085
                : ambiance.energy === 'high'
                    ? 0.04
                    : 0;
            const highNoisePenalty = ambiance.noise === 'high'
                ? 0.095
                : ambiance.noise === 'medium' && mix.activity > 0.42 && recoverySupport < 0.55
                    ? 0.03
                    : 0;
            const activityDominancePenalty = mix.activity > 0.48 ? 0.07 : mix.activity > 0.4 ? 0.03 : mix.activity > 0.34 ? 0.015 : 0;
            const activityLedPenalty = tags.has('activity-led') && mix.culture < 0.22
                ? 0.07
                : tags.has('activity-led') && lowFrictionEnvironment && sequenceQuality >= 0.28
                    ...
                    : tags.has('activity-led')
                        ? 0.055
                        : 0;
            const fragmentationPenalty = (sequenceQuality < 0.28 ? 0.05 : sequenceQuality < 0.4 ? 0.03 : 0) +
                (momentSeedCount < 2 ? 0.03 : 0) +
                (momentSeedCount >= 2 && !hasFlowSeed && movementFriendly < 0.55 ? 0.015 : 0);
            const mixedProgramChaosPenalty = tags.has('mixed-program') && mixDiversity > 0.85 && sequenceQuality < 0.6 && wonderLearningProxy < 0.58
                ? 0.08
                : tags.has('mixed-program') && sequenceQuality < 0.45
                    ? 0.045
                    : 0;
            const highNoiseWeakCulturePenalty = ambiance.noise === 'high' && mix.culture < 0.24
                ? 0.065
                : ambiance.noise === 'medium' && mix.culture < 0.18
                    ? 0.02
                    : 0;
            const drinksNightlifePenalty = (mix.drinks > 0.24
                ? 0.045
                : mix.drinks > 0.18
                    ? 0.028
            ...
            const nightlifeContinuationPenalty = hasNightlifeContinuationSeed && (mix.culture < 0.26 || recoverySupport < 0.55)
                ? 0.03
                : hasNightlifeContinuationSeed
                    ? 0.015
                    : 0;
            const weakLandingPenalty = recoverySupport < 0.45 && momentSeedCount < 2
                ? 0.035
                : recoverySupport < 0.5 && sequenceQuality < 0.35
                    ? 0.02
                    : 0;
            const brittleFlowPenalty = lowFrictionContinuation < 0.5 && sequenceQuality < 0.4
                ? 0.03
                : lowFrictionContinuation < 0.6 && movementFriendly < 0.55
                    ? 0.015
                    : 0;
            const highFrictionMovementPenalty = movementFriendly < 0.45 && sequenceQuality < 0.38 ? 0.025 : 0;
            const antiSaturationOffset = 0.335;
            const personaBoostRaw = flowContinuityBoost +
        ...
        if (persona === 'friends') {
            ...
            const seeds = candidate.derivedFrom.momentSeeds
                ...
                .map((seed) => seed.toLowerCase());
            const momentSeedCount = seeds.length;
            const lateNightSupport = hasSeedTerm(seeds, ['late', 'night', 'wine bar', 'cocktail', 'bar', 'music', 'after']) ||
                mix.drinks >= 0.16;
            const sceneEdgeSignal = hasSeedTerm(seeds, ['detour', 'wander', 'gallery', 'speakeasy', 'offbeat']) ||
                tags.has('mixed-program') ||
                tags.has('lively');
            const eventPresenceSignal = hasSeedTerm(seeds, ['performance', 'music', 'show', 'event', 'gallery']) ||
                tags.has('arts-adjacent');
            const culturalAnchorSignal = hasSeedTerm(seeds, ['gallery', 'museum', 'historic', 'architecture', 'performance']) ||
                tags.has('arts-adjacent');
            const curiositySignal = hasSeedTerm(seeds, ['detour', 'wander', 'surprise', 'offbeat', 'speakeasy']) ||
                tags.has('mixed-program');
            const socialEnergyBoost = mix.activity * 0.14 +
                mix.drinks * 0.12 +
                (ambiance.energy === 'high' ? 0.04 : ambiance.energy === 'medium' ? 0.03 : 0);
            const varietyBoost = mixDiversity * 0.04 + laneDiversity * 0.025;
            const momentumBoost = movementFriendly * 0.04 +
            ...
            const sceneCredibilityBoost = (sceneEdgeSignal ? 0.02 : 0) +
                (eventPresenceSignal ? 0.015 : 0) +
                (tags.has('activity-led') ? 0.01 : 0);
            const mixedProgramMomentumBoost = tags.has('mixed-program') && movementFriendly >= 0.55
                ? 0.02
                : tags.has('mixed-program')
                    ? 0.01
                    : 0;
            const cultureCuriosityBoost = mix.culture * 0.05 +
                (tags.has('arts-adjacent') ? 0.02 : 0) +
                (culturalAnchorSignal ? 0.015 : 0) +
                (curiositySignal ? 0.01 : 0);
            const conversationSpaceBoost = (ambiance.noise === 'low' ? 0.02 : ambiance.noise === 'medium' ? 0.014 : 0) +
                (mix.dining + mix.cafe + mix.culture) * 0.03 +
                (mix.activity < 0.4 ? 0.012 : 0) +
                sequenceQuality * 0.015;
            const atmosphericMemoryBoost = candidate.derivedFrom.momentPotential * 0.015 +
                (momentSeedCount >= 3 ? 0.015 : 0) +
                (culturalAnchorSignal && momentSeedCount >= 2 ? 0.01 : 0);
            const lowNoiseSoftBoost = ambiance.noise === 'medium' ? 0.012 : ambiance.noise === 'low' ? 0.008 : 0;
            const intimacySoftBoost = ambiance.intimacy === 'high' ? 0.003 : ambiance.intimacy === 'medium' ? 0.0015 : 0;
            const deadEnergyPenalty = ambiance.energy === 'low' ? 0.06 : 0;
            const weakMomentumPenalty = movementFriendly < 0.45 && sequenceQuality < 0.4
                ? 0.05
                : movementFriendly < 0.55 && sequenceQuality < 0.45
                    ? 0.02
                    : 0;
            const narrowPenalty = mixDiversity < 0.45 ? 0.05 : mixDiversity < 0.6 ? 0.02 : 0;
            const noContinuationPenalty = !lateNightSupport && mix.activity < 0.34 && mix.drinks < 0.14 ? 0.045 : 0;
            const overlyQuietPenalty = ambiance.noise === 'low' && ambiance.energy === 'low' ? 0.04 : 0;
            const overlyIntimatePenalty = ambiance.intimacy === 'high' && ambiance.energy === 'low' && ambiance.noise === 'low' ? 0.03 : 0;
            const chaoticNoisePenalty = tags.has('mixed-program') && ambiance.noise === 'high' && movementFriendly < 0.5 ? 0.03 : 0;
            const genericActivityPenalty = tags.has('activity-led') && mix.culture < 0.24 ? 0.065 : mix.activity > 0.44 && mix.culture < 0.2 ? 0.03 : 0;
            const highNoiseWeakCulturePenalty = ambiance.noise === 'high' && mix.culture < 0.22
                ? 0.05
                : ambiance.noise === 'medium' && mix.culture < 0.16
                    ? 0.02
                    : 0;
            const broadSocialNoAnchorPenalty = mix.activity + mix.drinks > 0.55 && !culturalAnchorSignal && !curiositySignal ? 0.04 : 0;
            const lowMemoryPenalty = momentSeedCount < 2 && candidate.derivedFrom.momentPotential < 0.9 && !culturalAnchorSignal ? 0.03 : 0;
            const antiSaturationOffset = 0.22;
            const personaBoostRaw = socialEnergyBoost +
                varietyBoost +
                momentumBoost +
                groupAssemblyBoost +
                lateNightBoost +
        ...
        const chaoticMixedProgram = tags.has('mixed-program') &&
        ...
        const noisePenalty = ambiance.noise === 'high'
            ? 0.08
            : ambiance.noise === 'medium'
                ? 0.02 + (mix.activity >= 0.38 ? 0.02 : 0)
                : 0;
        const personaBoostRaw = (ambiance.intimacy === 'high' ? 0.06 : ambiance.intimacy === 'medium' ? 0.03 : 0) +
