...
import { buildContractGateWorld, } from '../bearings/buildContractGateWorld';
import { buildStrategyAdmissibleWorlds, resolveDirectionStrategiesForFamily, } from '../bearings/buildStrategyAdmissibleWorlds';
...
function deriveArchetype(profile) {
    ...
    if (tags.has('arts-adjacent') &&
        (tags.has('mixed-program') ||
            profile.tasteSignals.momentSeeds.some((seed) => seed.toLowerCase().includes('gallery -> wine bar')))) {
        return 'cultural';
    }
    if ((ambiance.energy === 'high' || tags.has('lively')) &&
...
function getPreferredCluster(archetype) {
    if (archetype === 'chill') {
        return 'chill';
    }
...
function resolveCandidatePoolLimit(requestedLimit, availableCount) {
    if (availableCount <= 0) {
        return 0;
    }
    const normalizedRequested = typeof requestedLimit === 'number' && Number.isFinite(requestedLimit)
        ? Math.floor(requestedLimit)
        : DEFAULT_CANDIDATE_POOL_LIMIT;
    const boundedRequested = clamp(normalizedRequested, MIN_CANDIDATE_POOL_LIMIT, MAX_CANDIDATE_POOL_LIMIT);
    return Math.min(availableCount, boundedRequested);
}
function buildLabel(archetype, pocketLabel) {
...
function buildSubtitle(archetype, profile) {
...
function getStrategyDensityTargetValue(shape) {
    if (shape === 'low') {
        return 0.28;
    }
    if (shape === 'medium') {
...
function getStrategyExplorationTargetValue(shape) {
    if (shape === 'low') {
        return 0.28;
    }
    if (shape === 'medium') {
...
function getSocialShapeTargetValue(shape) {
    if (shape === 'intimate') {
        return 0.24;
    }
    if (shape === 'balanced') {
...
function getAnchorShapeTargetValue(shape) {
...
function getVibeBand(context) {
    const raw = context?.vibe ?? context?.experienceContract?.vibe;
    if (raw === 'cultured') {
        return 'cultured';
    }
    if (raw === 'cozy' || raw === 'chill') {
        return 'cozy';
    }
    if (raw === 'playful' || raw === 'lively' || raw === 'adventurous-outdoor' || raw === 'adventurous-urban') {
        return 'lively';
    }
    if (context?.experienceContract?.coordinationMode === 'narrative') {
        return 'cultured';
    }
    if (context?.experienceContract?.coordinationMode === 'depth') {
        return 'cozy';
    }
    return 'lively';
}
function resolveStrategyFamily(context) {
    const persona = context?.persona ?? context?.experienceContract?.persona ?? 'romantic';
    const vibeBand = getVibeBand(context);
    if (persona === 'romantic') {
...
function getPeakShapeTargetValue(shape) {
    if (shape === 'single') {
        return 0.78;
    }
    if (shape === 'multi') {
...
function getPeakModelStrength(peakModel) {
...
function getMovementShapeFit(strategy, features) {
...
function getDistanceFit(current, target) {
    return clamp(1 - Math.abs(current - target) * 1.9, 0, 1);
}
function getStrategyKeywordBonus(strategy, features) {
...
function getDirectionStrategyFeatures(entry) {
    ...
    const socialDensity = clamp(mix.drinks + mix.activity + (ambiance.energy === 'high' ? 0.14 : ambiance.energy === 'medium' ? 0.05 : 0), 0, 1);
    const calmness = clamp((1 - toAmbianceNumeric(ambiance.energy)) * 0.52 + (1 - toAmbianceNumeric(ambiance.noise)) * 0.48, 0, 1);
    const intimacy = clamp(toAmbianceNumeric(ambiance.intimacy), 0, 1);
    const culturalPotential = clamp(mix.culture +
        (tags.some((tag) => tag.includes('arts') || tag.includes('curated') || tag.includes('museum')) ? 0.18 : 0) +
        (categories.some((category) => category.includes('museum') || category.includes('gallery') || category.includes('theater')) ? 0.14 : 0), 0, 1);
    const scenicPotential = clamp(calmness * 0.58 +
        intimacy * 0.22 +
        (tags.some((tag) => tag.includes('scenic') || tag.includes('calm') || tag.includes('quiet')) ? 0.24 : 0), 0, 1);
    const familyPotential = clamp((categories.some((category) => ['park', 'playground', 'aquarium', 'zoo'].includes(category)) ? 0.32 : 0) +
        (tags.some((tag) => tag.includes('family') || tag.includes('kid') || tag.includes('play')) ? 0.28 : 0) +
        calmness * 0.25 +
        walkability * 0.15, 0, 1);
    const anchorPotential = clamp(profile.tasteSignals.momentPotential * 0.5 +
        mix.dining * 0.25 +
        mix.culture * 0.15 +
        intimacy * 0.1, 0, 1);
    const distributedPotential = clamp(mix.activity * 0.32 + mix.drinks * 0.28 + mix.cafe * 0.2 + mix.dining * 0.2, 0, 1);
    const explorationPotential = clamp((1 - compactness) * 0.42 +
        profile.coreSignals.categoryDiversity * 0.28 +
        profile.coreSignals.density * 0.16 +
        (tags.some((tag) => tag.includes('mixed') || tag.includes('offbeat') || tag.includes('detour')) ? 0.14 : 0), 0, 1);
    return {
        compactness,
        walkability,
        socialDensity,
...
function getStrategyContractMatch(strategy, context) {
    ...
    const movementMatch = (constraints.movementTolerance === 'contained' && strategy.movementShape === 'contained') ||
...
function scoreEntryForStrategy(params) {
    ...
    const explorationFit = getDistanceFit(features.explorationPotential, getStrategyExplorationTargetValue(strategy.explorationShape));
    const anchorValue = strategy.anchorShape === 'distributed'
    ...
    const peakFit = getDistanceFit(getPeakModelStrength(candidate.directionPeakModel), getPeakShapeTargetValue(strategy.peakShape));
    ...
    const base = clamp(candidate.confidence * 0.44 +
        entry.score * 0.16 +
        movementFit * 0.11 +
        densityFit * 0.08 +
        socialFit * 0.06 +
...
function getDirectionPeakModelLabel(experienceContract, contractConstraints) {
...
function getDirectionMovementStyleLabel(experienceContract, contractConstraints) {
...
function getMovementIdentityPrefix(experienceContract, contractConstraints) {
    const movement = contractConstraints?.movementTolerance ?? experienceContract?.movementStyle;
    if (movement === 'contained') {
        return 'Contained';
    }
...
function getExperienceSubjectToken(experienceContract) {
...
function getExperienceCorePhrase(experienceContract, contractConstraints) {
...
function compactTitle(value, maxWords = 5) {
    const words = value
        .replace(/\s+/g, ' ')
        .trim()
        .split(' ')
...
function deriveDirectionExperienceIdentity(params) {
    ...
    const includeMovement = !(movementPrefix.toLowerCase() === 'momentum' && coreLower.includes('momentum'));
...
function getSocialDensityBandSupport(socialDensity, band) {
    if (!band) {
...
function getMovementSupport(profile) {
...
function getPeakSupport(profile, experienceContract, contractConstraints) {
    const peakModel = getDirectionPeakModelLabel(experienceContract, contractConstraints);
...
function getContractFitSignal(entry) {
...
function deriveDirectionDistrictSupportSummary(params) {
    ...
    const densitySupport = getSocialDensityBandSupport(profile.tasteSignals.ambianceProfile.energy === 'high'
        ? Math.max(profile.tasteSignals.hospitalityMix.drinks + profile.tasteSignals.hospitalityMix.activity, 0.62)
        : profile.tasteSignals.hospitalityMix.drinks + profile.tasteSignals.hospitalityMix.activity, contractConstraints?.socialDensityBand);
...
function truncateStrategySummary(summary) {
...
function getNarrativePeakPhrase(peakModel) {
    if (peakModel === 'single-peak') {
...
function getNarrativeMovementPhrase(movementStyle) {
...
function getNarrativeLandingPhrase(contractConstraints) {
...
function getNarrativeActEdge(experienceContract) {
...
function deriveDirectionNarrativeMode(params) {
    ...
    const familyPosture = experienceContract.socialPosture === 'family_unit' ||
        experienceContract.socialPosture === 'parallel_tracks';
    const groupPosture = experienceContract.socialPosture === 'group_internal' ||
        experienceContract.socialPosture === 'social';
    if (experienceContract.socialPosture === 'intimate' &&
        contractConstraints.peakCountModel === 'single') {
        return 'intimate_centerpiece';
    }
    if (groupPosture && experienceContract.coordinationMode === 'momentum') {
        return 'group_momentum';
    }
    if (contractConstraints.peakCountModel === 'multi' &&
    ...
    if (experienceContract.coordinationMode === 'narrative' ||
...
function deriveDirectionNarrativeSummary(params) {
    const { experienceContract, contractConstraints, directionPeakModel, directionMovementStyle, directionExperienceIdentity, directionDistrictSupportSummary, strategy, } = params;
    ...
    const socialDensityPhrase = contractConstraints?.socialDensityBand === 'high' ||
...
function buildNearbyExamples(pocketId, debug) {
...
function getLaneIdentity(archetype, profile) {
    const mix = profile.tasteSignals.hospitalityMix;
    const ambiance = profile.tasteSignals.ambianceProfile;
    const tags = new Set(profile.tasteSignals.experientialTags.map((tag) => tag.toLowerCase()));
    const hasOutdoor = profile.categories.includes('park') ||
...
function getDistrictIdentityStrength(profile) {
    const mix = profile.tasteSignals.hospitalityMix;
    const tags = new Set(profile.tasteSignals.experientialTags.map((tag) => tag.toLowerCase()));
    ...
    const distinctiveTags = ['arts-adjacent', 'mixed-program', 'intentional', 'curated', 'activity-led', 'intimate'].filter((tag) => tags.has(tag)).length / 6;
    ...
    return toFixed(clamp(mixShape * 0.22 +
        categoryBreadth * 0.19 +
        distinctiveTags * 0.24 +
        momentSupport * 0.19 +
        viability * 0.16 -
        activityDominancePenalty, 0, 1));
}
function getMomentumProfile(archetype, profile) {
    const mix = profile.tasteSignals.hospitalityMix;
    const ambiance = profile.tasteSignals.ambianceProfile;
...
function getMacroLane(archetype, laneIdentity, profile) {
    const ambiance = profile.tasteSignals.ambianceProfile;
    const mix = profile.tasteSignals.hospitalityMix;
...
function getContrastProfile(archetype, profile) {
    ...
    const viability = profile.coreSignals.viability;
    const laneSpecificIdentity = laneIdentity !== 'district_core' &&
        laneIdentity !== 'social_mixed' &&
        laneIdentity !== 'lively_core';
    const momentumSpecific = momentumProfile === 'focused_flow' ||
        momentumProfile === 'soft_start' ||
        momentumProfile === 'late_pull';
    const contrastEligible = viability >= 0.58 &&
...
function buildDirectionCandidateFromRanked(entry, input, strategyMeta) {
    const profile = entry.profile;
    ...
    const directionMovementStyle = getDirectionMovementStyleLabel(experienceContract, contractConstraints);
    ...
    const experienceFamily = detectExperienceFamily({
        dominantCategories: profile.hyperlocal?.primaryMicroPocket.dominantCategories ??
        ...
        environmentalInfluencePotential: profile.hyperlocal?.primaryMicroPocket.environmentalInfluencePotential,
    ...
    return {
        ...
        directionStrategySummary: strategy?.summary ?? 'Balanced movement with one clear center and stable flow.',
        directionStrategySource: strategyMeta?.strategySource ??
...
function hasBaselineCandidateQuality(candidate, profile) {
...
function getCandidateDominantCategories(candidate) {
    return (candidate.derivedFrom.hyperlocal?.primaryMicroPocket.dominantCategories ??
        []).slice(0, 4);
}
function getCandidateAnchorKey(candidate) {
    return (candidate.derivedFrom.hyperlocal?.primaryAnchor?.entityId ??
        candidate.derivedFrom.hyperlocal?.primaryAnchor?.entityName ??
        undefined);
}
function getCandidatePocketTypeBucket(candidate) {
...
function getCandidateSimilarity(left, right) {
    const sameFamily = left.experienceFamily === right.experienceFamily ? 1 : 0;
    const sameArchetype = left.archetype === right.archetype ? 1 : 0;
    const categoryOverlap = getSetOverlap(getCandidateDominantCategories(left), getCandidateDominantCategories(right));
    const samePocketType = getCandidatePocketTypeBucket(left) === getCandidatePocketTypeBucket(right) ? 1 : 0;
    ...
    return clamp(sameFamily * 0.31 +
        sameArchetype * 0.24 +
        categoryOverlap * 0.27 +
        samePocketType * 0.12 +
        sameAnchor * 0.06, 0, 1);
}
function getStrategyOverlapScore(left, right) {
    ...
    const sameMomentum = left.contrastProfile.momentumProfile === right.contrastProfile.momentumProfile ? 0.08 : 0;
    return clamp(similarity + samePeak + sameMovement + sameMomentum, 0, 1);
}
function normalizeWithinPool(values) {
    if (values.length === 0) {
        return [];
    }
...
function buildStrategyCandidates(params) {
    const fallbackFamily = resolveStrategyFamily(params.input.context);
    const fallbackStrategies = resolveDirectionStrategiesForFamily(fallbackFamily);
    const hasBearingsWorlds = params.strategyAdmissibleWorlds.length > 0;
    const strategyWorldById = new Map(params.strategyAdmissibleWorlds.map((world) => [world.strategyId, world]));
    const seededStrategyPlan = (hasBearingsWorlds
    ...
    const fallbackSeedPool = params.sortedRanked.slice(0, Math.max(STRATEGY_POOL_WINDOW, TARGET_DIRECTION_CANDIDATES * 6));
    for (const planEntry of strategyPlan) {
        ...
        const strategyWorldPrimarySeed = strategyWorld?.admittedPockets.length
            ? strategyWorld.admittedPockets
            : strategyWorld
                ? [...strategyWorld.suppressedPockets, ...strategyWorld.rejectedPockets]
                : [];
        const strategySeedPool = strategyWorldPrimarySeed.length > 0
        ...
        const baseOptions = strategySeedPool
            ...
            if (shouldEnforceBaselineQuality &&
        ...
        const strategyPoolSize = strategyWorld?.debug.admittedCount ??
            baseOptions.length;
        ...
        const strategyFallbackApplied = !strategyWorld || strategyWorld.admittedPockets.length === 0;
        const strategyFallbackReason = strategyFallbackApplied
            ...
            : strategyWorld.admittedPockets.length > 0
                ...
                : 'strategy_world_non_viable_using_local_fallback';
        const rawScoredOptions = baseOptions.map((option) => {
            ...
            const worldShapeScore = option.strategyWorldDecision?.shapeScore ??
                option.entry.score;
            return {
                ...option,
                rawStrategyScore,
        ...
        const normalizedRawScores = normalizeWithinPool(rawScoredOptions.map((option) => option.rawStrategyScore));
        const normalizedWorldShapeScores = normalizeWithinPool(rawScoredOptions.map((option) => option.worldShapeScore));
        const scoredOptions = rawScoredOptions
            .map((option, index) => {
            const strategyScore = toFixed(clamp(normalizedRawScores[index] * 0.76 +
                normalizedWorldShapeScores[index] * 0.24, 0, 1));
            return {
                ...option,
                strategyScore,
            };
        ...
        if (selected.length > 0) {
            for (let index = 0; index < selectionPool.length; index += 1) {
                ...
                const maxOverlap = overlapRows.reduce((maxValue, row) => (row.overlap > maxValue ? row.overlap : maxValue), 0);
        ...
        const hardGuardStatus = worldDecision && !worldDecision.hardGuardPassed ? 'degraded' : 'pass';
        const hardGuardReason = hardGuardStatus === 'degraded'
        ...
        const strategyConstraintStatus = {
            required: worldDecision?.requiredPass === false ? 'fail' : 'pass',
            preferred: worldDecision?.preferredScore ?? 0.5,
            suppressed: worldDecision?.suppressedTriggered ? 'triggered' : 'not_triggered',
        };
        const strategyCandidate = buildDirectionCandidateFromRanked(chosen.entry, params.input, {
            strategy,
            collapseGuardApplied: chosenIndex > 0,
            overlapSummary: chosenOverlapSummary,
            strategySource: strategyWorld
            ...
            directionContractGateReasonSummary: chosen.baseCandidate.directionContractGateReasonSummary,
            ...
            directionStrategyWorldStatus: worldDecision?.status ?? 'not_evaluated',
            directionStrategyWorldReasonSummary: worldDecision?.reasonSummary ?? strategyFallbackReason,
        });
...
function getRichnessAssessment(candidate, existing) {
    ...
    for (const existingCandidate of existing) {
        ...
        const overlap = getSetOverlap(candidateCategories, getCandidateDominantCategories(existingCandidate));
        if (overlap < 0.38) {
            categoryContrast += 1;
        }
    }
    ...
    const richnessBoostApplied = familyContrastScore * 0.04 +
    ...
    const contrastReason = familyContrastScore >= 0.6
        ? 'family_contrast'
...
function hasConflictingFamilyExtremes(left, right) {
    const calmFamilies = new Set([
        'ambient',
        'intimate',
        'ritual',
        'indulgent',
    ...
    return ((leftEnergy === 'low' && rightEnergy === 'high') ||
        (leftEnergy === 'high' && rightEnergy === 'low'));
}
function areComposableCandidates(leftEntry, rightEntry, leftCandidate, rightCandidate) {
    ...
    const adjacent = haversineDistanceM(leftEntry.profile.centroid, rightEntry.profile.centroid) <=
        COMPOSED_MAX_DISTANCE_M;
    const identityOverlap = getSetOverlap(leftEntry.profile.categories, rightEntry.profile.categories) >=
        COMPOSED_MIN_IDENTITY_OVERLAP;
    const identityStrengthNear = Math.abs(leftCandidate.contrastProfile.districtIdentityStrength -
        rightCandidate.contrastProfile.districtIdentityStrength) <= 0.18;
    return adjacent || identityOverlap || identityStrengthNear;
}
function buildComposedCandidate(leftEntry, rightEntry, leftCandidate, rightCandidate, input, existingSingles) {
    if (!areComposableCandidates(leftEntry, rightEntry, leftCandidate, rightCandidate)) {
        return undefined;
    }
    const primary = leftCandidate.confidence >= rightCandidate.confidence ? leftCandidate : rightCandidate;
    const secondary = primary.pocketId === leftCandidate.pocketId ? rightCandidate : leftCandidate;
    const primaryProfile = primary.pocketId === leftEntry.profile.pocketId ? leftEntry.profile : rightEntry.profile;
    const secondaryProfile = secondary.pocketId === leftEntry.profile.pocketId ? leftEntry.profile : rightEntry.profile;
    const mergedMix = {
        drinks: toFixed(primary.derivedFrom.hospitalityMix.drinks * 0.56 +
            secondary.derivedFrom.hospitalityMix.drinks * 0.44),
        dining: toFixed(primary.derivedFrom.hospitalityMix.dining * 0.56 +
            secondary.derivedFrom.hospitalityMix.dining * 0.44),
        culture: toFixed(primary.derivedFrom.hospitalityMix.culture * 0.56 +
            secondary.derivedFrom.hospitalityMix.culture * 0.44),
        cafe: toFixed(primary.derivedFrom.hospitalityMix.cafe * 0.56 +
            secondary.derivedFrom.hospitalityMix.cafe * 0.44),
        activity: toFixed(primary.derivedFrom.hospitalityMix.activity * 0.56 +
            secondary.derivedFrom.hospitalityMix.activity * 0.44),
    };
    const mergedAmbiance = {
        energy: scoreToLevel(toLevelScore(primary.derivedFrom.ambianceProfile.energy) * 0.56 +
            toLevelScore(secondary.derivedFrom.ambianceProfile.energy) * 0.44),
        intimacy: scoreToLevel(toLevelScore(primary.derivedFrom.ambianceProfile.intimacy) * 0.56 +
            toLevelScore(secondary.derivedFrom.ambianceProfile.intimacy) * 0.44),
        noise: scoreToLevel(toLevelScore(primary.derivedFrom.ambianceProfile.noise) * 0.56 +
    ...
    const mergedSeeds = Array.from(new Set([...primary.derivedFrom.momentSeeds, ...secondary.derivedFrom.momentSeeds])).slice(0, 4);
    const mergedMomentPotential = toFixed(primary.derivedFrom.momentPotential * 0.56 +
        secondary.derivedFrom.momentPotential * 0.44);
    const composedConfidence = toFixed(primary.confidence * 0.56 + secondary.confidence * 0.44);
    const dominantCategories = Array.from(new Set([
    ...
    const whyHereSignals = Array.from(new Set([
    ...
    const activationStrength = toFixed((primaryProfile.hyperlocal?.primaryMicroPocket.activationStrength ?? 0.5) * 0.56 +
        (secondaryProfile.hyperlocal?.primaryMicroPocket.activationStrength ?? 0.5) *
            0.44);
    const environmentalInfluencePotential = toFixed((primaryProfile.hyperlocal?.primaryMicroPocket.environmentalInfluencePotential ?? 0.5) *
        0.56 +
    ...
    const composedCandidate = {
        ...
        highlights: Array.from(new Set([...primary.highlights.slice(0, 2), ...secondary.highlights.slice(0, 2)])).slice(0, 3),
        nearbyExamples: Array.from(new Set([
        ...
        contrastProfile: {
            ...
            districtIdentityStrength: toFixed((primary.contrastProfile.districtIdentityStrength +
                secondary.contrastProfile.districtIdentityStrength) /
                2),
            momentumProfile: primary.contrastProfile.momentumProfile,
            contrastEligible: primary.contrastProfile.contrastEligible ||
    ...
    const hasInterpretableIdentity = dominantCategories.length >= 2 ||
        whyHereSignals.length >= 1 ||
        mergedTags.length >= 3;
    const hasCohesiveExpressionInputs = mergedSeeds.length >= 1 &&
    ...
    const coherent = hasInterpretableIdentity &&
    ...
    composedCandidate.richnessDebug = {
        ...
        richnessContrastReason: coherent && assessment.netRichness >= MIN_RICHNESS_GAIN
...
export function buildDirectionCandidates(input) {
    ...
    const contractGateWorld = input.contractGateWorld ??
        buildContractGateWorld({
            ...
            source: 'direction.buildDirectionCandidates.fallback',
        });
    const strategyAdmissibleWorlds = input.strategyAdmissibleWorlds ??
        buildStrategyAdmissibleWorlds({
            contractGateWorld,
            strategyFamily: resolveStrategyFamily(input.context),
            strategySummary: undefined,
        });
    const gatedRanked = contractGateWorld.admittedPockets;
    const gateDecisionByPocketId = new Map(Object.entries(contractGateWorld.decisionByPocketId).map(([pocketId, decision]) => [
        pocketId,
        decision,
    ]));
    const poolLimit = resolveCandidatePoolLimit(input.candidatePoolLimit, gatedRanked.length);
    ...
    const allStrategyWorldsCollapsedOrEmpty = strategyWorldsPresent &&
        strategyAdmissibleWorlds.every((world) => world.debug.survivabilityStatus === 'collapsed' &&
            world.admittedPockets.length === 0 &&
            world.suppressedPockets.length === 0 &&
            world.rejectedPockets.length === 0);
    const preserveStrategyBackedCandidates = strategyWorldsPresent &&
    ...
    const candidates = preserveStrategyBackedCandidates
        ? [...strategyCandidates]
        : gatedRanked
            .slice(0, poolLimit)
    ...
    while (candidates.length < TARGET_DIRECTION_CANDIDATES) {
        const remainingPool = gatedRanked
            ...
            const richnessAdjustedScore = candidate.confidence +
    ...
    if (candidates.length < TARGET_DIRECTION_CANDIDATES) {
        ...
        const candidateEntries = [...entryCandidateByPocketId.values()].filter(({ entry, candidate }) => hasBaselineCandidateQuality(candidate, entry.profile));
        for (let leftIndex = 0; leftIndex < candidateEntries.length; leftIndex += 1) {
            ...
            for (let rightIndex = leftIndex + 1; rightIndex < candidateEntries.length; rightIndex += 1) {
                ...
                const composed = buildComposedCandidate(left.entry, right.entry, left.candidate, right.candidate, input, candidates);
                if (!composed) {
                    continue;
                }
                const assessment = getRichnessAssessment(composed, candidates);
                if (assessment.netRichness < MIN_RICHNESS_GAIN ||
                    assessment.maxSimilarity >= RICHNESS_SIMILARITY_BLOCK) {
                    continue;
                }
                const composedWithDebug = {
                    ...composed,
                    richnessDebug: {
                        richnessBoostApplied: assessment.richnessBoostApplied,
                        similarityPenaltyApplied: assessment.similarityPenaltyApplied,
                ...
                if (candidates.some((candidate) => candidate.pocketId === composedWithDebug.pocketId) ||
                    composedCandidates.some((candidate) => candidate.pocketId === composedWithDebug.pocketId)) {
                    continue;
                }
                composedCandidates.push(composedWithDebug);
                if (candidates.length + composedCandidates.length >= TARGET_DIRECTION_CANDIDATES ||
