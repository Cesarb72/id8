...
export function applyTargetedRefinement({ currentArc, scoredVenues, intent, crewPolicy, lens, directive, targetedRoles, }) {
    let targetedCandidateCount = 0;
    let winnerInertiaDetected = false;
    let winnerInertiaReduced = false;
    const winnerInertiaNotes = [];
    for (const role of targetedRoles) {
        ...
        for (const candidate of candidates) {
            ...
            const specificityGain = candidate.scoredVenue.contextSpecificity.byRole[internalRole] -
            ...
            const specificityOverride = specificityGain >= 0.09 && arcDelta >= -(directive.maxArcScoreDrop + 0.02);
            if (objectiveImproved && (acceptableArcCost || specificityOverride)) {
                ...
                if (specificityOverride && !acceptableArcCost) {
                    winnerInertiaNotes.push(`Accepted ${role} challenger due to stronger context specificity (+${specificityGain.toFixed(3)}).`);
                }
                return {
                    nextArc: swapped,
                    targetedRoles,
            ...
            if (candidate === axisLeader && objectiveImproved && !acceptableArcCost) {
                axisLeaderCouldNotWin = true;
                winnerInertiaNotes.push(`Axis-leading ${role} challenger rejected due to arc quality drop (${arcDelta.toFixed(3)}).`);
            }
        }
