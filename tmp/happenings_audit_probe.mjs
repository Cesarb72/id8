...
function applyTasteModeToLens(lens, tasteMode) {
  const nextLens = {
    ...lens,
    tasteMode,
    energyBand: unique2([...lens.energyBand, ...tasteMode.energyBand]),
    preferredCategories: unique2([...lens.preferredCategories, ...tasteMode.favoredCategories]),
    discouragedCategories: unique2([
      ...lens.discouragedCategories,
      ...tasteMode.discouragedCategories
    ]),
    preferredTags: unique2([...lens.preferredTags, ...tasteMode.favoredTags]),
    discouragedTags: unique2([...lens.discouragedTags, ...tasteMode.discouragedTags]),
...
function pickStarterPackTerms(starterPack) {
  ...
  return unique4(preferred.slice(0, 3).map(normalizeTerm).filter((value) => value.length >= 4));
}
...
function buildQueryText(kind, descriptors, locationLabel) {
  const phrase = unique4(descriptors).slice(0, 4).join(" ");
...
function computeLaneDiversityScore(entities) {
  ...
  return clamp3(uniqueCategories.size / Math.min(entities.length, 6), 0, 1);
}
...
function buildStableRawPocketId(entities) {
  ...
  return `raw-pocket-${hashString3(signature)}`;
}
...
function resolveLocation(input) {
  ...
  if (input.userLatLng) {
    return {
      ...
      radiusM: clamp5(input.searchRadiusM ?? DEFAULT_RADIUS_M, MIN_RADIUS_M, MAX_RADIUS_M),
      confidence: "high",
      source: "user_lat_lng",
      meta: {
  ...
  if (hint) {
    return {
      ...
      center: hint.center,
      radiusM: clamp5(input.searchRadiusM ?? hint.radiusM, MIN_RADIUS_M, MAX_RADIUS_M),
  ...
  if (parsedCityState) {
    return {
      ...
      radiusM: clamp5(input.searchRadiusM ?? 6200, MIN_RADIUS_M, MAX_RADIUS_M),
      confidence: "low",
      source: "query_lookup",
      meta: {
...
function getSecondaryVarietyGain(profile, retainedProfiles) {
  ...
  const interpretableShapeScore = clamp6(
...
function computeDistrictScores(fieldSignals, viabilityClass) {
  const normalizedEntityCount = clamp7(fieldSignals.entityCount / 10, 0, 1);
  ...
  const totalScore = clamp7(fieldScore + viabilityBonus, 0, 1);
  return {
...
function computeFieldSignals(pocket) {
  return {
    ...
    density: toFixed5(clamp8(pocket.viability.signals.densityScore, 0, 1)),
    walkability: toFixed5(clamp8(pocket.viability.signals.walkabilityScore, 0, 1)),
    viability: toFixed5(clamp8(pocket.viability.score, 0, 1))
  };
}
...
function computeAnchorCandidateIds(entities, dominantCategories, dominantLanes, centroid, radiusM) {
  ...
  return entities.map((entity) => {
    ...
    const proximityScore = clamp9(1 - distanceToCentroid / Math.max(120, radiusM * 1.2), 0, 1);
    ...
    const supportDensityScore = clamp9(supportCount / Math.max(1, entities.length - 1), 0, 1);
...
function buildMicroPocket(pocketId, entities, index) {
  ...
  const categoryDiversity = clamp9(categoryCounts.length / 5, 0, 1);
  const laneDiversity = clamp9(laneCounts.length / 4, 0, 1);
  const compactness = clamp9(1 - radiusM / 260, 0, 1);
  ...
  const experienceForwardSignal = clamp9(
    experienceForwardCount / Math.max(1, entities.length),
    0,
    1
  );
  const identityStrength = clamp9(
...
function resolveMicroPockets(pocket) {
  ...
  const epsM = clamp9(pocket.geometry.maxDistanceFromCentroidM * 0.42, 70, 180);
...
function buildAnchorScore(entity, pocketEntities, primaryMicroPocket) {
  ...
  const proximityScore = clamp10(1 - distanceToMicroCenter / proximityBandM, 0, 1);
  ...
  const supportDensity = clamp10(supportCount / Math.max(1, pocketEntities.length - 1), 0, 1);
...
function toFixed8(value) {
  return Number(clamp11(value, 0, 1).toFixed(3));
}
...
function computeViabilitySignals(pocket, thresholds) {
  ...
  const densityScore = clamp13(pocket.geometry.densityEntitiesPerKm2 / DENSITY_SCORE_BASELINE, 0, 1);
  const walkabilityScore = clamp13(1 - pocket.geometry.avgDistanceFromCentroidM / 260, 0, 1) * 0.75 + clamp13(1 - pocket.geometry.maxPairwiseDistanceM / 950, 0, 1) * 0.25;
  const compactnessScore = clamp13(
    1 - clamp13((pocket.geometry.elongationRatio - 1) / 3, 0, 1),
    0,
    1
  );
...
function computeViabilityScore(signals) {
  const entityCountScore = clamp13(signals.entityCount / 9, 0, 1);
  ...
  return toFixed11(clamp13(score, 0, 1));
}
...
function mergeRule(base, patch) {
  return {
    ...
    requiredCategories: unique7([...base.requiredCategories ?? [], ...patch.requiredCategories ?? []]),
    preferredCategories: unique7([...base.preferredCategories ?? [], ...patch.preferredCategories ?? []]),
    discouragedCategories: unique7([...base.discouragedCategories ?? [], ...patch.discouragedCategories ?? []]),
    requiredTags: unique7([...base.requiredTags ?? [], ...patch.requiredTags ?? []]),
    preferredTags: unique7([...base.preferredTags ?? [], ...patch.preferredTags ?? []]),
    discouragedTags: unique7([...base.discouragedTags ?? [], ...patch.discouragedTags ?? []]),
...
function getDensityScore(candidateCount, maxCandidateCount) {
  ...
  return clamp0111(candidateCount / maxCandidateCount);
}
function getAdjustedDensityScore(rawDensity) {
  return clamp0111(Math.pow(rawDensity, 0.72));
}
function getRelevanceScore(candidates) {
  ...
  return clamp0111(total / candidates.length);
}
function getDiversityScore(group) {
  ...
  const categoryCoverage = clamp0111(group.categories.size / 4);
  return clamp0111(coreRoleCoverage * 0.65 + categoryCoverage * 0.35);
}
...
function getDominanceScore(values) {
  ...
  return clamp0111(maxCount / values.length);
}
function getCoherenceScore(group) {
  ...
  return clamp0111(neighborhoodDominance * 0.58 + experienceFamilyDominance * 0.42);
}
function getSequenceSupportScore(group) {
  ...
  return clamp0111(sequenceMean * 0.68 + coverage * 0.22 + sequenceBalance * 0.1);
}
function getTasteAverages(candidates) {
  ...
  return {
    intimacy: clamp0111(totals.intimacy / candidates.length),
    linger: clamp0111(totals.linger / candidates.length),
    conversation: clamp0111(totals.conversation / candidates.length),
    calm: clamp0111(totals.calm / candidates.length),
    social: clamp0111(totals.social / candidates.length),
    culturalDepth: clamp0111(totals.culturalDepth / candidates.length),
    discovery: clamp0111(totals.discovery / candidates.length),
    experiential: clamp0111(totals.experiential / candidates.length),
    outdoor: clamp0111(totals.outdoor / candidates.length),
    playful: clamp0111(totals.playful / candidates.length)
  };
}
function getCategoryMix(candidates) {
  ...
  return {
    restaurant: clamp0111(totals.restaurant / candidates.length),
    bar: clamp0111(totals.bar / candidates.length),
    cafe: clamp0111(totals.cafe / candidates.length),
    dessert: clamp0111(totals.dessert / candidates.length),
    liveMusic: clamp0111(totals.liveMusic / candidates.length),
    activity: clamp0111(totals.activity / candidates.length),
    park: clamp0111(totals.park / candidates.length),
    museum: clamp0111(totals.museum / candidates.length),
    event: clamp0111(totals.event / candidates.length)
  };
}
...
function getWalkabilitySignal(candidates) {
  ...
  return clamp0111(matched / candidates.length);
}
...
function getRoleStrengths(group) {
  ...
  const chaosPenalty = clamp0111(Math.max(0, social - 0.58) * 0.9 + Math.max(0, energy - 0.62) * 0.8);
  const overCalmPenalty = clamp0111(Math.max(0, taste.calm - 0.72) * 0.9);
  ...
  const startCategorySupport = clamp0111(
  ...
  const highlightCategorySupport = clamp0111(
  ...
  const winddownCategorySupport = clamp0111(
  ...
  const highlightStrength = clamp0111(
  ...
  const winddownStrength = clamp0111(
...
function getAffinityScores(group, intent, sequenceSupport, signature) {
  ...
  return {
    affinity: clamp0111(vibeAffinity * 0.58 + crewAffinity * 0.42),
    vibeAffinity: clamp0111(vibeAffinity),
    crewAffinity: clamp0111(crewAffinity)
  };
}
function getAlignmentLift(density, affinity, signature, coherence, relevance, diversity) {
  const contextStrength = clamp0111(relevance * 0.55 + diversity * 0.45);
  return clamp0111(
...
function recommendDistricts({
  ...
  return districtGroups.map((group) => {
    ...
    const score = roundToHundredths2(
      clamp0111(baseScore + alignmentLift - breadthPenalty - sparsityPenalty)
    );
    const signals = {
      density,
      relevance,
...
function buildDistrictTasteAggregationByDistrictId(scoredVenues, city, context) {
...
function topEntries(record, limit = 10) {
  return Object.entries(record).sort((a, b) => b[1] - a[1] || String(a[0]).localeCompare(String(b[0]))).slice(0, limit).map(([key, value]) => ({ key, count: value }));
}
...
async function main() {
  ...
  const lateCloseHints = retrieved.filter(
    (v) => `${v.shortDescription} ${v.narrativeFlavor} ${v.tags.join(" ")}`.toLowerCase().includes("late") || `${v.shortDescription} ${v.narrativeFlavor} ${v.tags.join(" ")}`.toLowerCase().includes("night")
  ).length;
