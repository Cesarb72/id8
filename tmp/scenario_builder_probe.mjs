...
function getWhyThisWorks(stops) {
  const start = stops[0];
  const highlight = stops[2];
  const closer = stops[4];
...
function getScenarioCoherence(scenarioFamily, stops) {
  ...
  if (scenarioFamily === "romantic_lively") {
    ...
    return clamp01(
      highlight.authorityScore * 0.34 + highlight.currentRelevance * 0.2 + getStopQuality(stops[3]) * 0.16 + getStopQuality(closer) * 0.14 + livelySignal * 0.16
    );
  }
  ...
  return clamp01(
    highlight.authorityScore * 0.3 + getStopQuality(stops[0]) * 0.2 + getStopQuality(stops[1]) * 0.14 + getStopQuality(closer) * 0.14 + culturalSignal * 0.22
  );
}
function getCandidateNightScore(scenarioFamily, stops) {
  const anchorAuthority = stops[2]?.authorityScore ?? 0;
  const startQuality = getStopQuality(stops[0]);
  const landingQuality = clamp01((getStopQuality(stops[3]) + getStopQuality(stops[4])) / 2);
...
function overlapScore(left, right) {
  ...
  return clamp01(sharedVenues / 5 * 0.78 + sharedDistricts / 5 * 0.22);
}
...
function selectDistinctNights(candidates, targetCount) {
  ...
  const sorted = candidates.slice().sort((left, right) => right.score - left.score || left.stops[2].name.localeCompare(right.stops[2].name));
  const selected = [];
  if (sorted.length > 0) {
...
function ensureHiddenGemVariant(selected, allCandidates) {
  ...
  const hiddenGemCandidate = allCandidates.filter((night) => night.hiddenGemPresence >= 0.45).sort((left, right) => right.score - left.score || left.stops[2].name.localeCompare(right.stops[2].name))[0];
...
function buildScenarioNightsFromCandidateBoard(board, options = {}) {
  ...
  const rankedCandidates = candidateNights.slice().sort((left, right) => right.score - left.score || left.stops[2].name.localeCompare(right.stops[2].name));
  ...
  return selected.map((night, index) => {
    ...
    return {
      ...
      flavorLine: SCENARIO_FLAVOR_LINE[board.scenarioFamily],
      stops: night.stops,
      whyThisWorks: getWhyThisWorks(night.stops),
      complete: true
    };
  });
}
...
function normalizePersona(value) {
  const token = normalizeToken3(value);
...
function normalizeCity4(value) {
  const normalized = normalizeToken4(value).replace(/\./g, "");
  const [head] = normalized.split(",");
  return (head ?? normalized).trim();
}
...
function uniqueLowerTokens(venue) {
  ...
  return new Set(
    normalizeToken4(seed).split(" ").map((token) => token.trim()).filter(Boolean)
  );
}
function hasAnyToken2(tokens, values) {
  return values.some((value) => tokens.has(normalizeToken4(value)));
}
function hasAnyPhrase(value, phrases) {
  const normalized = normalizeToken4(value);
  return phrases.some((phrase) => normalized.includes(normalizeToken4(phrase)));
}
