...
function getHappeningsSignal(happenings2, key) {
  return clamp14(happenings2?.[key] ?? 0, 0, 1);
}
...
function getIntentFit(input) {
  ...
  const happenings2 = input.happenings;
  ...
  if (persona2.includes("romantic")) {
    personaFit = clamp14(
      tasteSignals.romanticScore * 0.46 + tasteSignals.intimacy * 0.22 + tasteSignals.conversationFriendliness * 0.16 + tasteSignals.lingerFactor * 0.12 + getHappeningsSignal(happenings2, "culturalAnchorPotential") * 0.04,
      0,
      1
    );
  ...
  return toFixed12(
    clamp14(
      base * 0.2 + vibeFit * 0.43 + personaFit * 0.33 + getHappeningsSignal(happenings2, "currentRelevance") * 0.02 + getHappeningsSignal(happenings2, "hotspotStrength") * 0.02,
      0,
      1
    )
  );
...
function getTimingRelevance(input) {
  ...
  return toFixed12(
    clamp14(
      signals.hyperlocalActivation.temporalRelevance * 0.42 + hoursAvailability * 0.26 + (hasEvent ? 0.14 : 0) + (hasPerformance ? 0.1 : 0) + getHappeningsSignal(happenings2, "lateNightPotential") * 0.06 + getHappeningsSignal(happenings2, "currentRelevance") * 0.06 + happyHourBonus + temporalWindowBonus,
      0,
      1
    )
  );
...
function detectAnchorMoment(input, timingRelevance, intentFit) {
  ...
  const anchorStrength = clamp14(
    signals.roleSuitability.highlight * 0.34 + tierScore * 0.2 + signals.anchorStrength * 0.18 + signals.momentIntensity.score * 0.16 + signals.momentPotential.score * 0.08 + getHappeningsSignal(happenings2, "majorVenueStrength") * 0.03 + getHappeningsSignal(happenings2, "culturalAnchorPotential") * 0.03 + getHappeningsSignal(happenings2, "hotspotStrength") * 0.02 + intentFit * 0.04,
    0,
    1
  );
...
function detectTemporalMoment(input, timingRelevance, intentFit) {
  ...
  const temporalSignalsPresent = Boolean(
    input.sourceFlags?.eventCapable || input.sourceFlags?.musicCapable || input.sourceFlags?.performanceCapable || input.sourceFlags?.hasHappyHour || getHappeningsSignal(happenings2, "eventPotential") >= 0.48 || getHappeningsSignal(happenings2, "lateNightPotential") >= 0.48 || signals.hyperlocalActivation.temporalLabel !== "background"
  );
  if (!temporalSignalsPresent) {
    return null;
  }
  const strength = clamp14(
    timingRelevance * 0.56 + Math.max(signals.momentPotential.score, signals.momentIntensity.score) * 0.2 + signals.roleSuitability.highlight * 0.12 + getHappeningsSignal(happenings2, "currentRelevance") * 0.08 + getHappeningsSignal(happenings2, "lateNightPotential") * 0.06 + intentFit * 0.12,
    0,
    1
  );
  if (strength < 0.58) {
...
function detectDiscoveryMoment(input, timingRelevance, intentFit) {
  ...
  const discoveryStrength = clamp14(
    signals.noveltyWeight * 0.34 + signals.categorySpecificity * 0.24 + signals.personalityStrength * 0.18 + signals.momentPotential.score * 0.14 + signals.experientialFactor * 0.06 + getHappeningsSignal(happenings2, "hiddenGemStrength") * 0.04,
    0,
    1
  );
...
function detectCommunityMoment(input, timingRelevance, intentFit) {
  ...
  const communityStrength = clamp14(
    signals.momentEnrichment.culturalDepth * 0.33 + signals.conversationFriendliness * 0.2 + signals.lingerFactor * 0.16 + signals.experientialFactor * 0.11 + getHappeningsSignal(happenings2, "culturalAnchorPotential") * 0.08 + getHappeningsSignal(happenings2, "eventPotential") * 0.04 + hintScore + activationScore + intentFit * 0.1,
    0,
    1
  );
...
function mapVenueToTasteInput(venue) {
  ...
  const happenings2 = venue.source.happenings ?? deriveVenueHappeningsSignals(venue);
  return {
    id: venue.id,
    name: venue.name,
    ...
    happenings: happenings2,
    hotspotStrength: happenings2.hotspotStrength,
    eventPotential: happenings2.eventPotential,
    performancePotential: happenings2.performancePotential,
    liveNightlifePotential: happenings2.liveNightlifePotential,
    culturalAnchorPotential: happenings2.culturalAnchorPotential,
    lateNightPotential: happenings2.lateNightPotential,
    currentRelevance: happenings2.currentRelevance,
    hiddenGemStrength: happenings2.hiddenGemStrength,
    majorVenueStrength: happenings2.majorVenueStrength
  };
}
...
function deriveDiscoveryBalance2(original, moments, contract) {
  const all2 = [...moments.primary, ...moments.secondary];
  if (all2.length === 0) {
    return original;
  }
  const discoveryLike = all2.filter(
    (moment) => moment.momentType === "discovery" || moment.momentType === "community"
  ).length;
  const ratio = discoveryLike / all2.length;
...
// src/domain/interpretation/taste/step2AuthorityConviction.ts
var SAN_JOSE_ROMANTIC_COZY = {
  anchors: [
    { venue: "La For\xEAt", weight: 1, buckets: ["anchor", "hidden_gem", "closer"] },
...
var SAN_JOSE_ROMANTIC_CULTURED = {
  anchors: [
...
function normalizeToken4(value) {
  return (value ?? "").toLowerCase().replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();
}
function matchesVenueName(value, target) {
  const normalizedValue = normalizeToken4(value);
  const normalizedTarget = normalizeToken4(target);
...
function getScenarioMap(city2, persona2, vibe) {
  const normalizedCity = normalizeToken4(city2);
  const normalizedPersona = normalizeToken4(persona2);
  const normalizedVibe = normalizeToken4(vibe);
...
function getBucketConviction(name, map, bucket) {
  if (!name) {
    return 0;
  }
  let best = 0;
...
function getAggregateBucketConviction(names, map, bucket) {
  if (names.length === 0) {
    return 0;
  }
  const values = names.map((name) => getBucketConviction(name, map, bucket));
  return clampScore3(values.reduce((sum, value) => sum + value, 0) / values.length);
}
function getReasonSignalScore(corpus) {
  const normalized = normalizeToken4(corpus);
  if (!normalized) {
    return {
      nightlife: 0,
      event: 0,
  ...
  const hasAny3 = (tokens) => tokens.some((token) => normalized.includes(token));
  return {
    nightlife: hasAny3(["late", "nightlife", "cocktail", "live", "jazz", "music", "after show"]) ? 1 : 0,
    event: hasAny3(["event", "show", "performance", "theatre", "opera", "ticketed"]) ? 1 : 0,
    cultural: hasAny3(["museum", "gallery", "cultural", "curated", "exhibit", "art"]) ? 1 : 0,
    discovery: hasAny3(["discovery", "hidden", "local", "insider", "neighborhood"]) ? 1 : 0,
    hiddenGem: hasAny3(["hidden", "insider", "garden", "lantern", "atelier"]) ? 1 : 0,
    close: hasAny3(["close", "landing", "nightcap", "wind down", "soft"]) ? 1 : 0,
    start: hasAny3(["start", "entry", "opener", "aperitivo"]) ? 1 : 0
  };
}
function getTimingSignalScore(value) {
  const normalized = normalizeToken4(value);
  if (!normalized) {
    return 0;
  }
...
function getHappeningsAuthority(happenings2) {
  if (happenings2.length === 0) {
  ...
  let discovery = 0;
  let currentness = 0;
  happenings2.slice(0, 2).forEach((entry) => {
    const weight = clampScore3(entry.strength ?? 0.62);
  ...
  return {
    nightlife: clampScore3(nightlife),
    event: clampScore3(event),
    cultural: clampScore3(cultural),
    discovery: clampScore3(discovery),
    currentness: clampScore3(currentness)
  };
}
function deriveStep2AuthoritySignals(input) {
  ...
  const reasonCorpus = `${anchorName} ${input.anchorReason ?? ""} ${startNames.map((name, index) => `${name} ${input.starts[index]?.reason ?? ""}`).join(" ")} ${closeNames.map((name, index) => `${name} ${input.closes[index]?.reason ?? ""}`).join(" ")}`.trim();
  const reasonSignals = getReasonSignalScore(reasonCorpus);
  const happeningsSignals = getHappeningsAuthority(input.happenings);
  const anchorTimingSignal = clampScore3(input.anchorTimingRelevance ?? 0);
  const anchorLiveSignal = clampScore3(
    (input.anchorLiveContext?.hasEvent ? 0.26 : 0) + (input.anchorLiveContext?.hasPerformance ? 0.24 : 0) + (input.anchorLiveContext?.hasHappyHour ? 0.18 : 0) + getTimingSignalScore(input.anchorLiveContext?.timeWindowLabel) * 0.2
  );
  ...
  const mappedHiddenGem = map ? getAggregateBucketConviction([anchorName, ...startNames, ...closeNames], map, "hidden_gem") : 0;
  const mappedCultural = map ? getAggregateBucketConviction([anchorName, ...startNames, ...closeNames], map, "cultural") : 0;
  const mappedNightlife = map ? getAggregateBucketConviction([anchorName, ...startNames, ...closeNames], map, "nightlife") : 0;
  const mappedMajorEvent = map ? getAggregateBucketConviction([anchorName, ...startNames, ...closeNames], map, "major_event") : 0;
  const mappedDiscovery = map ? getAggregateBucketConviction([anchorName, ...startNames], map, "discovery") : 0;
  const summaryNightlife = input.summary?.dominantEnergy === "lively" ? 1 : input.summary?.dominantEnergy === "balanced" ? 0.62 : 0.34;
  const summaryCultural = input.summary?.discoveryBalance === "novel" ? 1 : input.summary?.discoveryBalance === "balanced" ? 0.64 : 0.34;
  const summaryAnchor = input.summary?.highlightPotential === "high" ? 1 : input.summary?.highlightPotential === "medium" ? 0.72 : 0.44;
  const summaryContainment = input.summary?.movementProfile === "tight" ? 0.9 : input.summary?.movementProfile === "moderate" ? 0.65 : 0.36;
  const anchorContextSignal = Math.max(
    reasonSignals.event,
    reasonSignals.cultural,
    reasonSignals.nightlife
  );
  const anchorConviction = clampScore3(
    mappedAnchor * 0.56 + summaryAnchor * 0.34 + anchorContextSignal * 0.1
  );
  const startConviction = clampScore3(
    mappedStart * 0.5 + reasonSignals.start * 0.28 + summaryContainment * 0.12 + mappedAnchor * 0.1
  );
  const windDownConviction = clampScore3(
    mappedWindDown * 0.52 + reasonSignals.close * 0.26 + mappedHiddenGem * 0.1 + summaryContainment * 0.12
  );
  const hiddenGemConviction = clampScore3(
    mappedHiddenGem * 0.6 + reasonSignals.hiddenGem * 0.16 + happeningsSignals.discovery * 0.24
  );
  const culturalConviction = clampScore3(
    mappedCultural * 0.56 + happeningsSignals.cultural * 0.24 + summaryCultural * 0.2
  );
  const nightlifeConviction = clampScore3(
    mappedNightlife * 0.5 + happeningsSignals.nightlife * 0.26 + summaryNightlife * 0.24
  );
  const majorEventConviction = clampScore3(
    mappedMajorEvent * 0.54 + happeningsSignals.event * 0.3 + reasonSignals.event * 0.1 + summaryNightlife * 0.06
  );
  const discoveryConviction = clampScore3(
    mappedDiscovery * 0.46 + happeningsSignals.discovery * 0.26 + reasonSignals.discovery * 0.14 + summaryCultural * 0.14
  );
  const happeningAuthority = clampScore3(
    happeningsSignals.event * 0.32 + happeningsSignals.nightlife * 0.24 + happeningsSignals.cultural * 0.24 + happeningsSignals.discovery * 0.2 + summaryNightlife * 0.06 + summaryCultural * 0.06
  );
  const whyTonightPressure = clampScore3(
    happeningsSignals.currentness * 0.38 + happeningsSignals.event * 0.18 + happeningsSignals.nightlife * 0.14 + anchorTimingSignal * 0.14 + anchorLiveSignal * 0.16
  );
  const normalizedVibe = normalizeToken4(input.vibe);
  const modeAuthority = normalizedVibe === "lively" ? nightlifeConviction * 0.34 + majorEventConviction * 0.24 + anchorConviction * 0.18 + startConviction * 0.12 + whyTonightPressure * 0.12 : normalizedVibe === "cultured" ? culturalConviction * 0.3 + discoveryConviction * 0.2 + majorEventConviction * 0.14 + windDownConviction * 0.12 + anchorConviction * 0.1 + whyTonightPressure * 0.14 : hiddenGemConviction * 0.26 + windDownConviction * 0.24 + anchorConviction * 0.18 + discoveryConviction * 0.1 + culturalConviction * 0.1 + whyTonightPressure * 0.12;
  const overallAuthority = clampScore3(
    modeAuthority * 0.66 + happeningAuthority * 0.14 + whyTonightPressure * 0.12 + (anchorConviction + startConviction + windDownConviction) / 3 * 0.08
  );
  return {
    anchorConviction,
    startConviction,
...
function reshapeMom(ms, e) {
  ...
  const pc = Math.max(1, ms.primary.length), sc = ms.secondary.length;
  return { primary: r.slice(0, pc), secondary: r.slice(pc, pc + sc) };
}
...
function toStops(cs) {
  return (cs ?? []).slice(0, 2).map((c) => ({ id: c.venueId, name: c.venueName, score: c.score, reason: c.reason }));
}
function happenings(a) {
  if (!a) return [];
  const p = { temporal: 5, discovery: 4, community: 3, supporting: 2, anchor: 1 };
  return [...a.moments.primary, ...a.moments.secondary].filter((m) => m.momentType !== "anchor" && m.strength >= 0.6).sort((l, r) => {
    const pd = (p[r.momentType] ?? 0) - (p[l.momentType] ?? 0);
    if (pd !== 0) return pd;
    if (r.strength !== l.strength) return r.strength - l.strength;
    return l.id.localeCompare(r.id);
  }).filter((m, i, all2) => all2.findIndex((x) => x.title === m.title) === i).slice(0, 2).map((m) => ({ id: m.id, type: m.momentType, strength: m.strength, hasEvent: m.liveContext?.hasEvent, hasPerformance: m.liveContext?.hasPerformance }));
}
function qscore(a, starts, closes, hs, vibe) {
  const anchor = a.anchors.strongestHighlight;
  const as = anchor?.score ?? 0.44;
  const ss = starts.length ? starts.reduce((s, x) => s + x.score, 0) / starts.length : 0;
  const cs = closes.length ? closes.reduce((s, x) => s + x.score, 0) / closes.length : 0;
  const sup = (ss + cs) / 2;
  const temp = a.moments.primary.filter((m) => m.momentType === "temporal").length;
  const disc = a.moments.primary.filter((m) => m.momentType === "discovery" || m.momentType === "community").length;
  const live = clamp17(temp * 0.28 + (hs.some((h) => h.hasEvent) ? 0.22 : 0) + (hs.some((h) => h.hasPerformance) ? 0.18 : 0) + (hs.length ? 0.12 : 0));
  const auth = deriveStep2AuthoritySignals({ city, persona, vibe, anchorName: anchor?.venueName, anchorReason: anchor?.reason, starts: starts.map((x) => ({ name: x.name, reason: x.reason })), closes: closes.map((x) => ({ name: x.name, reason: x.reason })), happenings: hs.map((h) => ({ name: h.id, type: h.type, reason: "", strength: h.strength, hasEvent: h.hasEvent, hasPerformance: h.hasPerformance })), summary: a.summary });
  const score = as * 0.32 + sup * 0.2 + auth.overallAuthority * 0.14 + live * 0.08 + clamp17(disc * 0.24 + (hs.length ? 0.2 : 0)) * 0.06 + (a.summary.movementProfile === "tight" ? 1 : a.summary.movementProfile === "moderate" ? 0.72 : 0.46) * 0.03 + (starts.length && closes.length ? 1 : starts.length || closes.length ? 0.58 : 0) * 0.03;
  return { score, auth };
}
...
function dedupe(cards) {
  ...
  for (const c of cards) {
    const k = (c.anchorId ?? "").trim().toLowerCase();
    const ex = by.get(k) ?? [];
    if (!ex.length) {
      sel.push(c);
      by.set(k, [c]);
    ...
    const fp = (x) => `${x.label.toLowerCase()}::${x.starts.map((s) => s.name.toLowerCase()).join("|")}::${x.closes.map((s) => s.name.toLowerCase()).join("|")}::${x.happenings.map((h) => h.type ?? "generic").join("|")}`;
    const keep = ex.every((o) => fp(o) !== fp(c));
    if (keep) {
      sel.push(c);
      by.set(k, [...ex, c]);
    } else drop.push({ ...c, droppedReason: "anchor_dedupe" });
  }
  return { kept: sel, dropped: drop };
}
function stageCount(ids, meta) {
  const ms = [...ids].map((i) => meta.get(i)).filter(Boolean);
  return { totalVenueIds: ms.length, eventBearing: ms.filter((m) => m.eventBearing).length, performanceCapable: ms.filter((m) => m.performanceCapable).length, musicCapable: ms.filter((m) => m.musicCapable).length, nightlifeCapable: ms.filter((m) => m.nightlifeCapable).length, culturallyStrong: ms.filter((m) => m.culturallyStrong).length, majorVenueStrong: ms.filter((m) => m.majorVenueStrong).length, currentRelevant: ms.filter((m) => m.currentRelevant).length };
}
...
for (const vibe of vibes) {
  ...
  const base = /* @__PURE__ */ new Map(), afterS = /* @__PURE__ */ new Map(), afterE = /* @__PURE__ */ new Map(), afterX = /* @__PURE__ */ new Map();
  for (const c of cands) {
  ...
  const idsB = /* @__PURE__ */ new Set(), idsS = /* @__PURE__ */ new Set(), idsE = /* @__PURE__ */ new Set(), idsX = /* @__PURE__ */ new Set();
  ...
  const traces = cands.map((c) => {
    const a = afterX.get(c.p.pocketId);
    if (!a) return { pocket: c.p.pocketId, label: c.p.label, join: c.join, hasAgg: false, hasAnchor: false, hasSupport: false, score: -1, anchor: null, anchorId: null, starts: [], closes: [], happenings: [] };
    const anchor = a.anchors.strongestHighlight;
    const starts = toStops(a.ingredients.startCandidates), closes = toStops(a.ingredients.windDownCandidates), hs = happenings(a);
    const hasAnchor = Boolean(anchor?.venueId && anchor.venueName), hasSupport = starts.length > 0 || closes.length > 0;
    const qs = qscore(a, starts, closes, hs, vibe);
    return { pocket: c.p.pocketId, label: c.p.label, join: c.join, hasAgg: true, hasAnchor, hasSupport, score: qs.score, anchor: anchor?.venueName ?? null, anchorId: anchor?.venueId ?? null, starts, closes, happenings: hs, authority: qs.auth };
  });
  ...
  const surfIds = /* @__PURE__ */ new Set();
  for (const s of surv) {
    if (s.anchorId) surfIds.add(s.anchorId);
    for (const st of s.starts) surfIds.add(st.id);
    for (const wd of s.closes) surfIds.add(wd.id);
  }
  all.push({
    scenario: { city, persona, vibe },
    retrieval: { stageCounts: retrieval.stageCounts, totalScored: scored.length, categories: scored.reduce((a, s) => {
      a[s.venue.category] = (a[s.venue.category] ?? 0) + 1;
      return a;
    }, {}) },
