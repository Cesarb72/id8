...
function inferVibeTags(raw, category, inferredFields) {
  ...
  const vibes = new Set(base2[category]);
...
function inferLocalSignals(raw, signatureScore, inferredFields) {
  ...
  const base2 = clamp2(0.58 + signatureScore * 0.24, 0.45, 0.92);
  return {
    localFavoriteScore: Number(base2.toFixed(2)),
    neighborhoodPrideScore: Number((base2 + 0.04).toFixed(2)),
    repeatVisitorScore: Number((base2 - 0.02).toFixed(2))
  };
}
...
function parseHourFromTimeWindow(timeWindow) {
  ...
  const match2 = normalized.match(/(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/);
  if (!match2) {
    return void 0;
  }
  let hour = Number(match2[1]);
  const meridiem = match2[3];
...
function inferDriveMinutes(context, placeNeighborhood) {
  ...
  return Math.max(6, Math.min(22, base2 + rankPenalty * 2));
}
...
function buildLocation(venue) {
  ...
  const base2 = cityNeighborhoodAnchors[neighborhoodKey] ?? dynamicNeighborhoodAnchor ?? CITY_DEFAULT_ANCHORS[cityKey] ?? buildDynamicCityAnchor(cityKey) ?? DEFAULT_ANCHOR;
  const angleRadians = hashToUnit4(`${venue.id}:angle`) * Math.PI * 2;
  const radialScale = Math.sqrt(hashToUnit4(`${venue.id}:radius`));
  const jitterMeters = radialScale * base2.jitterRadiusM;
  ...
  const lngJitter = metersToLngDegrees2(jitterEastM, base2.lat);
  return {
    lat: base2.lat + latJitter,
    lng: base2.lng + lngJitter,
    normalizedNeighborhoodKey: neighborhoodKey,
    anchorKey: base2.key,
    jitterRadiusM: base2.jitterRadiusM
  };
}
...
function parseCityStateQuery(query) {
  const match2 = query.trim().match(/^([a-zA-Z][a-zA-Z .'-]{1,})\s*,\s*([a-zA-Z]{2})$/);
  if (!match2) {
    return void 0;
  }
  const city2 = toTitleCase2(match2[1].trim().replace(/\s+/g, " "));
  const stateCode = match2[2].toUpperCase();
  if (!city2 || !stateCode) {
    return void 0;
  }
...
function parseLatLngQuery(query) {
  const match2 = query.match(/(-?\d{1,2}\.\d+)\s*,\s*(-?\d{1,3}\.\d+)/);
  if (!match2) {
    return void 0;
  }
  const lat = Number(match2[1]);
  const lng = Number(match2[2]);
...
function computeBearingsLite(pocket) {
  ...
  const base2 = Math.max(1, width + height);
  const northSouthBias = toFixed3((height - width) / base2);
  const eastWestBias = toFixed3((width - height) / base2);
...
function contractMinCount(role, strictShapeEnabled) {
  const base2 = role === "peak" ? 3 : role === "cooldown" ? 2 : 1;
  return strictShapeEnabled ? base2 + 1 : base2;
}
...
function evaluatePreferredRoleAdmission(candidate, role, lensRole, crewPolicy, intent2) {
...
function pickRoleCandidates(scoredVenues, role, crewPolicy, lens2, intent2, roleContracts, strictShapeEnabled = false, contractConstraints, experienceContract) {
  const lensRole = roleToLensStop(role);
...
function buildRolePools(scoredVenues, crewPolicy, lens2, intent2, roleContracts, strictShapeEnabled = false, contractConstraints, experienceContract) {
...
function mergeShape(base2, patch) {
  return {
    preferredCategories: unique4([
      ...base2.preferredCategories ?? [],
      ...patch.preferredCategories ?? []
    ]),
    discouragedCategories: unique4([
      ...base2.discouragedCategories ?? [],
      ...patch.discouragedCategories ?? []
    ]),
    preferredTags: unique4([...base2.preferredTags ?? [], ...patch.preferredTags ?? []]),
    discouragedTags: unique4([...base2.discouragedTags ?? [], ...patch.discouragedTags ?? []]),
    energyPreference: unique4([
      ...base2.energyPreference ?? [],
...
function getBlendMode(vibe2, persona2) {
  if (!persona2) {
    return {
      blendMode: "vibe_only",
  ...
  if (persona2 === "romantic") {
    if (vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured" || vibe2 === "adventurous-outdoor") {
      return {
        blendMode: "aligned",
        compatibility: "reinforcing",
        resolutionSummary: vibe2 === "cultured" ? "Cultured supplies thoughtful setting while romantic requires an intimate highlight moment." : vibe2 === "adventurous-outdoor" ? "Outdoor exploration supplies the setting while romantic resolves the highlight into a scenic shared moment." : "Cozy pacing and romantic intent reinforce each other into a low-energy intimate moment.",
        priority: {
  ...
  if (persona2 === "friends") {
    if (vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured") {
      return {
        blendMode: "tension",
        compatibility: "tension",
  ...
  if (vibe2 === "lively" || vibe2 === "playful" || vibe2 === "adventurous-urban") {
    return {
      blendMode: "selective_energy",
      compatibility: "tension",
...
function buildRomanticContract(vibe2) {
  ...
  if (vibe2 === "lively" || vibe2 === "playful" || vibe2 === "adventurous-urban") {
    highlight = mergeShape(highlight, {
      preferredCategories: vibe2 === "playful" ? ["activity", "dessert"] : ["live_music", "activity", "restaurant"],
      preferredTags: vibe2 === "lively" ? ["listening", "stylish", "intimate"] : vibe2 === "adventurous-urban" ? ["walkable", "local", "design-forward"] : ["playful", "shared"],
    ...
    energyBandRemovals = vibe2 === "playful" ? ["high"] : [];
    windDownExpectation = {
      closeToBase: true,
      maxEnergy: "low"
    };
    highlightEnergyPreference = ["medium"];
  } else if (vibe2 === "cultured") {
    highlight = mergeShape(highlight, {
  ...
  } else if (vibe2 === "adventurous-outdoor") {
    highlight = mergeShape(highlight, {
...
function buildFriendsContract(vibe2) {
  const highlight = mergeShape(emptyShape(), {
    preferredCategories: vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured" ? ["activity", "live_music", "restaurant"] : ["activity", "live_music", "bar", "event"],
    preferredTags: ["social", "interactive"],
    discouragedCategories: vibe2 === "cozy" || vibe2 === "chill" ? ["park"] : [],
    energyPreference: vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured" ? ["medium"] : ["medium", "high"]
  });
  ...
  const resolvedContract = {
    primaryVibe: vibe2,
    persona: "friends",
    ...getBlendMode(vibe2, "friends"),
    toneOverride: vibe2 === "cozy" || vibe2 === "chill" ? void 0 : "electric",
    movementToleranceOverride: vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured" ? void 0 : "high",
    repetitionToleranceOverride: vibe2 === "cozy" || vibe2 === "chill" ? "medium" : void 0,
    wildcardAggressivenessMin: 0.58,
    energyBandAdditions: vibe2 === "cozy" || vibe2 === "chill" || vibe2 === "cultured" ? [] : ["high"],
    energyBandRemovals: [],
...
function buildFamilyContract(vibe2) {
  const highlight = mergeShape(emptyShape(), {
    preferredCategories: vibe2 === "lively" || vibe2 === "playful" || vibe2 === "adventurous-urban" ? ["museum", "activity", "event", "park"] : ["museum", "park", "activity", "event", "cafe"],
  ...
  const resolvedContract = {
    primaryVibe: vibe2,
    persona: "family",
    ...getBlendMode(vibe2, "family"),
    movementToleranceOverride: vibe2 === "lively" || vibe2 === "playful" || vibe2 === "adventurous-urban" ? void 0 : "low",
    movementToleranceCap: "medium",
    repetitionToleranceOverride: "low",
...
function buildVibeOnlyContract(vibe2) {
  return {
    resolvedContract: {
      primaryVibe: vibe2,
      ...getBlendMode(vibe2, void 0),
      energyBandAdditions: [],
      energyBandRemovals: [],
      preferredCategories: [],
      discouragedCategories: [],
...
function resolveHospitalityContract(intent2) {
  const persona2 = intent2.personaSource === "explicit" ? intent2.persona : void 0;
  if (persona2 === "romantic") {
...
function mergeRule(base2, patch) {
  return {
    ...base2,
    label: patch.label ?? base2.label,
    strength: patch.strength ? stronger(base2.strength, patch.strength) : base2.strength,
    requiredCategories: unique5([...base2.requiredCategories ?? [], ...patch.requiredCategories ?? []]),
    preferredCategories: unique5([...base2.preferredCategories ?? [], ...patch.preferredCategories ?? []]),
    discouragedCategories: unique5([...base2.discouragedCategories ?? [], ...patch.discouragedCategories ?? []]),
    requiredTags: unique5([...base2.requiredTags ?? [], ...patch.requiredTags ?? []]),
    preferredTags: unique5([...base2.preferredTags ?? [], ...patch.preferredTags ?? []]),
    discouragedTags: unique5([...base2.discouragedTags ?? [], ...patch.discouragedTags ?? []]),
    maxEnergyLevel: typeof patch.maxEnergyLevel === "number" ? typeof base2.maxEnergyLevel === "number" ? Math.min(base2.maxEnergyLevel, patch.maxEnergyLevel) : patch.maxEnergyLevel : base2.maxEnergyLevel
  };
}
...
function getRoleContract({
  ...
  const merged = {
    ...
    byRole: {
      start: mergeRule(mergeRule(base2.start, persona2.start ?? {}), pack.start ?? {}),
      highlight: mergeRule(mergeRule(base2.highlight, persona2.highlight ?? {}), pack.highlight ?? {}),
      surprise: mergeRule(mergeRule(base2.surprise, persona2.surprise ?? {}), pack.surprise ?? {}),
      windDown: mergeRule(mergeRule(base2.windDown, persona2.windDown ?? {}), pack.windDown ?? {})
    }
  };
...
function getIntentFit(input) {
  ...
  const vibe2 = (context?.vibe ?? "").toLowerCase();
  ...
  } else if (vibe2.includes("cozy") || vibe2.includes("chill") || vibe2.includes("calm")) {
    vibeFit = clamp14(
  ...
  } else if (vibe2.includes("cultured")) {
    vibeFit = clamp14(
  ...
  } else if (vibe2.includes("playful")) {
    vibeFit = clamp14(
  ...
  } else if (vibe2.includes("adventurous")) {
    vibeFit = clamp14(
  ...
  return toFixed12(
    clamp14(
      base2 * 0.2 + vibeFit * 0.43 + personaFit * 0.33 + getHappeningsSignal(happenings, "currentRelevance") * 0.02 + getHappeningsSignal(happenings, "hotspotStrength") * 0.02,
      0,
      1
    )
  );
...
function formatDistrictLabel(value) {
  return value.trim().replace(/[_-]+/g, " ").replace(/\b\w/g, (match2) => match2.toUpperCase());
}
...
function matchDistrictAlias(citySlug, value) {
  ...
  const match2 = getCityAliases(citySlug).find(
  ...
  return {
    districtId: `${citySlug}.${match2.idSuffix}`,
    districtLabel: match2.label
  };
}
...
function getSummaryLead(vibe2) {
  if (vibe2 === "lively" || vibe2 === "playful") {
    return "A lively, well-connected area";
  }
  if (vibe2 === "cozy" || vibe2 === "chill") {
...
function getVenueClusterId(venue) {
  ...
  for (const alias of cityAliases) {
    if (alias.matches.some((match2) => normalizedNeighborhood.includes(match2))) {
...
function getDistrictGroups(viableCandidates, rolePools, intent2) {
...
function recommendDistricts({
  ...
  const districtGroups = getDistrictGroups(viableCandidates, rolePools, intent2);
...
function getVibeProfile(vibe2) {
  return vibeProfiles[vibe2];
}
function normalizeTag(tag) {
  return tag.trim().toLowerCase();
}
function venueMatchesVibeTag(venue, vibe2) {
  const profile = getVibeProfile(vibe2);
...
function scoreVibeTagAffinity(venue, vibe2) {
  const profile = getVibeProfile(vibe2);
  ...
  const vibeTagFit = venueMatchesVibeTag(venue, vibe2) ? 1 : 0.18;
...
function getRoleShapeForVibe(vibe2, role) {
  const profile = getVibeProfile(vibe2);
  if (role === "start") {
...
function applyTasteModeToLens(lens2, tasteMode) {
  const nextLens = {
    ...lens2,
    tasteMode,
    energyBand: unique6([...lens2.energyBand, ...tasteMode.energyBand]),
    preferredCategories: unique6([...lens2.preferredCategories, ...tasteMode.favoredCategories]),
    discouragedCategories: unique6([
      ...lens2.discouragedCategories,
      ...tasteMode.discouragedCategories
    ]),
    preferredTags: unique6([...lens2.preferredTags, ...tasteMode.favoredTags]),
    discouragedTags: unique6([...lens2.discouragedTags, ...tasteMode.discouragedTags]),
...
function mergeStopShape(base2, patch) {
  return {
    ...
    preferredTags: unique7([...base2.preferredTags ?? [], ...patch.preferredTags ?? []]),
    discouragedTags: unique7([
      ...base2.discouragedTags ?? [],
      ...patch.discouragedTags ?? []
    ]),
...
function buildExperienceLens({
  ...
  const resolvedContractPackage = resolveHospitalityContract(intent2);
  const base2 = buildNeutralBaseline();
  const afterVibe = applyVibeShaping(base2, intent2);
...
function applyContextTonePressure(profile, venue, context) {
  ...
  const vibe2 = context?.vibe ?? void 0;
  if (!vibe2) {
    return { profile, signals: [] };
  }
  const keywords = buildKeywordCorpus(venue);
  const livelyLike = vibe2 === "lively" || vibe2 === "playful";
  const cozyLike = vibe2 === "cozy" || vibe2 === "chill";
  ...
  } else if (vibe2 === "adventurous-outdoor" || vibe2 === "adventurous-urban") {
...
function scoreRoleProfileFit(venue, role, vibe2) {
  const shape = getRoleShapeForVibe(vibe2, role);
  ...
  const generalVibeFit = scoreVibeTagAffinity(venue, vibe2);
  return clamp0122(
...
function computeVibeAuthority(venue, intent2, lens2, starterPack) {
  ...
  const adventureSplitLift = intent2.primaryAnchor === "adventurous-outdoor" ? clamp0122(adventureRead.outdoor) * 0.22 - clamp0122(adventureRead.urban) * 0.18 : intent2.primaryAnchor === "adventurous-urban" ? clamp0122(adventureRead.urban) * 0.22 - clamp0122(adventureRead.outdoor) * 0.18 : 0;
  const byRole = {
    start: clamp0122(
...
function scoreVenueFit(venue, intent2, crewPolicy, lens2, roleContracts, starterPack) {
  ...
  const hiddenGemScore = clamp0124(
    hiddenGemFit * (1 + crewPolicy.hiddenGemBias) + (intent2.prefersHiddenGems ? 0.03 : 0)
  );
  ...
  const cozyRomanticHighlightMode = crewPolicy.crew === "romantic" && intent2.primaryAnchor === "cozy";
  ...
  const cozyMusicPenalty = intent2.primaryAnchor === "cozy" && crewPolicy.crew === "romantic" && (venue.category === "live_music" || venue.category === "event") ? vibeAuthority.musicSupportSource === "pack" || vibeAuthority.musicSupportSource === "both" ? 0.04 : 0.2 : 0;
...
function scoreVenueCollection(venues, intent2, crewPolicy, lens2, roleContracts, starterPack) {
  const baseCandidates = venues.map(
    (venue) => scoreVenueFit(venue, intent2, crewPolicy, lens2, roleContracts, starterPack)
  );
  const momentCandidates = deriveMomentVenueRecords({
    intent: intent2,
    venuePool: venues
  }).map(({ moment, venue }) => {
    const candidate = scoreVenueFit(venue, intent2, crewPolicy, lens2, roleContracts, starterPack);
...
async function previewDistrictRecommendations(input, options = {}) {
  const intent2 = normalizeIntent({
    ...input,
    district: void 0
  });
  const lens2 = buildExperienceLens({
    intent: intent2,
    starterPack: options.starterPack,
    strictShape: options.strictShape
  });
  const crewPolicy = getCrewPolicy(intent2.crew);
  const roleContracts = getRoleContract({
    intent: intent2,
    starterPack: options.starterPack,
    strictShapeEnabled: options.strictShape
  });
  const retrieval2 = await retrieveVenues(intent2, lens2, {
...
function getHospitalityScenarioContract(input) {
  ...
  const vibe2 = normalizeVibe(input.vibe);
  if (!persona2 || !vibe2) {
    return null;
  }
  ...
  return HOSPITALITY_SCENARIO_CONTRACTS.find(
    (contract) => contract.city === "San Jose" && contract.persona === persona2 && contract.vibe === vibe2
  ) ?? null;
}
...
// tmp/cozy_authority_trace.ts
var city = "San Jose";
var persona = "romantic";
var vibe = "cozy";
var targets = ["La Foret", "La For\uFFFDt", "Hakone Gardens", "Japanese Friendship Garden", "Friendship Garden", "Hedley Club Lounge", "Willow Glen", "Willow Court Wine Bar"];
...
function inc(s, t) {
  if (t === "anchor") s.anchor++;
  else if (t === "supporting") s.supporting++;
  else if (t === "temporal") s.temporal++;
  else if (t === "discovery") s.discovery++;
  else s.community++;
}
function mvStats(ms) {
  const m = /* @__PURE__ */ new Map();
  for (const x of ms) {
    if (!x.venueId) continue;
    const s = m.get(x.venueId) ?? { maxStrength: 0, anchor: 0, supporting: 0, temporal: 0, discovery: 0, community: 0 };
    s.maxStrength = Math.max(s.maxStrength, x.strength);
    inc(s, x.momentType);
    m.set(x.venueId, s);
  }
  return m;
...
function mscore(m, e) {
  let s = m.strength * 0.72 + m.intentFit * 0.2 + m.timingRelevance * 0.08;
  if (e.exploration === "focused") {
    if (m.momentType === "anchor") s += 0.08;
    else if (m.momentType === "supporting") s += 0.04;
  } else {
    if (m.momentType === "discovery") s += 0.11;
    else if (m.momentType === "community") s += 0.09;
    else if (m.momentType === "temporal") s += 0.05;
    else if (m.momentType === "anchor") s -= 0.03;
  }
  if (e.discovery === "discover") {
    if (m.momentType === "discovery") s += 0.16;
    else if (m.momentType === "community") s += 0.12;
    else if (m.momentType === "anchor") s -= 0.05;
  } else if (m.momentType === "anchor") s += 0.1;
  else if (m.momentType === "supporting") s += 0.05;
  if (e.highlight === "standout") {
    if (m.momentType === "anchor") s += 0.14;
    else if (m.momentType === "temporal") s += 0.06;
  } else if (m.momentType === "anchor") s -= 0.1;
  else if (m.momentType === "supporting") s += 0.07;
  return s;
}
function reshapeMom(ms, e) {
  const c = [...ms.primary, ...ms.secondary];
  if (!c.length) return { primary: [], secondary: [] };
  const r = c.slice().sort((a, b) => {
    const d = mscore(b, e) - mscore(a, e);
    if (d !== 0) return d;
    if (b.strength !== a.strength) return b.strength - a.strength;
    return a.id.localeCompare(b.id);
  });
  const pc = Math.max(1, ms.primary.length), sc2 = ms.secondary.length;
  return { primary: r.slice(0, pc), secondary: r.slice(pc, pc + sc2) };
}
function cscore(c, role, stats, e, sh) {
  const s = stats.get(c.venueId);
  const ha = (s?.anchor ?? 0) > 0, hs = (s?.supporting ?? 0) > 0, hd = (s?.discovery ?? 0) > 0, hc = (s?.community ?? 0) > 0, ht = (s?.temporal ?? 0) > 0;
  let v = c.score;
  if (e.exploration === "focused") {
    v += c.score * 0.06;
    if (ha) v += 0.03;
  ...
  const hw = role === "highlight" ? 1 : 0.45;
  if (e.highlight === "standout") {
    v += hw * (ha ? 0.12 : 0);
    v += hw * (ht ? 0.05 : 0);
    v += hw * c.score * 0.04;
    if (role === "highlight" && c.venueId === sh) v += 0.05;
  } else {
    v -= hw * (ha ? 0.08 : 0);
    v += hw * (hs ? 0.06 : 0);
    v += hw * (hd ? 0.04 : 0);
  ...
  v += (s?.maxStrength ?? 0) * 0.03;
  return clamp17(v);
}
function rankRole(cs, role, stats, e, sh) {
  return cs.slice().sort((a, b) => {
    const d = cscore(b, role, stats, e, sh) - cscore(a, role, stats, e, sh);
    if (d !== 0) return d;
    if (b.score !== a.score) return b.score - a.score;
    return a.venueName.localeCompare(b.venueName);
  });
}
function htier(c, orig) {
  if (!c) return void 0;
  if (orig?.venueId === c.venueId) return orig.tier;
  if (c.score >= 0.72) return 1;
  if (c.score >= 0.56) return 2;
  return 3;
}
function shiftHP(v, dir) {
  if (dir === "up") {
...
function shiftDB(v, dir) {
  if (dir === "toward_discover") {
    if (v === "familiar") return "balanced";
    if (v === "balanced") return "novel";
    return "novel";
  }
...
function applyEcs(a, e) {
  const nm = reshapeMom(a.moments, e);
  const stats = mvStats([...nm.primary, ...nm.secondary]);
  const sh = a.anchors.strongestHighlight?.venueId;
  const st = rankRole(a.ingredients.startCandidates, "start", stats, e, sh);
  const hi = rankRole(a.ingredients.highlightCandidates, "highlight", stats, e, sh);
  const wd = rankRole(a.ingredients.windDownCandidates, "windDown", stats, e, sh);
  const top = hi[0], tier = htier(top, a.anchors.strongestHighlight);
  const mp = e.exploration === "exploratory" ? a.summary.movementProfile === "tight" ? "moderate" : a.summary.movementProfile : a.summary.movementProfile === "spread" ? "moderate" : a.summary.movementProfile;
  return { ...a, summary: { ...a.summary, movementProfile: mp, highlightPotential: e.highlight === "standout" ? shiftHP(a.summary.highlightPotential, "up") : shiftHP(a.summary.highlightPotential, "down"), discoveryBalance: e.discovery === "discover" ? shiftDB(a.summary.discoveryBalance, "toward_discover") : shiftDB(a.summary.discoveryBalance, "toward_reliable") }, ingredients: { startCandidates: st, highlightCandidates: hi, windDownCandidates: wd }, anchors: { strongestStart: st[0], strongestHighlight: top && typeof tier === "number" ? { ...top, tier } : void 0, strongestWindDown: wd[0] }, moments: nm };
}
function aggIds(a) {
  ...
  if (a.anchors.strongestStart?.venueId) s.add(a.anchors.strongestStart.venueId);
  if (a.anchors.strongestHighlight?.venueId) s.add(a.anchors.strongestHighlight.venueId);
  if (a.anchors.strongestWindDown?.venueId) s.add(a.anchors.strongestWindDown.venueId);
...
function buildDistrictCandidates(ranked, recs2) {
  const entries = recs2.filter((r) => Boolean(r.tasteAggregation)).map((r) => ({ r, keys: [.../* @__PURE__ */ new Set([...dkeys(r.districtId), ...dkeys(r.label)])], tokens: [.../* @__PURE__ */ new Set([...dtokens(r.label), ...dtokens(r.districtId)])] }));
  ...
  const cands2 = ranked.slice(0, 6).map((entry) => {
    ...
    return { entry, p, rec, join };
  });
  const fq = entries.map((e) => e.r).filter((r) => !used.has(r.districtId)).sort((l, r) => r.score - l.score || l.label.localeCompare(r.label));
  for (const c of cands2) {
    ...
    if (f) {
      c.rec = f;
      c.join = "fallback";
      used.add(f.districtId);
    }
  }
  return cands2;
}
var intentInput = { persona, primaryVibe: vibe, city, distanceMode: "nearby", budget: "balanced" };
var intent = normalizeIntent(intentInput);
var lens = buildExperienceLens({ intent });
var retrieval = await retrieveVenues(intent, lens);
var scored = scoreVenueCollection(retrieval.venues, intent, getCrewPolicy(intent.crew), lens, getRoleContract({ intent }));
var rawIds = new Set(scored.map((s) => s.venue.id));
var districtResult = await buildDistrictOpportunityProfiles({ locationQuery: city, includeDebug: true });
var recs = (await previewDistrictRecommendations(intentInput)).recommendedDistricts;
var cands = buildDistrictCandidates(districtResult.ranked, recs);
var sc = getHospitalityScenarioContract({ city, persona, vibe });
var ec = buildExperienceContractFromScenarioContract(sc);
var base = /* @__PURE__ */ new Map();
var afterS = /* @__PURE__ */ new Map();
var afterE = /* @__PURE__ */ new Map();
var afterX = /* @__PURE__ */ new Map();
for (const c of cands) {
  const a = c.rec?.tasteAggregation;
  if (!a) continue;
  base.set(c.p.pocketId, a);
  const s = applyScenarioContractToAggregation(a, sc);
  afterS.set(c.p.pocketId, s);
  const e = applyExperienceContractToAggregation(s, ec);
  afterE.set(c.p.pocketId, e);
  afterX.set(c.p.pocketId, applyEcs(e, ecs));
}
var idsB = /* @__PURE__ */ new Set();
var idsS = /* @__PURE__ */ new Set();
var idsE = /* @__PURE__ */ new Set();
var idsX = /* @__PURE__ */ new Set();
...
var survivors = [...afterX.entries()].map(([p, a]) => ({ p, anchor: a.anchors.strongestHighlight?.venueName ?? null, anchorId: a.anchors.strongestHighlight?.venueId ?? null, start: a.ingredients.startCandidates[0]?.venueName ?? null, windDown: a.ingredients.windDownCandidates[0]?.venueName ?? null, score: a.anchors.strongestHighlight?.score ?? 0 })).sort((l, r) => r.score - l.score).slice(0, 4);
var surfacedIds = /* @__PURE__ */ new Set();
for (const s of survivors) {
  if (s.anchorId) surfacedIds.add(s.anchorId);
}
var namesById = new Map(scored.map((s) => [s.venue.id, s.venue.name]));
var targetRows = targets.map((t) => {
  const ids = [...rawIds].filter((id) => match(namesById.get(id), t));
  const inRaw = ids.length > 0;
  const inBase = ids.some((id) => idsB.has(id));
  const inScenario = ids.some((id) => idsS.has(id));
  const inExperience = ids.some((id) => idsE.has(id));
  const inEcs = ids.some((id) => idsX.has(id));
  const inSurfaced = ids.some((id) => surfacedIds.has(id));
  const surfacedAsAnchor = survivors.some((s) => match(s.anchor, t));
  let droppedWhy = null;
  if (!inRaw) droppedWhy = "not_in_retrieval_field";
  else if (!inBase) droppedWhy = "not_in_aggregation_top_sets";
  else if (!inScenario || !inExperience || !inEcs) droppedWhy = "demoted_in_shaping";
  else if (!inSurfaced && !surfacedAsAnchor) droppedWhy = "lost_in_final_surfacing";
  else if (inSurfaced && !surfacedAsAnchor) droppedWhy = "support_only_not_anchor";
  return { target: t, matchedVenueNames: ids.map((id) => namesById.get(id)).slice(0, 8), inRaw, inBase, inScenario, inExperience, inEcs, inSurfaced, surfacedAsAnchor, droppedWhy };
});
