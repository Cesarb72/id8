...
function toFixed(value) {
  return Number(clamp2(value, 0, 1).toFixed(3));
}
function getHappeningsSignal(happenings, key) {
  return clamp2(happenings?.[key] ?? 0, 0, 1);
}
...
function getHoursAvailabilityScore(hoursStatus, hoursConfidence) {
  ...
  const confidence = clamp2(hoursConfidence ?? 0.5, 0, 1);
  if (status === "open") {
...
function detectDiscoveryMoment(input, timingRelevance, intentFit) {
  const signals = input.tasteSignals;
  const happenings = input.happenings;
  const discoveryStrength = clamp2(
...
function detectCommunityMoment(input, timingRelevance, intentFit) {
  ...
  const communityStrength = clamp2(
...
function getFitWeight(input) {
  return clamp3((input.fitScore ?? 0.5) * 0.5 + 0.5, 0.4, 1);
}
function getHappeningsMetric(input, key) {
  return clamp3(input.happenings?.[key] ?? 0, 0, 1);
}
...
function deriveReadableDistrictName(district, cityContext) {
  ...
  const normalizedCityKey = normalizeCityKey(city2);
...
function resolveDistrictAnchor({
  ...
  if (promotedDistrict) {
    return {
      ...
      confidence: roundToHundredths(clamp4(0.75 + strongestCluster.clusterShare * 0.15, 0.75, 0.9)),
  ...
  if (strongestCluster.clusterShare >= 0.5) {
    ...
    return {
      ...
      confidence: roundToHundredths(clamp4(0.65 + strongestCluster.clusterShare * 0.2, 0.65, 0.85)),
      reason: "inferred from strongest cluster"
    };
  }
  return createCityFallback(trimmedCity, citySlug);
...
function getDensityScore(candidateCount, maxCandidateCount) {
  ...
  return clamp014(candidateCount / maxCandidateCount);
}
function getAdjustedDensityScore(rawDensity) {
  return clamp014(Math.pow(rawDensity, 0.72));
}
function getRelevanceScore(candidates) {
  ...
  return clamp014(total / candidates.length);
}
function getDiversityScore(group) {
  ...
  const categoryCoverage = clamp014(group.categories.size / 4);
  return clamp014(coreRoleCoverage * 0.65 + categoryCoverage * 0.35);
}
...
function getDominanceScore(values) {
  ...
  return clamp014(maxCount / values.length);
}
function getCoherenceScore(group) {
  ...
  return clamp014(neighborhoodDominance * 0.58 + experienceFamilyDominance * 0.42);
}
function getSequenceSupportScore(group) {
  ...
  return clamp014(sequenceMean * 0.68 + coverage * 0.22 + sequenceBalance * 0.1);
}
function getTasteAverages(candidates) {
  ...
  return {
    intimacy: clamp014(totals.intimacy / candidates.length),
    linger: clamp014(totals.linger / candidates.length),
    conversation: clamp014(totals.conversation / candidates.length),
    calm: clamp014(totals.calm / candidates.length),
    social: clamp014(totals.social / candidates.length),
    culturalDepth: clamp014(totals.culturalDepth / candidates.length),
    discovery: clamp014(totals.discovery / candidates.length),
    experiential: clamp014(totals.experiential / candidates.length),
    outdoor: clamp014(totals.outdoor / candidates.length),
    playful: clamp014(totals.playful / candidates.length)
  };
}
function getCategoryMix(candidates) {
  ...
  return {
    restaurant: clamp014(totals.restaurant / candidates.length),
    bar: clamp014(totals.bar / candidates.length),
    cafe: clamp014(totals.cafe / candidates.length),
    dessert: clamp014(totals.dessert / candidates.length),
    liveMusic: clamp014(totals.liveMusic / candidates.length),
    activity: clamp014(totals.activity / candidates.length),
    park: clamp014(totals.park / candidates.length),
    museum: clamp014(totals.museum / candidates.length),
    event: clamp014(totals.event / candidates.length)
  };
}
...
function getWalkabilitySignal(candidates) {
  ...
  return clamp014(matched / candidates.length);
}
...
function getRoleStrengths(group) {
  ...
  const chaosPenalty = clamp014(Math.max(0, social - 0.58) * 0.9 + Math.max(0, energy - 0.62) * 0.8);
  const overCalmPenalty = clamp014(Math.max(0, taste.calm - 0.72) * 0.9);
  ...
  const startCategorySupport = clamp014(
  ...
  const highlightCategorySupport = clamp014(
  ...
  const winddownCategorySupport = clamp014(
  ...
  const highlightStrength = clamp014(
  ...
  const winddownStrength = clamp014(
...
function getAffinityScores(group, intent, sequenceSupport, signature) {
  ...
  return {
    affinity: clamp014(vibeAffinity * 0.58 + crewAffinity * 0.42),
    vibeAffinity: clamp014(vibeAffinity),
    crewAffinity: clamp014(crewAffinity)
  };
}
function getAlignmentLift(density, affinity, signature, coherence, relevance, diversity) {
  const contextStrength = clamp014(relevance * 0.55 + diversity * 0.45);
  return clamp014(
...
function recommendDistricts({
  ...
  return districtGroups.map((group) => {
    ...
    const score = roundToHundredths2(
      clamp014(baseScore + alignmentLift - breadthPenalty - sparsityPenalty)
    );
    const signals = {
      density,
      relevance,
...
function applyQualityGate(venue) {
  ...
  const effectiveGenericScore = clamp016(venue.signature.genericScore - liveFairness.genericRelief);
  const effectiveSourceConfidence = clamp016(
  ...
  const completenessScore = clamp016(1 - completenessPenalty);
  const qualityScore = clamp016(
...
function inferHoursPressure({
  ...
  const softenedConfidence = clamp017(
...
function inferSocialDensity(raw, category, energyLevel, inferredFields) {
  ...
  return clamp5(Math.round(socialDensity), 1, 5);
}
...
function inferSignatureSignals(raw, category) {
  ...
  return {
    chainLike,
    genericScore: clamp5(Number(genericScore.toFixed(2)), 0, 1),
    signatureScore: clamp5(Number(signatureScore.toFixed(2)), 0, 1)
  };
}
...
function inferLocalSignals(raw, signatureScore, inferredFields) {
  ...
  const base = clamp5(0.58 + signatureScore * 0.24, 0.45, 0.92);
  return {
...
function inferVenueSignals({
  ...
  const uniquenessScore = inferScore(
    raw.uniquenessScore,
    clamp5(0.52 + signature.signatureScore * 0.38, 0.4, 0.95),
    inferredFields,
    "uniquenessScore"
  );
  const distinctivenessScore = inferScore(
    raw.distinctivenessScore,
    clamp5(0.5 + signature.signatureScore * 0.42, 0.38, 0.96),
    inferredFields,
    "distinctivenessScore"
  );
  const underexposureScore = inferScore(
    raw.underexposureScore,
    clamp5(0.42 + (raw.isHiddenGem ? 0.22 : 0) + (signature.signatureScore - signature.genericScore) * 0.12, 0.25, 0.92),
    inferredFields,
    "underexposureScore"
  );
  const shareabilityScore = inferScore(
    raw.shareabilityScore,
    clamp5(0.48 + socialDensity * 0.06 + signature.signatureScore * 0.12, 0.35, 0.94),
    inferredFields,
    "shareabilityScore"
  );
...
function normalizeRawPlace(raw, options = {}) {
  ...
  const completenessScore = clamp019(1 - missingFields.length * 0.1);
...
function normalizeRawEvent(raw, options = {}) {
  ...
  const completenessScore = clamp0110(1 - missingFields.length * 0.11);
  const sourceConfidence = clamp0110(
...
function getCityCenter(city2) {
  const cityKey = normalizeCity2(city2);
...
function dedupeByPlaceId(venues) {
  ...
  for (const venue of venues) {
    const key = venue.source.providerRecordId ?? `${normalizeCity2(venue.city)}|${venue.name.trim().toLowerCase()}|${venue.category}`;
...
function getCityCenter2(city2) {
  const key = normalizeCity3(city2);
...
function dedupeVenues(venues) {
  const bestByKey = /* @__PURE__ */ new Map();
  for (const venue of venues) {
    const key = venue.source.providerRecordId ?? `${normalizeCity3(venue.city)}|${slugify(venue.name)}|${slugify(venue.neighborhood)}|${venue.category}`;
    const existing = bestByKey.get(key);
...
function summarizeGeoSpread(venues) {
  ...
  const bucketComponent = clamp0111((bucketCount - 1) / 4);
  const balanceComponent = clamp0111(1 - dominantAreaShare);
...
function projectNeighborhoodByGeoArea(venues, city2) {
  ...
  return venues.map((venue) => {
    ...
    return {
      ...venue,
      neighborhood: `${toTitleCase2(normalizeCity3(city2) || city2)} ${areaLabel} District`
    };
  });
}
...
function buildPortableBootstrapVenues(city2) {
  const cityLabel = toTitleCase2(normalizeCity3(city2) || city2);
...
async function fetchHybridPortableVenues(city2) {
  const normalizedCity = toTitleCase2(normalizeCity3(city2) || city2);
  if (!normalizedCity) {
    return {
      venues: [],
      diagnostics: {
...
function isSanJoseCity(value) {
  const city2 = normalizeCity4(value);
...
function reshapeRoleCandidates2(candidates, role, contract, statsByVenueId) {
  return candidates.map((candidate) => ({
    ...candidate,
    score: toFixed4(scoreRoleCandidate3(candidate, role, contract, statsByVenueId))
  })).sort((left, right) => right.score - left.score || left.venueName.localeCompare(right.venueName));
}
...
function getHappeningsAuthority(happenings) {
...
// tmp/step2_authority_probe.ts
var ecs = {
  exploration: "focused",
  discovery: "reliable",
  highlight: "standout"
};
function clampScore4(value) {
  return Math.max(0, Math.min(1, value));
}
function toConciseRoleReason(role, rawReason, fallbackReason) {
...
function toCityOpportunityStopOptions(candidates, role) {
...
function getCityOpportunityHappenings(aggregation) {
  ...
  return [...aggregation.moments.primary, ...aggregation.moments.secondary].filter((moment) => moment.momentType !== "anchor" && moment.strength >= 0.6).sort((left, right) => {
    ...
    return left.id.localeCompare(right.id);
  }).filter((moment, index, all) => all.findIndex((entry) => entry.title === moment.title) === index).slice(0, 2).map((moment) => ({
    name: moment.title,
...
function shiftHighlightPotential2(value, direction) {
  if (direction === "up") {
...
function shiftDiscoveryBalance(value, direction) {
...
function incrementMomentTypeStat(stats, momentType) {
  if (momentType === "anchor") {
    stats.anchor += 1;
    return;
  }
...
function buildMomentVenueStats(moments) {
  const statsByVenue = /* @__PURE__ */ new Map();
  moments.forEach((moment) => {
...
function getMomentEcsScore(moment) {
...
function reshapeMoments3(moments) {
  const combined = [...moments.primary, ...moments.secondary];
...
function getRoleCandidateEcsScore(candidate, role, statsByVenue, strongestHighlightVenueId) {
  ...
  return clampScore4(score);
}
function reshapeRoleCandidates3(candidates, role, statsByVenue, strongestHighlightVenueId) {
...
function applyExplorationControlsToAggregation(aggregation) {
  const nextMoments = reshapeMoments3(aggregation.moments);
  ...
  const startCandidates = reshapeRoleCandidates3(aggregation.ingredients.startCandidates, "start", momentStatsByVenue, strongestHighlightVenueId);
  const highlightCandidates = reshapeRoleCandidates3(aggregation.ingredients.highlightCandidates, "highlight", momentStatsByVenue, strongestHighlightVenueId);
  const windDownCandidates = reshapeRoleCandidates3(aggregation.ingredients.windDownCandidates, "windDown", momentStatsByVenue, strongestHighlightVenueId);
  const nextMovementProfile = aggregation.summary.movementProfile === "spread" ? "moderate" : aggregation.summary.movementProfile;
  const nextHighlightPotential = shiftHighlightPotential2(aggregation.summary.highlightPotential, "up");
...
function vibeAuthorityLift(vibe, signals) {
  if (vibe === "lively") {
    return signals.nightlifeConviction * 0.16 + signals.majorEventConviction * 0.12 + signals.happeningAuthority * 0.06;
  }
  if (vibe === "cultured") {
    return signals.culturalConviction * 0.18 + signals.discoveryConviction * 0.12 + signals.majorEventConviction * 0.08;
  }
  return signals.hiddenGemConviction * 0.16 + signals.windDownConviction * 0.12 + signals.discoveryConviction * 0.07;
}
var city = "San Jose";
...
for (const vibe of vibes) {
  ...
  const rows = preview.recommendedDistricts.filter((d) => Boolean(d.tasteAggregation)).map((district) => {
    ...
    const score = (shaped.anchors.strongestHighlight?.score ?? 0.44) * 0.32 + authority.overallAuthority * 0.26 + authority.anchorConviction * 0.14 + authority.startConviction * 0.09 + authority.windDownConviction * 0.09 + vibeAuthorityLift(vibe, authority) + (happenings[0]?.strength ?? 0) * 0.04;
    return {
