...
function formatReasonBuckets(reasons) {
  const entries2 = Object.entries(reasons).filter(([, count]) => count > 0).sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0])).slice(0, 3).map(([reason, count]) => `${reason} (${count})`);
  return entries2.length > 0 ? entries2.join(", ") : "none";
}
...
function isOpenDuringWindow(periods, signal2) {
  if (!periods || periods.length === 0) {
    return void 0;
  }
  const targetMinute = toWeeklyMinute(signal2.day, signal2.hour, signal2.minute);
...
function inferLikelyOpenFromCategory(raw, category, signal2) {
  const notes = [];
  const placeSignals = [
    ...raw.placeTypes ?? [],
    ...raw.sourceTypes ?? [],
  ...
  if (category === "bar") {
    ...
    return {
      likelyOpen: true,
      confidence: signal2.phase === "late-night" ? 0.66 : 0.6,
  ...
  notes.push(signal2.phase === "evening" ? "restaurant-hour heuristic favors dinner windows" : "restaurant-hour heuristic supports daytime service");
  return {
    likelyOpen: true,
    confidence: signal2.phase === "evening" ? 0.58 : 0.54,
    notes
  };
}
...
function summarizeGeoSpread(venues) {
  ...
  for (const venue of venues) {
    ...
    const entries2 = bucketMap.get(key) ?? [];
    entries2.push(venue);
    bucketMap.set(key, entries2);
  }
...
function selectGeoBalancedVenues(venues, maxCount) {
  ...
  while (selected.length < maxCount) {
    ...
    for (const bucketKey of bucketOrder) {
      const entries2 = bucketMap.get(bucketKey);
      if (!entries2 || entries2.length === 0) {
        continue;
      }
      const nextVenue = entries2.shift();
      if (!nextVenue) {
        continue;
      }
...
function getDominantCategory(pocket) {
  const entries2 = Object.entries(pocket.categoryCounts);
  if (entries2.length === 0) {
    return { category: "mixed", share: 0 };
  }
  const [category, count] = entries2.sort((left, right) => {
    if (right[1] !== left[1]) {
      return right[1] - left[1];
    }
    return left[0].localeCompare(right[0]);
...
function evaluateSignalThresholds(signals, thresholds) {
  ...
  const entries2 = Object.entries(thresholds);
  let passCount = 0;
  let cumulativeScore = 0;
  for (const [key, threshold] of entries2) {
  ...
  return {
    pass: passCount === entries2.length,
    score: entries2.length > 0 ? toFixed12(cumulativeScore / entries2.length) : 0.5,
...
// src/domain/bearings/buildGreatStopAdmissibilitySignal.ts
...
function buildGreatStopAdmissibilitySignal(evaluation) {
  if (!evaluation) {
    return void 0;
  }
  ...
  const severeFailureCount = severeCriteriaCounts.real + severeCriteriaCounts.place_right + severeCriteriaCounts.moment_right;
  const failedStopCount = evaluation.failedStops.length;
  const totalStopCount = evaluation.stopEvaluations.length;
  const riskTier = severeFailureCount >= 2 || severeCriteriaCounts.real > 0 ? "severe" : failedStopCount > 0 ? "warning" : "none";
  const suppressionRecommended = severeCriteriaCounts.real > 0 || severeFailureCount >= 3 || severeFailureCount >= 2 && failedStopCount >= 2;
  const degradedConfidencePenalty = toFixed13(
...
// tmp/gate_reason_probe.ts
var district = await buildDistrictOpportunityProfiles({ locationQuery: "San Jose", includeDebug: true });
var canonical = buildCanonicalInterpretationBundle({ persona: "friends", vibe: "lively", city: "San Jose" });
var signal = buildGreatStopAdmissibilitySignal({
...
var entries = gate.contractAwareRanking.ranked.slice(0, 4).map((entry) => ({
  pocketId: entry.profile.pocketId,
