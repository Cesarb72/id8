...
// tmp/step2_experience_alignment_probe.ts
...
for (const vibe of vibes) {
  ...
  const rows = result.recommendedDistricts.filter((d) => Boolean(d.tasteAggregation)).map((d) => {
  ...
  }).sort((a, b) => b.align - a.align).slice(0, 4);
  console.log("\n===ALIGN===", vibe);
