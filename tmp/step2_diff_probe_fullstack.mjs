...
// tmp/step2_diff_probe_fullstack.ts
var DEFAULT_ECS_STATE = {
  exploration: "focused",
  discovery: "reliable",
  highlight: "standout"
};
...
function getMomentEcsScore(moment, ecs) {
...
function reshapeMoments3(moments, ecs) {
  const combined = [...moments.primary, ...moments.secondary];
...
function getRoleCandidateEcsScore(candidate, role, statsByVenue, ecs, strongestHighlightVenueId) {
...
function reshapeRoleCandidates3(candidates, role, statsByVenue, ecs, strongestHighlightVenueId) {
  return candidates.slice().sort((left, right) => {
    const scoreDiff = getRoleCandidateEcsScore(right, role, statsByVenue, ecs, strongestHighlightVenueId) - getRoleCandidateEcsScore(left, role, statsByVenue, ecs, strongestHighlightVenueId);
    if (scoreDiff !== 0) return scoreDiff;
    if (right.score !== left.score) return right.score - left.score;
    return left.venueName.localeCompare(right.venueName);
  });
}
function inferHighlightTier4(candidate, originalStrongestHighlight) {
...
function applyExplorationControlsToAggregation(aggregation, ecs) {
  const nextMoments = reshapeMoments3(aggregation.moments, ecs);
  ...
  const startCandidates = reshapeRoleCandidates3(
    aggregation.ingredients.startCandidates,
    "start",
    momentStatsByVenue,
    ecs,
  ...
  const highlightCandidates = reshapeRoleCandidates3(
    aggregation.ingredients.highlightCandidates,
    "highlight",
    momentStatsByVenue,
    ecs,
  ...
  const windDownCandidates = reshapeRoleCandidates3(
    aggregation.ingredients.windDownCandidates,
    "windDown",
    momentStatsByVenue,
    ecs,
  ...
  const strongestHighlight = highlightCandidates[0];
  const strongestHighlightTier = inferHighlightTier4(strongestHighlight, aggregation.anchors.strongestHighlight);
  const nextMovementProfile = ecs.exploration === "exploratory" ? aggregation.summary.movementProfile === "tight" ? "moderate" : aggregation.summary.movementProfile : aggregation.summary.movementProfile === "spread" ? "moderate" : aggregation.summary.movementProfile;
  const nextHighlightPotential = ecs.highlight === "standout" ? shiftHighlightPotential2(aggregation.summary.highlightPotential, "up") : shiftHighlightPotential2(aggregation.summary.highlightPotential, "down");
  const nextDiscoveryBalance = ecs.discovery === "discover" ? shiftDiscoveryBalance(aggregation.summary.discoveryBalance, "toward_discover") : shiftDiscoveryBalance(aggregation.summary.discoveryBalance, "toward_reliable");
  return {
    ...aggregation,
    summary: {
      ...aggregation.summary,
...
for (const vibe of vibes) {
  ...
  const cards = result.recommendedDistricts.filter((district) => Boolean(district.tasteAggregation)).map((district) => {
