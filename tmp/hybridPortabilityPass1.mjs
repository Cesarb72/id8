...
function getRejectionReasonSummary(reasons) {
  ...
  const classificationReject = reasons.find(
    (reason) => reason.includes("Rejected pocket:")
  );
...
function buildDistrictDebugTrace(input) {
  const rankedByPocketId = new Map(
    input.ranked.map((entry) => [entry.profile.pocketId, entry])
  );
...
function rankAndSelectPockets(profiles, selectLimit = DEFAULT_SELECT_LIMIT) {
  ...
  return {
    ranked,
    selected: ranked.slice(0, Math.max(1, selectLimit))
  };
}
...
function computeDistrictScores(fieldSignals, viabilityClass) {
  const normalizedEntityCount = clamp5(fieldSignals.entityCount / 10, 0, 1);
  ...
  const totalScore = clamp5(fieldScore + viabilityBonus, 0, 1);
  return {
...
function computeFieldSignals(pocket) {
  return {
    ...
    density: toFixed5(clamp6(pocket.viability.signals.densityScore, 0, 1)),
    walkability: toFixed5(clamp6(pocket.viability.signals.walkabilityScore, 0, 1)),
    viability: toFixed5(clamp6(pocket.viability.score, 0, 1))
  };
}
...
function toFixed6(value) {
  return Number(clamp7(value, 0, 1).toFixed(3));
}
...
function normalizeMix(rawMix) {
  ...
  return {
    drinks: toFixed6(rawMix.drinks / total),
    dining: toFixed6(rawMix.dining / total),
    culture: toFixed6(rawMix.culture / total),
    cafe: toFixed6(rawMix.cafe / total),
    activity: toFixed6(rawMix.activity / total)
  };
}
...
function computeViabilitySignals(pocket, thresholds) {
  ...
  const densityScore = clamp8(pocket.geometry.densityEntitiesPerKm2 / DENSITY_SCORE_BASELINE, 0, 1);
  const walkabilityScore = clamp8(1 - pocket.geometry.avgDistanceFromCentroidM / 260, 0, 1) * 0.75 + clamp8(1 - pocket.geometry.maxPairwiseDistanceM / 950, 0, 1) * 0.25;
  const compactnessScore = clamp8(
    1 - clamp8((pocket.geometry.elongationRatio - 1) / 3, 0, 1),
    0,
    1
  );
  ...
  return {
    entityCount,
    categoryDiversity: toFixed8(pocket.laneDiversityScore),
    densityScore: toFixed8(densityScore),
    walkabilityScore: toFixed8(walkabilityScore),
    compactnessScore: toFixed8(compactnessScore),
    radiusPenalty: toFixed8(radiusPenalty)
  };
}
function computeViabilityScore(signals) {
  const entityCountScore = clamp8(signals.entityCount / 9, 0, 1);
  ...
  return toFixed8(clamp8(score, 0, 1));
}
...
function computeHybridLiveLift(venue) {
  ...
  const dedupePriorityScore = clamp9(
...
function pickRoleCandidates(scoredVenues, role, crewPolicy, lens, intent, roleContracts, strictShapeEnabled = false) {
  const lensRole = roleToLensStop(role);
...
function buildRolePools(scoredVenues, crewPolicy, lens, intent, roleContracts, strictShapeEnabled = false) {
...
function deriveReadableDistrictName(district, cityContext) {
  ...
  const normalizedCityKey = normalizeCityKey2(city);
...
function resolveDistrictAnchor({
  ...
  if (promotedDistrict) {
    return {
      ...
      confidence: roundToHundredths(clamp10(0.75 + strongestCluster.clusterShare * 0.15, 0.75, 0.9)),
  ...
  if (strongestCluster.clusterShare >= 0.5) {
    ...
    return {
      ...
      confidence: roundToHundredths(clamp10(0.65 + strongestCluster.clusterShare * 0.2, 0.65, 0.85)),
      reason: "inferred from strongest cluster"
    };
  }
  return createCityFallback(trimmedCity, citySlug);
...
function getRelevanceScore(candidates) {
  ...
  return clamp019(total / candidates.length);
}
...
function getTasteAverages(candidates) {
  ...
  return {
    intimacy: clamp019(totals.intimacy / candidates.length),
    linger: clamp019(totals.linger / candidates.length),
    conversation: clamp019(totals.conversation / candidates.length),
    calm: clamp019(totals.calm / candidates.length),
    social: clamp019(totals.social / candidates.length),
    culturalDepth: clamp019(totals.culturalDepth / candidates.length),
    discovery: clamp019(totals.discovery / candidates.length),
    experiential: clamp019(totals.experiential / candidates.length),
    outdoor: clamp019(totals.outdoor / candidates.length),
    playful: clamp019(totals.playful / candidates.length)
  };
}
function getCategoryMix(candidates) {
  ...
  return {
    restaurant: clamp019(totals.restaurant / candidates.length),
    bar: clamp019(totals.bar / candidates.length),
    cafe: clamp019(totals.cafe / candidates.length),
    dessert: clamp019(totals.dessert / candidates.length),
    liveMusic: clamp019(totals.liveMusic / candidates.length),
    activity: clamp019(totals.activity / candidates.length),
    park: clamp019(totals.park / candidates.length),
    museum: clamp019(totals.museum / candidates.length),
    event: clamp019(totals.event / candidates.length)
  };
}
...
function getWalkabilitySignal(candidates) {
  ...
  return clamp019(matched / candidates.length);
}
...
function interpretVenueTaste(venue, context) {
  ...
  const profile = mergeProfile(venue.seedCalibratedProfile, inferred.profile);
...
function deriveRoleSuitability(venue, profile, noveltyWeight, calibration, experienceStrengths, momentPotential, experienceArchetypes, momentEnrichment, hyperlocalActivation) {
  ...
  const start = clamp0120(
    profile.conversationFriendliness * 0.28 + profile.socialDensity * 0.2 + profile.energy * 0.16 + profile.lingerFactor * 0.12 + calibration.categorySpecificity * 0.06 + calibration.personalityStrength * 0.04 + experienceStrengths.outdoor * 0.06 + experienceStrengths.interactive * 0.03 + hoursConfidence * 0.12 + startHoursSupport + (1 - calibration.nightlifeStrength) * 0.04 - calibration.signatureDiningStrength * 0.03 + temporalAdjustments.start + (venue.category === "cafe" ? 0.06 : 0)
  );
...
function deriveHighlightTier(highlightSuitability, profile, anchorStrength, experienceStrengths) {
  const highlightStrength = highlightSuitability * 0.48 + profile.destinationFactor * 0.16 + profile.experientialFactor * 0.16 + Math.max(experienceStrengths.outdoor, experienceStrengths.interactive) * 0.12 + anchorStrength * 0.2;
  if (highlightStrength >= 0.8) {
    return 1;
  }
  if (highlightStrength >= 0.62) {
    return 2;
  }
  return 3;
}
...
function deriveDurationEstimate(venue, profile) {
  ...
  const durationScore = profile.lingerFactor * 0.5 + profile.experientialFactor * 0.18 + profile.destinationFactor * 0.12 + (venue.category === "restaurant" ? 0.1 : 0) + (venue.category === "cafe" || venue.category === "dessert" ? -0.06 : 0);
  if (durationScore >= 0.72) {
    return "extended";
  }
  if (durationScore >= 0.44) {
    return "moderate";
  }
  return "quick";
}
...
// tmp/hybridPortabilityPass1.ts
var cityCases = [
  { label: "San Jose, CA", city: "San Jose, CA", sourceMode: "curated" },
  { label: "Denver, CO", city: "Denver, CO", sourceMode: "curated" },
  { label: "Austin, TX", city: "Austin, TX", sourceMode: "hybrid" }
];
var scenarios = [
