...
function computeLiveQualityFairness(venue) {
  ...
  const supportedCategory = venue.category === "restaurant" || venue.category === "bar" || venue.category === "cafe";
  const supportFriendly = venue.category === "cafe" || venue.category === "restaurant" || venue.category === "bar" && venue.settings.dateFriendly || venue.settings.supportOnly;
...
function buildGenericDisplayName(rawDistrict, city) {
  ...
  const cleanCity = city.trim() || "San Jose";
...
function resolveDistrictAnchor({
  ...
  if (promotedDistrict) {
    return {
      ...
      confidence: roundToHundredths(clamp2(0.75 + strongestCluster.clusterShare * 0.15, 0.75, 0.9)),
  ...
  if (strongestCluster.clusterShare >= 0.5) {
    ...
    return {
      ...
      confidence: roundToHundredths(clamp2(0.65 + strongestCluster.clusterShare * 0.2, 0.65, 0.85)),
      reason: "inferred from strongest cluster"
    };
  }
  return createCityFallback(trimmedCity, citySlug);
...
function getVenueClusterId(venue) {
  ...
  for (const alias of CLUSTER_ALIASES) {
...
function getDensityScore(candidateCount, maxCandidateCount) {
  ...
  return clamp013(candidateCount / maxCandidateCount);
}
function getAdjustedDensityScore(rawDensity) {
  return clamp013(Math.pow(rawDensity, 0.72));
}
function getRelevanceScore(candidates) {
  ...
  return clamp013(total / candidates.length);
}
function getDiversityScore(group) {
  ...
  const categoryCoverage = clamp013(group.categories.size / 4);
  return clamp013(coreRoleCoverage * 0.65 + categoryCoverage * 0.35);
}
...
function getDominanceScore(values) {
  ...
  return clamp013(maxCount / values.length);
}
function getCoherenceScore(group) {
  ...
  return clamp013(neighborhoodDominance * 0.58 + experienceFamilyDominance * 0.42);
}
function getSequenceSupportScore(group) {
  ...
  return clamp013(sequenceMean * 0.68 + coverage * 0.22 + sequenceBalance * 0.1);
}
function getTasteAverages(candidates) {
  ...
  return {
    intimacy: clamp013(totals.intimacy / candidates.length),
    linger: clamp013(totals.linger / candidates.length),
    conversation: clamp013(totals.conversation / candidates.length),
    calm: clamp013(totals.calm / candidates.length),
    social: clamp013(totals.social / candidates.length),
    culturalDepth: clamp013(totals.culturalDepth / candidates.length),
    discovery: clamp013(totals.discovery / candidates.length),
    experiential: clamp013(totals.experiential / candidates.length),
    outdoor: clamp013(totals.outdoor / candidates.length),
    playful: clamp013(totals.playful / candidates.length)
  };
}
...
function getAffinityScores(group, intent, sequenceSupport, signature) {
  ...
  return {
    affinity: clamp013(vibeAffinity * 0.58 + crewAffinity * 0.42),
    vibeAffinity: clamp013(vibeAffinity),
    crewAffinity: clamp013(crewAffinity)
  };
}
function getAlignmentLift(density, affinity, signature, coherence, relevance, diversity) {
  const contextStrength = clamp013(relevance * 0.55 + diversity * 0.45);
  return clamp013(
...
function recommendDistricts({
  ...
  return districtGroups.map((group) => {
    ...
    const componentBreakdownRaw = {
      density: adjustedDensity * 0.18,
      relevance: relevance * 0.22,
      diversity: diversity * 0.18,
      energy: energy * 0.08,
    ...
    const score = roundToHundredths2(
      clamp013(baseScore + alignmentLift - breadthPenalty - sparsityPenalty)
    );
    const signals = {
      density,
      relevance,
...
function buildNameSignature(value) {
  return normalizeValue2(value).split(/\s+/).filter((part) => part.length > 1).join(" ");
}
...
function getDuplicateReason(left, right) {
  ...
  const sameNeighborhood = normalizeValue2(left.neighborhood) === normalizeValue2(right.neighborhood);
  const sameCity = normalizeValue2(left.city) === normalizeValue2(right.city);
...
function mergeVenueSources(curatedVenues, liveVenues, requestedSourceMode) {
  const sourcePool = requestedSourceMode === "curated" ? curatedVenues : requestedSourceMode === "live" ? liveVenues : [...curatedVenues, ...liveVenues];
...
function applyQualityGate(venue) {
  ...
  const weakVenueCategory = venue.category !== "restaurant" && venue.category !== "bar" && venue.category !== "cafe";
  ...
  const effectiveGenericScore = clamp015(venue.signature.genericScore - liveFairness.genericRelief);
  const effectiveSourceConfidence = clamp015(
  ...
  const completenessScore = clamp015(1 - completenessPenalty);
  const qualityScore = clamp015(
  ...
  if (isLiveSource && weakVenueCategory) {
    notes.push("outside first live adapter category slice");
  }
  if (fastFoodLike) {
  ...
  if (isLiveSource && weakVenueCategory) {
    suppressionReasons.push("unsupported category for the first live place adapter");
  }
...
function mapBusinessStatus(value) {
  const normalized = value ? normalizeValue4(value) : "unknown";
...
function tokenizeQueryLabel(value) {
  ...
  return value.split(/[^a-z0-9]+/i).map((part) => normalizeValue5(part)).filter(Boolean);
}
function countMatches2(values, candidates) {
  return candidates.filter((candidate) => values.has(normalizeValue5(candidate))).length;
}
function computeLiveSignatureStrength(raw, category) {
  ...
  const normalizedSignals = /* @__PURE__ */ new Set([
    ...(raw.tags ?? []).map(normalizeValue5),
    ...(raw.sourceTypes ?? []).map(normalizeValue5),
    ...(raw.queryTerms ?? []).map(normalizeValue5),
...
function inferSocialDensity(raw, category, energyLevel, inferredFields) {
  ...
  return clamp3(Math.round(socialDensity), 1, 5);
}
...
function inferSignatureSignals(raw, category) {
  ...
  return {
    chainLike,
    genericScore: clamp3(Number(genericScore.toFixed(2)), 0, 1),
    signatureScore: clamp3(Number(signatureScore.toFixed(2)), 0, 1)
  };
}
...
function inferLocalSignals(raw, signatureScore, inferredFields) {
  ...
  const base = clamp3(0.58 + signatureScore * 0.24, 0.45, 0.92);
  return {
...
function inferVenueSignals({
  ...
  const uniquenessScore = inferScore(
    raw.uniquenessScore,
    clamp3(0.52 + signature.signatureScore * 0.38, 0.4, 0.95),
    inferredFields,
    "uniquenessScore"
  );
  const distinctivenessScore = inferScore(
    raw.distinctivenessScore,
    clamp3(0.5 + signature.signatureScore * 0.42, 0.38, 0.96),
    inferredFields,
    "distinctivenessScore"
  );
  const underexposureScore = inferScore(
    raw.underexposureScore,
    clamp3(0.42 + (raw.isHiddenGem ? 0.22 : 0) + (signature.signatureScore - signature.genericScore) * 0.12, 0.25, 0.92),
    inferredFields,
    "underexposureScore"
  );
  const shareabilityScore = inferScore(
    raw.shareabilityScore,
    clamp3(0.48 + socialDensity * 0.06 + signature.signatureScore * 0.12, 0.35, 0.94),
    inferredFields,
    "shareabilityScore"
  );
...
function getVibeTerms(intent) {
  ...
  if (primary === "cultured") {
    return ["thoughtful", "design forward", "wine"];
  }
  if (primary === "chill") {
...
function getGooglePlacesConfig() {
  return {
    ...
    endpoint: import.meta.env.VITE_GOOGLE_PLACES_ENDPOINT ?? "https://places.googleapis.com/v1/places:searchText",
    languageCode: import.meta.env.VITE_GOOGLE_PLACES_LANGUAGE_CODE ?? "en",
    regionCode: import.meta.env.VITE_GOOGLE_PLACES_REGION_CODE ?? "US",
    pageSize: Number(import.meta.env.VITE_GOOGLE_PLACES_PAGE_SIZE ?? 8)
  };
}
...
function getNormalizedTypes(place) {
  return unique6(
    [place.primaryType, ...place.types ?? []].filter((value) => Boolean(value)).map(normalizeValue6).filter((value) => !ignoredTypes.has(value))
  );
}
...
function getAddressComponent(components, candidates) {
  return components?.find(
    (component) => component.types?.some((type) => candidates.includes(normalizeValue6(type)))
  )?.longText;
}
...
function inferDriveMinutes(context, placeNeighborhood) {
  ...
  const normalizedIntentNeighborhood = context.neighborhood ? normalizeValue6(context.neighborhood) : void 0;
  const normalizedPlaceNeighborhood = placeNeighborhood ? normalizeValue6(placeNeighborhood) : void 0;
  ...
  let base = context.requestedKind === "bar" ? 10 : context.requestedKind === "restaurant" ? 12 : 9;
  if (sameNeighborhood) {
    base -= 3;
  }
...
function mapLivePlaceToRawPlace(place, context) {
...
function normalizeRawPlaces(rawPlaces, intent) {
  ...
  return rawPlaces.map(
    (rawPlace) => normalizeVenue(rawPlace, {
      timeWindowSignal
    })
  );
}
async function fetchLivePlaces(intent, starterPack) {
  ...
  const queryPlan = buildLiveQueryPlan(intent, starterPack);
  const queryTemplatesUsed = [...new Set(queryPlan.map((entry) => entry.template))];
  const queryLabelsUsed = queryPlan.map((entry) => entry.label);
  const roleIntentQueryNotes = [...new Set(queryPlan.flatMap((entry) => entry.notes))];
  const requestedKindsForPlan = [...new Set(queryPlan.map((entry) => entry.kind))];
  if (!config.apiKey) {
    return {
      venues: [],
      diagnostics: {
  ...
  const settled = await Promise.allSettled(
    queryPlan.map(async (query) => ({
      query,
      places: await queryGooglePlacesTextSearch(query)
    }))
  );
  const errors = [];
  let fetchedCount = 0;
  ...
  for (const result of settled) {
    ...
    places.forEach((place, index) => {
      const mapped = mapLivePlaceToRawPlace(place, {
        city: intent.city,
      ...
      if (mapped) {
        rawPlaces.push(mapped);
      }
    });
  }
  const venues = normalizeRawPlaces(rawPlaces, intent);
  ...
  const liveCandidatesByQuery = queryPlan.map((query) => {
    ...
    const normalized = venues.filter((venue) => venue.source.sourceQueryLabel === query.label);
    return {
      label: query.label,
      ...
      approvedCount: countByGateStatus(normalized, "approved"),
      demotedCount: countByGateStatus(normalized, "demoted"),
      suppressedCount: countByGateStatus(normalized, "suppressed")
    };
  });
  return {
    venues,
...
async function retrieveVenues(intent, lens, options = {}) {
  const curatedVenues = options.seedVenues ? [...options.seedVenues, ...sanJoseVenues] : sanJoseVenues;
  const requestedSourceMode = options.requestedSourceMode ?? "curated";
  const cityQuery = sanitize(intent.city);
  ...
  const mergedRequested = mergeVenueSources(curatedVenues, liveFetch.venues, requestedSourceMode);
  const liveTrustBreakdown = buildLiveTrustBreakdown(liveFetch.venues, mergedRequested.dedupeLosses);
  const liveHoursDemotedCount = liveFetch.venues.filter((venue) => venue.source.hoursDemotionApplied).length;
  const liveHoursSuppressedCount = liveFetch.venues.filter((venue) => venue.source.hoursSuppressionApplied).length;
  const shouldFallbackToCurated = requestedSourceMode !== "curated" && (!liveFetch.diagnostics.success || mergedRequested.countsBySource.live === 0 || requestedSourceMode === "live" && mergedRequested.venues.length < 10);
  const sourcePool = shouldFallbackToCurated ? curatedVenues : buildSourcePool(requestedSourceMode, curatedVenues, liveFetch.venues);
  const mergedPool = mergeVenueSources(curatedVenues, liveFetch.venues, shouldFallbackToCurated ? "curated" : requestedSourceMode);
  ...
  const cityMatches = qualityEligibleVenues.filter(
    (venue) => sanitize(venue.city) === cityQuery && venue.driveMinutes <= maxDriveMinutes
  );
  const fallbackMatches = cityMatches.length > 0 ? cityMatches : shouldFallbackToCurated || requestedSourceMode === "curated" ? qualityEligibleVenues.filter(
    (venue) => sanitize(venue.city) === "san jose" && venue.driveMinutes <= maxDriveMinutes
  ) : qualityEligibleVenues.filter((venue) => venue.driveMinutes <= maxDriveMinutes);
  ...
  const buildResult = (venues, neighborhoodPreferred) => {
    ...
    return {
      ...
      sourceMode: {
        ...
        mappedCount: liveFetch.diagnostics.mappedCount,
        normalizedCount: liveFetch.diagnostics.normalizedCount,
        approvedCount: liveFetch.diagnostics.approvedCount,
        demotedCount: liveFetch.diagnostics.demotedCount,
        suppressedCount: liveFetch.diagnostics.suppressedCount,
      ...
      stageCounts: {
        ...
        liveMapped: liveFetch.diagnostics.mappedCount,
        liveNormalized: liveFetch.diagnostics.normalizedCount,
        liveApproved: liveFetch.diagnostics.approvedCount,
        liveDemoted: liveFetch.diagnostics.demotedCount,
        liveSuppressed: liveFetch.diagnostics.suppressedCount,
