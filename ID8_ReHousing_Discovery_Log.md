...
| geo/pocket/viability → split (Field/Bearings/Interp) | partial | Structural clustering fan-in complete for `formRawPockets.ts` and `dbscan.ts`; `geoDistance.ts` held in legacy path due mixed consumers; viability/entity/admission splits remain pending. |
