...
1. **Stage 2E-2 - scoring candidate-identity preservation** *(immediate next scope-design boundary; not started; implementation not authorized).* Scope: scoring-side candidate projection, including `scoreVenueFit.ts`. Purpose: ensure scoring consumes admitted venue identity without minting or substituting route identity.
2. **Stage 2F - Application and product-mode materialization** *(held; depends on stable ingress and candidate identity).* Scope: Curate, Surprise, Build, public-card, seed, sandbox-preview, and App materialization bridges. Purpose: ensure Application consumes resolved/admitted identity and does not repair, synthesize, substitute, or promote provider identity.
...
- **Stage 2E held files/systems:** `scoreVenueFit.ts`, `hybridPortableAdapter.ts`, direct Application scoring/materialization, provider/hosted proof, and scoring candidate-identity preservation remain unstarted after Stage 2E-1.
