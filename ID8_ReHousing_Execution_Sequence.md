...
1. **Stage 2E - retrieval merge/dedupe/scoring identity guard** *(previously approved next stage boundary; scope design pending; implementation not authorized).* Scope: `scoreVenueFit`, `retrieveVenues`, `dedupeVenues`, and `mergeVenueSources`. Purpose: require resolved, admitted, non-provider-derived identity before route scoring and align merge/dedupe with canonical identity where available.
