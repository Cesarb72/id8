...
function countMatches(values, candidates2) {
  return candidates2.filter((candidate) => values.has(normalizeValue(candidate))).length;
}
...
function hasAnySignal(venue, candidates2) {
  const normalized = new Set(
    [...venue.tags, ...venue.source.sourceTypes].map((value) => value.trim().toLowerCase())
  );
  return candidates2.some((candidate) => normalized.has(candidate.toLowerCase()));
}
...
function firstMatch(values, candidates2) {
  return values.find((value) => candidates2.includes(value));
}
...
function hasAny(values, candidates2) {
  return candidates2.some((candidate) => values.includes(candidate));
}
...
function countMatches2(values, candidates2) {
  return candidates2.filter((candidate) => values.has(normalizeValue4(candidate))).length;
}
...
function hasAnyTag(raw, candidates2) {
  const tags = new Set((raw.tags ?? []).map((tag) => tag.toLowerCase()));
  return candidates2.some((candidate) => tags.has(candidate.toLowerCase()));
}
...
function inferScore(rawValue, fallback2, inferredFields, field) {
...
function parseHourFromTimeWindow(timeWindow) {
  ...
  for (const [candidates2, hour2] of directMatches) {
    if (candidates2.some((candidate) => normalized.includes(candidate))) {
      return hour2;
    }
  }
...
function buildLiveQueryPlan(intent, starterPack) {
  ...
  const deduped2 = /* @__PURE__ */ new Map();
  for (const entry of withText) {
    const key = `${entry.kind}:${entry.textQuery.toLowerCase()}`;
    if (!deduped2.has(key)) {
      deduped2.set(key, entry);
    }
  }
  return [...deduped2.values()];
}
...
function hasAny2(values, candidates2) {
  return candidates2.some((candidate) => values.includes(candidate));
}
...
function getAddressComponent(components, candidates2) {
  return components?.find(
    (component) => component.types?.some((type) => candidates2.includes(normalizeValue5(type)))
  )?.longText;
}
function inferNeighborhood(place, fallback2) {
  ...
  return firstSegment && firstSegment.length >= 3 ? firstSegment : fallback2;
}
function inferCity(place, fallback2) {
  return getAddressComponent(place.addressComponents, ["locality"]) ?? getAddressComponent(place.addressComponents, ["administrative-area-level-2"]) ?? fallback2;
}
...
function dedupeByPlaceId(venues) {
  ...
  const deduped2 = [...bestByKey.values()].sort((left, right) => left.id.localeCompare(right.id));
  return {
    venues: deduped2,
    dropped: Math.max(0, venues.length - deduped2.length)
  };
}
...
async function fetchLivePlaces(intent, starterPack) {
  ...
  const deduped2 = dedupeByPlaceId(normalized.venues);
  const venues = deduped2.venues;
  ...
  return {
    ...
    diagnostics: {
      ...
      dedupedByPlaceIdCount: deduped2.dropped,
...
function summarizeGeoSpread(venues) {
  ...
  const bucketComponent = clamp017((bucketCount - 1) / 4);
  const balanceComponent = clamp017(1 - dominantAreaShare);
...
async function fetchHybridPortableVenues(city2) {
  ...
  const dedupeDropped2 = liveAcceptedBeforeDedupe.length - liveVenuesDeduped.length;
    ...
    return {
      ...
      diagnostics: {
        ...
        notes: [
          ...
          ...dedupeDropped2 > 0 ? [`${dedupeDropped2} duplicate live entities collapsed during merge.`] : [],
...
function scopeRomanticHighlightCandidatesByMomentTier(candidates2, lens) {
  if (!lens || !isRomanticPersonaContractActive(lens) || candidates2.length === 0) {
    return {
  ...
  const anchorCandidates = candidates2.filter(
  ...
  const builderCandidates = candidates2.filter(
  ...
  const supportCandidates = candidates2.filter(
    (candidate) => candidate.taste.signals.momentTier === "support"
  );
  return {
    candidates: supportCandidates.length > 0 ? supportCandidates : candidates2,
    appliedTier: "support",
    fallbackUsed: true
  };
}
...
function assessGenericHospitalityFallbackPenalty(candidate, candidates2) {
  ...
  const strongestAlternative = [...candidates2].filter((alternative) => {
...
function computeRolePoolRankingBreakdown(candidate, role, lens, intent) {
  ...
  const contract2 = candidate.roleContract[role];
  const contractInfluence = contractWeight(role, contract2.strength);
  ...
  const contractBonusContribution = contract2.score * contractInfluence.boost;
  const contractPenaltyContribution = contract2.satisfied ? 0 : (1 - contract2.score) * contractInfluence.penalty;
...
function starterPackContracts(starterPack) {
  ...
  const build = (role, label) => {
    const contract2 = starterPack.roleContracts?.[role];
    if (!contract2) {
      return void 0;
    }
    return {
      label,
      strength: contract2.strength,
      requiredCategories: asCategories(contract2.requiredCategories),
      preferredCategories: asCategories(contract2.preferredCategories),
      discouragedCategories: asCategories(contract2.discouragedCategories),
      requiredTags: contract2.requiredTags ?? [],
      preferredTags: contract2.preferredTags ?? [],
      discouragedTags: contract2.discouragedTags ?? [],
      maxEnergyLevel: contract2.maxEnergyLevel
    };
  };
  return {
...
function getIntentFit(input) {
  ...
  return toFixed12(clamp14(base * 0.2 + vibeFit * 0.45 + personaFit * 0.35, 0, 1));
}
...
function detectTemporalMoment(input, timingRelevance, intentFit) {
  ...
  const temporalSignalsPresent = Boolean(
    input.sourceFlags?.eventCapable || input.sourceFlags?.musicCapable || input.sourceFlags?.performanceCapable || input.sourceFlags?.hasHappyHour || signals.hyperlocalActivation.temporalLabel !== "background"
  );
...
function detectMomentsFromTaste(input) {
  ...
  return candidates2.sort((left, right) => {
...
function getDiscoveryBalance(inputs) {
  const noveltySignal = weightedAverage(
    inputs.map((input) => ({
      value: input.tasteSignals.noveltyWeight * 0.55 + input.tasteSignals.momentPotential.score * 0.2 + input.tasteSignals.experientialFactor * 0.15 + input.tasteSignals.momentIntensity.score * 0.1,
...
function getStrongestHighlightCandidate(candidates2, inputById) {
  const top = candidates2[0];
  if (!top) {
    return void 0;
  }
...
function buildHighlights(signals) {
  ...
  const candidates2 = buildHighlightCandidates(signals).sort((left, right) => {
    if (right.score !== left.score) {
      return right.score - left.score;
    }
...
function getDensityScore(candidateCount, maxCandidateCount) {
  ...
  return clamp019(candidateCount / maxCandidateCount);
}
function getAdjustedDensityScore(rawDensity) {
  return clamp019(Math.pow(rawDensity, 0.72));
}
function getRelevanceScore(candidates2) {
  if (candidates2.length === 0) {
    return 0;
  }
  const total = candidates2.reduce((sum, candidate) => {
  ...
  return clamp019(total / candidates2.length);
}
function getDiversityScore(group) {
  ...
  const categoryCoverage = clamp019(group.categories.size / 4);
  return clamp019(coreRoleCoverage * 0.65 + categoryCoverage * 0.35);
}
function getEnergyScore(candidates2) {
  if (candidates2.length === 0) {
    return 0;
  }
  const total = candidates2.reduce(
    (sum, candidate) => sum + candidate.venue.source.qualityScore,
    0
  );
  return clamp019(total / candidates2.length);
}
function getSignatureScore(candidates2) {
  if (candidates2.length === 0) {
    return 0;
  }
  const total = candidates2.reduce((sum, candidate) => {
...
function getDominanceScore(values) {
  ...
  return clamp019(maxCount / values.length);
}
function getCoherenceScore(group) {
  ...
  return clamp019(neighborhoodDominance * 0.58 + experienceFamilyDominance * 0.42);
}
function getSequenceSupportScore(group) {
  ...
  return clamp019(sequenceMean * 0.68 + coverage * 0.22 + sequenceBalance * 0.1);
}
function getTasteAverages(candidates2) {
  if (candidates2.length === 0) {
    return {
      intimacy: 0,
      linger: 0,
  ...
  const totals = candidates2.reduce(
    (sum, candidate) => {
      const signals = candidate.taste.signals;
      return {
  ...
  return {
    intimacy: clamp019(totals.intimacy / candidates2.length),
    linger: clamp019(totals.linger / candidates2.length),
    conversation: clamp019(totals.conversation / candidates2.length),
    calm: clamp019(totals.calm / candidates2.length),
    social: clamp019(totals.social / candidates2.length),
    culturalDepth: clamp019(totals.culturalDepth / candidates2.length),
    discovery: clamp019(totals.discovery / candidates2.length),
    experiential: clamp019(totals.experiential / candidates2.length),
    outdoor: clamp019(totals.outdoor / candidates2.length),
    playful: clamp019(totals.playful / candidates2.length)
  };
}
function getCategoryMix(candidates2) {
  if (candidates2.length === 0) {
    return {
      restaurant: 0,
      bar: 0,
  ...
  return {
    restaurant: clamp019(totals.restaurant / candidates2.length),
    bar: clamp019(totals.bar / candidates2.length),
    cafe: clamp019(totals.cafe / candidates2.length),
    dessert: clamp019(totals.dessert / candidates2.length),
    liveMusic: clamp019(totals.liveMusic / candidates2.length),
    activity: clamp019(totals.activity / candidates2.length),
    park: clamp019(totals.park / candidates2.length),
    museum: clamp019(totals.museum / candidates2.length),
    event: clamp019(totals.event / candidates2.length)
  };
}
...
function getWalkabilitySignal(candidates2) {
  if (candidates2.length === 0) {
    return 0;
  }
  const matched = candidates2.filter(
    (candidate) => ["walkable", "stroll", "promenade", "courtyard", "main-street", "main street", "open air"].some(
      (term) => hasTag(candidate.venue.tags, term)
    )
  ).length;
  return clamp019(matched / candidates2.length);
}
function getEventMomentumSignal(candidates2) {
  if (candidates2.length === 0) {
    return 0;
  }
  const total = candidates2.reduce((sum, candidate) => {
...
function getRoleStrengths(group) {
  ...
  const chaosPenalty = clamp019(Math.max(0, social - 0.58) * 0.9 + Math.max(0, energy - 0.62) * 0.8);
  const overCalmPenalty = clamp019(Math.max(0, taste.calm - 0.72) * 0.9);
  ...
  const startCategorySupport = clamp019(
  ...
  const highlightCategorySupport = clamp019(
  ...
  const winddownCategorySupport = clamp019(
  ...
  const highlightStrength = clamp019(
  ...
  const winddownStrength = clamp019(
...
function getAffinityScores(group, intent, sequenceSupport, signature) {
  ...
  return {
    affinity: clamp019(vibeAffinity * 0.58 + crewAffinity * 0.42),
    vibeAffinity: clamp019(vibeAffinity),
    crewAffinity: clamp019(crewAffinity)
  };
}
function getAlignmentLift(density, affinity, signature, coherence, relevance, diversity) {
  const contextStrength = clamp019(relevance * 0.55 + diversity * 0.45);
  return clamp019(
...
function recommendDistricts({
  ...
  return districtGroups.map((group) => {
    ...
    const score = roundToHundredths2(
      clamp019(baseScore + alignmentLift - breadthPenalty - sparsityPenalty)
    );
    const signals = {
      density,
      relevance,
...
function hasAnyTag2(tags, candidates2) {
  return candidates2.some((candidate) => tags.has(normalizeTag2(candidate)));
}
...
function dedupeVenues2(venues) {
  ...
  const losses = [];
  for (const venue of venues) {
    const duplicateIndex = deduped2.findIndex((candidate) => Boolean(getDuplicateReason(candidate, venue)));
    if (duplicateIndex === -1) {
      deduped2.push(venue);
      continue;
    }
    const current = deduped2[duplicateIndex];
...
function mergeVenueSources(curatedVenues2, liveVenues, requestedSourceMode) {
  ...
  const deduped2 = dedupeVenues2(sourcePool);
  const countsBySource = deduped2.venues.reduce(
    (acc, venue) => {
  ...
  return {
    venues: deduped2.venues,
    dedupedCount: deduped2.dedupedCount,
    dedupedLiveCount: deduped2.dedupedLiveCount,
    liveDedupedAgainstCuratedCount: deduped2.liveDedupedAgainstCuratedCount,
    liveNoveltyCollapsedCount: deduped2.liveNoveltyCollapsedCount,
    dedupeLosses: deduped2.losses,
    countsBySource
  };
}
...
function scoreAnchorCategoryPressure(venue, anchor) {
  ...
  return clamp0111(categoryFit - categoryPenalty);
}
function scoreAnchorFit(venue, intent) {
  ...
  return clamp0111(weightedSum / totalWeight);
}
...
function scoreCrewFit(venue, crewPolicy) {
  ...
  const energyScore = clamp0113(1 - energyDistance / 4);
...
function scoreHiddenGemFit(venue, fitScore, intent) {
  ...
  return clamp0114(weighted - chainPenalty + localBonus + hiddenGemLift + intentLift);
}
...
function scoreProximityFit(venue, intent) {
  ...
  const drivePenalty = clamp0115(venue.driveMinutes / cap);
  const driveScore = clamp0115(1 - drivePenalty);
  const neighborhoodBonus = sameNeighborhood ? 1 : 0;
  const neighborhoodPenalty = neighborhoodQuery && !sameNeighborhood ? 0.18 : 0;
  if (intent.distanceMode === "nearby") {
    const strictDriveScore = clamp0115(1 - venue.driveMinutes / nearbyCap);
    return clamp0115(
      strictDriveScore * 0.62 + neighborhoodBonus * 0.38 - neighborhoodPenalty
    );
  }
  return clamp0115(driveScore * 0.8 + neighborhoodBonus * 0.2 - neighborhoodPenalty * 0.5);
}
...
function roleAffinityUniversality(venue) {
  ...
  const spreadPenalty = clamp0117(1 - (max - min));
  return clamp0117(mean * 0.58 + spreadPenalty * 0.42);
}
...
function evaluateRoleContract(venue, rule) {
  ...
  const maxEnergyPenalty = typeof rule.maxEnergyLevel === "number" && venue.energyLevel > rule.maxEnergyLevel ? clamp0118((venue.energyLevel - rule.maxEnergyLevel) / 4) : 0;
...
function hasAnyTag3(tags, candidates2) {
  return candidates2.some((candidate) => tags.has(normalizeTag4(candidate)));
}
...
function deriveSeedCalibratedTasteProfile(venue) {
  ...
  const energy = clamp0119(venue.energyLevel / 5);
  const socialDensity = clamp0119(venue.socialDensity / 5);
  ...
  const conversationFriendliness = clamp0119(
...
function inferRuleProfile(venue, context) {
  ...
  profile.experientialFactor = clamp0120(
  ...
  profile.conversationFriendliness = clamp0120(
  ...
  if (venue.priceLevel === "$$$" || venue.priceLevel === "$$$$") {
    profile.destinationFactor = clamp0120(profile.destinationFactor + 0.08);
    profile.experientialFactor = clamp0120(profile.experientialFactor + 0.06);
  ...
  if (hasHighRatingCue) {
    profile.destinationFactor = clamp0120(profile.destinationFactor + 0.04);
    profile.experientialFactor = clamp0120(profile.experientialFactor + 0.04);
  }
  if (hasHighReviewCue) {
    profile.socialDensity = clamp0120(profile.socialDensity + 0.06);
    profile.destinationFactor = clamp0120(profile.destinationFactor + 0.04);
  }
...
function applyContextTonePressure(profile, venue, context) {
  ...
  const containedSocialCue = clamp0120(
  ...
  const broadSocialCue = clamp0120(
  ...
  if (livelyLike) {
    ...
    nextProfile.energy = clamp0120(nextProfile.energy + livelyLift);
    nextProfile.socialDensity = clamp0120(nextProfile.socialDensity + 0.07 + broadSocialCue * 0.07);
    nextProfile.experientialFactor = clamp0120(nextProfile.experientialFactor + 0.06);
    nextProfile.destinationFactor = clamp0120(
      nextProfile.destinationFactor + 0.03 + energeticCue * 0.03
    );
    if (persona2 === "romantic") {
      nextProfile.intimacy = clamp0120(nextProfile.intimacy - 0.01 + containedSocialCue * 0.05);
      nextProfile.conversationFriendliness = clamp0120(
      ...
      nextProfile.intimacy = clamp0120(nextProfile.intimacy - 0.06);
      nextProfile.conversationFriendliness = clamp0120(nextProfile.conversationFriendliness - 0.03);
      signals.push("tone:vibe-lively-friends-broad-social-lift");
    } else {
      nextProfile.intimacy = clamp0120(nextProfile.intimacy - 0.03);
      nextProfile.conversationFriendliness = clamp0120(nextProfile.conversationFriendliness - 0.02);
      signals.push("tone:vibe-lively-general-lift");
    }
  } else if (cozyLike) {
    const cozySoftening = 0.09 + calmCue * 0.06;
    nextProfile.energy = clamp0120(nextProfile.energy - cozySoftening);
    nextProfile.socialDensity = clamp0120(nextProfile.socialDensity - 0.08 - broadSocialCue * 0.04);
    nextProfile.intimacy = clamp0120(nextProfile.intimacy + 0.08 + calmCue * 0.06);
    nextProfile.conversationFriendliness = clamp0120(
      nextProfile.conversationFriendliness + 0.07 + calmCue * 0.04
    );
    nextProfile.lingerFactor = clamp0120(nextProfile.lingerFactor + 0.05);
    nextProfile.destinationFactor = clamp0120(nextProfile.destinationFactor - 0.02 + calmCue * 0.02);
    signals.push("tone:vibe-cozy-softening");
  } else if (vibe2 === "cultured") {
    nextProfile.destinationFactor = clamp0120(nextProfile.destinationFactor + 0.05);
    nextProfile.experientialFactor = clamp0120(nextProfile.experientialFactor + 0.05);
    nextProfile.conversationFriendliness = clamp0120(nextProfile.conversationFriendliness + 0.03);
    nextProfile.energy = clamp0120(nextProfile.energy + 0.01);
    signals.push("tone:vibe-cultured-curation-lift");
  } else if (vibe2 === "adventurous-outdoor" || vibe2 === "adventurous-urban") {
    nextProfile.energy = clamp0120(nextProfile.energy + 0.06);
    nextProfile.socialDensity = clamp0120(nextProfile.socialDensity + 0.03);
    nextProfile.experientialFactor = clamp0120(nextProfile.experientialFactor + 0.07);
    nextProfile.destinationFactor = clamp0120(nextProfile.destinationFactor + 0.03);
    nextProfile.lingerFactor = clamp0120(nextProfile.lingerFactor + 0.02);
...
function deriveRoleSuitability(venue, profile, durationEstimate, noveltyWeight, calibration, experienceStrengths, momentPotential, experienceArchetypes, momentEnrichment, hyperlocalActivation, context) {
  ...
  const centerpieceSignal = clamp0120(
  ...
  const startFrictionSignal = clamp0120(
  ...
  const nightlifePressure = clamp0120(
  ...
  if (highlight >= 0.72) {
    const dominance = clamp0120((highlight - 0.72) * 1.18 + centerpieceSignal * 0.28);
    start = clamp0120(start - 0.14 * dominance);
    windDown = clamp0120(windDown - 0.2 * dominance);
  }
  if (start >= 0.66 && centerpieceSignal >= 0.58) {
    start = clamp0120(start - 0.12);
  }
  if (windDown >= 0.62 && (profile.energy >= 0.66 || nightlifePressure >= 0.62 || centerpieceSignal >= 0.66)) {
    windDown = clamp0120(windDown - 0.16);
  }
  if (landingSignal >= 0.58) {
    highlight = clamp0120(highlight - 0.08 * landingSignal);
  }
  const vibe2 = context?.vibe ?? void 0;
  const persona2 = context?.persona ?? void 0;
  const livelyLike = vibe2 === "lively" || vibe2 === "playful";
  const cozyLike = vibe2 === "cozy" || vibe2 === "chill";
  const livelyPulseSignal = clamp0120(
  ...
  if (livelyLike) {
    ...
    start = clamp0120(start - livelyPulseSignal * 0.08);
    windDown = clamp0120(windDown - livelyPulseSignal * 0.06);
    if (persona2 === "romantic") {
      highlight = clamp0120(highlight + containedPulseSignal * 0.08 - broadChaosSignal * 0.1);
      if (softCenterSignal >= 0.62 && centerpieceSignal < 0.62) {
        highlight = clamp0120(highlight - 0.1);
      }
    } else if (persona2 === "friends") {
      highlight = clamp0120(highlight + livelyPulseSignal * 0.08);
      start = clamp0120(start + profile.socialDensity * 0.04);
    } else if (persona2 === "family") {
      highlight = clamp0120(highlight - broadChaosSignal * 0.08);
      windDown = clamp0120(windDown + profile.conversationFriendliness * 0.05);
    }
  } else if (cozyLike) {
    highlight = clamp0120(
      highlight - livelyPulseSignal * 0.14 + profile.intimacy * 0.08 + landingSignal * 0.06
    );
    start = clamp0120(start + profile.conversationFriendliness * 0.06 + profile.intimacy * 0.04);
    windDown = clamp0120(windDown + landingSignal * 0.08 + profile.conversationFriendliness * 0.06);
  }
  const surprise = clamp0120(
...
function deriveHighlightTier(venue, highlightSuitability, profile, anchorStrength, experienceStrengths, durationEstimate, context) {
  ...
  const vibeAdjustedStrength = clamp0120(
...
function deriveMomentIntensity(venue, profile, experienceStrengths, experienceArchetypes, momentPotential, momentIdentity, romanticSignals, noveltyWeight, calibration, anchorStrength, highlightTier, momentEnrichment, hyperlocalActivation) {
  ...
  const scenicStrength = clamp0120(
  ...
  const activityRichness = clamp0120(
  ...
  const culturalImmersion = clamp0120(
  ...
  const hyperlocalLift = clamp0120(
...
function deriveTasteCalibration(venue, profile, noveltyWeight) {
  ...
  const tagRichness = clamp0120(venue.tags.length / 5);
  const placeTypeRichness = clamp0120((venue.placeTypes?.length ?? 0) / 4);
  const categorySpecificity = clamp0120(
  ...
  const personalityStrength = clamp0120(
...
function deriveExperienceStrengths(venue, keywordSignals) {
  ...
  return {
    ...
    interactive: clamp0120(interactive),
    passiveHospitality: clamp0120(passiveHospitality)
  };
}
...
function deriveExperienceFamily(venue, profile, experienceStrengths, experienceArchetypes, romanticSignals, momentIntensity, calibration, momentEnrichment, hyperlocalActivation) {
  ...
  const activitySignal = clamp0120(
  ...
  const immersiveCultureSignal = clamp0120(
  ...
  const familyScores = [
    {
      ...
      score: clamp0120(scenicSignal * 0.66 + quietSignal * 0.12 + richnessSignal * 0.12)
    },
    {
      family: "quiet_activity",
      score: clamp0120(activitySignal * 0.5 + quietSignal * 0.22 + richnessSignal * 0.16)
    },
    {
      family: "cultural",
      score: clamp0120(cultureSignal * 0.62 + ambientSignal * 0.12 + richnessSignal * 0.16)
    },
    {
    ...
    },
    {
      ...
      score: clamp0120(ambientSignal * 0.58 + quietSignal * 0.14 + richnessSignal * 0.14)
    },
    {
      family: "intimate_dining",
      score: clamp0120(diningSignal * 0.62 + quietSignal * 0.14 + richnessSignal * 0.12)
    }
  ];
  const priority = [
    "outdoor_scenic",
...
function deriveExpandedExperienceFamily(venue, profile, momentPotential, momentIntensity, momentEnrichment, hyperlocalActivation, baseFamily) {
  ...
  const activationStrength = clamp0120(
...
function applyExpandedFamilyRoleAdjustment(roleSuitability, experienceFamily, baseExperienceFamily, hyperlocalActivation) {
  ...
  const activationScale = clamp0120(
  ...
  if (experienceFamily === "tasting_experience") {
    adjusted.highlight = clamp0120(adjusted.highlight + 0.026 * activationScale);
    adjusted.windDown = clamp0120(adjusted.windDown + 0.018 * activationScale);
  } else if (experienceFamily === "live_experience") {
    adjusted.highlight = clamp0120(adjusted.highlight + 0.028 * activationScale);
    adjusted.surprise = clamp0120(adjusted.surprise + 0.02 * activationScale);
  } else if (experienceFamily === "immersive_experience") {
    adjusted.highlight = clamp0120(adjusted.highlight + 0.024 * activationScale);
    adjusted.surprise = clamp0120(adjusted.surprise + 0.022 * activationScale);
  } else if (experienceFamily === "atmospheric_experience") {
    adjusted.highlight = clamp0120(adjusted.highlight + 0.022 * activationScale);
    adjusted.windDown = clamp0120(adjusted.windDown + 0.02 * activationScale);
  }
  return adjusted;
}
...
function deriveRomanticSignals(venue, profile, experienceStrengths, experienceArchetypes, momentEnrichment) {
  ...
  const sharedActivity = clamp0120(
  ...
  const ambientExperience = clamp0120(
...
function deriveHyperlocalActivation(venue, profile, experienceArchetypes, context) {
  const keywords = buildKeywordCorpus(venue);
  const archetypes = new Set(experienceArchetypes.archetypes);
  const livePerformance = clamp0120(
  ...
  const tastingActivation = clamp0120(
  ...
  const culturalActivation = clamp0120(
  ...
  const temporalRelevance = clamp0120(
  ...
  const intensityContribution = activationTypes.length === 0 ? 0 : clamp0120(
  ...
  return {
    ...
    interpretationImpact: {
      highlightSuitability: clamp0120(highlightSuitabilityImpact),
      momentPotential: clamp0120(momentPotentialImpact),
      novelty: clamp0120(noveltyImpact),
      momentIntensity: clamp0120(momentIntensityImpact),
      familyRefinements
    },
...
function deriveHyperlocalTemporalCompatibility(venue, keywords, activationTypes, temporalRelevance, temporalLabel, intensityContribution, context) {
  ...
  const eveningCueScore = clamp0120(
...
function deriveMomentEnrichment(venue, profile, experienceStrengths, experienceArchetypes, hyperlocalActivation) {
  ...
  const temporalEnergy = clamp0120(
  ...
  const ambientUniqueness = clamp0120(
  ...
  const highlightSurfaceBoost = clamp0120(
...
function normalizeCount(value, ceiling) {
  ...
  return clamp0120(value / ceiling);
}
function clampProfile(profile) {
  return {
    energy: clamp0120(profile.energy),
    socialDensity: clamp0120(profile.socialDensity),
    intimacy: clamp0120(profile.intimacy),
    lingerFactor: clamp0120(profile.lingerFactor),
    destinationFactor: clamp0120(profile.destinationFactor),
    experientialFactor: clamp0120(profile.experientialFactor),
    conversationFriendliness: clamp0120(profile.conversationFriendliness)
  };
}
...
function scorePackHighlightPressure(venue, starterPack) {
  ...
  const contract2 = starterPack.roleContracts?.highlight;
  const contractCategoryMatch = contract2?.requiredCategories?.includes(venue.category) || contract2?.preferredCategories?.includes(venue.category) ? 1 : 0;
  const presetCategoryMatch = preferredCategories.includes(venue.category) ? 1 : 0;
  const tagFit = tagOverlapScore3(venue.tags, [
    ...preferredTags,
    ...contract2?.requiredTags ?? [],
    ...contract2?.preferredTags ?? []
  ]);
  return clamp0121(
...
function computeVibeAuthority(venue, intent, lens, starterPack) {
  ...
  const adventureSplitLift = intent.primaryAnchor === "adventurous-outdoor" ? clamp0121(adventureRead.outdoor) * 0.22 - clamp0121(adventureRead.urban) * 0.18 : intent.primaryAnchor === "adventurous-urban" ? clamp0121(adventureRead.urban) * 0.22 - clamp0121(adventureRead.outdoor) * 0.18 : 0;
  const byRole = {
    start: clamp0121(
...
function buildDistrictTasteAggregationByDistrictId(scoredVenues, city2) {
...
function getConciergeSocialEnergy(vibe2) {
  if (vibe2 === "lively" || vibe2 === "playful") {
    return "high";
  }
  if (vibe2 === "cozy" || vibe2 === "chill") {
...
function getConciergeExplorationTolerance(params) {
  const { persona: persona2, vibe: vibe2 } = params;
  if (vibe2 === "adventurous-outdoor" || vibe2 === "adventurous-urban" || vibe2 === "cultured") {
    return "high";
  }
  if (vibe2 === "chill" || vibe2 === "cozy" || persona2 === "family") {
    return "low";
  }
  return "medium";
}
function getConciergePacing(params) {
  ...
  if (vibe2 === "chill" && persona2 !== "family") {
    return "linger";
  }
  if (persona2 === "romantic" && (vibe2 === "cozy" || vibe2 === "chill")) {
    return "linger";
  }
  return "balanced";
}
function buildConciergeIntent(params) {
  ...
  const socialEnergy = getConciergeSocialEnergy(vibe2);
  ...
  const travelTolerance = vibe2 === "lively" ? "tight" : vibe2 === "chill" || vibe2 === "cozy" ? "flexible" : "balanced";
  const structureRigidity = persona2 === "family" ? "tight" : persona2 === "friends" ? "flexible" : "balanced";
  const swapTolerance = persona2 === "friends" || vibe2 === "lively" || vibe2 === "playful" ? "high" : persona2 === "family" ? "low" : "medium";
  ...
  const certaintyPriority = persona2 === "family" ? "high" : "medium";
  const cityToken = normalizeConciergeIntentToken(city2);
  const id = `cintent_v0_1_${intentMode}_${controlMode}_${persona2}_${vibe2}_${cityToken}`;
  return {
    id,
    intentMode,
    objective: {
...
function normalizeExperienceContractVibe(vibe2) {
  if (vibe2 === "cozy" || vibe2 === "chill") {
    return "cozy";
  }
  if (vibe2 === "lively" || vibe2 === "playful") {
...
function buildExperienceContract(params) {
  const { persona: persona2, vibe: vibe2, conciergeIntent, selectedDirectionContext } = params;
  const normalizedVibe = normalizeExperienceContractVibe(vibe2);
  const template = EXPERIENCE_CONTRACT_V0_1_MATRIX[persona2][normalizedVibe];
  ...
  const id = `xcontract_v0_1_${persona2}_${normalizedVibe}`;
  return {
    id,
    persona: persona2,
    vibe: normalizedVibe,
...
function toFixed14(value) {
  return Number(clamp17(value, 0, 1).toFixed(3));
}
...
function hasTokenSignal(values, tokens2) {
  const normalized = values.map((value) => value.toLowerCase());
  return normalized.some((value) => tokens2.some((token) => value.includes(token)));
}
...
function getDirectionStrategyFeatures(entry) {
  ...
  const compactness = clamp17(1 - profile.radiusM / 2200, 0, 1);
  const walkability = clamp17(profile.coreSignals.walkability, 0, 1);
  const socialDensity = clamp17(
    mix.drinks + mix.activity + (ambiance.energy === "high" ? 0.14 : ambiance.energy === "medium" ? 0.05 : 0),
    0,
    1
  );
  const calmness = clamp17(
    (1 - toAmbianceNumeric(ambiance.energy)) * 0.52 + (1 - toAmbianceNumeric(ambiance.noise)) * 0.48,
    0,
    1
  );
  const intimacy = clamp17(toAmbianceNumeric(ambiance.intimacy), 0, 1);
  const culturalPotential = clamp17(
...
function getLaneDiversityScore(mix) {
  ...
  const activeLaneScore = clamp17((activeLaneCount - 1) / 4, 0, 1);
  return clamp17(activeLaneScore * 0.68 + (1 - maxLaneValue) * 0.32, 0, 1);
}
function getSignalThresholdPassScore(signalValue, threshold) {
  ...
  return clamp17(signalValue / threshold, 0, 1);
}
function evaluateSignalThresholds(signals, thresholds) {
  ...
  return {
    ...
    score: entries.length > 0 ? toFixed14(cumulativeScore / entries.length) : 0.5,
...
function getContractGateProfile(context, resolvedFamilyOverride) {
  ...
  const bump = (target, key, delta) => {
    ...
    target[key] = toFixed14(clamp17(current + delta, 0, 1));
  };
...
function getContractGateSignals(entry) {
  ...
  const categoryDiversity = clamp17(profile.coreSignals.categoryDiversity, 0, 1);
  const density = clamp17(profile.coreSignals.density, 0, 1);
  const momentPotential = clamp17(profile.tasteSignals.momentPotential, 0, 1);
  ...
  const nightlifeAdjacency = clamp17(
  ...
  const multiPeakCapability = clamp17(
    momentPotential * 0.5 + (mix.activity + mix.drinks) * 0.28 + categoryDiversity * 0.22,
    0,
    1
  );
  const experientialRichness = clamp17(
    momentPotential * 0.36 + categoryDiversity * 0.26 + laneDiversity * 0.18 + clamp17(tags.size / 7, 0, 1) * 0.2,
    0,
    1
  );
  const containedMovement = clamp17(features.compactness * 0.58 + features.walkability * 0.42, 0, 1);
  const softLandingCompatibility = clamp17(
    features.calmness * 0.5 + (1 - energy) * 0.2 + (mix.cafe + mix.dining) * 0.3,
    0,
    1
  );
  const scenicDestinationIntimacy = clamp17(features.scenicPotential * 0.62 + features.intimacy * 0.38, 0, 1);
  const thematicProgression = clamp17(
    (hasFlowSeed(profile.tasteSignals.momentSeeds) ? 0.3 : 0) + features.culturalPotential * 0.4 + categoryDiversity * 0.3,
    0,
    1
  );
  const groupThroughput = clamp17(
    features.socialDensity * 0.52 + features.distributedPotential * 0.28 + features.walkability * 0.2,
    0,
    1
  );
  const lateCapability = clamp17(
    nightlifeAdjacency * 0.66 + (hasLateSeed(profile.tasteSignals.momentSeeds) ? 0.34 : 0),
    0,
    1
  );
  const easyHangComfort = clamp17(
    features.calmness * 0.34 + features.intimacy * 0.2 + (mix.dining + mix.cafe) * 0.32 + containedMovement * 0.14,
    0,
    1
  );
  const moderateDensity = clamp17(1 - Math.abs(features.socialDensity - 0.56) * 1.9, 0, 1);
  const conversationalAnchorStrength = clamp17(
    features.culturalPotential * 0.38 + features.intimacy * 0.28 + features.calmness * 0.34,
    0,
    1
  );
  const exploratoryVariety = clamp17(
    features.explorationPotential * 0.48 + categoryDiversity * 0.26 + laneDiversity * 0.26,
    0,
    1
  );
  const multiAnchorEnrichment = clamp17(
    features.culturalPotential * 0.45 + exploratoryVariety * 0.35 + momentPotential * 0.2,
    0,
    1
  );
  const familyCompatibility = clamp17(
    features.familyPotential * 0.5 + features.calmness * 0.22 + containedMovement * 0.28,
    0,
    1
  );
  const resetRecoverySupport = clamp17(
    features.calmness * 0.52 + containedMovement * 0.28 + familyCompatibility * 0.2,
    0,
    1
  );
  const activeEngagement = clamp17(
    features.familyPotential * 0.42 + features.socialDensity * 0.24 + momentPotential * 0.34,
    0,
    1
  );
  const boundedEnergy = clamp17(
    (1 - Math.abs(energy - 0.58) * 1.8) * 0.58 + containedMovement * 0.42,
    0,
    1
  );
  const learningEnrichment = clamp17(features.culturalPotential * 0.62 + momentPotential * 0.38, 0, 1);
  const clusteredEnrichment = clamp17(learningEnrichment * 0.56 + containedMovement * 0.44, 0, 1);
  const decompressionSupport = clamp17(features.calmness * 0.56 + (1 - features.socialDensity) * 0.24 + containedMovement * 0.2, 0, 1);
  const lowMeaningFiller = clamp17(
  ...
  return {
    ...
    activationLevel: clamp17(energy * 0.42 + features.socialDensity * 0.4 + momentPotential * 0.18, 0, 1),
    nightlifeAdjacency,
    ...
    culturalAnchorStrength: clamp17(features.culturalPotential * 0.56 + momentPotential * 0.26 + mix.culture * 0.18, 0, 1),
    reflectiveContinuity: clamp17(features.calmness * 0.36 + features.culturalPotential * 0.34 + (hasFlowSeed(profile.tasteSignals.momentSeeds) ? 0.3 : 0), 0, 1),
    thematicProgression,
    groupThroughput,
    lateCapability,
    crawlBasecampCompatibility: clamp17(containedMovement * 0.46 + features.socialDensity * 0.34 + features.distributedPotential * 0.2, 0, 1),
    easyHangComfort,
    moderateDensity,
    ...
    flatCalmOnly: clamp17(features.calmness * 0.58 + (1 - momentPotential) * 0.42, 0, 1),
    utilitarianFoodCluster: clamp17((mix.dining + mix.cafe) * 0.54 + (1 - features.culturalPotential) * 0.22 + (1 - momentPotential) * 0.24, 0, 1),
    homogeneousLowContrast: clamp17((1 - categoryDiversity) * 0.5 + (1 - laneDiversity) * 0.3 + (1 - features.explorationPotential) * 0.2, 0, 1),
    loudDiffuseNightlife: clamp17(features.socialDensity * 0.42 + energy * 0.3 + noise * 0.16 + (1 - features.compactness) * 0.12, 0, 1),
    highChurnNightlife: clamp17(features.socialDensity * 0.4 + energy * 0.26 + features.explorationPotential * 0.22 + (1 - features.intimacy) * 0.12, 0, 1),
    overSocialDensity: features.socialDensity,
    genericNightlifeFirst: clamp17(nightlifeAdjacency * 0.56 + (1 - features.culturalPotential) * 0.44, 0, 1),
    lowMeaningFiller,
    quietIntimacyFirst: clamp17(features.calmness * 0.42 + features.intimacy * 0.34 + (1 - features.socialDensity) * 0.24, 0, 1),
    lowCarryMomentum: clamp17((1 - groupThroughput) * 0.7 + (1 - momentPotential) * 0.3, 0, 1),
    brittleHighPressureNightlife: clamp17(nightlifeAdjacency * 0.44 + energy * 0.2 + (1 - easyHangComfort) * 0.36, 0, 1),
    overFormalDestinationIntensity: clamp17(scenicDestinationIntimacy * 0.5 + (1 - moderateDensity) * 0.3 + (1 - groupThroughput) * 0.2, 0, 1),
    lowSubstanceSocialFiller: clamp17(features.socialDensity * 0.44 + (1 - features.culturalPotential) * 0.3 + (1 - momentPotential) * 0.26, 0, 1),
    nightlifeHeavyDistrict: nightlifeAdjacency,
    highFrictionSpread: clamp17((1 - features.compactness) * 0.54 + (1 - features.walkability) * 0.26 + features.explorationPotential * 0.2, 0, 1),
    adultNightlifePulse: clamp17(nightlifeAdjacency * 0.54 + (1 - familyCompatibility) * 0.46, 0, 1),
    chaoticLateEnergy: clamp17(lateCapability * 0.44 + energy * 0.24 + (1 - containedMovement) * 0.32, 0, 1),
    nightlifeFirstLowEnrichment: clamp17(nightlifeAdjacency * 0.56 + (1 - learningEnrichment) * 0.44, 0, 1)
  };
}
function evaluateContractGateDecision(params) {
  const { entry, gateProfile } = params;
  ...
  const gateAdjustedScore = toFixed14(
    clamp17(
      entry.score + allowed.score * (CONTRACT_GATE_BOOST_WEIGHT * 0.42) + preferred.score * (CONTRACT_GATE_BOOST_WEIGHT * 0.58) - suppressionScore * CONTRACT_GATE_SUPPRESSION_WEIGHT - (hardPassed ? 0 : 0.36),
      0,
      1
    )
  );
  ...
  return {
    ...
    suppressedScore: suppressionScore,
    hardPassed,
    reasonSummary: "contract_aligned",
    gateAdjustedScore,
    hardFailureReasons
  };
}
...
function buildContractGateWorld(input) {
  ...
  const decisionLog = contractAwareRanking.ranked.map((entry) => {
    ...
    return decision ?? {
      ...
      suppressedScore: 0,
      hardPassed: true,
      reasonSummary: "gate_not_evaluated",
      gateAdjustedScore: entry.score,
      hardFailureReasons: []
    };
  });
...
function includesVenueNameMatch(name, candidates2) {
  if (!candidates2 || candidates2.length === 0) {
    return false;
  }
  const normalizedName = normalizeToken(name);
  return candidates2.some((candidate) => {
...
function splitMeaningfulTokens(value) {
  return normalizeToken(value).split(" ").map((token) => token.trim()).filter((token) => token.length >= 4);
}
function hasPatternSignal(corpus, pattern) {
  ...
  const tokens2 = splitMeaningfulTokens(pattern);
  if (tokens2.length === 0) {
    return false;
  }
  let matched = 0;
  ...
  return matched >= Math.min(2, tokens2.length);
}
...
function getRoleKeywords(contract2, role) {
  const tokens2 = /* @__PURE__ */ new Set();
  contract2.stopRules.forEach((rule) => {
    const ruleRole = getStopRuleRole(rule);
    if (ruleRole !== role) {
      return;
    }
    splitMeaningfulTokens(rule.stopType).forEach((token) => tokens2.add(token));
    splitMeaningfulTokens(rule.purpose).forEach((token) => tokens2.add(token));
    rule.examples?.forEach(
      (example) => splitMeaningfulTokens(example).forEach((token) => tokens2.add(token))
    );
  });
...
function getPreferredVenueBoost(venueName, contract2, role) {
  const preferredBoost = contract2.selectionBias?.preferredVenueBoost ?? 0.14;
  let boost = 0;
  if (includesVenueNameMatch(venueName, contract2.anchorRules.stronglyPreferredVenues) || includesVenueNameMatch(venueName, contract2.anchorRules.allowedVenues)) {
    boost += preferredBoost;
  }
  if (role === "highlight" && includesVenueNameMatch(venueName, contract2.anchorRules.defaultPrimaryAnchors)) {
    boost += preferredBoost * 0.7;
  }
  if (includesVenueNameMatch(venueName, contract2.hiddenGemRules.preferredHiddenGemVenues) && contract2.hiddenGemRules.routeLevelHiddenGemBias) {
    boost += (contract2.selectionBias?.hiddenGemBoost ?? 0.08) * 0.8;
  }
  return boost;
}
function getMomentTypeBoost(stats, contract2) {
  if (!stats) {
    return 0;
  }
  const momentTypeBoosts = contract2.selectionBias?.momentTypeBoosts;
  if (!momentTypeBoosts) {
    return 0;
  }
...
function scoreRoleCandidate2(candidate, role, contract2, statsByVenueId, roleKeywords) {
  const stats = statsByVenueId.get(candidate.venueId);
  let score = candidate.score;
  score += contract2.selectionBias?.roleBoosts?.[role] ?? 0;
  score += getPreferredVenueBoost(candidate.venueName, contract2, role);
  score += getMomentTypeBoost(stats, contract2) * 0.42;
  if (contract2.hiddenGemRules.routeLevelHiddenGemBias && stats) {
    if (stats.discovery > 0 || stats.community > 0) {
      score += contract2.selectionBias?.hiddenGemBoost ?? 0.08;
    }
  }
  if (matchesRoleKeywords(`${candidate.venueName} ${candidate.reason}`, roleKeywords)) {
    score += 0.06;
  }
  if (includesVenueNameMatch(candidate.venueName, contract2.anchorRules.avoidVenues)) {
    score -= 0.24;
  }
  score -= getForbiddenPatternPenalty(
    `${candidate.venueName} ${candidate.reason}`,
    contract2.anchorRules.forbiddenPatterns
  );
  if (stats) {
...
function reshapeRoleCandidates(candidates2, role, contract2, statsByVenueId) {
  const roleKeywords = getRoleKeywords(contract2, role);
  return candidates2.map((candidate) => ({
    ...candidate,
    score: toFixed15(
      scoreRoleCandidate2(candidate, role, contract2, statsByVenueId, roleKeywords)
    )
  })).sort((left, right) => right.score - left.score || left.venueName.localeCompare(right.venueName));
}
function scoreMoment(moment, contract2) {
  const momentTypeBoosts = contract2.selectionBias?.momentTypeBoosts ?? {};
  ...
  if (includesVenueNameMatch(moment.title, contract2.anchorRules.stronglyPreferredVenues) || includesVenueNameMatch(moment.title, contract2.anchorRules.defaultPrimaryAnchors)) {
    score += (contract2.selectionBias?.preferredVenueBoost ?? 0.14) * 0.8;
  }
  if (contract2.hiddenGemRules.routeLevelHiddenGemBias && (moment.momentType === "discovery" || moment.momentType === "community")) {
    score += contract2.selectionBias?.hiddenGemBoost ?? 0.08;
  }
  if (includesVenueNameMatch(moment.title, contract2.anchorRules.avoidVenues)) {
    score -= 0.22;
  }
  score -= getForbiddenPatternPenalty(moment.reason, contract2.anchorRules.forbiddenPatterns);
  return clampScore(score);
}
function reshapeMoments(moments, contract2) {
  const combined = [...moments.primary, ...moments.secondary];
  if (combined.length === 0) {
    return moments;
  }
  const ranked = combined.slice().sort((left, right) => {
    const scoreDiff = scoreMoment(right, contract2) - scoreMoment(left, contract2);
    if (scoreDiff !== 0) {
      return scoreDiff;
    }
    if (right.strength !== left.strength) {
...
function deriveDiscoveryBalance(original, moments, contract2) {
  ...
  if (contract2.hiddenGemRules.routeLevelHiddenGemBias) {
    return "balanced";
  }
  return "familiar";
}
...
function applyScenarioContractToAggregation(aggregation, contract2) {
  if (!contract2) {
    return aggregation;
  }
  const nextMoments = reshapeMoments(aggregation.moments, contract2);
...
function getHospitalityScenarioContract(input) {
  ...
  return HOSPITALITY_SCENARIO_CONTRACTS.find(
    (contract2) => contract2.city === "San Jose" && contract2.persona === persona2 && contract2.vibe === vibe2
  ) ?? null;
}
...
function scoreSubset(cand, rec) {
  if (!cand.length || !rec.length) return 0;
  const set = new Set(rec);
  let overlap = 0;
  for (const t of cand) {
    if (set.has(t)) overlap++;
  }
  const cov = overlap / cand.length;
  if (overlap === 0) return 0;
  if (cand.length <= 2) return cov >= 0.5 ? cov : 0;
  if (overlap >= 2 && cov >= 0.5) return cov;
  return 0;
}
var city = "San Jose";
var persona = "romantic";
var vibe = "lively";
var districtPreviewResult = await buildDistrictOpportunityProfiles({ locationQuery: city, includeDebug: true });
var districtRecommendations = (await previewDistrictRecommendations({ persona, primaryVibe: vibe, city, distanceMode: "nearby" })).recommendedDistricts;
var bundle = buildCanonicalInterpretationBundle({ persona, vibe, city, planningMode: "engine-led", entryPoint: "direction_selection", hasAnchor: false });
var gate = buildContractGateWorld({ ranked: districtPreviewResult.ranked ?? [], context: { canonicalStrategyFamily: bundle.strategyFamily, canonicalStrategyFamilyResolution: bundle.strategyFamilyResolution, experienceContract: bundle.experienceContract, contractConstraints: bundle.contractConstraints }, source: "audit.join" });
var rankedSource = gate.contractAwareRanking.ranked.length > 0 ? gate.contractAwareRanking.ranked : districtPreviewResult?.ranked ?? [];
var recEntries = districtRecommendations.filter((r) => Boolean(r.tasteAggregation)).map((r) => ({ r, lookup: [.../* @__PURE__ */ new Set([...keys(r.label), ...keys(r.districtId)])], tok: [.../* @__PURE__ */ new Set([...tokens(r.label), ...tokens(r.districtId)])] }));
var recByKey = /* @__PURE__ */ new Map();
for (const e of recEntries) {
  for (const k of e.lookup) {
    if (!recByKey.has(k)) recByKey.set(k, e.r);
  }
}
var used = /* @__PURE__ */ new Set();
var candidates = rankedSource.slice(0, 6).map((entry) => {
  const p = entry.profile;
  const ckeys = [.../* @__PURE__ */ new Set([...keys(p.label), ...keys(p.pocketId), ...keys(p.meta.sourcePocketId)])];
  let rec = ckeys.map((k) => recByKey.get(k)).find(Boolean);
  if (!rec) {
    const ct = tokens(p.label);
    const best = recEntries.map((e) => ({ r: e.r, s: scoreSubset(ct, e.tok) })).filter((e) => e.s > 0).sort((a, b) => b.s - a.s || b.r.score - a.r.score || a.r.label.localeCompare(b.r.label))[0];
    rec = best?.r;
  }
  if (rec) used.add(rec.districtId);
  return { id: p.pocketId, label: p.label, rec };
});
var fallback = recEntries.map((e) => e.r).filter((r) => !used.has(r.districtId)).slice().sort((a, b) => b.score - a.score || a.label.localeCompare(b.label));
for (const c of candidates) {
  if (!c.rec) {
    const next = fallback.shift();
    if (next) {
      c.rec = next;
      used.add(next.districtId);
    }
  }
}
var contract = getHospitalityScenarioContract({ city, persona, vibe });
var stage = candidates.map((c) => {
  const agg = c.rec?.tasteAggregation ? applyScenarioContractToAggregation(c.rec.tasteAggregation, contract) : void 0;
  const hasAgg = Boolean(agg);
  const hasAnchor = Boolean(agg?.anchors.strongestHighlight?.venueId && agg?.anchors.strongestHighlight?.venueName);
  const hasSupport = (agg?.ingredients.startCandidates.length ?? 0) > 0 || (agg?.ingredients.windDownCandidates.length ?? 0) > 0;
  return { pocketId: c.id, label: c.label, attachedLabel: c.rec?.label, hasAgg, hasAnchor, hasSupport, anchor: agg?.anchors.strongestHighlight?.venueName, anchorId: agg?.anchors.strongestHighlight?.venueId };
});
var survivors = stage.filter((s) => s.hasAgg && s.hasAnchor && s.hasSupport);
var anchorKeys = /* @__PURE__ */ new Set();
var deduped = [];
var dedupeDropped = [];
for (const s of survivors) {
  const k = (s.anchorId || "").toLowerCase();
  if (!k) {
    dedupeDropped.push(s);
    continue;
  }
  if (!anchorKeys.has(k)) {
    anchorKeys.add(k);
