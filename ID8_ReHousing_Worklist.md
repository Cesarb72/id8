...
| Stage 2F-1 - Waypoint/Arc candidate-materialization identity parity | Waypoint consumes; Field/Interpretation/Bearings remain identity owners | `ScoredVenue` -> role pools -> Arc candidate assembly -> combination validation -> route selection before Application/protected handoff | Stage 2E globally closed at `99f9bed` | Characterize candidate-vs-physical identity semantics before correction: `candidateId` for wrapper competition/order/equality/diagnostics; `baseVenueId` for duplicate prevention, physical exclusion, and required-stop preservation; raw `candidate.venue.id` has no universal meaning without characterization. | Do not touch Application projection, routeAuthority, ContractEntryArtifact, Review/Lock, RuntimeRouteArtifact, Great Stop, LCE, Stage 2G, Stage 3, provider/hosted proof. | Behavior-woven; production correction deltas are not approved until characterization establishes current behavior. | Closed-valve characterization matrix: base only; base + Moment same base; base + activation same base; base + Moment + activation same base; presentation-similar distinct bases; required Build anchor as wrapper/base; same physical venue across role pools; distinct wrappers competing; provider provenance; reversed order; Build/Curate/Surprise parity. | Any need to alter scoring/ranking thresholds, Application behavior, protected route truth, or behavior beyond named identity-correction deltas. | Immediate next gate; characterization proof required first, implementation not authorized. |
...
## CURRENT NEXT
- **Stage 2F-1 closed-valve characterization proof:** characterize Waypoint/Arc candidate-materialization identity parity before any production correction. No Stage 2F-1 implementation, Stage 2F-2/Application work, A3/protected artifact work, provider/hosted proof, global A1/A2/A4 pass, or MVP-green claim is authorized by this row.
