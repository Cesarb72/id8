...
| Stage 1 - Interpretation venue identity resolution | complete | Interpretation | `src/domain/interpretation/venueIdentity/*`, focused offline proof | Static convergence, provider-only deterministic IDs, pending/ambiguous states, city portability, duplicate convergence, provider IDs provenance-only. |
...
| Stage 2B - Bearings admission | complete | Bearings | venue identity admission helper and proof | Resolved identities may be admitted; pending/ambiguous/provider-derived identities remain diagnostic-only; no routeAuthority or lock work. |
...
|---|---|---|---|---|---|---|---|---|---|
| Stage 2D - Build anchor ingress | Field -> Interpretation -> Bearings; Application consumes | `src/domain/providers/ProviderAdapter.ts` (`searchAnchorPlaces`), `src/app/services/buildAnchorOrchestrationService.ts`, `src/domain/providers/buildProviderSupplyDryRunHarness.ts` | Stage 2C closed | Direct Build anchor search cannot materialize `live_google_` or `providerRecordId` as route-bearing anchor identity; provider-only anchor candidates are resolved/admitted or diagnostic-only. | Do not touch `routeAuthority`, `RuntimeRouteArtifact`, Review/Lock, corpus, generated artifacts. | Likely behavior-changing for provider-only anchor candidates that previously surfaced with provider-derived IDs. | Mocked/offline anchor fixtures for static convergence, provider-only issuance, pending, ambiguity, and hostile provider ID; zero provider calls. | Any need to alter A3, lock authority, corpus, routing quality, or provider env. | Required before implementation. |
| Stage 2E - retrieval merge/dedupe/scoring | Field/Bearings identity preservation; Waypoint consumes | `src/domain/retrieval/scoreVenueFit.ts`, `retrieveVenues.ts`, `dedupeVenues.ts`, `mergeVenueSources.ts` | Stage 2D stable ingress | No route candidate enters scoring with provider-derived or unresolved identity; merge/dedupe preserves canonical identity and candidate evidence. | Do not touch routeAuthority, Review/Lock, RuntimeRouteArtifact, Great Stop criteria, Taste scoring thresholds. | Behavior-changing risk if dedupe currently prefers a live/static duplicate by non-identity heuristics. | Offline retrieval/scoring guards proving admitted identity only, same-place merge, distinct-place separation, no false MVP green. | Any scoring/ranking/quality change not attributable to identity preservation. | Required before implementation. |
| Stage 2F - Application and product-mode materialization | Application consumes; owners remain Field/Interpretation/Bearings/Waypoint | `src/app/services/curate/*`, `src/app/AppShell.tsx`, public-card, seed, sandbox-preview, Curate, Surprise, and Build materialization bridges | Stage 2D + Stage 2E | Application presents/selects resolved/admitted identity only; no App repair, synthesis, substitution, fallback canonicalization, or provider identity promotion. | Do not touch routeAuthority, RuntimeRouteArtifact, Review/Lock, Stage 3 corpus, Great Stop purity. | Behavior-changing where Application fallbacks currently mask missing engine identity. | Offline product-mode fixtures proving Curate/Surprise/Build materialization use admitted identity and retain diagnostic evidence. | Any product-mode difference that requires UX/product approval rather than identity-boundary cleanup. | Required before implementation. |
...
| A3 - routeAuthority / Review / Lock protected packet | Protected Application/lifecycle authority | `src/app/services/routeAuthority/routeAuthorityService.ts`, `src/domain/artifacts/runtimeRouteArtifact.ts`, `src/domain/artifacts/contractEntryArtifact.ts`, Review/Lock consumers | After A1/A2/A4 identity seams are proven | Provider/source/display fallbacks cannot become lock authority except through approved migration; routeAuthority remains separate from Stage 2D-2G. | Entire A3 scope is protected from ordinary Stage 2 cleanup. | High-risk behavior-changing lock authority work. | Dedicated lock-authority proof; RuntimeRouteArtifact and Review/Lock regression; no bundled cleanup. | Any attempt to bundle into Stage 2D-2G. | Required as separate packet. |
...
## NEXT (steps 2 and 3 of your plan)
