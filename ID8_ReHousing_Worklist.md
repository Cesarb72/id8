...
|---|---|---|---|---|---|---|---|---|---|
| Stage 2E-2 - scoring candidate-identity preservation | Waypoint consumes; scoring remains consumer only | `src/domain/retrieval/scoreVenueFit.ts` and scoring-side candidate projection | Stage 2E-1 complete at `6bcbf4e` | Scored route candidates preserve admitted venue identity and do not mint, repair, or substitute route identity; no provider-derived or unresolved identity enters candidate projection. | Do not touch routeAuthority, Review/Lock, RuntimeRouteArtifact, Great Stop criteria, Taste scoring thresholds, `hybridPortableAdapter.ts`, direct Application materialization, Stage 2F-2G, or Stage 3. | Scope design required before implementation; scoring/ranking/quality changes are held unless proven strictly identity-preservation-only. | Offline scoring candidate-identity guard proving no scoring dependence on provider provenance and no false MVP green. | Any ranking, quality, Taste, Waypoint, Application, hybrid portable, or route-authority behavior change outside identity preservation. | Immediate next scope-design boundary; implementation not authorized. |
