...
const DEFAULT_ANCHOR: NeighborhoodAnchor = {
...
const CITY_DEFAULT_ANCHORS: Record<string, NeighborhoodAnchor> = {
  'san jose': {
...
const CITY_NEIGHBORHOOD_ANCHORS: Record<string, Record<string, NeighborhoodAnchor>> = {
  'san jose': {
  downtown: {
    key: 'downtown-core',
    lat: 37.3384,
...
function normalizeNeighborhoodKey(value: string | undefined): string {
...
function toEntityType(category: VenueCategory): PlaceEntity['type'] {
  if (category === 'event') {
    return 'event'
  }
...
function buildDynamicCityAnchor(cityKey: string): NeighborhoodAnchor {
...
function buildDynamicNeighborhoodAnchor(
  cityKey: string,
...
function buildLocation(venue: Venue): {
  ...
  const dynamicNeighborhoodAnchor = neighborhoodKey
    ...
    : undefined
  const base =
    cityNeighborhoodAnchors[neighborhoodKey] ??
