...
export function haversineDistanceM(a: DistrictPoint, b: DistrictPoint): number {
...
export function centroidOf(points: DistrictPoint[]): DistrictPoint {
  if (points.length === 0) {
    return { lat: 0, lng: 0 }
  }
...
export function computeBoundingBoxMetrics(points: DistrictPoint[]): {
  widthM: number
  heightM: number
} {
  if (points.length <= 1) {
