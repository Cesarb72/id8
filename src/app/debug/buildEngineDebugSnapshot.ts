...
import {
  ...
  getScoredVenueTraceLabel,
  isHyperlocalActivationVariant,
  isMomentCandidate,
} from '../../domain/candidates/candidateIdentity'
import {
  getCanonicalDistanceStatus,
  isCandidateUsedByStretch,
  isCandidateWithinActiveDistanceWindow,
} from '../../domain/constraints/localStretchPolicy'
import {
  assessRomanticPersonaHighlightQualification,
  getRomanticPersonaHighlightType,
  isExpandedRomanticHighlightCandidate,
  isRomanticPersonaContractActive,
  requiresRomanticPersonaMoment,
  satisfiesRomanticPersonaHighlightContract,
} from '../../domain/contracts/romanticPersonaContract'
import type { ArcCandidate, ScoredVenue } from '../../domain/types/arc'
import type { ExperienceLens } from '../../domain/types/experienceLens'
import { getVibeLabel, type IntentProfile, type PersonaMode } from '../../domain/types/intent'
...
import type { IntentDraft } from '../state/sessionStore'
...
export interface EngineDebugSection {
  title: string
  entries: EngineDebugEntry[]
}

export interface EngineDebugSnapshot {
  summary: string
  sections: EngineDebugSection[]
}

function formatNullable(value: string | undefined): string {
...
function formatRole(role: UserStopRole | undefined): string {
  if (role === 'start') {
    return 'Start'
  }
  if (role === 'highlight') {
...
function formatInternalRole(role: ArcCandidate['stops'][number]['role']): string {
  if (role === 'warmup') {
    return 'Start'
  }
  if (role === 'peak') {
...
function formatRefinements(refinements: string[] | undefined): string {
  return refinements && refinements.length > 0 ? refinements.join(', ') : 'None'
}

function formatPersona(persona: PersonaMode | null | undefined): string {
  if (persona === 'romantic') {
    return 'Romantic'
  }
  if (persona === 'friends') {
...
function buildIntentSummary(intent: {
  primaryAnchor?: IntentProfile['primaryAnchor']
  secondaryAnchors?: IntentProfile['secondaryAnchors']
  persona?: IntentProfile['persona']
  mode?: IntentProfile['mode']
  planningMode?: IntentProfile['planningMode']
  distanceMode?: IntentProfile['distanceMode']
}): string {
...
function mapCategoryLabelToLane(value: string): string {
...
function formatExperienceFamily(value: string): string {
  return value.replace(/_/g, ' ')
}

function formatFamilyCompetitionWinnerMode(
  value: ArcCandidate['scoreBreakdown']['familyCompetitionWinnerMode'],
): string {
...
function formatExpressionWidth(
  value: ArcCandidate['scoreBreakdown']['expressionWidth'],
): string {
  if (value === 'broad') {
    return 'Broad'
  }
...
function formatExpressionReleaseSelectionMode(
  value: ArcCandidate['scoreBreakdown']['expressionReleaseSelectionMode'],
): string {
...
function summarizeDiscoveryDistribution(discoveryGroups?: DiscoveryDirection[]): string {
...
function summarizeRouteDistribution(itinerary: Itinerary): string {
  const categoryCounts = new Map<string, number>()
  const laneCounts = new Map<string, number>()
...
function summarizeArcArchetypeDistribution(arc: ArcCandidate): string {
...
function summarizeHighlightArchetype(arc: ArcCandidate): string {
...
function summarizeMomentPresence(arc: ArcCandidate): string {
  const highMomentStops = arc.stops.filter((stop) =>
...
function summarizeMomentBreakdown(arc: ArcCandidate): string {
  return arc.stops
    .map((stop) => {
...
function summarizeStrongMomentPresence(arc: ArcCandidate): string {
  return arc.scoreBreakdown.strongMomentPresent ? 'Yes' : 'No'
}

function summarizeMomentVariance(arc: ArcCandidate): string {
...
function summarizeMomentQualityNote(arc: ArcCandidate): string {
  return arc.scoreBreakdown.momentQualityNote ?? 'Moment layer not scored.'
}

function summarizeThemeSpread(arc: ArcCandidate): string {
  const alignedStops = arc.stops.filter(
    (stop) => stop.scoredVenue.taste.modeAlignment.score >= 0.48,
  ).length
  return `${alignedStops} / ${arc.stops.length}`
}

function summarizeThemeNote(arc: ArcCandidate): string {
  return arc.scoreBreakdown.themeSpreadNote ?? 'highlight-only alignment'
}

function summarizeRomanticSignalsSample(scoredVenues: ScoredVenue[]): string {
  const topCandidate = [...scoredVenues]
    .sort((left, right) => {
      return (
        right.taste.signals.romanticScore - left.taste.signals.romanticScore ||
...
function summarizeRoleEnergySummary(arc: ArcCandidate): string {
...
function summarizeRoleEnergyNote(arc: ArcCandidate): string {
  return arc.scoreBreakdown.roleEnergyNote ?? 'flat arc detected'
}

function formatRomanticMomentRole(
  role: ArcCandidate['scoreBreakdown']['romanticMomentRole'],
): string {
  if (!role || role === 'none') {
    return 'none'
  }
...
function summarizeRomanticContractOutcome(
  ...
  if (
    arc.surpriseInjection?.promotionOutcome === 'promoted_to_highlight' &&
    highlight &&
...
function summarizePersonaContractRequiresMoment(
  lens: ExperienceLens,
  intentProfile: IntentProfile,
): string {
...
function summarizePersonaContractPreferredHighlight(
  lens: ExperienceLens,
  intentProfile: IntentProfile,
): string {
  if (intentProfile.persona !== 'romantic' || lens.resolvedContract?.persona !== 'romantic') {
...
function summarizePersonaContractSatisfied(
  arc: ArcCandidate,
  lens: ExperienceLens,
  intentProfile: IntentProfile,
): string {
...
function summarizeContractBlendMode(lens: ExperienceLens): string {
  return lens.resolvedContract?.blendMode.replace(/_/g, ' ') ?? 'Not active'
}

function summarizeContractResolution(lens: ExperienceLens): string {
  return lens.resolvedContract?.resolutionSummary ?? 'No resolved contract'
}

function summarizeResolvedHighlightRequirements(lens: ExperienceLens): string {
  ...
  const momentRequirement = highlight.requiresMomentPresence
    ...
    : 'moment none'
  const types =
    highlight.preferredHighlightTypes.length > 0
...
function summarizeResolvedRoleExpectations(lens: ExperienceLens): string {
  ...
  const start =
    resolved.rolePreferences.start?.preferredCategories?.slice(0, 3).join('/') ?? 'baseline'
  const highlight =
    resolved.highlight.preferredCategories.slice(0, 3).join('/') || 'baseline'
  const windDown =
    resolved.rolePreferences.windDown?.preferredCategories?.slice(0, 3).join('/') ?? 'baseline'
  return `Start ${start} | Highlight ${highlight} | Wind Down ${windDown}`
}

function summarizeResolvedPacingExpectations(lens: ExperienceLens): string {
  const resolved = lens.resolvedContract
  if (!resolved) {
...
function buildRomanticContractEntries(
  arc: ArcCandidate,
  ...
  const strongestRejected = getStrongestRejectedRomanticCandidate(
    arc,
    scoredVenues,
    intentProfile,
    lens,
...
function buildLocalStretchEntries(
  arc: ArcCandidate,
  intentProfile: IntentProfile,
): EngineDebugEntry[] {
  return [
    ...
    },
    {
      ...
      value:
        arc.scoreBreakdown.localStretchCandidateSetBasis ??
    ...
    },
    {
      ...
      value:
        arc.scoreBreakdown.localSupplyDerivedFrom ??
    ...
    },
    {
      ...
      value:
        arc.scoreBreakdown.strictNearbyFailedDerivedFrom ??
...
function hasHighlightAnchorConflict(
...
function isHighlightRoleValidForRomanticContract(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
): boolean {
  return (
...
function isRomanticContractFeasible(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): boolean {
...
function getRomanticDistanceStatus(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
): string {
...
function getRomanticQualificationFailureReason(candidate: ScoredVenue): string {
...
function computeRomanticTracePriority(candidate: ScoredVenue): number {
  const assessment = assessRomanticPersonaHighlightQualification(candidate)
  return (
    computePeakMomentTraceScore(candidate) * 0.52 +
...
function isRelevantRomanticTraceCandidate(candidate: ScoredVenue): boolean {
  const assessment = assessRomanticPersonaHighlightQualification(candidate)
  return (
...
function getRomanticContractExcludedReason(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
  arc: ArcCandidate,
  lens: ExperienceLens,
  ...
  if (
    !isCandidateWithinActiveDistanceWindow(candidate, intentProfile, {
  ...
  if (
    !isHighlightRoleValidForRomanticContract(candidate, intentProfile)
  ) {
    return 'role invalid'
  }
  if (
...
function summarizeRomanticSignalBreakdown(candidate: ScoredVenue): string {
...
function summarizeRomanticQualificationExpansionState(
  scoredVenues: ScoredVenue[],
  lens: ExperienceLens,
): string {
  ...
  const expandedCount = scoredVenues.filter((candidate) =>
    isExpandedRomanticHighlightCandidate(candidate),
  ).length
  return expandedCount > 0 ? `Yes | ${expandedCount} expanded candidates` : 'No'
}

function summarizeExpandedRomanticCandidates(
  scoredVenues: ScoredVenue[],
  lens: ExperienceLens,
): string {
  if (!isRomanticPersonaContractActive(lens)) {
  ...
  const expanded = [...scoredVenues]
    ...
    .sort((left, right) => {
      return (
        computeRomanticTracePriority(right) - computeRomanticTracePriority(left) ||
        right.fitScore - left.fitScore
      )
    })
    .slice(0, 4)
...
function summarizeEligibleRomanticFamilies(
  scoredVenues: ScoredVenue[],
  lens: ExperienceLens,
): string {
  if (!isRomanticPersonaContractActive(lens)) {
  ...
  const qualifying = scoredVenues.filter((candidate) =>
  ...
  return summarizeTopFamilies(
    qualifying,
    (candidate) => candidate.taste.signals.experienceFamily,
  )
}

function buildRomanticCandidateTraceEntries(
  arc: ArcCandidate,
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
  ...
  return candidates.map((candidate, index) => {
    ...
    return {
      ...
      value:
        `${candidate.venue.name} | ${candidate.momentIdentity.type}/${candidate.momentIdentity.strength} | ` +
        `${formatMomentIntensity(candidate)} | ` +
        ...
        `usedByContractFeasibility ${formatYesNoCompact(
          isCandidateWithinActiveDistanceWindow(candidate, intentProfile, {
            allowMeaningfulStretch: true,
          }),
        )} | ` +
...
function getStrongestQualifyingRomanticCandidate(
  scoredVenues: ScoredVenue[],
  lens: ExperienceLens,
...
function getStrongestRejectedRomanticCandidate(
  arc: ArcCandidate,
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
  ...
  return [...scoredVenues]
    .filter(
      (candidate) =>
        isRelevantRomanticTraceCandidate(candidate) &&
...
function formatMomentScore(value: number): string {
  return value.toFixed(2)
}

function formatMomentIntensity(candidate: ScoredVenue): string {
  return `${candidate.taste.signals.momentIntensity.tier} ${candidate.taste.signals.momentIntensity.score.toFixed(2)}`
}

function formatMomentIntensityDrivers(candidate: ScoredVenue): string {
  return candidate.taste.signals.momentIntensity.drivers.join(' + ')
}

function formatMomentEnrichmentSignals(candidate: ScoredVenue): string {
  const signals = candidate.taste.signals.momentEnrichment.signals
  return signals.length > 0 ? signals.join(' + ') : 'no enriched signals'
}

function formatHyperlocalActivationType(value: string | undefined): string {
  return value ? value.replace(/_/g, ' ') : 'none'
}

function formatHyperlocalActivation(candidate: ScoredVenue): string {
...
function formatHyperlocalActivationImpact(candidate: ScoredVenue): string {
  const activation = candidate.taste.signals.hyperlocalActivation
  if (!activation.primaryActivationType) {
  ...
  const family =
    impact.familyRefinements.length > 0 ? impact.familyRefinements.join(' / ') : 'none'
...
function formatExperienceFamilyExpansion(candidate: ScoredVenue): string {
  ...
  return (
    `yes | ${formatExperienceFamily(candidate.taste.signals.baseExperienceFamily)} -> ` +
    `${formatExperienceFamily(candidate.taste.signals.experienceFamily)} | ` +
    `${candidate.taste.signals.experienceFamilyExpansionReason ?? 'activation changed family'}`
  )
}

function summarizeExpandedFamilyCandidates(scoredVenues: ScoredVenue[]): string {
  const expanded = [...scoredVenues]
    .filter((candidate) => candidate.taste.signals.experienceFamilyExpanded)
    .sort((left, right) => {
      return (
        right.taste.signals.hyperlocalActivation.intensityContribution -
  ...
  return expanded
    .map((candidate) => {
      return (
        `${getScoredVenueTraceLabel(candidate)} | ` +
...
function formatHyperlocalTemporalCompatibility(candidate: ScoredVenue): string {
  const activation = candidate.taste.signals.hyperlocalActivation
  if (!activation.primaryActivationType) {
  ...
  return (
    `${temporal.timePresenceState} | context ${contextWindow} -> activation ${activationWindow} | ` +
    `${formatTemporalRoleAdjustments(candidate)} | ${changed} | ${signals}`
  )
}

function formatMomentElevation(candidate: ScoredVenue): string {
  const eligible = candidate.taste.signals.isElevatedMomentCandidate ? 'yes' : 'no'
  return (
    `${eligible} | potential ${candidate.taste.signals.momentElevationPotential.toFixed(2)} | ` +
    `${candidate.taste.signals.momentElevationReason ?? 'no elevation signal'}`
  )
}

function summarizeActivationTemporalFit(scoredVenues: ScoredVenue[]): string {
  const activated = [...scoredVenues]
    ...
    .sort((left, right) => {
      ...
      return (
        Number(rightActivation.temporalCompatibility.materiallyChangesViability) -
  ...
  return activated
    .map((candidate) => {
      ...
      return (
        `${getScoredVenueTraceLabel(candidate)} | ${temporal.timePresenceState} | ` +
...
function getInjectedHyperlocalCandidates(scoredVenues: ScoredVenue[]): ScoredVenue[] {
  return scoredVenues.filter(isHyperlocalActivationVariant)
}

function getMomentCandidates(scoredVenues: ScoredVenue[]): ScoredVenue[] {
  return scoredVenues.filter(isMomentCandidate)
}

function formatMomentCandidateType(candidate: ScoredVenue): string {
  return candidate.candidateIdentity.momentType?.replace(/_/g, ' ') ?? 'unknown'
}

function formatMomentCandidateSource(candidate: ScoredVenue): string {
  return candidate.candidateIdentity.momentSourceType ?? 'unknown'
}

function summarizeMomentCandidateField(
  scoredVenues: ScoredVenue[],
  highlightPoolCandidateIds?: string[],
): string {
  ...
  return moments
    ...
    .map((candidate) => {
      ...
      return (
        `${candidate.venue.name} | ${formatMomentCandidateType(candidate)} | ` +
...
function summarizeMomentTypeDistribution(scoredVenues: ScoredVenue[]): string {
  const moments = getMomentCandidates(scoredVenues)
  if (moments.length === 0) {
    return 'n/a'
  }
...
function summarizeHyperlocalInjection(
  scoredVenues: ScoredVenue[],
  highlightPoolCandidateIds?: string[],
): string {
...
function summarizeHyperlocalInjectionReasons(scoredVenues: ScoredVenue[]): string {
  const injected = getInjectedHyperlocalCandidates(scoredVenues)
  if (injected.length === 0) {
    return 'n/a'
  }
  ...
  return injected
    ...
    .map((candidate) => {
      const activation = candidate.taste.signals.hyperlocalActivation
      return (
        `${candidate.venue.name} -> ${formatHyperlocalActivationType(
          candidate.candidateIdentity.activationType,
        )}` +
...
function formatYesNoCompact(value: boolean): string {
  return value ? 'yes' : 'no'
}

function getMomentStrengthWeight(strength: ScoredVenue['momentIdentity']['strength']): number {
  if (strength === 'strong') {
    return 1
  }
  if (strength === 'medium') {
...
function getMomentTypeWeight(type: ScoredVenue['momentIdentity']['type']): number {
  if (type === 'anchor') {
    return 1
  }
  if (type === 'explore') {
...
function computePeakMomentTraceScore(candidate: ScoredVenue): number {
...
function isHoursOk(candidate: ScoredVenue): boolean {
...
function isDistanceOk(candidate: ScoredVenue, intentProfile: IntentProfile): boolean {
  return isCandidateWithinActiveDistanceWindow(candidate, intentProfile, {
    allowMeaningfulStretch: true,
  })
}

function hasConstraintConflict(candidate: ScoredVenue): boolean {
...
function getAssignedRoleLabel(
  candidate: ScoredVenue,
  arc: ArcCandidate,
): 'highlight' | 'start' | 'wildcard' | 'unused' {
  const stop = arc.stops.find(
    (item) => getArcStopCandidateId(item) === getScoredVenueCandidateId(candidate),
  )
  if (!stop) {
    return 'unused'
  }
...
function getPeakTraceDecisionReason(params: {
  strongest: ScoredVenue
  strongestScore: number
  finalHighlight: ScoredVenue
  finalHighlightScore: number
  arc: ArcCandidate
  intentProfile: IntentProfile
}): string {
  if (
    getScoredVenueCandidateId(params.strongest) ===
  ...
  if (
    hasConstraintConflict(params.strongest) ||
  ...
  if (
    params.finalHighlight.taste.signals.momentIntensity.score >=
    params.strongest.taste.signals.momentIntensity.score + 0.04
  ) {
    return 'won on higher moment intensity'
  }
  if (
    params.finalHighlight.roleScores.peak >= params.strongest.roleScores.peak + 0.04 ||
...
function buildPeakTraceEntries(
  arc: ArcCandidate,
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
): EngineDebugEntry[] {
  ...
  const strongest = [...scoredVenues].sort((left, right) => {
    return (
      computePeakMomentTraceScore(right) - computePeakMomentTraceScore(left) ||
...
function buildSurprisePromotionEntries(arc: ArcCandidate): EngineDebugEntry[] {
...
function summarizeRedundancyPenalty(arc: ArcCandidate): string {
...
function summarizeFallbackSuppression(arc: ArcCandidate): string {
...
function summarizeFallbackSuppressionReason(arc: ArcCandidate): string {
  if (!(arc.scoreBreakdown.fallbackHighlightSignal ?? 0)) {
...
function summarizeSuppressedFallbackCandidates(scoredVenues: ScoredVenue[]): string {
  const suppressed = scoredVenues
    .filter((candidate) => candidate.taste.fallbackPenalty.applied)
    .sort(
      (left, right) =>
        right.taste.fallbackPenalty.appliedPenalty - left.taste.fallbackPenalty.appliedPenalty,
    )
    .slice(0, 3)
  ...
  return suppressed
    .map(
      (candidate) =>
        `${candidate.venue.name} -> ${candidate.taste.fallbackPenalty.strongerAlternativeName ?? 'stronger alternative'} (${candidate.taste.fallbackPenalty.appliedPenalty.toFixed(2)})`,
    )
    .join(' | ')
}

function getHighlightSampleCandidates(scoredVenues: ScoredVenue[], limit = 24): ScoredVenue[] {
  return [...scoredVenues]
    .sort((left, right) => {
      return (
        right.roleScores.peak - left.roleScores.peak ||
...
function isDebugHighlightFeasibleCandidate(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
): boolean {
  return (
...
function satisfiesResolvedHighlightContract(candidate: ScoredVenue, lens: ExperienceLens): boolean {
  if (!requiresRomanticPersonaMoment(lens)) {
    return true
  }
...
function isLowIntensityHighlightCandidate(candidate: ScoredVenue): boolean {
  return (
...
function summarizeTopFamilies(
  candidates: ScoredVenue[],
  selector: (candidate: ScoredVenue) => string,
  limit = 3,
): string {
  if (candidates.length === 0) {
    return 'None'
...
function summarizeDistributionRecord(
  distribution: Partial<Record<string, number>> | undefined,
  limit = 4,
): string {
...
function summarizeDominantFamily(
  distribution: Partial<Record<string, number>> | undefined,
): string {
  if (!distribution || Object.keys(distribution).length === 0) {
    return 'No'
...
function summarizeFamilyCompetitionState(arc: ArcCandidate): string {
  if (!arc.scoreBreakdown.familyCompetitionActive) {
    return `No | ${formatFamilyCompetitionWinnerMode(
      arc.scoreBreakdown.familyCompetitionWinnerMode,
    )}`
  }

  return `Yes | ${formatFamilyCompetitionWinnerMode(
    arc.scoreBreakdown.familyCompetitionWinnerMode,
  )}`
}

function summarizeFamilyCompetitionFamilies(arc: ArcCandidate): string {
...
function summarizeFamilyCompetitionSpread(arc: ArcCandidate): string {
...
function summarizeExpressionWidth(arc: ArcCandidate): string {
  return formatExpressionWidth(arc.scoreBreakdown.expressionWidth)
}

function summarizeExpressionWidthBasis(
  arc: ArcCandidate,
  generationTrace: GenerationTrace,
): string {
  const familyCount = arc.scoreBreakdown.expressionWidthFamilyCount ?? 0
  const competitiveFamilyCount =
    arc.scoreBreakdown.expressionWidthCompetitiveFamilyCount ?? 0
...
function summarizeExpressionWidthOutcome(arc: ArcCandidate): string {
...
function summarizeExpressionReleaseEligibility(arc: ArcCandidate): string {
...
function summarizeExpressionReleaseEliteSet(arc: ArcCandidate): string {
  ...
  return names
    .map(
      (name, index) =>
        `${name} | ${formatExperienceFamily(families[index] ?? 'unknown')} | ${lanes[index]?.replace(/_/g, ' / ') ?? 'unknown'}`,
    )
    .join(' ; ')
}

function summarizeExpressionReleaseSelectionMode(arc: ArcCandidate): string {
  return formatExpressionReleaseSelectionMode(
    arc.scoreBreakdown.expressionReleaseSelectionMode,
  )
}

function summarizeExpressionReleaseDecision(arc: ArcCandidate): string {
...
function summarizeActivationMomentElevationEligibility(arc: ArcCandidate): string {
...
function summarizeActivationMomentElevationCandidates(arc: ArcCandidate): string {
...
function summarizeActivationMomentElevationApplied(arc: ArcCandidate): string {
  ...
  return (
    `${applied} | ${winnerElevated} | ${topCandidate} | ` +
    `potential ${(topPotential ?? 0).toFixed(2)}`
  )
}

function summarizeEliteFieldDiversification(arc: ArcCandidate): string {
...
function summarizeEliteFieldLanes(arc: ArcCandidate): string {
...
function summarizeEliteFieldLaneCandidates(arc: ArcCandidate): string {
...
function buildCandidateShapingEntries(params: {
  ...
  const qualifying = sample.filter(
    (candidate) =>
      satisfiesResolvedHighlightContract(candidate, params.lens) &&
  ...
  return [
    ...
    },
    {
      ...
      value: `${getMomentCandidates(params.scoredVenues).length} | ${summarizeMomentTypeDistribution(
        params.scoredVenues,
      )}`,
    },
    {
      label: 'Moment highlight pool',
      value: summarizeMomentCandidateField(
        params.scoredVenues,
        highlightDiagnostics?.rolePoolCandidateIds,
      ),
    },
    {
    ...
    },
    {
      ...
      value:
        highlightDiagnostics?.contractRelaxed && highlightDiagnostics.contractFallbackReason
    ...
    },
    {
      ...
      value:
        highlightDiagnostics?.familyDistribution && Object.keys(highlightDiagnostics.familyDistribution).length > 0
    ...
    },
    {
      ...
      value: summarizeEnrichmentPromotedCandidates(
        params.scoredVenues,
        params.intentProfile,
        params.lens,
      ),
    },
    {
      label: 'Activation layer',
      value: summarizeActivationPromotedCandidates(
        params.scoredVenues,
        params.intentProfile,
        params.lens,
      ),
    },
    {
      ...
      value: summarizeHyperlocalInjection(
        params.scoredVenues,
        highlightDiagnostics?.rolePoolCandidateIds,
      ),
    },
    ...
    },
    {
      ...
      value: summarizeEnrichmentExpandedFamilies(
        params.scoredVenues,
        params.intentProfile,
        params.lens,
      ),
    ...
    },
    {
      ...
      value: summarizeTopFamilies(
        qualifying,
        (candidate) => candidate.taste.signals.primaryExperienceArchetype,
      ),
    },
  ]
}

function inferSelectedDirection(
  discoveryGroups: DiscoveryDirection[] | undefined,
  selectedVenueIds: string[],
  fallbackVenueIds: string[] = [],
): DiscoveryDirection | undefined {
  if (!discoveryGroups || discoveryGroups.length === 0) {
    return undefined
  }
  ...
  return [...discoveryGroups]
    .map((direction, index) => {
      const candidateIds = direction.groups.flatMap((group) =>
        group.candidates.map((candidate) => candidate.venueId),
      )
...
function buildDirectionArcSummary(direction?: DiscoveryDirection): string {
  if (!direction) {
...
function buildDraftArcSummary(itinerary: Itinerary): string {
  return itinerary.stops.map((stop) => `${stop.title}: ${stop.venueName}`).join(' | ')
}

function summarizeConstraintTrace(trace?: GenerationTrace['constraintTrace']): string {
...
function buildTasteEntries(
  lens: ExperienceLens | undefined,
  distribution: string,
): EngineDebugEntry[] {
  ...
  return [
    ...
    },
    {
      ...
      value:
        tasteMode && tasteMode.discouragedLanes.length > 0
...
function summarizeCandidateAlignment(scoredVenues: ScoredVenue[]): string {
...
function summarizeAlignedLanes(scoredVenues: ScoredVenue[]): string {
  const sample = scoredVenues.slice(0, 24)
  if (sample.length === 0) {
    return 'No scored candidates'
  }
...
function summarizeFinalArcAlignment(arc: ArcCandidate): string {
  const alignedStops = arc.stops.filter((stop) => stop.scoredVenue.taste.modeAlignment.score >= 0.48)
  const strongAlignedStops = arc.stops.filter(
    (stop) =>
      stop.scoredVenue.taste.modeAlignment.score >= 0.68 ||
  ...
  return `Aligned stops ${alignedStops.length} | strong ${strongAlignedStops.length} | light ${lightAlignedStops} | score ${(
    arc.scoreBreakdown.tasteModeAlignmentScore ?? 0
  ).toFixed(2)}`
}

function summarizeAlignmentAvailability(arc: ArcCandidate): string {
...
function summarizeAlignmentSatisfiedStops(arc: ArcCandidate): string {
  const alignedStops = arc.stops.filter(
    (stop) => stop.scoredVenue.taste.modeAlignment.score >= 0.48,
  )
...
function buildAlignmentOutcomeNote(arc: ArcCandidate): string {
  const availableAligned = arc.scoreBreakdown.tasteModeAvailableAlignedCount ?? 0
  const availableStrong = arc.scoreBreakdown.tasteModeAvailableStrongCount ?? 0
...
function computeHighlightDecisionPriority(
  candidate: ScoredVenue,
  lens: ExperienceLens,
  intentProfile: IntentProfile,
): number {
  return (
    computePeakMomentTraceScore(candidate) +
...
function getHighlightAlternativeLossReason(
  candidate: ScoredVenue,
  finalHighlight: ScoredVenue,
  arc: ArcCandidate,
  intentProfile: IntentProfile,
...
function summarizeWinningDrivers(
  finalHighlight: ScoredVenue,
  lens: ExperienceLens,
  arc?: ArcCandidate,
): string {
  ...
  if (
    arc?.scoreBreakdown.familyCompetitionActive &&
    (arc.scoreBreakdown.familyCompetitionEligibleFamilies ?? []).includes(
      finalHighlight.taste.signals.experienceFamily,
    )
  ) {
    drivers.push('won competitive family field')
  }
  if (
    arc?.scoreBreakdown.expressionReleaseEligible &&
...
function getTopQualifyingHighlightCandidates(
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): ScoredVenue[] {
  return [...scoredVenues]
    ...
    .sort((left, right) => {
      return (
        computeHighlightDecisionPriority(right, lens, intentProfile) -
...
function isEnrichmentPromotedHighlightCandidate(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): boolean {
...
function isActivationPromotedHighlightCandidate(
  candidate: ScoredVenue,
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): boolean {
  const activation = candidate.taste.signals.hyperlocalActivation
  return (
    Boolean(activation.primaryActivationType) &&
...
function summarizeEnrichmentPromotedCandidates(
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): string {
  const promoted = [...scoredVenues]
    .filter((candidate) =>
      isEnrichmentPromotedHighlightCandidate(candidate, intentProfile, lens),
    )
    .sort((left, right) => {
      return (
        right.taste.signals.momentEnrichment.highlightSurfaceBoost -
  ...
  return promoted
    .map(
      (candidate) =>
        `${candidate.venue.name} | ${formatExperienceFamily(candidate.taste.signals.experienceFamily)} | boost ${candidate.taste.signals.momentEnrichment.highlightSurfaceBoost.toFixed(2)}`,
    )
    .join(' ; ')
}

function summarizeEnrichmentExpandedFamilies(
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): string {
  const promoted = scoredVenues.filter((candidate) =>
    isEnrichmentPromotedHighlightCandidate(candidate, intentProfile, lens),
  )
  if (promoted.length === 0) {
...
function summarizeActivationPromotedCandidates(
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): string {
  const promoted = [...scoredVenues]
    .filter((candidate) =>
      isActivationPromotedHighlightCandidate(candidate, intentProfile, lens),
    )
    .sort((left, right) => {
      return (
        right.taste.signals.hyperlocalActivation.intensityContribution -
...
function getStrongestBlockedHighlightCandidate(
  scoredVenues: ScoredVenue[],
  intentProfile: IntentProfile,
  lens: ExperienceLens,
): ScoredVenue | undefined {
...
function buildHighlightDecisionEntries(params: {
  ...
  const strongest = [...params.scoredVenues].sort((left, right) => {
    return (
      computeHighlightDecisionPriority(right, params.lens, params.intentProfile) -
        computeHighlightDecisionPriority(left, params.lens, params.intentProfile) ||
      right.fitScore - left.fitScore
    )
  })[0]
  const topQualifying = getTopQualifyingHighlightCandidates(
    params.scoredVenues,
    params.intentProfile,
    params.lens,
  )
  const blocked = getStrongestBlockedHighlightCandidate(
    params.scoredVenues,
    params.intentProfile,
    params.lens,
  )
  ...
  return [
    ...
    },
    {
      label:
        ...
        getScoredVenueCandidateId(strongest) ===
          ...
          : 'Why strongest lost',
      value: strongest
        ? getHighlightAlternativeLossReason(
            strongest,
            finalHighlight,
            params.arc,
            params.intentProfile,
    ...
    ...topQualifying.map((candidate, index) => ({
      label: `Top ${index + 1}`,
      value:
        `${candidate.venue.name} | ${formatExperienceFamily(candidate.taste.signals.experienceFamily)} | ${candidate.taste.signals.primaryExperienceArchetype} | ` +
...
function buildExpressionCompressionEntries(params: {
  ...
  const families = new Set(
    sample.slice(0, 8).map((candidate) => candidate.taste.signals.experienceFamily),
  )
  ...
  const dominantShare =
    dominantLane && sample.slice(0, 8).length > 0
  ...
  return [
    ...
    },
    {
      label: 'Width basis',
      value:
        params.arc.scoreBreakdown.expressionWidthReason ??
    ...
    },
    {
      ...
      value:
        typeof params.arc.scoreBreakdown.expressionWidthIntensitySpread === 'number'
    ...
    },
    {
      label: 'Dominant lane',
      value:
        dominantLane && dominantShare >= 0.6
...
function summarizeFinalFallbackRoles(arc: ArcCandidate): string {
  const fallbackStops = arc.stops.filter(
    (stop) =>
      stop.scoredVenue.taste.fallbackPenalty.signalScore > 0 &&
...
function buildFallbackTraceEntries(params: {
  ...
  const suppressedCount = params.scoredVenues.filter(
    (candidate) => candidate.taste.fallbackPenalty.applied,
  ).length
  return [
    {
...
function buildFinalArcPreservationEntries(params: {
  ...
  const strongestRole =
    strongest && finalHighlight ? getAssignedRoleLabel(strongest, params.arc) : 'unused'
  const contractPreserved =
    !requiresRomanticPersonaMoment(params.lens) ||
  ...
  return [
    ...
    },
    {
      ...
      value:
        strongest && strongestRole !== 'unused'
          ? `Yes | ${strongestRole}`
          : 'No',
    },
    {
...
export function buildExploreDebugSnapshot(params: {
  ...
  const selectedDirection = inferSelectedDirection(
    params.discoveryGroups,
    params.selectedVenueIds,
  )
  ...
  return {
    ...
    sections: [
        ...
        entries: [
          {
            ...
            value: params.intentDraft.primaryVibe
              ? `${getVibeLabel(params.intentDraft.primaryVibe)}${
                  params.intentDraft.secondaryVibe
          ...
          },
          {
            ...
            value:
              params.lens?.interpretation?.personaEffectSummary ??
              'Waiting for a primary vibe before interpretation runs.',
          },
          {
            label: 'Derived traits',
            value: `Refinements: ${formatRefinements(
              params.intentProfile?.refinementModes,
            )} | Hidden gems: ${formatYesNo(
              params.intentProfile?.prefersHiddenGems ?? params.intentDraft.prefersHiddenGems,
            )}`,
          },
        ],
      },
      {
        title: 'Taste',
        entries: buildTasteEntries(
          params.lens,
          summarizeDiscoveryDistribution(params.discoveryGroups),
        ),
      },
      {
        title: 'District',
      ...
      },
      {
        ...
        entries: [
          ...
          },
          {
            ...
            value: `Distance: ${params.intentDraft.distanceMode} | Planning: ${
              params.intentProfile?.planningMode ?? params.intentDraft.planningMode ?? 'engine-led'
            }`,
          },
        ],
      },
...
export function buildDraftDebugSnapshot(params: {
  ...
  const selectedDirection = inferSelectedDirection(
    params.discoveryGroups,
    params.selectedVenueIds,
    params.itinerary.stops.map((stop) => stop.venueId),
  )
  ...
  return {
    ...
    sections: [
        ...
        entries: [
          {
            ...
            value: `${getVibeLabel(params.intentProfile.primaryAnchor)}${
              params.intentProfile.secondaryAnchors?.[0]
          ...
          },
          {
            ...
            value: `Refinements: ${formatRefinements(
              params.intentProfile.refinementModes,
            )} | Hidden gems: ${formatYesNo(params.intentProfile.prefersHiddenGems)}`,
          },
        ],
      },
      {
        ...
        ? [
            {
              ...
              entries: buildRomanticContractEntries(
                params.arc,
                params.scoredVenues,
                params.intentProfile,
                params.lens,
            ...
            },
            {
              ...
              entries: buildRomanticCandidateTraceEntries(
                params.arc,
                params.scoredVenues,
                params.intentProfile,
                params.lens,
      ...
      {
        title: 'Local Stretch',
        entries: buildLocalStretchEntries(
          params.arc,
          params.intentProfile,
        ),
      },
      {
        ...
        entries: [
          {
            label: 'City / Area hint',
            value: `${params.intentProfile.city} / ${formatNullable(
              params.intentProfile.neighborhood,
            )}`,
          },
          {
          ...
          },
          {
            label: 'Pocket spread',
            value:
              itineraryNeighborhoods.length > 1
