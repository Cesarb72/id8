...
import type { PreviewControls } from '../../domain/types/previewControls'
...
export type GenerationTarget = 'preview' | 'final'
...
export interface UserComposedStop {
  ownedStopId: string
  role: UserStopRole
  label: string
  venue: Venue
  kind: 'candidate' | 'custom'
  sourceAction: 'replace-stop' | 'add-before' | 'add-after'
}

export interface SessionState {
  currentStep: FlowStep
  generationTarget: GenerationTarget
  activeEditor?: 'crew' | 'mood'
  mode: ExperienceMode | null
  intentDraft: IntentDraft
  selectedStarterPackId?: string
  districtRecommendations?: DistrictRecommendation[]
  topDistrictId?: string
  districtSelectionLoading: boolean
...
type SessionAction =
  ...
  | { type: 'SET_GENERATION_TARGET'; payload: GenerationTarget }
  | { type: 'SET_ACTIVE_EDITOR'; payload?: 'crew' | 'mood' }
  | { type: 'SET_MODE'; payload: ExperienceMode }
  | { type: 'PATCH_INTENT_DRAFT'; payload: Partial<IntentDraft> }
  | { type: 'PATCH_PREVIEW_CONTROLS'; payload: Partial<PreviewControls> }
  | { type: 'SELECT_STARTER_PACK'; payload?: string }
  | { type: 'BEGIN_DISTRICT_PREVIEW' }
  | {
  ...
  | { type: 'CLEAR_DISTRICT_PREVIEW' }
  | { type: 'BEGIN_DISCOVERY_PREVIEW' }
  ...
  | {
      ...
      payload: Partial<Record<UserStopRole, UserComposedStop>>
    }
  | { type: 'SET_ROUTE_EDITED_BY_USER'; payload: boolean }
  | { type: 'SET_COMPOSITION_CONFLICT_MESSAGE'; payload?: string }
  | {
      type: 'SET_STOP_ALTERNATIVES'
      payload: {
        role: UserStopRole
        alternatives: StopAlternative[]
        kind: StopAlternativeKind
      }
    }
  | {
  ...
  | { type: 'CLEAR_STOP_ALTERNATIVES'; payload?: UserStopRole }
  | { type: 'SET_REFINEMENTS'; payload: RefinementMode[] }
  | { type: 'LOCK_PLAN' }
  | { type: 'RESET' }

const initialIntentDraft: IntentDraft = {
  persona: null,
  primaryVibe: null,
  city: 'San Jose',
  distanceMode: 'nearby',
...
function sessionReducer(state: SessionState, action: SessionAction): SessionState {
  if (action.type === 'SET_STEP') {
    ...
    return {
      ...
      activeEditor:
        action.payload === 'crew' || action.payload === 'mood'
          ? state.activeEditor
          : undefined,
    }
  }
  ...
  if (action.type === 'SET_DISTRICT_PREVIEW') {
    return {
      ...
      previewControls: {
        ...state.previewControls,
        districtPreference:
          action.payload.selectedDistrictId ?? state.previewControls.districtPreference,
      },
...
interface SessionActions {
  ...
  setActiveEditor: (step?: 'crew' | 'mood') => void
  setMode: (mode: ExperienceMode) => void
  patchIntentDraft: (patch: Partial<IntentDraft>) => void
  patchPreviewControls: (patch: Partial<PreviewControls>) => void
  selectStarterPack: (packId?: string) => void
  beginDistrictPreview: () => void
  setDistrictPreview: (
    recommendations: DistrictRecommendation[],
    topDistrictId?: string,
    selectedDistrictId?: string,
  ) => void
  clearDistrictPreview: () => void
  beginDiscoveryPreview: () => void
  ...
  setArcAndItinerary: (itinerary: Itinerary, arc: ArcCandidate) => void
  setUserComposedStops: (
    stopsByRole: Partial<Record<UserStopRole, UserComposedStop>>,
  ) => void
  setRouteEditedByUser: (value: boolean) => void
  setCompositionConflictMessage: (message?: string) => void
  setStopAlternatives: (
    role: UserStopRole,
    alternatives: StopAlternative[],
    kind: StopAlternativeKind,
  ) => void
  setTraceAlternativeCount: (
    role: UserStopRole,
    count: number,
    kind: 'swap' | 'nearby',
  ) => void
  clearStopAlternatives: (role?: UserStopRole) => void
  setRefinements: (refinements: RefinementMode[]) => void
  lockPlan: () => void
  reset: () => void
}
...
function buildActions(dispatch: Dispatch<SessionAction>): SessionActions {
  return {
    setStep(step) {
