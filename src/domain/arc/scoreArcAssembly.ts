...
import { getRolePoolForRole, type RolePools } from './buildRolePools'
import type { ArcCandidate, ArcScoreBreakdown, ArcStop } from '../types/arc'
...
function isHospitalityArchetype(archetype: ReturnType<typeof getPrimaryExperienceArchetype>): boolean {
...
function getPeakMomentStrengthWeight(
  strength: ArcStop['scoredVenue']['momentIdentity']['strength'],
): number {
  if (strength === 'strong') {
    return 1
  }
...
function getPeakMomentTypeWeight(
  type: ArcStop['scoredVenue']['momentIdentity']['type'],
): number {
  if (type === 'anchor') {
    return 1
  }
...
function computePeakMomentLeadScore(candidate: ArcStop['scoredVenue']): number {
...
function getUniqueRolePoolCandidates(rolePools?: RolePools): ArcStop['scoredVenue'][] {
  if (!rolePools) {
    return []
  }
  ...
  return [
    ...new Map(
      [
        ...
        ...rolePools.wildcard,
        ...rolePools.cooldown,
      ].map((candidate) => [getScoredVenueCandidateId(candidate), candidate] as const),
    ).values(),
  ]
}
...
function computeRomanticMomentLeadScore(candidate: ArcStop['scoredVenue']): number {
  const archetype = candidate.taste.signals.primaryExperienceArchetype
  const experientialArchetypeLift =
...
function isFeasibleRomanticMomentCandidate(
  candidate: ArcStop['scoredVenue'],
  intent: IntentProfile,
): boolean {
  ...
  const wildcardFeasible =
    candidate.roleScores.wildcard >= 0.57 && candidate.stopShapeFit.surprise >= 0.44

  return highlightFeasible || warmupFeasible || wildcardFeasible
}

function isFeasibleRomanticHighlightCandidate(
  candidate: ArcStop['scoredVenue'],
  lens: ExperienceLens,
  intent: IntentProfile,
): boolean {
...
function isGenericHospitalityHighlightCandidate(candidate: ArcStop['scoredVenue']): boolean {
  const archetype = candidate.taste.signals.primaryExperienceArchetype
  return (
    archetype === 'dining' ||
    archetype === 'drinks' ||
...
function getHighlightExperienceFamily(candidate: ArcStop['scoredVenue']): string {
  return candidate.taste.signals.experienceFamily
}

function computeFamilyCompetitionLeadScore(candidate: ArcStop['scoredVenue']): number {
  return (
    computePeakMomentLeadScore(candidate) +
...
function isFeasibleFamilyCompetitionHighlightCandidate(
  candidate: ArcStop['scoredVenue'],
  intent: IntentProfile,
  lens: ExperienceLens,
): boolean {
...
interface FamilyCompetitionFieldEntry {
  family: string
  candidate: ArcStop['scoredVenue']
  score: number
  poolCount: number
...
interface FamilyCompetitionField {
  finalHighlight?: ArcStop['scoredVenue']
  competitionCandidates: ArcStop['scoredVenue'][]
  familyEntries: FamilyCompetitionFieldEntry[]
  threshold: number
  intensitySpread: number
  peakPoolSize: number
}

interface DiversifiedEliteFieldEntry {
  lane: EliteExperienceLane
  family: string
  candidate: ArcStop['scoredVenue']
  score: number
}

function buildFamilyCompetitionField(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
): FamilyCompetitionField {
  const finalHighlight = stops.find((stop) => stop.role === 'peak')?.scoredVenue
  ...
  const competitionCandidates = peakPool.filter((candidate) =>
    isFeasibleFamilyCompetitionHighlightCandidate(candidate, intent, lens),
  )
  ...
  const familyLeaders = new Map<string, ArcStop['scoredVenue']>()
  for (const candidate of competitionCandidates) {
    const family = getHighlightExperienceFamily(candidate)
...
function getEliteExperienceLane(candidate: ArcStop['scoredVenue']): EliteExperienceLane {
...
function buildDiversifiedEliteField(
  field: FamilyCompetitionField,
  width: ReturnType<typeof computeExpressionWidthFromField>,
  lens: ExperienceLens,
): {
  entries: DiversifiedEliteFieldEntry[]
  applied: boolean
  reason: string
  detectedLanes: EliteExperienceLane[]
  laneCandidates: string[]
} {
  ...
  const romanticRichField =
    requiresRomanticPersonaMoment(lens) &&
    width.classification === 'moderate' &&
    field.peakPoolSize >= 4 &&
    field.familyEntries.length >= 3
  const eliteScoreThreshold = Math.min(
    field.threshold,
    width.classification === 'broad' ? 0.05 : romanticRichField ? 0.06 : 0.04,
  )
  const eliteIntensityThreshold =
    width.classification === 'broad' ? 0.06 : romanticRichField ? 0.06 : 0.05
  ...
  const nearBestCandidates = [...field.competitionCandidates]
    .filter((candidate) => {
      ...
      return (
        bestScore - score <= eliteScoreThreshold &&
  ...
  const laneLeaderMap = new Map<EliteExperienceLane, DiversifiedEliteFieldEntry>()
  ...
  const laneEntries = [...laneLeaderMap.values()].sort(
    (left, right) => right.score - left.score || left.lane.localeCompare(right.lane),
  )
  const detectedLanes = laneEntries.map((entry) => entry.lane)
  const laneCandidates = laneEntries.map(
    (entry) => `${entry.lane} => ${getScoredVenueTraceLabel(entry.candidate)}`,
  )
  ...
  const fallbackEntries = field.familyEntries
    .filter(
      (entry) =>
        bestEntry.score - entry.score <= eliteScoreThreshold &&
...
function computeFamilyCompetitionPressure(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  topSpread: number
  threshold: number
  winnerMode: NonNullable<ArcScoreBreakdown['familyCompetitionWinnerMode']>
} {
  ...
  const competingFamilies = field.familyEntries.filter(
    (entry) => bestEntry.score - entry.score <= field.threshold,
  )
  ...
  const dominantPoolEntry = [...new Map(field.familyEntries.map((entry) => [entry.family, entry.poolCount] as const)).entries()].sort(
    (left, right) => right[1] - left[1] || left[0].localeCompare(right[0]),
  )[0]
  const dominantPoolFamily = dominantPoolEntry?.[0]
  const dominantPoolShare =
    dominantPoolEntry && field.competitionCandidates.length > 0
  ...
  const penalty =
    dominantPoolFamily === finalFamily &&
...
function computeExpressionWidthFromField(
  field: FamilyCompetitionField,
  lens: ExperienceLens,
  rolePools?: RolePools,
): {
  classification: NonNullable<ArcScoreBreakdown['expressionWidth']>
  reason: string
  familyCount: number
  competitiveFamilyCount: number
  intensitySpread: number
  peakPoolSize: number
  fallbackReliance: boolean
  topSpread: number
} {
  const finalHighlight = field.finalHighlight
  const familyCount = field.familyEntries.length
  const topSpread =
    field.familyEntries.length >= 2
      ? Math.max(0, field.familyEntries[0]!.score - field.familyEntries[1]!.score)
      : 0
  const competitiveFamilyCount =
    field.familyEntries.length === 0
      ? 0
      : field.familyEntries.filter(
          (entry) => field.familyEntries[0]!.score - entry.score <= field.threshold,
        ).length
  const fallbackReliance = Boolean(
    finalHighlight &&
      (finalHighlight.highlightValidity.validityLevel === 'fallback' ||
        finalHighlight.taste.fallbackPenalty.applied),
  )
  const contractCompressing =
    requiresRomanticPersonaMoment(lens) ||
    rolePools?.contractPoolStatus.peak.contractStrength === 'hard'

  let classification: NonNullable<ArcScoreBreakdown['expressionWidth']> = 'narrow'
  if (
    familyCount >= 3 &&
...
function computeExpressionWidth(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
): ReturnType<typeof computeExpressionWidthFromField> {
  const field = buildFamilyCompetitionField(stops, intent, lens, rolePools)
...
function computeExpressionRelease(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  eligible: boolean
  reason: string
  eliteCandidateNames: string[]
  eliteFamilies: string[]
  eliteCandidateLanes: string[]
  detectedLanes: string[]
  laneCandidates: string[]
  diversified: boolean
  diversificationReason: string
  selectedCandidateName?: string
  selectedFamily?: string
  selectionMode: NonNullable<ArcScoreBreakdown['expressionReleaseSelectionMode']>
  decision: string
} {
  const field = buildFamilyCompetitionField(stops, intent, lens, rolePools)
  ...
  const bestEntry = field.familyEntries[0]
  const competingFamilies = field.familyEntries.filter(
    (entry) => bestEntry && bestEntry.score - entry.score <= field.threshold,
  )
  const romanticRichField =
    requiresRomanticPersonaMoment(lens) &&
    width.classification === 'moderate' &&
  ...
  const finalIsSelected =
    finalHighlightId === getScoredVenueCandidateId(selectedEntry.candidate)
  const finalIsElite = eliteEntries.some(
    (entry) => getScoredVenueCandidateId(entry.candidate) === finalHighlightId,
  )
  ...
  const releaseMode =
    selectedEntry.family === bestEntry.family
...
function computeActivationMomentElevation(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  topCandidatePotential?: number
  winnerElevated: boolean
} {
  const field = buildFamilyCompetitionField(stops, intent, lens, rolePools)
  const width = computeExpressionWidthFromField(field, lens, rolePools)
  const finalHighlight = field.finalHighlight
  ...
  const candidateScoreThreshold =
    width.classification === 'broad' ? 0.05 : requiresRomanticPersonaMoment(lens) ? 0.045 : 0.04
  ...
  const elevatedCandidates = eliteField.entries
    ...
    .filter((candidate) => {
      ...
      if (
        bestCandidateScore - computeFamilyCompetitionLeadScore(candidate) >
    ...
    .sort((left, right) => {
      return (
        right.taste.signals.momentElevationPotential -
  ...
  const finalHighlightId = getScoredVenueCandidateId(finalHighlight)
  const finalWinnerElevated = elevatedCandidates.some(
    (candidate) => getScoredVenueCandidateId(candidate) === finalHighlightId,
  )
  const topGapToLeader = Math.max(
    0,
    bestCandidateScore - computeFamilyCompetitionLeadScore(topElevatedCandidate),
  )
  ...
  const score = finalWinnerElevated
    ...
    : 0
  const penalty =
    !finalWinnerElevated && topGapToLeader <= candidateScoreThreshold
  ...
  const candidateFamilies = elevatedCandidates.map(
...
function getCoarseArcCategory(stop: ArcStop): CoarseArcCategory {
...
function getExperienceLane(stop: ArcStop): ExperienceLane {
...
function isCasualVenue(stop: ArcStop): boolean {
  const { venue } = stop.scoredVenue
  const tagSet = new Set(venue.tags.map((tag) => tag.toLowerCase()))
  return (
    venue.energyLevel <= 3 ||
...
function computeRoleFlowScore(stops: ArcStop[]): number {
...
function computeDiversityScore(stops: ArcStop[], crewPolicy: CrewPolicy, lens: ExperienceLens): number {
...
function computeCategoryDiversityGuardrail(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
): {
  ...
  const groupedStops = new Map<CoarseArcCategory, ArcStop[]>()
  ...
  const repeatedArchetypes = [...archetypeGroups.entries()].filter(
    (entry) => entry[1].length > 1,
  )
  const repeatedCategoryCount = repeatedCategories.reduce(
    (sum, [, grouped]) => sum + (grouped.length - 1),
    0,
  )
  const repeatedLaneCount = repeatedLanes.reduce(
    (sum, [, grouped]) => sum + (grouped.length - 1),
    0,
  )
  const repeatedArchetypeCount = repeatedArchetypes.reduce(
    (sum, [, grouped]) => sum + (grouped.length - 1),
    0,
  )
  if (repeatedCategoryCount === 0 && repeatedLaneCount === 0 && repeatedArchetypeCount === 0) {
    const distinctNonAnchorCategories = new Set(
      nonAnchorCoreStops.map((stop) => getCoarseArcCategory(stop)),
    ).size
    const distinctNonAnchorArchetypes = new Set(
      nonAnchorCoreStops.map((stop) => getPrimaryExperienceArchetype(stop)),
    ).size
    const bonus =
  ...
  const warmupPeakRepeat =
    warmup && peak && getCoarseArcCategory(warmup) === getCoarseArcCategory(peak)
      ? 1
      : 0
  const peakCooldownRepeat =
    peak && cooldown && getCoarseArcCategory(peak) === getCoarseArcCategory(cooldown)
      ? 1
      : 0
    ...
    const bucketPenalty =
      totalStops >= 3
        ...
        : totalStops === 2
          ? 0.15
          : 0
    const crossRolePenalty = Math.max(
      0,
      new Set(nonAnchorGroup.map((stop) => stop.role)).size - 1,
    ) * 0.03
  ...
  const lanePenalty = repeatedLanes.reduce((sum, [lane, grouped]) => {
    ...
    const repeatedLanePenalty =
      totalStops >= 3
        ? 0.1
        : totalStops === 2
          ? 0.05
          : 0
    const crossRoleLanePenalty = Math.max(
      0,
      new Set(nonAnchorGroup.map((stop) => stop.role)).size - 1,
    ) * 0.02
    const hospitalityLanePenalty =
      (lane === 'dining' || lane === 'drinks' || lane === 'sweet') && totalStops >= 2
        ? 0.02
        : 0
    return (
      sum +
    ...
    const repeatPenalty =
      totalStops >= 3
        ...
        : totalStops === 2
          ? 0.055
          : 0
    const lowMomentRepeats = nonAnchorGroup.filter(
      (stop) => !isHighMomentPotential(stop.scoredVenue.taste.signals.momentPotential),
    ).length
    const lowMomentMultiplier =
      lowMomentRepeats === nonAnchorStops
        ? 1.18
        : lowMomentRepeats > 0
          ? 1.08
          : 0.94
    const hospitalityPenalty =
      isHospitalityArchetype(archetype) && totalStops >= 2 ? 0.03 : 0
  ...
  const hospitalityStopCount = nonAnchorArchetypes.filter((archetype) =>
    isHospitalityArchetype(archetype),
  ).length
  const lowMomentHospitalityCount = nonAnchorCoreStops.filter(
    (stop) =>
      isHospitalityArchetype(getPrimaryExperienceArchetype(stop)) &&
  ...
  const lowVarianceHospitalityPenalty =
    hospitalityStopCount >= 2 &&
    distinctNonAnchorArchetypes <= 2 &&
...
function computeRoleAwareCategoryLift(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
): number {
  const coreStops = stops.filter(
    (stop) =>
      (stop.role === 'warmup' || stop.role === 'peak' || stop.role === 'cooldown') &&
...
function computeAlignmentPreservationContract(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  alignedStopCount: number
  strongAlignedStopCount: number
  availableAlignedCount: number
  availableStrongCount: number
} {
  if (!lens.tasteMode) {
    return {
  ...
  const strongAlignedStops = nonProtectedStops.filter(
    (stop) =>
  ...
  const uniqueCandidates = [
    ...new Map(
      rolePoolCandidates.map((item) => [getScoredVenueCandidateId(item), item] as const),
    ).values(),
  ]
  const availableAlignedCount = uniqueCandidates.filter(
    (candidate) => candidate.taste.modeAlignment.score >= 0.48,
  ).length
  const availableStrongCount = uniqueCandidates.filter(
    (candidate) =>
      candidate.taste.modeAlignment.score >= 0.68 ||
      candidate.taste.modeAlignment.tier === 'primary',
  ).length
  const highlightAligned = nonProtectedStops.some(
    (stop) =>
      stop.role === 'peak' && stop.scoredVenue.taste.modeAlignment.score >= 0.48,
  )
  const strongTasteModeActive =
    lens.tasteMode.enforcementStrength === 'strong' ||
  ...
  const enforcementWeight =
    lens.tasteMode.enforcementStrength === 'strong'
      ? 1
      : lens.tasteMode.enforcementStrength === 'moderate'
        ? 0.8
        : 0.55
  const alignmentPreservationScore = clamp01(
    (availableStrongCount > 0
  ...
  const penalty =
    availableStrongCount > 0 && strongAlignedStops.length === 0
      ? alignedStops.length > 0
      ...
      : availableAlignedCount > alignedStops.length && alignedStops.length === 0
          ...
          : 0
  const themeSpreadScore =
    strongTasteModeActive && themeSpreadAvailable
      ? themeReinforced
        ...
        : highlightAligned
          ...
          : alignedStops.length > 0
            ...
            : 0
      : 0
  const themeSpreadPenalty =
    strongTasteModeActive &&
...
function computeWindDownScore(stops: ArcStop[], lens: ExperienceLens): number {
  ...
  const distancePenalty =
    lens.windDownExpectation.closeToBase && venue.driveMinutes > 16
      ? 0.1
  ...
  return clamp01(
    cooldown.scoredVenue.roleScores.cooldown +
      lowEnergyBonus +
      categoryBonus +
      momentBonus +
      momentStrengthBonus +
...
function deriveHighlightHospitalityMix(
  highlight: ArcStop['scoredVenue'],
): {
  drinks: number
  dining: number
  culture: number
  ...
  if (
    tags.some((tag) =>
      ['cocktail', 'late-night', 'nightlife', 'social', 'speakeasy'].some((term) =>
        tag.includes(term),
      ),
    )
  ) {
...
function deriveHighlightFamilyFromStop(
  highlight: ArcStop['scoredVenue'],
  intent: IntentProfile,
): ExperienceFamily {
  ...
  const family = detectExperienceFamily(
    {
      ...
      activationStrength: clamp01(
        signals.experientialFactor * 0.42 +
          signals.socialDensity * 0.28 +
          signals.momentPotential.score * 0.3,
      ),
      environmentalInfluencePotential: clamp01(
        signals.destinationFactor * 0.44 +
...
function areDirectionFamiliesCompatible(
  selectedFamily: ExperienceFamily,
  highlightFamily: ExperienceFamily,
): boolean {
  if (selectedFamily === highlightFamily) {
    return true
  }
  const compatibleFamilies: Record<ExperienceFamily, ExperienceFamily[]> = {
...
function computeHighlightIntegrityAdjustments(
  stops: ArcStop[],
  intent: IntentProfile,
): {
  ...
  const dominanceBoost =
    highlightIntensity >= HIGHLIGHT_DOMINANCE_THRESHOLD ? HIGHLIGHT_DOMINANCE_BOOST : 0
  const weakPenalty =
    highlightIntensity < HIGHLIGHT_DOMINANCE_THRESHOLD ? HIGHLIGHT_WEAK_PENALTY : 0

  let familyAlignmentBoost = 0
  let familyMismatchPenalty = 0
  const selectedDirectionFamily =
    intent.selectedDirectionContext?.familyConfidence !== undefined &&
    intent.selectedDirectionContext.familyConfidence >= FAMILY_ALIGNMENT_CONFIDENCE_MIN
      ? (intent.selectedDirectionContext.family as ExperienceFamily | undefined)
      : undefined
  ...
  if (start && windDown) {
    ...
    const startTooSimilar =
      start.scoredVenue.venue.category === highlight.scoredVenue.venue.category &&
  ...
  return {
    ...
    highlightFamilyAlignmentApplied:
      familyAlignmentBoost > 0 || familyMismatchPenalty > 0,
...
function computeMissedPeakPenalty(
  ...
  const strongestFeasiblePeakMoment = availableCandidates
    ...
    .sort((left, right) => {
      return (
        computePeakMomentLeadScore(right) - computePeakMomentLeadScore(left) ||
  ...
  if (
    getScoredVenueCandidateId(strongestFeasiblePeakMoment) ===
    getScoredVenueCandidateId(finalHighlight)
  ) {
    return { penalty: 0, applied: false }
  }

  const finalHighlightPassiveFallback =
    isHospitalityArchetype(finalHighlight.taste.signals.primaryExperienceArchetype) &&
    finalHighlight.momentIdentity.strength !== 'strong'
  const finalHighlightWeakMoment =
    (finalHighlight.momentIdentity.strength !== 'strong' &&
  ...
  const scoreDelta = Math.max(
    0,
    computePeakMomentLeadScore(strongestFeasiblePeakMoment) -
...
function computeRomanticMomentContract(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  present: boolean
  romanticMomentRole: ArcScoreBreakdown['romanticMomentRole']
  ...
  const romanticCandidates = uniqueCandidates.filter(
    (candidate) => assessRomanticPersonaHighlightQualification(candidate).qualifies,
  )
  const feasibleRomanticCandidates = romanticCandidates.filter((candidate) =>
    isFeasibleRomanticMomentCandidate(candidate, intent),
  )
  const strongestRomanticCandidate =
    [...feasibleRomanticCandidates].sort((left, right) => {
      return (
...
function computeRomanticPersonaContract(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
  rolePools?: RolePools,
  ...
  feasible: boolean
  feasibleCount: number
  result: NonNullable<ArcScoreBreakdown['romanticHighlightArbitrationResult']>
} {
  if (
    !requiresRomanticPersonaMoment(lens)
  ) {
  ...
  const feasibleRomanticHighlights = uniqueCandidates.filter((candidate) =>
    isFeasibleRomanticHighlightCandidate(candidate, lens, intent),
  )
  const finalHighlight = stops.find((stop) => stop.role === 'peak')?.scoredVenue
  ...
  if (isFeasibleRomanticHighlightCandidate(finalHighlight, lens, intent)) {
    ...
    const bestAvailableGap = strongestFeasibleRomanticHighlight
      ? Math.max(
          0,
          strongestFeasibleRomanticHighlight.taste.signals.momentIntensity.score -
            finalHighlight.taste.signals.momentIntensity.score,
        )
        : 0
    const strongestAvailableTierGap =
      strongestFeasibleRomanticHighlight &&
      getScoredVenueCandidateId(strongestFeasibleRomanticHighlight) !==
        getScoredVenueCandidateId(finalHighlight)
        ? Math.max(
            0,
            getMomentIntensityRank(strongestFeasibleRomanticHighlight.taste.signals.momentIntensity.tier) -
              getMomentIntensityRank(finalHighlight.taste.signals.momentIntensity.tier),
          )
        : 0
    const strongestAvailableTierBoost =
      strongestFeasibleRomanticHighlight &&
      getScoredVenueCandidateId(strongestFeasibleRomanticHighlight) !==
        getScoredVenueCandidateId(finalHighlight)
        ? getMomentIntensityTierBoost(strongestFeasibleRomanticHighlight.taste.signals.momentIntensity)
        : 0
    const strongestAvailableBonus =
      !strongestFeasibleRomanticHighlight ||
      getScoredVenueCandidateId(strongestFeasibleRomanticHighlight) ===
        getScoredVenueCandidateId(finalHighlight)
        ? clamp01(
        ...
        : 0
    const strongestAvailablePenalty =
      strongestFeasibleRomanticHighlight &&
      getScoredVenueCandidateId(strongestFeasibleRomanticHighlight) !==
        getScoredVenueCandidateId(finalHighlight)
        ? clamp01(
...
function computeMeaningfulMomentStretchScore(candidate: ArcStop['scoredVenue']): number {
  return (
    computePeakMomentLeadScore(candidate) +
...
interface EffectiveContractEvaluation {
  score: number
  satisfied: boolean
  strength: ArcStop['scoredVenue']['roleContract'][InternalRole]['strength']
  overrideApplied: boolean
}

function getPreferredDiscoveryVenueIdForRole(
  intent: IntentProfile,
  role: InternalRole,
): string | undefined {
...
function isUserLedAnchorStop(stop: ArcStop, intent: IntentProfile): boolean {
  if (intent.planningMode !== 'user-led' || !intent.anchor?.venueId) {
    return false
  }
...
function getEffectiveContractEvaluation(
  stop: ArcStop,
  intent: IntentProfile,
  rolePools?: RolePools,
): EffectiveContractEvaluation {
  ...
  const overrideApplied =
    contractStatus.preferredDiscoveryVenueAdmitted === true &&
...
function resolveEffectiveContractEvaluations(
  stops: ArcStop[],
  intent: IntentProfile,
  rolePools?: RolePools,
): Array<{ stop: ArcStop; evaluation: EffectiveContractEvaluation }> {
...
function computeContractCompliance(
  effectiveEvaluations: Array<{ stop: ArcStop; evaluation: EffectiveContractEvaluation }>,
): number {
...
function computeContractViolationPenalty(
  effectiveEvaluations: Array<{ stop: ArcStop; evaluation: EffectiveContractEvaluation }>,
): number {
  const penalties = effectiveEvaluations.map(({ stop, evaluation }) => {
    ...
    const strengthWeight =
      evaluation.strength === 'soft' ? 0.08 : evaluation.strength === 'strong' ? 0.16 : 0.24
...
function computeLiveRolePromotionScore(stops: ArcStop[]): number {
  const promotionTotal = stops.reduce(
    (sum, stop) => sum + computeRoleAwareLiveLift(stop.scoredVenue, stop.role).promotion,
    0,
  )
  return clamp01(promotionTotal / 0.1)
}

function toInternalRole(preferenceRole: 'start' | 'highlight' | 'windDown'): InternalRole {
...
function computeDiscoveryContract(
  stops: ArcStop[],
  intent: IntentProfile,
  rolePools?: RolePools,
): {
  ...
  for (const preference of preferences) {
    ...
    const acceptedForRole = rolePools
      ? getRolePoolForRole(role, rolePools).some(
          (candidate) => getScoredVenueBaseVenueId(candidate) === preference.venueId,
        )
      : true
    const exactRoleMatch = stops.some(
      (stop) => stop.role === role && getArcStopBaseVenueId(stop) === preference.venueId,
    )
    const wrongRoleMatch = stops.some(
      (stop) => stop.role !== role && getArcStopBaseVenueId(stop) === preference.venueId,
    )
...
export function scoreArcAssembly(
  stops: ArcStop[],
  intent: IntentProfile,
  ...
  const breakdown: ArcScoreBreakdown = {
  ...
  const contractViolationPenalty = computeContractViolationPenalty(
    effectiveContractEvaluations,
  )
  ...
  const alignmentPreservation = computeAlignmentPreservationContract(
    stops,
    intent,
    lens,
    rolePools,
  ...
  const expressionWidth = computeExpressionWidth(
    stops,
    intent,
    lens,
    rolePools,
  )
  const expressionRelease = computeExpressionRelease(
    stops,
    intent,
    lens,
    rolePools,
  )
  const activationMomentElevation = computeActivationMomentElevation(
    stops,
    intent,
    lens,
    rolePools,
  ...
  return {
    ...
    scoreBreakdown: {
      ...
      tasteModeAlignmentScore: clamp01(
        alignmentPreservation.alignmentPreservationScore,
      ),
      ...
      highlightFamilyAlignmentApplied:
        highlightIntegrity.highlightFamilyAlignmentApplied,
      highlightSupportPenaltyApplied:
        highlightIntegrity.highlightSupportPenaltyApplied,
    },
  }
}
