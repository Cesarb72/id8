...
import type {
  DerivedNextContext,
  ExplorationContinuationMode,
  ExplorationDistrictStrategy,
  ExplorationResolutionStrength,
  ExplorationVibeShift,
} from './types'

const timePhaseOrder: PlanningTimePhase[] = ['morning', 'afternoon', 'evening', 'late-night']
...
function getAlternateDistrict(result: GeneratePlanResult) {
  return result.trace.recommendedDistricts.find(
    (district) => district.districtId !== result.trace.selectedDistrictId,
  )
}

function deriveDistrictStrategy(result: GeneratePlanResult): {
  ...
  const selectedDistrict = result.trace.recommendedDistricts.find(
    (district) => district.districtId === result.trace.selectedDistrictId,
  )
  const alternateDistrict = getAlternateDistrict(result)
  const continuationNeighborhood =
    result.itinerary.stops[result.itinerary.stops.length - 1]?.neighborhood ??
  ...
  const districtFeelsSpent =
    selectedDiversity < 0.58 ||
...
function mapShiftedVibe(
  currentVibe: VibeAnchor,
  vibeShift: ExplorationVibeShift,
  endCategory?: VenueCategory,
): VibeAnchor {
...
function deriveVibeShift(
  result: GeneratePlanResult,
  nextPhase: PlanningTimePhase,
): {
  vibeShift: ExplorationVibeShift
  nextPrimaryVibe: VibeAnchor
  nextSecondaryVibe?: VibeAnchor
  vibeReason: string
  ...
  let vibeShift: ExplorationVibeShift = 'MAINTAIN'
  let repetitionReason =
    'Arc 1 ends on an open enough category mix, so Arc 2 can continue without forcing a reset.'

  if (
    nextPhase === 'late-night' ||
    elapsedMinutes >= 250 ||
  ...
  const nextSecondaryVibe =
    nextPrimaryVibe === currentPrimary ? currentSecondary : currentPrimary
  const vibeReason =
    vibeShift === 'MAINTAIN'
...
function isMajorAnchorCategory(category?: VenueCategory): boolean {
  return (
    category === 'restaurant' ||
    category === 'live_music' ||
    category === 'event' ||
...
function evaluateResolution(
  ...
  const hasMeaningfulSequence =
    roles.has('start') && roles.has('highlight') && roles.has('windDown')
  ...
  const majorAnchorUsed =
    isMajorAnchorCategory(highlightStop?.category) ||
  ...
  let resolutionStrength: ExplorationResolutionStrength = 'NONE'
  if (
    hasMeaningfulSequence &&
    endsOnWindDown &&
    landsSoftly &&
    strongDuration &&
    majorAnchorUsed
    ...
    (hasMeaningfulSequence && endsOnWindDown && (landsSoftly || moderateDuration)) ||
  ...
  const continuationMode: ExplorationContinuationMode =
    resolutionStrength === 'STRONG'
  ...
  const continuationReason =
    continuationMode === 'CONTINUATION_FRAGMENT'
...
function softenContinuationPrimaryVibe(
  continuationMode: ExplorationContinuationMode,
  vibeShift: ExplorationVibeShift,
  nextPrimaryVibe: VibeAnchor,
): VibeAnchor {
...
export function deriveNextContext(result: GeneratePlanResult): DerivedNextContext {
  ...
  const nextPrimaryVibe = softenContinuationPrimaryVibe(
    resolutionDecision.continuationMode,
    vibeDecision.vibeShift,
    vibeDecision.nextPrimaryVibe,
  )
  const nextSecondaryVibe =
    resolutionDecision.continuationMode === 'FULL_CONTINUATION'
  ...
  const nextIntent = {
    ...
    persona:
      result.intentProfile.crew === 'romantic'
        ? 'romantic'
    ...
    neighborhood:
      districtDecision.districtStrategy === 'stay-put'
  ...
  const summary =
    resolutionDecision.continuationMode === 'CONTINUATION_FRAGMENT'
