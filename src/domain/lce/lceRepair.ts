/**
 * ARC BOUNDARY: LCE runtime continuity repair policy.
 *
 * Owns:
 * - proposing deterministic continuity-preserving repairs for runtime breakage
 *
 * Does NOT own:
...
import type { ArcCandidate, ScoredVenue, StopAlternative } from '../types/arc'
...
export interface LceRepairProposal {
  role: UserStopRole
  trigger: LceRepairTrigger
  brokenStopVenueId: string
  brokenStopVenueName: string
...
function uniqueByCandidateId(candidates: CandidateSource[]): CandidateSource[] {
...
function computeLocalContinuityScore(
  currentArc: ArcCandidate,
  replacement: ScoredVenue,
  role: keyof ScoredVenue['roleScores'],
): number {
...
function buildRankedCandidates(
  ...
  return [...scoredVenues]
    ...
    .map((scoredVenue) => ({ scoredVenue, source: 'ranked-candidates' as const }))
}

function formatSourceLabel(source: CandidateSource['source']): string {
  return source === 'role-pool' ? 'role pool' : 'ranked candidate field'
}

export function proposeLceRepair({
  currentArc,
  role,
  trigger,
  scoredVenues,
  ...
  const rolePoolCandidates: CandidateSource[] = rolePool.map((item) => ({
    scoredVenue: item.scoredVenue,
    source: 'role-pool',
  }))
  const rankedCandidates = buildRankedCandidates(scoredVenues, internalRole)
  const mergedCandidates = uniqueByCandidateId([...rolePoolCandidates, ...rankedCandidates]).filter(
    (candidate) =>
      !excludedVenueIds.has(candidate.scoredVenue.venue.id) &&
  ...
  const evaluated = mergedCandidates
    ...
    .sort(
      (left, right) =>
        ...
        left.scoredVenue.candidateIdentity.candidateId.localeCompare(
          right.scoredVenue.candidateIdentity.candidateId,
        ),
    )
  ...
  const rationale = `${formatSourceLabel(best.source)} | role ${best.scoredVenue.roleScores[
    internalRole
  ].toFixed(2)} | continuity ${best.continuity.toFixed(2)} | contract ${
    best.contractFit >= 1 ? 'fit' : 'soft-fit'
  }`
