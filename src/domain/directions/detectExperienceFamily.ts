...
type ExperienceFamilyDetectionCandidate = {
...
type ExperienceFamilyDetectionContext = {
  persona?: PersonaMode
  vibe?: VibeAnchor
}

export type ExperienceFamilyDetectionResult = {
...
function getCategoryDiversity(categories: string[]): number {
  const uniqueCount = new Set(categories).size
  return clamp(uniqueCount / 4, 0, 1)
}

function toLevel(value: 'low' | 'medium' | 'high', invert = false): number {
  const score = value === 'high' ? 1 : value === 'medium' ? 0.62 : 0.24
  return invert ? 1 - score : score
}

function getMixVariance(
  mix: ExperienceFamilyDetectionCandidate['hospitalityMix'],
): number {
  if (!mix) {
    return 0
  }
...
export function detectExperienceFamily(
  candidate: ExperienceFamilyDetectionCandidate,
  context?: ExperienceFamilyDetectionContext,
): ExperienceFamilyDetectionResult {
  const categories = (candidate.dominantCategories ?? []).map((value) =>
    value.toLowerCase(),
  )
  const signals = (candidate.whyHereSignals ?? []).map((value) =>
    value.toLowerCase(),
  )
  const tags = (candidate.experientialTags ?? []).map((value) =>
    value.toLowerCase(),
  )

  const activationStrength = clamp(candidate.activationStrength ?? 0.5, 0, 1)
  const environmentalInfluencePotential = clamp(
    candidate.environmentalInfluencePotential ?? 0.5,
    0,
    1,
  )
  ...
  const ambientLikeSignal =
    includesAny(tags, ['scenic', 'calm', 'quiet', 'atmospheric', 'ambient']) ||
    includesAny(categories, ['park', 'waterfront', 'garden']) ||
    ambiance.noise === 'low'
  const eventLikeSignal =
    includesAny(tags, ['event', 'live', 'festival', 'show', 'spike']) ||
    includesAny(categories, ['event', 'music', 'activity']) ||
    includesAny(signals, ['high_activation_core'])
  const diningForwardSignal =
    includesAny(categories, ['restaurant', 'dining', 'tasting', 'wine', 'dessert']) ||
    mix.dining >= 0.32
  const structuredTransitions =
    includesAny(signals, ['steady_activation_base', 'tight_walkable_micro_pocket']) ||
  ...
  let exploratoryScore =
    categoryDiversity * 0.32 +
  ...
  const ambientTieBreakEligible =
    mix.activity <= 0.24 && (ambiance.energy === 'low' || ambiance.noise === 'low')
  const ritualTieBreakEligible =
    mix.dining + mix.cafe >= 0.34 &&
  ...
  const ranked = [
    { family: 'social' as const, score: socialScore },
    { family: 'cultural' as const, score: culturalScore },
    { family: 'playful' as const, score: playfulScore },
    { family: 'intimate' as const, score: intimateScore },
    { family: 'exploratory' as const, score: exploratoryScore },
    { family: 'ambient' as const, score: ambientScore },
    { family: 'eventful' as const, score: eventfulScore },
    { family: 'ritual' as const, score: ritualScore },
    { family: 'indulgent' as const, score: indulgentScore },
  ].sort((left, right) => {
  ...
  const legacyFamilies = new Set<ExperienceFamily>([
    'social',
    'cultural',
    'playful',
    'intimate',
  ...
  const newFamilies = new Set<ExperienceFamily>([
    'ambient',
    'eventful',
    'ritual',
    'indulgent',
  ...
  const topLegacyCandidate =
    ranked.find((entry) => legacyFamilies.has(entry.family)) ?? provisionalRunnerUp
  const winner =
    newFamilies.has(provisionalWinner.family) &&
    topLegacyCandidate &&
    provisionalWinner.score - topLegacyCandidate.score < -NEW_FAMILY_COMPETITIVE_BAND
      ? topLegacyCandidate
      : provisionalWinner
  const runnerUp =
    ranked.find((entry) => entry.family !== winner.family) ?? provisionalRunnerUp
  const confidence = toFixed(
    clamp(
      0.42 + winner.score * 0.38 + (winner.score - runnerUp.score) * 0.42,
      0,
      1,
    ),
  )
...
export type {
  ExperienceFamilyDetectionCandidate,
  ExperienceFamilyDetectionContext,
}
