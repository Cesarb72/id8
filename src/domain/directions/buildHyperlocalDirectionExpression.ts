import {
  clampWords,
  formatList,
  getExpressionFamilyMode,
  getFamilyAnchorOrder,
  getFamilyBulletOrder,
  getFamilySubtitleTemplate,
  getFamilySupportEmphasis,
  getFamilyTemplateKeys,
  resolveReasonTemplate,
  resolveReasonTitleSignal,
  toDisplayToken,
  toKey,
} from './hyperlocalExpressionTemplates'
...
type PocketType = 'pocket' | 'cluster' | 'core' | 'strip' | 'zone'

type HyperlocalExpression = {
  title: string
  subtitle?: string
  anchorLine?: string
  supportLine?: string
...
type BuildExpressionInput = {
  districtLabel: string
  defaultTitle: string
  defaultSubtitle?: string
  defaultSupportLine?: string
  preferDefaultTitle?: boolean
  preferDefaultSubtitle?: boolean
  defaultSectionLabel?: string
  defaultBullets?: string[]
  candidate?: unknown
  hyperlocal?: HyperlocalLike | null
  usedPrimarySignals?: Iterable<string>
  usedPocketTypes?: Iterable<PocketType>
}
...
function toSet<T extends string>(value?: Iterable<T>): Set<T> {
...
function extractHyperlocal(input: BuildExpressionInput): HyperlocalLike | null {
  if (input.hyperlocal) {
  ...
  const candidate = input.candidate as
    | {
        derivedFrom?: { hyperlocal?: HyperlocalLike }
        districtProfile?: { hyperlocal?: HyperlocalLike }
        profile?: { hyperlocal?: HyperlocalLike }
      }
    | undefined

  return (
    candidate?.derivedFrom?.hyperlocal ??
...
function buildFallback(input: BuildExpressionInput): HyperlocalExpression {
  return {
...
function getCompactness(radiusM: number): number {
  return clamp(1 - radiusM / 260, 0, 1)
}

function getPocketTypeOrder(
  radiusM: number,
  compactness: number,
  activationStrength: number,
  identityStrength: number,
): PocketType[] {
  const preferred: PocketType =
    activationStrength >= 0.72 && compactness >= 0.62
      ? 'core'
  ...
  const alternativesByType: Record<PocketType, PocketType[]> = {
...
function pickFirstAvailable<T>(ordered: T[], used: Set<T>): T {
...
function buildSignalCandidates(
  ...
  return unique(
    [...categoryCandidates, categoryBlend, ...reasonCandidates]
      .filter((entry): entry is SignalCandidate => Boolean(entry))
      .map((entry) => `${entry.key}::${entry.label}` as const),
  ).map((value) => {
...
function sanitizeTitleBlend(signal: string, pocketType: PocketType): string {
...
function getAnchorPrefix(
  ...
  identityStrength: number,
  compactness: number,
  familyMode: ReturnType<typeof getExpressionFamilyMode>,
): 'Anchored by' | 'Centered around' | 'Built around' {
  const metricPreferred: 'Anchored by' | 'Centered around' | 'Built around' =
    compactness >= 0.66 && activationStrength >= 0.6
      ? 'Centered around'
...
function getSubtitle(compactness: number, activationStrength: number): string {
  const structure =
    compactness >= 0.66
      ? 'Dense overlap'
      : compactness >= 0.5
...
function buildDensityBullet(compactness: number, activationStrength: number): string {
...
function buildMixBullet(dominantCategories: string[], dominantLanes: string[]): string {
...
function buildTransitionBullet(
  reasonKeys: string[],
...
function getFamilyContrastBullet(
  familyMode: ReturnType<typeof getExpressionFamilyMode>,
): string | undefined {
...
function getSectionLabel(compactness: number, identityStrength: number): string {
  if (compactness >= 0.6) {
    return 'Pocket shape'
  }
  ...
  const activationBand =
    activationStrength >= 0.68
      ...
      : activationStrength >= 0.5
        ...
        : 'soft overlap'
  const influenceBand =
    environmentalInfluencePotential >= 0.66
...
export function buildHyperlocalDirectionExpression(
  input: BuildExpressionInput,
): HyperlocalExpression {
  ...
  const environmentalInfluencePotential = clamp(
    primaryPocket.environmentalInfluencePotential ?? 0.5,
    0,
    1,
  )
  ...
  const candidateIdentity = input.candidate as
    | { experienceFamily?: string; familyConfidence?: number }
    | undefined
  const familyMode = getExpressionFamilyMode(
    candidateIdentity?.experienceFamily,
    candidateIdentity?.familyConfidence,
  )
  ...
  const selectedSignal = pickFirstAvailable(
    signalCandidates.map((entry) => entry.key),
    usedPrimarySignals,
  )
  const selectedSignalLabel =
    signalCandidates.find((entry) => entry.key === selectedSignal)?.label ?? 'Balanced Mix'

  const pocketTypeOrder = getPocketTypeOrder(
    radiusM,
    compactness,
    activationStrength,
    identityStrength,
  ...
  const bulletByKey: Record<string, string | undefined> = {
...
export type { HyperlocalExpression, BuildExpressionInput, PocketType }
