import type {
  ...
  LiveTrustBreakdownDiagnostics,
  LiveTrustFailureCandidateDiagnostics,
  ReasonBreakdownEntry,
} from '../types/diagnostics'
import type { Venue } from '../types/venue'

function countReasons(reasons: string[]): ReasonBreakdownEntry[] {
...
function upstreamStrength(venue: Venue): number {
  return (
...
function toFailureCandidate(venue: Venue): LiveTrustFailureCandidateDiagnostics {
  const helpedBy = [
    ...venue.source.qualityGateNotes.filter((note) =>
      /(supports|viable path|distinctive|fairly|metadata|signal)/i.test(note),
    ),
    ...venue.source.hoursPressureNotes.filter((note) =>
      /(supports|accepted|positive|open)/i.test(note),
    ),
  ].slice(0, 4)
  const hurtBy = [
    ...
    ...venue.source.hoursPressureNotes.filter((note) =>
      /(conflicts|unknown|closed|soft)/i.test(note),
    ),
  ].slice(0, 6)
...
export function buildLiveTrustBreakdown(
  liveVenues: Venue[],
