import type {
  TasteMomentIdentity,
  TasteRomanticFlavor,
  TasteRomanticSignals,
  TasteHyperlocalActivation,
  TasteHyperlocalActivationType,
  TasteInterpretationContext,
  NormalizedVenueTasteInput,
  SeedCalibratedTasteProfile,
  ...
  TasteMomentTier,
  TasteMomentEnrichment,
  TasteMomentPotential,
  TasteTemporalWindow,
  TasteRoleSuitability,
  TasteSignals,
  TasteVenuePersonality,
  TasteVenuePersonalityTag,
} from './types'

type TasteCoreProfile = SeedCalibratedTasteProfile

interface ProfileInference {
  profile: TasteCoreProfile
  supportingSignals: string[]
  experienceStrengths: ExperienceStrengths
}

interface TasteCalibrationProfile {
  categorySpecificity: number
  personalityStrength: number
  nightlifeStrength: number
  signatureDiningStrength: number
  eveningFit: number
  genericPremiumPenalty: number
}

interface ExperienceStrengths {
  outdoor: number
  interactive: number
  passiveHospitality: number
}

interface ExperienceArchetypeRead {
  primary: TasteExperienceArchetype
  archetypes: TasteExperienceArchetype[]
}

const CATEGORY_BASELINES: Record<
  NormalizedVenueTasteInput['category'],
  TasteCoreProfile
> = {
  activity: {
    energy: 0.74,
...
function normalizeInterpretationTimeWindow(value: string | undefined): string | undefined {
  const normalized = value?.trim().toLowerCase()
  return normalized ? normalized : undefined
}

function parseTemporalWindow(
  value: string | undefined,
): TasteTemporalWindow | undefined {
...
function createZeroRoleAdjustments(): TasteRoleSuitability {
  return {
    start: 0,
    highlight: 0,
    windDown: 0,
...
export function interpretVenueTaste(
  venue: NormalizedVenueTasteInput,
  context?: TasteInterpretationContext,
): TasteSignals {
...
function inferRuleProfile(
  venue: NormalizedVenueTasteInput,
  context?: TasteInterpretationContext,
): ProfileInference {
  const profile: TasteCoreProfile = { ...CATEGORY_BASELINES[venue.category] }
  ...
  profile.socialDensity = clamp01(
    profile.socialDensity +
      socialBump * 0.16 +
      energyBump * 0.08 +
      socialActivityBump * 0.08 -
      calmBump * 0.06,
  )
  profile.intimacy = clamp01(
    profile.intimacy +
      calmBump * 0.18 +
      scenicBump * 0.08 -
      socialBump * 0.08 -
  ...
  profile.lingerFactor = clamp01(
    profile.lingerFactor + lingerBump * 0.16 + outdoorBump * 0.06,
  )
  profile.destinationFactor = clamp01(
    profile.destinationFactor +
      destinationBump * 0.2 +
      surpriseBump * 0.08 +
      scenicBump * 0.2 +
  ...
  profile.experientialFactor = clamp01(
  ...
  profile.conversationFriendliness = clamp01(
    profile.conversationFriendliness +
...
function mergeProfile(
  seed: SeedCalibratedTasteProfile | undefined,
  inferred: TasteCoreProfile,
): TasteCoreProfile {
  if (!seed) {
    return inferred
  }
  ...
  return clampProfile({
    ...
    socialDensity:
      seed.socialDensity * seedWeight + inferred.socialDensity * inferredWeight,
    intimacy: seed.intimacy * seedWeight + inferred.intimacy * inferredWeight,
    lingerFactor:
      seed.lingerFactor * seedWeight + inferred.lingerFactor * inferredWeight,
    destinationFactor:
      seed.destinationFactor * seedWeight +
      inferred.destinationFactor * inferredWeight,
    experientialFactor:
      seed.experientialFactor * seedWeight +
      inferred.experientialFactor * inferredWeight,
    conversationFriendliness:
      seed.conversationFriendliness * seedWeight +
      inferred.conversationFriendliness * inferredWeight,
  })
}

function applyContextTonePressure(
  profile: TasteCoreProfile,
  venue: NormalizedVenueTasteInput,
  context?: TasteInterpretationContext,
): { profile: TasteCoreProfile; signals: string[] } {
  ...
  if (livelyLike) {
    ...
    nextProfile.destinationFactor = clamp01(
    ...
    if (persona === 'romantic') {
      ...
      nextProfile.conversationFriendliness = clamp01(
...
function deriveRoleSuitability(
  ...
  const startHoursSupport =
    venue.hoursStatus === 'open' || venue.hoursStatus === 'likely_open'
      ? 0.06
      : venue.hoursStatus === 'closed' || venue.hoursStatus === 'likely_closed'
        ? -0.1
        : 0

  const quickDurationPenalty =
    durationEstimate === 'quick'
      ? 0.14
      : durationEstimate === 'event'
        ...
        : durationEstimate === 'moderate'
          ? 0.04
          : 0
  const startDurationPenalty =
    durationEstimate === 'event' ? 0.18 : durationEstimate === 'extended' ? 0.06 : 0
  const windDownDurationPenalty =
    durationEstimate === 'event' ? 0.2 : durationEstimate === 'quick' ? 0.04 : 0
  ...
  const categoryIdentityBoost =
    experienceStrengths.interactive >= 0.7 || experienceStrengths.outdoor >= 0.76
      ? 0.11
  ...
  let windDown = clamp01(
    profile.intimacy * 0.26 +
  ...
  const livelyLike = vibe === 'lively' || vibe === 'playful'
  const cozyLike = vibe === 'cozy' || vibe === 'chill'
  const livelyPulseSignal = clamp01(
    profile.energy * 0.36 +
  ...
  const containedPulseSignal = clamp01(
    livelyPulseSignal * 0.54 +
      profile.intimacy * 0.2 +
      profile.conversationFriendliness * 0.16 +
      (1 - quickStopSignal) * 0.1,
  )
  const softCenterSignal = clamp01(
    (1 - profile.energy) * 0.44 +
      (1 - profile.socialDensity) * 0.26 +
      landingSignal * 0.16 +
      profile.intimacy * 0.14,
  )
  const broadChaosSignal = clamp01(
    profile.energy * 0.42 +
...
function deriveHighlightTier(
  ...
  const vibeAdjustedStrength = clamp01(
    highlightStrength +
  ...
  const exceptionalQuickCenterpiece =
    durationEstimate === 'quick' &&
...
function deriveMomentIntensity(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
  experienceArchetypes: ExperienceArchetypeRead,
  ...
  const signatureQuality = clamp01(
    anchorStrength * 0.42 +
      venue.signatureStrength * 0.34 +
      venue.qualityScore * 0.24,
  )
  const distinctiveIdentity = clamp01(
    calibration.categorySpecificity * 0.48 +
      calibration.personalityStrength * 0.32 +
      noveltyWeight * 0.2,
  )
  const experientialRichness = clamp01(
    profile.experientialFactor * 0.34 +
      profile.destinationFactor * 0.22 +
  ...
  const scenicStrength = clamp01(
    romanticSignals.scenic * 0.58 +
      experienceStrengths.outdoor * 0.28 +
      (primaryArchetype === 'scenic' ? 0.14 : primaryArchetype === 'outdoor' ? 0.08 : 0),
  )
  const activityRichness = clamp01(
    romanticSignals.sharedActivity * 0.52 +
      experienceStrengths.interactive * 0.34 +
      (primaryArchetype === 'activity' ? 0.14 : primaryArchetype === 'culture' ? 0.08 : 0),
  )
  const ambientPull = clamp01(
    Math.max(romanticSignals.ambiance, romanticSignals.ambientExperience) * 0.68 +
      profile.intimacy * 0.14 +
      (primaryArchetype === 'culture' || primaryArchetype === 'social' ? 0.1 : 0),
  )
  const temporalLift = clamp01(
    momentEnrichment.temporalEnergy * 0.56 +
  ...
  const culturalImmersion = clamp01(
    momentEnrichment.culturalDepth * 0.62 +
      momentEnrichment.ambientUniqueness * 0.12 +
      profile.destinationFactor * 0.08 +
      (primaryArchetype === 'culture' ? 0.12 : 0),
  )
  const vibeRichness = clamp01(
    momentEnrichment.ambientUniqueness * 0.62 +
      profile.intimacy * 0.1 +
      venue.signatureStrength * 0.08 +
      (venue.setting === 'indoor' || venue.setting === 'hybrid' ? 0.08 : 0),
  )
  const hyperlocalLift = clamp01(
    hyperlocalActivation.intensityContribution * 0.72 +
...
function deriveMomentTier(
  ...
  const identitySignal = clamp01(
    destinationKeyword * 0.44 +
      identityKeyword * 0.34 +
      venue.signatureStrength * 0.12 +
      venue.qualityScore * 0.1,
  )
  const destinationRead = clamp01(
    profile.destinationFactor * 0.28 +
  ...
  const experientialRichness = clamp01(
    profile.experientialFactor * 0.3 +
  ...
  const memorability = clamp01(
    venue.signatureStrength * 0.38 +
  ...
  const destinationLikeCharacteristics =
    destinationKeyword >= 0.34 ||
    identityKeyword >= 0.34 ||
    venue.eventCapable ||
    venue.performanceCapable ||
    venue.musicCapable
  const anchorTierQualified =
...
function deriveDurationEstimate(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
): TasteDurationEstimate {
...
function deriveVenuePersonality(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  ...
  const tags: TasteVenuePersonalityTag[] = []
  const keywords = buildKeywordCorpus(venue)
  const hasKeyword = (terms: string[], threshold = 0.28): boolean =>
    keywordScore(keywords, terms) >= threshold
  const quickCue = keywordScore(keywords, QUICK_STOP_TERMS)
  const landingCue = keywordScore(keywords, LANDING_TERMS)
  ...
  const intimate =
    (profile.intimacy >= intimacyThreshold &&
  ...
  const social =
    profile.socialDensity >= socialThreshold ||
  ...
  const lingering =
    ((profile.lingerFactor >= 0.62 || landingCue >= 0.4) &&
  ...
  const experiential =
    (profile.experientialFactor >= 0.64 &&
  ...
  if (tags.includes('quick_stop') && tags.includes('destination')) {
    const strongDestinationOverride =
      highlightTier === 1 ||
...
function deriveTasteCalibration(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  noveltyWeight: number,
  ...
  const subcategorySpecificity =
    venue.subcategory && !isGenericSubcategory(venue.subcategory, venue.category) ? 1 : 0
  const tagRichness = clamp01(venue.tags.length / 5)
  const placeTypeRichness = clamp01((venue.placeTypes?.length ?? 0) / 4)
  const categorySpecificity = clamp01(
    subcategorySpecificity * 0.34 +
      tagRichness * 0.24 +
  ...
  const personalityStrength = clamp01(
    categorySpecificity * 0.34 +
  ...
  const genericPremiumPenalty = clamp01(
    genericPremiumStrength * 0.56 +
...
function deriveExperienceStrengths(
  venue: NormalizedVenueTasteInput,
  keywordSignals: {
    outdoorBump: number
    scenicBump: number
    interactiveBump: number
    socialActivityBump: number
    passiveHospitalityBump: number
  },
): ExperienceStrengths {
...
function deriveExperienceArchetypes(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
): ExperienceArchetypeRead {
  const keywords = buildKeywordCorpus(venue)
  const scores: Record<TasteExperienceArchetype, number> = {
    dining:
      (venue.category === 'restaurant' ? 0.82 : 0) +
      keywordScore(keywords, DINING_SIGNATURE_TERMS) * 0.18 +
      profile.lingerFactor * 0.04,
    drinks:
      (venue.category === 'bar' ? 0.84 : venue.category === 'cafe' ? 0.58 : 0) +
      keywordScore(keywords, DRINKS_TERMS) * 0.22 +
      keywordScore(keywords, NIGHTLIFE_SIGNATURE_TERMS) * 0.1,
    sweet:
      (venue.category === 'dessert' ? 0.88 : 0) +
      keywordScore(keywords, SWEET_TERMS) * 0.26 +
      (venue.category === 'cafe' ? 0.08 : 0),
    outdoor:
      experienceStrengths.outdoor * 0.88 +
      (venue.category === 'park' ? 0.18 : 0),
    activity:
      experienceStrengths.interactive * 0.82 +
    ...
    scenic:
      keywordScore(keywords, SCENIC_TERMS) * 0.62 +
  ...
  const priority: TasteExperienceArchetype[] = [
    'scenic',
    'activity',
    'outdoor',
    'culture',
  ...
  const archetypes = priority
    ...
    .sort(
      (left, right) =>
        scores[right] - scores[left] || priority.indexOf(left) - priority.indexOf(right),
    )
    .slice(0, 3)
...
function deriveMomentPotential(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
  experienceArchetypes: ExperienceArchetypeRead,
...
function deriveExperienceFamily(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
  experienceArchetypes: ExperienceArchetypeRead,
  ...
  const culturalCue =
    keywords.includes('japanese') || keywords.includes('heritage') || keywords.includes('historic')
  const ambientCue =
    keywords.includes('greenhouse') ||
    keywords.includes('reflective') ||
    keywords.includes('atmosphere')
  const richnessSignal = clamp01(
    profile.experientialFactor * 0.42 +
      calibration.categorySpecificity * 0.28 +
      calibration.personalityStrength * 0.3,
  )
  const scenicSignal = clamp01(
    romanticSignals.scenic * 0.42 +
      experienceStrengths.outdoor * 0.26 +
      (archetypes.has('scenic') ? 0.18 : 0) +
      (archetypes.has('outdoor') ? 0.1 : 0) +
      momentIntensity.score * 0.08,
  )
  const quietSignal = clamp01(
    profile.conversationFriendliness * 0.34 +
  ...
  const activitySignal = clamp01(
    experienceStrengths.interactive * 0.42 +
  ...
  const cultureSignal = clamp01(
    (archetypes.has('culture') ? 0.34 : 0) +
      cultureKeyword * 0.24 +
  ...
  const ambientSignal = clamp01(
    Math.max(romanticSignals.ambiance, romanticSignals.ambientExperience) * 0.34 +
      profile.intimacy * 0.12 +
      profile.destinationFactor * 0.1 +
      momentIntensity.score * 0.08 +
  ...
  const immersiveCultureSignal = clamp01(
    cultureSignal * 0.44 +
  ...
  const diningSignal = clamp01(
    (archetypes.has('dining') ? 0.28 : 0) +
  ...
  const familyScores: Array<{ family: TasteExperienceFamily; score: number }> = [
    {
  ...
  const priority: TasteExperienceFamily[] = [
    'outdoor_scenic',
    'quiet_activity',
    'immersive_cultural',
    'cultural',
  ...
  const enrichedCultureBridge =
    immersiveCultureSignal >= 0.48 &&
...
function deriveExpandedExperienceFamily(
  ...
  momentEnrichment: TasteMomentEnrichment,
  hyperlocalActivation: TasteHyperlocalActivation,
  baseFamily: TasteExperienceFamily,
): { family: TasteExperienceFamily; expanded: boolean; reason?: string } {
  ...
  const activationStrength = clamp01(
    hyperlocalActivation.intensityContribution * 0.52 +
      hyperlocalActivation.interpretationImpact.highlightSuitability * 0.22 +
      hyperlocalActivation.interpretationImpact.momentIntensity * 0.18 +
  ...
  const distinctFromBase =
    (expandedFamily === 'tasting_experience' && baseFamily !== 'intimate_dining') ||
  ...
  const plausible =
    (expandedFamily === 'tasting_experience' &&
...
function applyExpandedFamilyRoleAdjustment(
  roleSuitability: TasteRoleSuitability,
  ...
  const activationScale = clamp01(
    hyperlocalActivation.intensityContribution * 0.44 +
...
function deriveActivationMomentElevation(
  profile: TasteCoreProfile,
  momentPotential: TasteMomentPotential,
  momentIntensity: TasteMomentIntensity,
  hyperlocalActivation: TasteHyperlocalActivation,
...
): { potential: number; eligible: boolean; reason?: string } {
  const activationType = hyperlocalActivation.primaryActivationType
  if (!activationType) {
    return {
      potential: 0,
  ...
  const strongActivationEffect =
    hyperlocalActivation.interpretationImpact.highlightSuitability >= 0.05 ||
...
function deriveMomentIdentity(
  ...
  const mediumThreshold =
    archetype === 'dining' || archetype === 'drinks' || archetype === 'sweet'
      ...
      : 0.38
  const strength =
    momentPotential.score >= strongThreshold
      ? 'strong'
  ...
  if (archetype === 'sweet') {
    return {
      type:
        profile.energy <= 0.44 || profile.intimacy >= 0.58 ? 'close' : 'linger',
      strength,
    }
  }
...
function deriveRomanticSignals(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
  experienceArchetypes: ExperienceArchetypeRead,
  ...
  const ambiance = clamp01(
    ambianceKeyword * 0.34 +
      profile.destinationFactor * 0.14 +
  ...
  const ambientExperience = clamp01(
    ambianceKeyword * 0.2 +
...
function deriveRomanticScore(romanticSignals: TasteRomanticSignals): number {
  return clamp01(
    romanticSignals.intimacy * 0.25 +
...
function deriveHyperlocalActivation(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceArchetypes: ExperienceArchetypeRead,
  context?: TasteInterpretationContext,
  ...
  const keywords = buildKeywordCorpus(venue)
  const archetypes = new Set(experienceArchetypes.archetypes)
  const livePerformance = clamp01(
    keywordScore(keywords, LIVE_PERFORMANCE_ACTIVATION_TERMS) * 0.54 +
      (venue.category === 'live_music' ? 0.24 : 0) +
      (venue.musicCapable ? 0.12 : 0) +
      (venue.performanceCapable ? 0.1 : 0) +
      (venue.liveSource ? 0.06 : 0),
  )
  const socialRitual = clamp01(
    keywordScore(keywords, SOCIAL_RITUAL_ACTIVATION_TERMS) * 0.5 +
      keywordScore(keywords, RECURRING_EVENT_TERMS) * 0.18 +
      (venue.category === 'bar' || venue.category === 'activity' ? 0.08 : 0) +
      (venue.eventCapable ? 0.08 : 0),
  )
  const tastingActivation = clamp01(
    keywordScore(keywords, TASTING_ACTIVATION_TERMS) * 0.58 +
      (venue.category === 'restaurant' || venue.category === 'bar' ? 0.08 : 0) +
      venue.signatureStrength * 0.08,
  )
  const culturalActivation = clamp01(
    keywordScore(keywords, CULTURAL_ACTIVATION_TERMS) * 0.54 +
  ...
  const seasonalMarket = clamp01(
    keywordScore(keywords, SEASONAL_MARKET_ACTIVATION_TERMS) * 0.6 +
      (venue.category === 'event' ? 0.16 : 0) +
      (venue.eventCapable ? 0.08 : 0),
  )
  const ambientActivation = clamp01(
    keywordScore(keywords, AMBIENT_ACTIVATION_TERMS) * 0.5 +
  ...
  const activationScoresBase: Array<{
    type: TasteHyperlocalActivationType
    score: number
    signal: string
  }> = [
    {
  ...
  const temporalRelevance = clamp01(
    (strongest?.score ?? 0) * 0.34 +
      ...
      : recurrenceShape === 'recurring'
        ...
        : recurrenceShape === 'seasonal'
          ...
          : recurrenceShape === 'ambient'
            ? 0.05
            : 0
  const intensityContribution =
    activationTypes.length === 0
      ? 0
      : clamp01(
          (strongest?.score ?? 0) * 0.4 +
  ...
  const familyRefinements: string[] = []
  const addFamilyRefinement = (value: string) => {
  ...
  const contractCompatibilityHints: TasteHyperlocalActivation['contractCompatibilityHints'] = []
  if (
  ...
  return {
    ...
    materiallyChangesHighlightPotential:
      activationTypes.length > 0 && intensityContribution >= 0.26,
    materiallyChangesInterpretation:
      activationTypes.length > 0 &&
...
function deriveHyperlocalTemporalCompatibility(
  venue: NormalizedVenueTasteInput,
  keywords: string[],
  activationTypes: TasteHyperlocalActivationType[],
  temporalRelevance: number,
  temporalLabel: TasteHyperlocalActivation['temporalLabel'],
  intensityContribution: number,
  context?: TasteInterpretationContext,
): TasteHyperlocalActivation['temporalCompatibility'] {
  const contextWindow = parseTemporalWindow(context?.timeWindow)
  const dayCueScore = clamp01(
    keywordScore(keywords, DAY_WINDOW_TERMS) * 0.56 +
      keywordScore(keywords, SEASONAL_MARKET_ACTIVATION_TERMS) * 0.22 +
      (activationTypes.includes('seasonal_market') ? 0.26 : 0) +
      (venue.category === 'cafe' ? 0.08 : 0),
  )
  const eveningCueScore = clamp01(
    keywordScore(keywords, EVENING_WINDOW_TERMS) * 0.44 +
  ...
  const lateCueScore = clamp01(
    keywordScore(keywords, LATE_WINDOW_TERMS) * 0.52 +
  ...
  let activationWindow: TasteTemporalWindow | undefined
  ...
  const materiallyChangesViability =
    activationTypes.length > 0 &&
...
function deriveMomentEnrichment(
  venue: NormalizedVenueTasteInput,
  profile: TasteCoreProfile,
  experienceStrengths: ExperienceStrengths,
  experienceArchetypes: ExperienceArchetypeRead,
  ...
  const keywords = buildKeywordCorpus(venue)
  const archetypes = new Set(experienceArchetypes.archetypes)
  const temporalKeyword = clamp01(
    keywordScore(keywords, TEMPORAL_ENERGY_TERMS) * 0.72 +
      keywordScore(keywords, RECURRING_EVENT_TERMS) * 0.28,
  )
  const socialKeyword = clamp01(
    keywordScore(keywords, SOCIAL_ENERGY_TERMS) * 0.7 +
  ...
  const hyperlocalAmbientLift =
    hyperlocalActivation.activationTypes.includes('ambient_activation') ||
    hyperlocalActivation.activationTypes.includes('live_performance')
      ? hyperlocalActivation.intensityContribution * 0.12
      : 0
  const hyperlocalCulturalLift =
    hyperlocalActivation.activationTypes.includes('cultural_activation') ||
    hyperlocalActivation.activationTypes.includes('live_performance') ||
    hyperlocalActivation.activationTypes.includes('seasonal_market')
      ? hyperlocalActivation.intensityContribution * 0.12
      : 0
  const temporalEnergy = clamp01(
    temporalKeyword * 0.46 +
  ...
  const ambientUniqueness = clamp01(
    ambientKeyword * 0.42 +
      profile.intimacy * 0.08 +
      venue.signatureStrength * 0.14 +
      venue.qualityScore * 0.08 +
  ...
  const culturalDepth = clamp01(
    culturalKeyword * 0.42 +
  ...
  const highlightSurfaceBoost = clamp01(
    temporalEnergy * 0.18 +
      socialEnergy * 0.08 +
...
function deriveRomanticFlavor(
  ...
  return strongest.key as TasteRomanticFlavor
}

function deriveRomanticMomentCandidate(
  venue: NormalizedVenueTasteInput,
  romanticSignals: TasteRomanticSignals,
  romanticScore: number,
  momentIdentity: TasteMomentIdentity,
  ...
  const primaryArchetype = experienceArchetypes.primary
  const hospitalityPrimary =
    primaryArchetype === 'dining' ||
    primaryArchetype === 'drinks' ||
    primaryArchetype === 'sweet'
  const atmosphericDiningSignal =
    hospitalityPrimary &&
  ...
  const strongRomanticDimension =
    romanticSignals.sharedActivity >= 0.48 ||
    romanticSignals.scenic >= 0.48 ||
...
function deriveDebugConfidence(
  venue: NormalizedVenueTasteInput,
  sourceMode: TasteSignals['debug']['sourceMode'],
): number {
  const modeBase =
    sourceMode === 'seed_calibrated'
      ? 0.82
      : sourceMode === 'hybrid'
        ? 0.76
        : 0.58
...
function hasMeaningfulDelta(
  seed: SeedCalibratedTasteProfile,
  inferred: TasteCoreProfile,
): boolean {
  return (
    Math.abs(seed.energy - inferred.energy) >= 0.05 ||
...
function isGenericSubcategory(
  subcategory: string,
  category: NormalizedVenueTasteInput['category'],
): boolean {
...
function buildKeywordCorpus(venue: NormalizedVenueTasteInput): string[] {
  const textBlocks = [
    venue.name,
    venue.subcategory,
...
function hasAnyFragment(values: string[] | undefined, fragments: string[]): boolean {
  if (!values || values.length === 0) {
    return false
  }

  return fragments.some((fragment) =>
    values.some((value) => value.toLowerCase().includes(fragment)),
  )
}
...
function normalizeCount(value: number | undefined, ceiling: number): number {
  if (!value || value <= 0) {
    return 0
  }
...
function clampProfile(profile: TasteCoreProfile): TasteCoreProfile {
  return {
...
function finalizeSupportingSignals(
  supportingSignals: string[],
  sourceMode: TasteSignals['debug']['sourceMode'],
  momentIntensity?: TasteMomentIntensity,
  experienceFamily?: TasteExperienceFamily,
  momentEnrichment?: TasteMomentEnrichment,
  hyperlocalActivation?: TasteHyperlocalActivation,
  elevatedMomentCandidate?: boolean,
): string[] {
