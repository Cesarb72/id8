import type { UserStopRole, ItineraryStop } from '../types/itinerary'
import type { PriceTier, VenueCategory } from '../types/venue'
import type { VenueVibeTag } from '../types/intent'

interface ReadableStopVenueInput {
...
function normalizeSignals(values: string[] | undefined): Set<string> {
  return new Set((values ?? []).map((value) => value.toLowerCase()))
}

function hasAnySignal(signals: Set<string>, candidates: string[]): boolean {
...
function getBaseLine(category: VenueCategory, subcategory?: string, signals?: Set<string>): string {
...
function buildIdentityLine(venue: ReadableStopVenueInput, role: UserStopRole): string {
...
function buildHighlightIdentityLine(venue: ReadableStopVenueInput): string | undefined {
  const signals = normalizeSignals([...(venue.tags ?? []), ...(venue.vibeTags ?? [])])
...
function buildRoleLine(role: UserStopRole): string {
  if (role === 'start') {
...
export function deriveReadableStopContent(
  stop: Pick<ItineraryStop, 'role'>,
  venue: ReadableStopVenueInput,
  role: UserStopRole,
): ReadableStopContent {
  const resolvedRole = role ?? stop.role
  const highlightIdentity =
    resolvedRole === 'highlight' ? buildHighlightIdentityLine(venue) : undefined
