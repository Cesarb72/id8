/**
 * ARC BOUNDARY: Interpretation canonical artifact builder.
 *
 * Owns:
 * - normalized intent semantics
 * - experience contract meaning
 * - strategy family semantics
 *
 * Does NOT own:
 * - admissibility/buildability gating decisions
 * - sequence coordination and structural route validation
 */
import type {
  ConciergeIntent,
  ContractConstraints,
...
export type ExperienceContractVibeAxis = 'cozy' | 'lively' | 'cultured'

interface ExperienceContractTemplate {
  coordinationMode: ExperienceContract['coordinationMode']
  contractIdentity: string
  summary: string
  actPattern: ExperienceContract['actStructure']['actPattern']
  highlightModel: ExperienceContract['highlightModel']
  highlightType: ExperienceContract['highlightType']
  movementStyle: ExperienceContract['movementStyle']
  socialPosture: ExperienceContract['socialPosture']
  pacingStyle: ExperienceContract['pacingStyle']
  constraintPriority: ExperienceContract['constraintPriority']
  venuePressure: ExperienceContract['venuePressure']
  contractReasonSummary: string
}

const EXPERIENCE_CONTRACT_V0_1_MATRIX: Record<
  PersonaMode,
  Record<ExperienceContractVibeAxis, ExperienceContractTemplate>
> = {
  romantic: {
    cozy: {
      ...
      contractReasonSummary:
        'Romantic + cozy emphasizes intimacy, low-friction movement, and one earned destination moment.',
    },
    lively: {
      ...
      contractReasonSummary:
        'Romantic + lively favors energetic pacing, alternating peaks, and a controlled late taper.',
    },
    cultured: {
      ...
      contractReasonSummary:
        'Romantic + cultured builds through thematic acts with an intentional culmination and reflective close.',
    },
  },
  friends: {
    cozy: {
      ...
      contractReasonSummary:
        'Friends + cozy keeps movement compact and optional so the group can settle in without pressure.',
    },
    lively: {
      ...
      contractReasonSummary:
        'Friends + lively optimizes for movement, distributed social moments, and periodic resets.',
    },
    cultured: {
      ...
      contractReasonSummary:
        'Friends + cultured leans on sequential learning anchors and shared debrief points.',
    },
  },
  family: {
    cozy: {
      ...
      contractReasonSummary:
        'Family + cozy emphasizes low-friction movement, frequent resets, and dependable engagement beats.',
    },
    lively: {
      ...
      contractReasonSummary:
        'Family + lively favors playful bursts with predictable reset points to preserve stamina.',
    },
    cultured: {
      coordinationMode: 'enrichment',
      ...
      contractReasonSummary:
        'Family + cultured uses staged learning anchors with decompression and parallel-track flexibility.',
    },
  },
}

export function normalizeExperienceContractVibe(vibe: VibeAnchor): ExperienceContractVibeAxis {
  if (vibe === 'cozy' || vibe === 'chill') {
    return 'cozy'
  }
  if (vibe === 'lively' || vibe === 'playful') {
...
export function formatExperienceContractActShape(
  actPattern: ExperienceContract['actStructure']['actPattern'],
): string {
...
interface ContractConstraintsTemplate {
  ...
  maxEnergyDropTolerance: ContractConstraints['maxEnergyDropTolerance']
  socialDensityBand: ContractConstraints['socialDensityBand']
  movementTolerance: ContractConstraints['movementTolerance']
  allowLateHighEnergy: boolean
  windDownStrictness: ContractConstraints['windDownStrictness']
...
const CONTRACT_CONSTRAINTS_V0_1_MATRIX: Record<
  PersonaMode,
  Record<ExperienceContractVibeAxis, ContractConstraintsTemplate>
> = {
  romantic: {
    cozy: {
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'Single-center romantic containment with strict continuity and a soft required landing.',
    },
    lively: {
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'Escalating romantic pulse with multiple peaks, continuity guardrails, and controlled wind-down.',
    },
    cultured: {
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'Cumulative romantic-cultural progression with continuity and strong thematic center pressure.',
    },
  },
  friends: {
    cozy: {
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'Distributed friend-hang structure with high tolerance for variance and flexible close.',
    },
    lively: {
      peakCountModel: 'multi',
      requireEscalation: true,
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'High-density group momentum with distributed peaks, multi-anchor freedom, and loose taper.',
    },
    cultured: {
      peakCountModel: 'cumulative',
      requireEscalation: false,
      ...
      adultPayoffRequired: false,
      constraintReasonSummary:
        'Cultural friends route with multi-anchor enrichment, moderate center pressure, and controlled close.',
    },
  },
  family: {
    cozy: {
      ...
      adultPayoffRequired: true,
      constraintReasonSummary:
        'Family-cozy pacing requires recovery windows, tight movement, and a soft required taper.',
    },
    lively: {
      peakCountModel: 'distributed',
      requireEscalation: false,
      ...
      adultPayoffRequired: true,
      constraintReasonSummary:
        'Family-lively stays active but controlled: recovery windows, contained movement, no late energy spikes.',
    },
    cultured: {
      peakCountModel: 'cumulative',
      requireEscalation: false,
      ...
      adultPayoffRequired: true,
      constraintReasonSummary:
        'Family-cultural cumulative flow with recovery-protected cadence and moderate center pressure.',
    },
  },
}

function buildContractConstraints(experienceContract: ExperienceContract): ContractConstraints {
...
interface InterpretationStrategySemantics {
  family: DirectionStrategyFamily
  summary: string
  source: 'persona_vibe_contract'
}

export interface InterpretationStrategyFamilyResolution {
  ...
  persona: PersonaMode
  normalizedVibe: ExperienceContractVibeAxis
}
...
export interface CanonicalInterpretationBundle {
  ...
  strategyFamilyResolution: InterpretationStrategyFamilyResolution
  strategySemantics: InterpretationStrategySemantics
  debug: {
    bundleSource: string
    derivedFrom: string[]
    contractReasonSummary: string
...
const STRATEGY_FAMILY_SUMMARY_BY_ID: Record<DirectionStrategyFamily, string> = {
...
function resolveInterpretationStrategyFamilyWithTrace(params: {
  ...
  const reasonSummary = [
    ...
    fallbackUsed
      ? 'fallback:adaptive_semantics_due_to_unmapped_persona_vibe'
      : 'fallback:none',
  ].join(' | ')
  return {
    resolvedFamily,
