...
interface DistrictInsiderSignals {
  affinity: number
  signature: number
  coherence: number
  ...
  categoryMix: DistrictCategoryMix
}

interface DistrictInsiderContext {
...
type DistrictInsider = DistrictRecommendation['districtInsider']

function getTimeBand(timeWindow?: string): 'early' | 'prime' | 'late' | undefined {
  if (!timeWindow) {
    return undefined
  }
...
function buildWhyNow(
  signals: DistrictInsiderSignals,
  context: DistrictInsiderContext,
): string {
  const energetic = signals.energy >= 0.64 || 1 - signals.calm >= 0.63
  const calmPocket = signals.calm >= 0.62 && signals.energy < 0.64
  const compactPocket =
    signals.coherence >= 0.66 || signals.walkability >= 0.55 || signals.density >= 0.56
...
function categoryLabel(category: keyof DistrictCategoryMix): string {
...
function hasCategory(
  mix: DistrictCategoryMix,
  category: keyof DistrictCategoryMix,
  threshold = 0.2,
): boolean {
  return mix[category] >= threshold
}

function buildCategoryMixLine(mix: DistrictCategoryMix): string {
  const rankedCategories = (Object.entries(mix) as Array<[keyof DistrictCategoryMix, number]>)
...
function buildWhatStandsOut(signals: DistrictInsiderSignals): string {
  const mix = signals.categoryMix

  if (
    hasCategory(mix, 'bar', 0.18) &&
  ...
  if (
    hasCategory(mix, 'restaurant', 0.2) &&
  ...
  if (
    (hasCategory(mix, 'museum', 0.1) || hasCategory(mix, 'event', 0.1)) &&
...
export function generateDistrictInsider(
  signals: DistrictInsiderSignals,
  context: DistrictInsiderContext,
): DistrictInsider {
  return {
