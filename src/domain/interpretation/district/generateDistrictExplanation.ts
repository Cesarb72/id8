...
interface DistrictExplanationSignals {
  affinity: number
  signature: number
  coherence: number
...
interface DistrictExplanationContext {
...
const toneByVibe: Record<VibeAnchor, string> = {
...
function getPersonaLine(context: DistrictExplanationContext): string {
...
function getSummaryLead(vibe: VibeAnchor): string {
...
function getFitClause(affinity: number, context: DistrictExplanationContext): string {
...
function getFlowClause(sequenceSupport: number): string {
...
function getStructureClause(roleFit: number): string {
  if (roleFit >= 0.76) {
...
function buildSummary(
  signals: DistrictExplanationSignals,
...
function buildHighlightCandidates(signals: DistrictExplanationSignals): HighlightCandidate[] {
...
function buildHighlights(signals: DistrictExplanationSignals): string[] {
...
export function generateDistrictExplanation(
  district: DistrictExplanationSignals,
  context: DistrictExplanationContext,
): DistrictRecommendation['districtExplanation'] {
  return {
