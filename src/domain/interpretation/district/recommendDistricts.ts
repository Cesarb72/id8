...
interface RecommendDistrictsInput {
...
interface DistrictCandidateGroup {
  districtId: string
  label: string
...
interface DistrictTasteAverages {
  intimacy: number
  linger: number
  conversation: number
  calm: number
...
interface DistrictRoleStrengths {
  startStrength: number
  highlightStrength: number
  winddownStrength: number
}

interface DistrictRoleWeights {
  start: number
  highlight: number
  winddown: number
}
...
function normalizeWeights(weights: DistrictRoleWeights): DistrictRoleWeights {
  const minFloor = 0.15
  const floored = {
...
function getRoleWeights(intent: IntentProfile): DistrictRoleWeights {
  let weights: DistrictRoleWeights
...
function getViableCandidates(rolePools: RolePools, scoredVenues: ScoredVenue[]): ScoredVenue[] {
  const candidatesById = new Map<string, ScoredVenue>()
  for (const pool of [
    rolePools.warmup,
    rolePools.peak,
    rolePools.cooldown,
...
function getDistrictGroups(
  viableCandidates: ScoredVenue[],
  rolePools: RolePools,
  intent: IntentProfile,
): DistrictCandidateGroup[] {
  ...
  const groups = new Map<string, DistrictCandidateGroup>()
  ...
  for (const candidate of viableCandidates) {
    ...
    const current = groups.get(districtIdentity.districtId) ?? {
      ...
      categories: new Set<VenueCategory>(),
    }
...
function getDensityScore(candidateCount: number, maxCandidateCount: number): number {
...
function getAdjustedDensityScore(rawDensity: number): number {
  return clamp01(Math.pow(rawDensity, 0.72))
}

function getRelevanceScore(candidates: ScoredVenue[]): number {
...
function getDiversityScore(group: DistrictCandidateGroup): number {
...
function getEnergyScore(candidates: ScoredVenue[]): number {
  if (candidates.length === 0) {
    return 0
  }
...
function getSignatureScore(candidates: ScoredVenue[]): number {
  if (candidates.length === 0) {
    return 0
  }
  ...
  const total = candidates.reduce((sum, candidate) => {
    const signatureSignal =
      candidate.venue.distinctivenessScore * 0.35 +
...
function getDominanceScore(values: string[]): number {
  if (values.length === 0) {
    return 0
  }
...
function getCoherenceScore(group: DistrictCandidateGroup): number {
...
function getSequenceSupportScore(group: DistrictCandidateGroup): number {
  if (group.candidates.length === 0) {
    return 0
  }
  ...
  const sequenceSpread =
    Math.max(bestWarmup, bestPeak, bestCooldown) - Math.min(bestWarmup, bestPeak, bestCooldown)
...
function getTasteAverages(candidates: ScoredVenue[]): DistrictTasteAverages {
  if (candidates.length === 0) {
    return {
      intimacy: 0,
      linger: 0,
...
function getCategoryMix(candidates: ScoredVenue[]): DistrictCategoryMix {
  if (candidates.length === 0) {
    return {
      restaurant: 0,
      bar: 0,
...
function getWalkabilitySignal(candidates: ScoredVenue[]): number {
  if (candidates.length === 0) {
    return 0
  }
  const matched = candidates.filter((candidate) =>
    ['walkable', 'stroll', 'promenade', 'courtyard', 'main-street', 'main street', 'open air'].some(
...
function getEventMomentumSignal(candidates: ScoredVenue[]): number {
  if (candidates.length === 0) {
    return 0
  }
  const total = candidates.reduce((sum, candidate) => {
    ...
    const activationBoost =
      ...
      activation === 'social_ritual'
        ...
        : activation === 'ambient_activation' || activation === 'cultural_activation'
          ...
          : 0
    const tagBoost =
      ['event', 'night market', 'market', 'live', 'performance', 'show', 'lineup'].some((term) =>
        hasTag(candidate.venue.tags, term),
      )
        ? 1
        : 0
...
function getRoleStrengths(group: DistrictCandidateGroup): DistrictRoleStrengths {
  if (group.candidates.length === 0) {
    return {
      startStrength: 0,
      highlightStrength: 0,
  ...
  const startCategorySupport = clamp01(
    categories.cafe * 0.28 +
  ...
  const highlightCategorySupport = clamp01(
    categories.bar * 0.22 +
  ...
  const winddownCategorySupport = clamp01(
    categories.dessert * 0.22 +
  ...
  const startStrength = clamp01(
    meanRoleSuitability.start * 0.25 +
      bestRole.start * 0.16 +
  ...
  const highlightStrength = clamp01(
    meanRoleSuitability.highlight * 0.23 +
  ...
  const winddownStrength = clamp01(
    meanRoleSuitability.windDown * 0.25 +
...
function getRoleFitScore(
  roleStrengths: DistrictRoleStrengths,
  roleWeights: DistrictRoleWeights,
): number {
  return clamp01(
    roleStrengths.startStrength * roleWeights.start +
      roleStrengths.highlightStrength * roleWeights.highlight +
      roleStrengths.winddownStrength * roleWeights.winddown,
  )
}

function getVibeAffinity(
  intent: IntentProfile,
  taste: DistrictTasteAverages,
...
function getAffinityScores(
  group: DistrictCandidateGroup,
  intent: IntentProfile,
  sequenceSupport: number,
  signature: number,
): { affinity: number; vibeAffinity: number; crewAffinity: number } {
  const taste = getTasteAverages(group.candidates)
...
function getAlignmentLift(
  ...
  return clamp01(
    (Math.max(0, affinity - density) * 0.07 +
...
function getBreadthPenalty(
  ...
  return clamp01(
    Math.max(0, density - affinity - 0.08) * 0.16 +
      Math.max(0, density - signature - 0.08) * 0.12 +
      Math.max(0, density - coherence - 0.05) * 0.14,
  )
}

function getSparsityPenalty(
  density: number,
  diversity: number,
  sequenceSupport: number,
): number {
  return clamp01(
    Math.max(0, 0.38 - density) * 0.18 +
...
function buildReason(input: {
  signals: DistrictRecommendation['signals']
  affinity: number
  signature: number
  coherence: number
  sequenceSupport: number
...
export function recommendDistricts({
  scoredVenues,
  rolePools,
  intent,
  limit = 5,
}: RecommendDistrictsInput): DistrictRecommendation[] {
  ...
  return districtGroups
    .map((group) => {
      ...
      const score = roundToHundredths(
        clamp01(baseScore + alignmentLift - breadthPenalty - sparsityPenalty),
      )
      const signals = {
        density,
        relevance,
