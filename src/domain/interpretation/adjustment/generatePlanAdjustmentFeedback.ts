import type { GenerationTrace } from '../../runGeneratePlan'
import type { Itinerary, UserStopRole } from '../../types/itinerary'
import type {
  PlanAdjustmentEffectStrength,
  PlanAdjustmentFeedback,
} from '../../types/planAdjustmentFeedback'
import type { PreviewControls } from '../../types/previewControls'
...
interface GeneratePlanAdjustmentFeedbackInput {
  previousPlan: PlanSnapshot
  nextPlan: PlanSnapshot
  controls: PreviewControls
}

const orderedRoles: UserStopRole[] = ['start', 'highlight', 'surprise', 'windDown']
...
function stopMapByRole(itinerary: Itinerary): Partial<Record<UserStopRole, string>> {
  return itinerary.stops.reduce<Partial<Record<UserStopRole, string>>>((accumulator, stop) => {
    accumulator[stop.role] = stop.venueId
    return accumulator
  }, {})
}

function countChangedStops(previous: Itinerary, next: Itinerary): number {
...
function hasRole(itinerary: Itinerary, role: UserStopRole): boolean {
  return itinerary.stops.some((stop) => stop.role === role)
}

function hasCentralMoment(itinerary: Itinerary): boolean {
...
function resolveEffectStrength({
  districtChanged,
  changedStopCount,
  highlightChanged,
  spatialModeChanged,
  ...
  spatialModeChanged: boolean
  frictionDelta: number
  durationDelta: number
  surpriseChanged: boolean
}): PlanAdjustmentEffectStrength {
  let magnitude = 0
  if (districtChanged) {
    magnitude += 2
...
function buildHeadline(strength: PlanAdjustmentEffectStrength, mostlySame: boolean): string {
  if (strength === 'strong') {
...
export function generatePlanAdjustmentFeedback({
  previousPlan,
  nextPlan,
  controls,
}: GeneratePlanAdjustmentFeedbackInput): PlanAdjustmentFeedback {
  ...
  const districtChanged =
    previousPlan.trace.selectedDistrictId !== nextPlan.trace.selectedDistrictId
  const frictionDelta =
    nextPlan.trace.routePacing.totalRouteFriction - previousPlan.trace.routePacing.totalRouteFriction
  const durationDelta =
    nextPlan.trace.routePacing.estimatedTotalMinutes -
    previousPlan.trace.routePacing.estimatedTotalMinutes
  const spatialModeChanged =
    previousPlan.trace.spatialCoherence.mode !== nextPlan.trace.spatialCoherence.mode
  const becameMoreCompact =
    (previousPlan.trace.spatialCoherence.mode === 'flexible' &&
      nextPlan.trace.spatialCoherence.mode === 'walkable') ||
    frictionDelta <= -1
  const becameMoreOpen =
    (previousPlan.trace.spatialCoherence.mode === 'walkable' &&
      nextPlan.trace.spatialCoherence.mode === 'flexible') ||
    frictionDelta >= 1
  const becameSofter =
    (!nextHasSurprise && previousHasSurprise) ||
    durationDelta <= -25 ||
    frictionDelta <= -1
  const becameStronger =
    (nextHasSurprise && !previousHasSurprise) ||
    durationDelta >= 25 ||
    frictionDelta >= 1
  ...
  const districtAccepted =
    Boolean(controls.districtPreference) &&
    nextPlan.trace.selectedDistrictId === controls.districtPreference
  const startTimeAccepted =
    Boolean(controls.startTime) && nextPlan.trace.intent.timeWindow === controls.startTime
  const distanceAccepted =
    controls.distanceTolerance === 'compact'
      ...
      : controls.distanceTolerance === 'open'
        ...
        : controls.distanceTolerance === 'balanced'
          ? nextPlan.trace.intent.distanceMode === 'nearby' &&
            !nextRefinementModes.includes('closer-by')
          : false
  const energyAccepted =
    controls.energyBias === 'softer'
  ...
  if (mostlySame) {
    ...
    if (controls.distanceTolerance === 'open' && distanceAccepted) {
      pushUnique(
        trustNotes,
        'This route already had enough range, so only light changes were needed.',
      )
    }
    if (controls.energyBias === 'softer' && energyAccepted) {
      pushUnique(
        trustNotes,
        'The current district already fit your softer energy preference.',
      )
    }
    if (controls.energyBias === 'stronger' && energyAccepted) {
      pushUnique(
        trustNotes,
        'The current district already supported a stronger energy curve.',
      )
    }
  }
