...
type NarrativeVibeBand = 'cozy' | 'lively' | 'cultured'
type NarrativeMode =
  | 'romantic_cozy'
  | 'romantic_lively'
  | 'romantic_cultured'
...
export interface StopNarrativeSnapshot {
  mode: string
  source: string
  roleMeaning: string
  whyNow: string
...
function toTagSet(values: string[]): Set<string> {
  return new Set(values.map((value) => value.toLowerCase()))
}

function hasAnyTag(tagSet: Set<string>, tags: string[]): boolean {
  return tags.some((tag) => tagSet.has(tag))
}

function normalizeVibeBand(
...
function resolveNarrativeMode(params: {
  ...
  const vibeBand = normalizeVibeBand(
    params.experienceContract?.vibe ?? params.intent.primaryAnchor,
    params.experienceContract,
    params.contractConstraints,
  )
  if (persona === 'romantic') {
...
function resolveRoleNeighbors(
  itineraryStops: ItineraryStop[] | undefined,
  role: UserStopRole,
): { previousRole?: UserStopRole; nextRole?: UserStopRole } {
...
function deriveFlavorTags(params: {
  ...
  const socialDensity = clampUnit(
    signals?.socialDensity ?? (hasAnyTag(tagSet, ['social', 'live', 'cocktails']) ? 0.7 : 0.45),
  )
  const intimacy = clampUnit(signals?.intimacy ?? (hasAnyTag(tagSet, ['intimate', 'cozy']) ? 0.68 : 0.42))
  const linger = clampUnit(
    signals?.lingerFactor ?? (hasAnyTag(tagSet, ['tasting', 'dessert', 'tea', 'wine']) ? 0.66 : 0.45),
  )
  const destination = clampUnit(
    signals?.destinationFactor ??
      (hasAnyTag(tagSet, ['chef-led', 'tasting', 'signature']) || role === 'highlight' ? 0.64 : 0.38),
  )
  const experiential = clampUnit(
  ...
  addScore(
    'recovery-friendly',
    (contractConstraints?.requireRecoveryWindows ? 0.3 : 0) + (1 - energy) * 0.66 + linger * 0.24,
  )
  addScore(
    'contained pulse',
    experienceContract?.coordinationMode === 'pulse' &&
...
function resolveTransitionLogic(params: {
  role: UserStopRole
  previousRole?: UserStopRole
  nextRole?: UserStopRole
  contractConstraints?: ContractConstraints
}): string {
