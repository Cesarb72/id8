...
export type LocalContextMarkerKind = 'food' | 'music' | 'quiet'

interface LocalContextMarker {
  kind: LocalContextMarkerKind
}

interface StopLocalContext {
  mapLabels: string[]
  aroundHereSignals: string[]
  nearbyMarkers: LocalContextMarker[]
}

const LOCAL_CONTEXT_BY_ROLE: Record<UserStopRole, StopLocalContext> = {
  start: {
...
export function getLocalContextForRole(role: UserStopRole): StopLocalContext {
