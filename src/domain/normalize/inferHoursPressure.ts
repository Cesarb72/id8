import type { HoursPressureAnalysis, HoursPeriod, PlanningTimeWindowSignal } from '../types/hours'
import type { RawPlace } from '../types/rawPlace'
import type { VenueCategory } from '../types/venue'

interface InferHoursPressureInput {
  raw: RawPlace
...
function mapBusinessStatus(value: string | undefined): HoursPressureAnalysis['businessStatus'] {
...
function isOpenDuringWindow(
  periods: HoursPeriod[] | undefined,
  signal: PlanningTimeWindowSignal,
): boolean | undefined {
...
function inferLikelyOpenFromCategory(
  raw: RawPlace,
  category: VenueCategory,
  signal: PlanningTimeWindowSignal,
): { likelyOpen: boolean; confidence: number; notes: string[] } {
  const notes: string[] = []
  const placeSignals = [
    ...(raw.placeTypes ?? []),
    ...(raw.sourceTypes ?? []),
...
export function inferHoursPressure({
  raw,
  category,
  timeWindowSignal,
}: InferHoursPressureInput): HoursPressureAnalysis {
  ...
  const heuristic = inferLikelyOpenFromCategory(raw, category, timeWindowSignal)
  const softenedConfidence = clamp01(
    heuristic.confidence +
