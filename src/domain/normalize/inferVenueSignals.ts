...
import type {
  HighlightCapabilityTier,
  VenueCapabilitySignals,
  VenueSignatureSignals,
  VenueDurationProfile,
  VenueSetting,
} from '../types/normalization'
import type { CrewProfile, VenueVibeTag } from '../types/intent'
import type { LocalSignals, RoleAffinity, VenueCategory } from '../types/venue'

interface InferVenueSignalsInput {
  raw: RawVenueInput
  category: VenueCategory
}

interface InferredVenueSignals {
...
function hasAnyTag(raw: RawVenueInput, candidates: string[]): boolean {
  const tags = new Set((raw.tags ?? []).map((tag) => tag.toLowerCase()))
  return candidates.some((candidate) => tags.has(candidate.toLowerCase()))
}

function inferSetting(raw: RawVenueInput, category: VenueCategory): VenueSetting {
  if (raw.settingHint) {
  ...
  if (
    hasAnyTag(raw, ['garden', 'trail', 'viewpoint', 'stargazing', 'walkable', 'outdoor-seating', 'rooftop'])
  ) {
...
function inferEnergyLevel(raw: RawVenueInput, category: VenueCategory, inferredFields: string[]): number {
  if (typeof raw.energyLevel === 'number') {
    return raw.energyLevel
  }
  inferredFields.push('energyLevel')
  const baseByCategory: Record<VenueCategory, number> = {
    restaurant: 3,
    bar: 4,
    cafe: 2,
    dessert: 2,
...
function inferSocialDensity(
  ...
  let socialDensity =
    category === 'bar' || category === 'event' || category === 'live_music'
      ? 4
...
function inferVibeTags(raw: RawVenueInput, category: VenueCategory, inferredFields: string[]): VenueVibeTag[] {
  if (raw.vibeTags && raw.vibeTags.length > 0) {
    return raw.vibeTags
  }
  inferredFields.push('vibeTags')
  const base: Record<VenueCategory, VenueVibeTag[]> = {
  ...
  const vibes = new Set<VenueVibeTag>(base[category])
...
function inferAudienceFlags(raw: RawVenueInput, category: VenueCategory): {
...
function inferUseCases(
  raw: RawVenueInput,
  audienceFlags: ReturnType<typeof inferAudienceFlags>,
  inferredFields: string[],
): CrewProfile[] {
  ...
  const useCases = new Set<CrewProfile>()
...
function inferHighlightTier(raw: RawVenueInput, category: VenueCategory): {
  tier: HighlightCapabilityTier
  confidence: number
} {
  ...
  if (
    ...
    return {
      tier:
        hasAnyTag(raw, ['scenic', 'trail', 'viewpoint', 'signature', 'historic', 'immersive', 'intimate'])
          ? 'highlight-capable'
          : 'support-only',
...
function inferRouteFootprint(raw: RawVenueInput): VenueCapabilitySignals['routeFootprint'] {
...
function inferSignatureSignals(raw: RawVenueInput, category: VenueCategory): VenueSignatureSignals {
  ...
  let signatureScore =
    typeof raw.distinctivenessScore === 'number'
...
function inferScore(rawValue: number | undefined, fallback: number, inferredFields: string[], field: string): number {
...
function inferLocalSignals(raw: RawVenueInput, signatureScore: number, inferredFields: string[]): LocalSignals {
  if (raw.localSignals) {
    return {
...
function inferRoleAffinity(
  raw: RawVenueInput,
  category: VenueCategory,
  highlightTier: HighlightCapabilityTier,
  inferredFields: string[],
): RoleAffinity {
  if (raw.roleAffinity) {
    return {
  ...
  const baseByCategory: Record<VenueCategory, RoleAffinity> = {
...
export function inferVenueSignals({
  raw,
  category,
}: InferVenueSignalsInput): InferredVenueSignals {
  ...
  const durationProfile: VenueDurationProfile = {
