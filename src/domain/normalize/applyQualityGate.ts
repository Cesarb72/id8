...
function meaningfulTagCount(venue: Venue): number {
  return venue.tags.filter((tag) => tag.trim().length >= 4).length
}

function hasAnySignal(venue: Venue, candidates: string[]): boolean {
...
export function applyQualityGate(venue: Venue): QualityGateDecision {
  ...
  let status: QualityGateDecision['status'] = 'approved'
  ...
  const supportApprovalCandidate =
  ...
  const effectiveSourceConfidence = clamp01(
  ...
  const qualityScore = clamp01(
    effectiveSourceConfidence * 0.36 +
