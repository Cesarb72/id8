...
interface ScoreContextSpecificityInput {
  venue: Venue
...
export interface ContextSpecificityScore {
  overall: number
...
function tagOverlapScore(venueTags: string[], preferredTags: string[]): number {
...
export function scoreContextSpecificity({
  venue,
  intent,
  crewPolicy,
  lens,
  fitBreakdown,
  stopShapeFit,
}: ScoreContextSpecificityInput): ContextSpecificityScore {
  const personaSignal = clamp01(
    fitBreakdown.crewFit * 0.66 +
      (venue.useCases.includes(crewPolicy.crew) ? 0.24 : 0) +
      (crewPolicy.preferredCategories.includes(venue.category) ? 0.1 : 0),
  )
  const vibeSignal = clamp01(
    fitBreakdown.anchorFit * 0.72 +
  ...
  const lensSignal = clamp01(
    (lens.preferredCategories.includes(venue.category) ? 0.52 : 0.18) +
