...
interface GetNearbyAlternativesInput {
  role: InternalRole
...
export function getNearbyAlternatives({
  ...
  lens,
  limit = 5,
}: GetNearbyAlternativesInput): StopAlternative[] {
  ...
  const lensRole: LensStopRole =
    role === 'warmup' ? 'start' : role === 'peak' ? 'highlight' : role === 'wildcard' ? 'surprise' : 'windDown'

  const scoreCandidate = (item: ScoredVenue): StopAlternative => {
    const sameNeighborhood = item.venue.neighborhood === originNeighborhood ? 1 : 0
    const neighborhoodIntentBonus =
      neighborhoodQuery && item.venue.neighborhood.toLowerCase() === neighborhoodQuery ? 1 : 0
  ...
  const strictCandidates = scoredVenues
    .filter(
      (item) =>
        item.venue.id !== roleStop.scoredVenue.venue.id &&
