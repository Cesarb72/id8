...
function buildNameSignature(value: string): string {
...
function isUnknownNeighborhood(value: string): boolean {
...
function getDuplicateReason(left: Venue, right: Venue): string | undefined {
  ...
  const sameNeighborhood =
    normalizeValue(left.neighborhood) === normalizeValue(right.neighborhood)
  const sameCity = normalizeValue(left.city) === normalizeValue(right.city)
  const ambiguousNeighborhood =
    isUnknownNeighborhood(left.neighborhood) || isUnknownNeighborhood(right.neighborhood)
...
function pickPreferredVenue(
  left: Venue,
  right: Venue,
): { preferred: Venue; reason: string } {
