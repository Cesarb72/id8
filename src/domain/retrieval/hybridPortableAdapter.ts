...
export type HybridPortableMode =
  | 'none'
  | 'hybrid_live'
  | 'hybrid_live_plus_bootstrap'
  | 'hybrid_bootstrap'

export interface HybridPortableDiagnostics {
  mode: HybridPortableMode
  city: string
  ...
  liveNormalizationDropped: number
  liveNormalizationDropReasons: Record<string, number>
  liveAcceptedPreGeo: number
  liveAccepted: number
...
interface PortableSeed {
  idSuffix: string
  name: string
  neighborhoodSuffix: string
...
const PORTABLE_SEEDS: PortableSeed[] = [
  {
...
function gateStatusRank(status: Venue['source']['qualityGateStatus']): number {
  if (status === 'approved') {
    return 3
  }
  if (status === 'demoted') {
...
function sortByQuality(venues: Venue[]): Venue[] {
...
function dedupeVenues(venues: Venue[]): Venue[] {
...
function getGeoBucketKey(venue: Venue): string {
...
function summarizeGeoSpread(venues: Venue[]): {
  geoBucketCount: number
  dominantAreaShare: number
  geoSpreadScore: number
  dominantBucketKey: string | undefined
  bucketMap: Map<string, Venue[]>
} {
  const bucketMap = new Map<string, Venue[]>()
  for (const venue of venues) {
...
function applyGeoDiversityShaping(venues: Venue[]): {
  venues: Venue[]
  geoBucketCount: number
  dominantAreaShare: number
  geoSpreadScore: number
...
function selectGeoBalancedVenues(venues: Venue[], maxCount: number): Venue[] {
...
function projectNeighborhoodByGeoArea(venues: Venue[], city: string): Venue[] {
...
function buildIntent(city: string, primaryAnchor: VibeAnchor, crew: CrewProfile): IntentProfile {
  return {
    crew,
    primaryAnchor,
    city,
...
export async function fetchHybridPortableVenues(
  ...
  const liveIntents: IntentProfile[] = [
  ...
  const liveMapDropReasons = liveResults.reduce(
    (merged, entry) => mergeReasonCounts(merged, entry.diagnostics.mappedDropReasons),
  ...
  const liveNormalizationDropReasons = liveResults.reduce(
    (merged, entry) => mergeReasonCounts(merged, entry.diagnostics.normalizationDropReasons),
    {} as Record<string, number>,
  )
  const liveSuppressionReasons = liveResults.reduce(
    (merged, entry) => mergeReasonCounts(merged, entry.diagnostics.acceptanceDropReasons),
    {} as Record<string, number>,
  )
  ...
  const mode: HybridPortableMode = liveVenues.length > 0
    ? 'hybrid_live_plus_bootstrap'
    : 'hybrid_bootstrap'
