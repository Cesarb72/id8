...
interface ComputeRefinementDeltaInput {
  requestedModes: RefinementMode[]
  nextArc: ArcCandidate
  previousArc?: ArcCandidate
  nextStopExplainability: Partial<Record<UserStopRole, StopExplainabilityDiagnostics>>
...
function formatRefinement(mode: RefinementMode): string {
...
function roleLabel(role: UserStopRole): string {
  if (role === 'start') {
    return 'Start'
  }
  if (role === 'highlight') {
...
export function computeRefinementDelta({
  requestedModes,
  nextArc,
  previousArc,
  ...
  winnerInertiaNotes = [],
}: ComputeRefinementDeltaInput): RefinementOutcome | undefined {
  if (!previousArc) {
    return undefined
  }

  const allRoles = new Set<UserStopRole>([
    ...previousArc.stops.map((stop) => roleProjection[stop.role]),
    ...nextArc.stops.map((stop) => roleProjection[stop.role]),
  ])

  const stopDeltas: StopDelta[] = [...allRoles].map((role) => {
    ...
    const confidenceDelta =
      typeof previousConfidence === 'number' && typeof nextConfidence === 'number'
    ...
    const materialScoreShift =
      typeof scoreDelta === 'number' && Math.abs(scoreDelta) >= SCORE_MATERIAL_THRESHOLD
    const materialConfidenceShift =
