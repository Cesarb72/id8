...
export interface TargetedRefinementCandidate {
...
interface FindTargetedRefinementCandidatesInput {
  role: UserStopRole
...
function priceTierNumber(value: '$' | '$$' | '$$$' | '$$$$'): number {
  if (value === '$') {
    return 1
  }
  if (value === '$$') {
...
function objectiveScoreForMode(
  mode: RefinementDirective['mode'],
  role: UserStopRole,
...
export function findTargetedRefinementCandidates({
  role,
  currentArc,
  scoredVenues,
  intent,
  directive,
  limit = 14,
}: FindTargetedRefinementCandidatesInput): TargetedRefinementCandidate[] {
  const internalRole = inverseRoleProjection[role]
  ...
  const currentObjective =
    objectiveScoreForMode(directive.mode, role, currentStop.scoredVenue, intent) -
    currentStop.scoredVenue.dominanceControl.byRole[internalRole]

  const baseCandidates = scoredVenues
    .filter(
      (candidate) =>
        candidate.venue.id !== currentStop.scoredVenue.venue.id &&
  ...
  const contractSatisfiedCandidates = baseCandidates.filter(
    (candidate) => candidate.roleContract[internalRole].satisfied,
  )
  const candidatePool =
    contractSatisfiedCandidates.length > 0
