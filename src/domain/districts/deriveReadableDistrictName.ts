...
interface ReadableDistrictName {
  displayName: string
  optionalAnchor?: string
}

interface DistrictNamingOverride {
  displayName: string
  optionalAnchor?: string
}

const DISTRICT_NAMING_OVERRIDES: Record<string, Record<string, DistrictNamingOverride>> = {
  'san jose': {
    downtown: {
...
function withAreaSuffix(value: string): string {
...
function buildGenericDisplayName(rawDistrict: string, city: string): string {
...
export function deriveReadableDistrictName(
  district: string,
  cityContext: CityContext,
): ReadableDistrictName {
  ...
  const cityOverrides =
    DISTRICT_NAMING_OVERRIDES[normalizedCity] ??
