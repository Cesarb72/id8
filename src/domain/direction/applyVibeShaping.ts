...
type SupportedPrimaryVibe = 'cozy' | 'lively' | 'playful' | 'cultured'

export interface DirectionShapingDebugMeta {
  persona?: PersonaMode
  personaBoost?: number
  vibe?: VibeAnchor
  vibeBoost?: number
  family?: DirectionCandidate['experienceFamily']
...
export type ShapableDirectionCandidate = DirectionCandidate & {
  finalScore?: number
  shapingDebug?: DirectionShapingDebugMeta
}

export interface VibeShapedDirectionCandidate extends DirectionCandidate {
  finalScore: number
  shapingDebug: DirectionShapingDebugMeta
}
...
function isSupportedPrimaryVibe(vibe: VibeAnchor): vibe is SupportedPrimaryVibe {
...
function computeCategoryDiversity(candidate: DirectionCandidate): number {
  const mix = candidate.derivedFrom.hospitalityMix
  const activeCategories = [mix.drinks, mix.dining, mix.culture, mix.cafe, mix.activity].filter(
    (value) => value >= 0.14,
  ).length
  return clamp(activeCategories / 5, 0, 1)
}

function computeNonDominantCategoryInclusion(candidate: DirectionCandidate): number {
...
function computeFamilyBias(candidate: DirectionCandidate): number {
  ...
  const environmentalInfluence =
    candidate.derivedFrom.hyperlocal?.primaryMicroPocket.environmentalInfluencePotential ?? 0.5
  ...
  const calmScenicSignal =
    tags.has('scenic') ||
    tags.has('calm') ||
    tags.has('quiet') ||
    tags.has('atmospheric')
  const eventSpikeSignal =
    tags.has('event') ||
    tags.has('live') ||
    tags.has('festival') ||
    tags.has('show') ||
    seeds.some((seed) =>
      ['event', 'live', 'show', 'festival', 'set', 'headline'].some((term) =>
        seed.includes(term),
      ),
    )
  const balancedMixSignal =
    Math.max(mix.dining, mix.cafe, mix.culture, mix.drinks, mix.activity) -
      Math.min(mix.dining, mix.cafe, mix.culture, mix.drinks, mix.activity) <
    0.28
  const noveltySignal =
    hasSeedTerm(seeds, ['detour', 'wander', 'switch-up', 'discovery', 'gallery']) ||
    tags.has('mixed-program') ||
    tags.has('offbeat')
  const chaoticOverlap =
    activationStrength >= 0.66 &&
  ...
  if (family === 'social') {
    ...
    const highDensityHighEnergyPenalty =
      (activationStrength >= 0.68 ? 0.008 : 0) +
...
function computeSequenceQuality(candidate: DirectionCandidate): number {
  ...
  const balancedMixSignal =
    mix.activity <= 0.5 && diversityCount >= 3 ? 0.25 : mix.activity <= 0.55 && diversityCount >= 2 ? 0.15 : 0
  return clamp(seedCountSignal + flowSignal + balancedMixSignal, 0, 1)
}

function computeDiversityScore(candidate: DirectionCandidate): number {
  const mix = candidate.derivedFrom.hospitalityMix
...
function computeMovementFriendlyScore(candidate: DirectionCandidate): number {
  ...
  const energySignal =
    candidate.derivedFrom.ambianceProfile.energy === 'high'
      ? 0.2
...
function computeNoveltyScore(candidate: DirectionCandidate): number {
  const tags = new Set(candidate.derivedFrom.experientialTags.map((tag) => tag.toLowerCase()))
  const seeds = candidate.derivedFrom.momentSeeds.map((seed) => seed.toLowerCase())
  const tagSignal = ['mixed-program', 'arts-adjacent', 'offbeat', 'curated'].some((tag) => tags.has(tag))
    ? 0.55
    : 0
  const seedSignal = seeds.some((seed) =>
    ['detour', 'stroll', 'wander', 'discovery', 'gallery'].some((term) => seed.includes(term)),
  )
    ? 0.45
    : 0
...
function computeVibeBoost(candidate: DirectionCandidate, vibe: SupportedPrimaryVibe): number {
  ...
  if (vibe === 'cozy') {
    ...
    const movementPenalty =
      movementFriendly > 0.75 && mix.activity > 0.36 ? 0.05 : 0
    const socialNoisePenalty =
      ambiance.noise === 'high'
        ? 0.07
    ...
    const broadMixedProgramPenalty =
      mixedProgram && movementFriendly > 0.85 && mix.activity > 0.34 ? 0.03 : 0
    return clamp(
      intimacyBoost +
        noiseBoost +
        energyBoost +
  ...
  if (vibe === 'lively') {
    ...
    if (romanticLivelyContext) {
      ...
      const socialWarmthBoost =
        (ambiance.intimacy === 'high' ? 0.018 : ambiance.intimacy === 'medium' ? 0.01 : 0) +
        (mix.dining + mix.culture) * 0.012
      const sceneCredibilityBoost =
        (tags.has('arts-adjacent') ? 0.015 : 0) +
      ...
      const activityLedMismatchPenalty =
        activityLed && (sequenceQuality < 0.55 || ambiance.intimacy === 'low') ? 0.04 : 0
    ...
    const socialDensity = clamp(
      (mix.drinks + mix.activity) * 0.55 +
        (ambiance.energy === 'high' ? 0.18 : ambiance.energy === 'medium' ? 0.1 : 0) +
        candidate.derivedFrom.momentPotential * 0.25,
      0,
      1,
    )
  ...
  if (vibe === 'playful') {
    ...
    const detourSeedBoost =
      candidate.derivedFrom.momentSeeds.some((seed) =>
        ['detour', 'stroll', 'wander', 'discovery', 'switch-up'].some((term) =>
          seed.toLowerCase().includes(term),
        ),
      )
        ? 0.07
        : 0
    ...
    const formalPenalty =
      mix.activity < 0.2 && mix.dining + mix.culture > 0.62 && ambiance.energy === 'low' ? 0.07 : 0
  ...
  if (romanticCulturedContext) {
    ...
    const aestheticCoherence = clamp(
      (tags.has('arts-adjacent') ? 0.32 : 0) +
    ...
    const conversationFriendlyPacing = clamp(
      (ambiance.noise === 'low' ? 1 : ambiance.noise === 'medium' ? 0.65 : 0.25) *
        (ambiance.intimacy === 'high' ? 1 : ambiance.intimacy === 'medium' ? 0.7 : 0.35),
      0,
      1,
    )
    ...
    const mixedProgramChaosPenalty =
      mixedProgram && (mix.activity > 0.36 || ambiance.noise === 'high') ? 0.05 : mixedProgram ? 0.025 : 0
    const noiseMismatchPenalty =
      ambiance.noise === 'high' && (mix.culture < 0.24 || ambiance.intimacy !== 'high')
        ? 0.06
  ...
  const intentionalPacingBoost =
    tags.has('intentional') || tags.has('curated') || candidate.derivedFrom.momentSeeds.some((seed) => hasFlowSignal(seed))
      ? 0.08
      : 0.01
...
function shapeSubtitleForVibe(subtitle: string, vibe: SupportedPrimaryVibe): string {
  if (vibe === 'cozy') {
...
export function applyVibeShaping(
  candidates: ShapableDirectionCandidate[],
  vibe: VibeAnchor,
): VibeShapedDirectionCandidate[] {
  ...
  const prelimShaped = candidates.map((candidate) => {
    ...
    return {
      ...candidate,
      subtitle:
        supportedVibe && !shouldPreserveExperienceSubtitle(candidate)
  ...
  const topShapedScore = prelimShaped.reduce(
    (maxValue, candidate) =>
      candidate.finalScore > maxValue ? candidate.finalScore : maxValue,
  ...
  const shaped = prelimShaped.map((candidate) => {
    ...
    const shapedScoreAfterCompression =
      spread > POST_SHAPING_MAX_SPREAD
