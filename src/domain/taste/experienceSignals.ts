...
import type { TasteModeId } from './selectTasteMode'
import type { ScoredVenue } from '../types/arc'
import type { VenueCategory } from '../types/venue'

const DISTINCTIVE_ARCHETYPES = new Set<TasteExperienceArchetype>([
  'outdoor',
  'activity',
  'culture',
  'scenic',
...
const HOSPITALITY_ARCHETYPES = new Set<TasteExperienceArchetype>([
  'dining',
  'drinks',
  'sweet',
])
...
export function isHighMomentPotential(
  value: TasteMomentPotential | number,
): boolean {
  const score = typeof value === 'number' ? value : value.score
  return score >= 0.48
}

export function getMomentStrengthRank(strength: TasteMomentStrength): number {
  if (strength === 'strong') {
    return 3
  }
  if (strength === 'medium') {
...
export function isStrongMomentIdentity(
  value: TasteMomentIdentity | undefined,
): boolean {
  return value?.strength === 'strong'
}

export function getMomentIntensityRank(tier: TasteMomentIntensityTier): number {
  if (tier === 'signature') {
    return 4
  }
  if (tier === 'exceptional') {
...
export function getMomentIntensityTierBoost(
  value: TasteMomentIntensity | TasteMomentIntensityTier,
): number {
...
export function getArchetypeRepeatTolerance(
  archetype: TasteExperienceArchetype,
  tasteModeId?: TasteModeId,
): number {
  if (!tasteModeId) {
    return 1
  }
...
export function getHighlightArchetypeLift(
  signals: Pick<
    TasteSignals,
    'experienceArchetypes' | 'primaryExperienceArchetype' | 'momentPotential'
  >,
  tasteModeId?: TasteModeId,
): number {
  const primary = signals.primaryExperienceArchetype
  const includes = (value: TasteExperienceArchetype) =>
    signals.experienceArchetypes.includes(value)
  let lift =
    primary === 'scenic'
      ? 0.13
      : primary === 'outdoor'
...
export function getGenericHospitalityFallbackPenalty(params: {
  ...
  signatureGenericScore: number
  uniquenessScore: number
  distinctivenessScore: number
  protectedCandidate: boolean
  signals: Pick<
    TasteSignals,
    | 'primaryExperienceArchetype'
    | 'momentPotential'
    | 'momentIntensity'
    | 'experientialFactor'
    | 'categorySpecificity'
  ...
  const archetypePenalty = HOSPITALITY_ARCHETYPES.has(
    ...
    : 0.7
  const lowMomentRead =
    (1 - params.signals.momentIntensity.score) * 0.32 +
    (1 - params.signals.momentPotential.score) * 0.24
  const lowRichness =
    (1 - params.signals.experientialFactor) * 0.24 +
    (1 - params.uniquenessScore) * 0.12 +
    (1 - params.distinctivenessScore) * 0.1
  const genericRead =
    (1 - params.signals.categorySpecificity) * 0.18 +
    (1 - params.signals.personalityStrength) * 0.14 +
    params.signatureGenericScore * 0.12
  const modePressure =
    params.tasteModeId === 'scenic-outdoor' || params.tasteModeId === 'activity-led'
      ? 0.03
  ...
  return clamp01(
    (lowMomentRead + lowRichness + genericRead) * categoryWeight * archetypePenalty * 0.18 +
      modePressure,
  )
}

export function isGenericHospitalityFallbackCandidate(candidate: ScoredVenue): boolean {
  ...
  const weakMomentRead =
    candidate.taste.signals.momentIntensity.score < 0.78 &&
...
function computeHighlightAlternativeStrength(candidate: ScoredVenue): number {
  return (
    candidate.roleScores.peak * 0.34 +
...
export function assessGenericHospitalityFallbackPenalty(
  ...
  const strongestAlternative = [...candidates]
    ...
    .sort((left, right) => {
      return (
        computeHighlightAlternativeStrength(right) - computeHighlightAlternativeStrength(left) ||
  ...
  const strengthGap =
    computeHighlightAlternativeStrength(strongestAlternative) -
...
export function getExperienceArchetypeSummary(
  archetypes: TasteExperienceArchetype[],
): string {
  return archetypes.join(' + ')
}

export function hasDistinctiveExperienceArchetype(
  archetypes: TasteExperienceArchetype[],
): boolean {
