...
export interface RouteShapeBiasResult {
  score: number
  appliedLabel: string
  notes: string[]
}
...
export function computeRouteShapeBias(
  stops: ArcStop[],
  intent: IntentProfile,
  lens: ExperienceLens,
): RouteShapeBiasResult {
  if (stops.length === 0) {
    return {
      score: 0,
  ...
  const clusterAroundHighlight =
    stops.reduce(
      (sum, stop) => sum + Math.abs(stop.scoredVenue.venue.driveMinutes - highlight.scoredVenue.venue.driveMinutes),
      0,
    ) / stops.length
  ...
  if (intent.primaryAnchor === 'adventurous-outdoor') {
    const scenicAnchor =
      highlight.scoredVenue.vibeAuthority.adventureRead === 'outdoor' ||
