...
import type {
  SpatialClusterAssignment,
  SpatialCoherenceAnalysis,
  SpatialTransitionAnalysis,
} from '../types/spatial'
...
function inferClusterId(stop: ArcStop): string {
  return getVenueClusterId(stop.scoredVenue.venue)
}

function uniqueConsecutive(values: string[]): string[] {
  return values.filter((value, index) => index === 0 || value !== values[index - 1])
}

function getWalkableTransitionDelta(
  sameCluster: boolean,
  driveGap: number,
): { bonus: number; penalty: number; longTransition: boolean; notes: string[] } {
  const notes: string[] = []
  let bonus = 0
  let penalty = 0
...
function getFlexibleTransitionDelta(
  sameCluster: boolean,
  driveGap: number,
): { bonus: number; penalty: number; longTransition: boolean; notes: string[] } {
  const notes: string[] = []
...
export function computeSpatialCoherence(
  stops: ArcStop[],
  intent: IntentProfile,
): SpatialCoherenceAnalysis {
  const mode = getSpatialMode(intent.distanceMode)
  const clusterAssignments: SpatialClusterAssignment[] = stops.map((stop) => ({
  ...
  const highlightClusterId =
    clusterAssignments[stops.findIndex((stop) => stop.role === 'peak')]?.clusterId ??
    homeClusterId
  ...
  const transitions: SpatialTransitionAnalysis[] = stops.slice(0, -1).map((stop, index) => {
    const fromAssignment = clusterAssignments[index]!
    const toAssignment = clusterAssignments[index + 1]!
    const driveGap = Math.abs(
      stop.scoredVenue.venue.driveMinutes - stops[index + 1]!.scoredVenue.venue.driveMinutes,
    )
  ...
  if (mode === 'walkable') {
    if (clusterSegments.length <= 1) {
      ...
      jumpUsed = true
      const jumpTransition = transitions.find(
        (transition) => transition.toClusterId === distinctNonHomeClusters[0],
      )
      if (jumpTransition) {
