...
interface BuildExperienceLensInput {
...
function toEnergyBand(value: number): LensEnergy {
  if (value <= 2) {
    return 'low'
  }
  if (value <= 3) {
...
function biasRank(value: 'low' | 'medium' | 'high'): number {
  if (value === 'low') {
    return 0
  }
  if (value === 'medium') {
...
function chooseStrongerBias<T extends 'low' | 'medium' | 'high'>(current: T, next: T): T {
  return biasRank(next) > biasRank(current) ? next : current
}

function mergeStopShape(
  base: StopShapeProfile,
  patch: Partial<StopShapeProfile>,
): StopShapeProfile {
  return {
...
function buildNeutralBaseline(): ExperienceLens {
  return {
    tone: 'refined',
...
function applyVibeShaping(lens: ExperienceLens, intent: IntentProfile): ExperienceLens {
  const anchors = [intent.primaryAnchor, ...(intent.secondaryAnchors ?? [])]
  const next: ExperienceLens = {
    ...lens,
...
function applyResolvedContractShaping(
  lens: ExperienceLens,
  ...
  next.energyBand = unique(
    next.energyBand
      .filter((band) => !resolvedContract.energyBandRemovals.includes(band))
      .concat(resolvedContract.energyBandAdditions),
  )

  const roleKeys = Object.keys(resolvedContract.rolePreferences) as Array<
    keyof typeof resolvedContract.rolePreferences
  >
...
function applyModeShaping(lens: ExperienceLens, intent: IntentProfile): ExperienceLens {
  const next = { ...lens }
...
function applyStarterPackShaping(lens: ExperienceLens, starterPack?: StarterPack): ExperienceLens {
  ...
  if (preset.preferredStopShapes) {
    ...
    ) as Array<keyof typeof preset.preferredStopShapes>
  ...
  if (preset.preferredCategories?.length) {
    const roleKeys = Object.keys(next.preferredStopShapes) as Array<
      keyof typeof next.preferredStopShapes
    >
    for (const role of roleKeys) {
...
function buildPersonaEffectSummary(intent: IntentProfile): string {
...
function applyStrictDebugShaping(
  lens: ExperienceLens,
  intent: IntentProfile,
  starterPack?: StarterPack,
): ExperienceLens {
...
export function buildExperienceLens({
  intent,
  starterPack,
  strictShape = false,
}: BuildExperienceLensInput): ExperienceLens {
  ...
  return {
    ...
    interpretation: {
      ...
      personaEffectSummary:
        resolvedContractPackage.resolvedContract.persona
