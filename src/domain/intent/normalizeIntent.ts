import type { CrewProfile, IntentInput, IntentProfile } from '../types/intent'

function mapPersonaToCrew(
  persona: IntentInput['persona'],
  primaryVibe: IntentInput['primaryVibe'],
): CrewProfile {
  if (persona === 'romantic') {
    return 'romantic'
  }
...
export function normalizeIntent(input: IntentInput): IntentProfile {
  ...
  const discoveryPreferences = discoveryPreferenceSource.length > 0
    ...
    : undefined
  const autoHiddenGemPreference =
    refinementModes.includes('more-unique') ||
