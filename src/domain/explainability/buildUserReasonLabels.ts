import type { SelectionStrengthLabel } from '../types/diagnostics'
import { getVibeLabel, type IntentProfile } from '../types/intent'
import type { UserStopRole } from '../types/itinerary'
import type { ScoredVenue } from '../types/arc'

interface BuildUserReasonLabelsInput {
  role: UserStopRole
  venue: ScoredVenue
...
function crewLabel(crew: IntentProfile['crew']): string {
  if (crew === 'romantic') {
    return 'Romantic'
  }
  if (crew === 'socialite') {
...
export function buildUserReasonLabels({
  role,
  venue,
  intent,
  starterPackTitle,
  ...
  refinementImpact,
  fallbackLabel,
}: BuildUserReasonLabelsInput): string[] {
  const labels: string[] = []
