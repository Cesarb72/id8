...
interface GetDiscoveryCandidatesOptions {
...
interface DirectionStrategy {
  ...
  title: string
  narrative: string
  selectionOrder: UserStopRole[]
  baseNeighborhood?: string
  focusNeighborhood?: string
  focusNeighborhoodWeight: number
  closenessBias: number
  highlightBias: number
  softLandingBias: number
  explorationBias: number
  contrastBias: number
  varietyBias: number
  uniquenessBias: number
}

type DiscoveryCoarseCategory =
  | 'coffee'
  | 'restaurant'
  | 'bar'
  | 'activity'
...
interface DiscoveryUsageState {
  venueIds: Set<string>
  categoryCounts: Map<DiscoveryCoarseCategory, number>
  laneCounts: Map<DiscoveryExperienceLane, number>
}

const ROLE_CONFIG: Array<{
  poolKey: keyof Pick<RolePools, 'warmup' | 'peak' | 'cooldown'>
  role: UserStopRole
  title: string
  subtitle: string
}> = [
...
function incrementMapCount<T extends string>(counts: Map<T, number>, key: T): void {
  counts.set(key, (counts.get(key) ?? 0) + 1)
}

function getCoarseDiscoveryCategory(candidate: ScoredVenue): DiscoveryCoarseCategory {
...
function getDiscoveryExperienceLane(candidate: ScoredVenue): DiscoveryExperienceLane {
...
function collectDirectionNeighborhoods(
  rolePools: RolePools,
  city: string,
): Array<{ neighborhood: string; label: string }> {
...
function buildDirectionStrategies(params: {
  ...
  alternateNeighborhoods: Array<{ neighborhood: string; label: string }>
  hasAnchor: boolean
}): DirectionStrategy[] {
  const vibeTone = getVibeTone(params.primaryVibe)
  const strategies: DirectionStrategy[] = [
    {
      id: 'close-knit',
      title: 'Keep it close',
...
function scoreDirectionCandidate(params: {
  ...
  usageState: DiscoveryUsageState
  directionCategoryCounts: Map<DiscoveryCoarseCategory, number>
  directionLaneCounts: Map<DiscoveryExperienceLane, number>
  protectedVenueIds: Set<string>
}): number {
  const {
    candidate,
    role,
  ...
  const samePocketBonus =
    strategy.focusNeighborhood && candidate.venue.neighborhood === strategy.focusNeighborhood
      ? strategy.focusNeighborhoodWeight * 14
      : 0
  const explorationPocketBonus =
    strategy.baseNeighborhood && candidate.venue.neighborhood !== strategy.baseNeighborhood
      ? strategy.explorationBias * 8
      : 0
  const fallbackToBasePocketPenalty =
    strategy.id === 'nearby-pocket' &&
  ...
  const highlightBonus =
    role === 'highlight'
      ...
      : 0
  const startBonus =
    role === 'start' ? strategy.closenessBias * candidate.stopShapeFit.start * 12 : 0
  const windDownBonus =
    role === 'windDown'
  ...
  const sameVenuePenalty =
    usageState.venueIds.has(candidate.venue.id) && !protectedCandidate ? 36 : 0
  const freshCategoryBonus =
    !protectedCandidate && (usageState.categoryCounts.get(coarseCategory) ?? 0) === 0
      ? 5 + strategy.contrastBias * 5
      : 0
  const freshLaneBonus =
    !protectedCandidate && (usageState.laneCounts.get(lane) ?? 0) === 0
...
function buildDirectionGroups(params: {
  ...
  const directionUsedIds = new Set<string>()
  const directionCategoryCounts = new Map<DiscoveryCoarseCategory, number>()
  const directionLaneCounts = new Map<DiscoveryExperienceLane, number>()
  const selectedByRole = new Map<UserStopRole, ScoredVenue>()
  const roleConfigByRole = new Map(ROLE_CONFIG.map((config) => [config.role, config] as const))
  ...
  for (const selected of selectedByRole.values()) {
    const protectedCandidate =
      selected.isAnchor || params.protectedVenueIds.has(selected.venue.id)
  ...
  }).filter((group): group is DiscoveryGroup => Boolean(group))
}

function buildProtectedVenueIds(intent: ReturnType<typeof normalizeIntent>): Set<string> {
  const protectedVenueIds = new Set<string>()
...
export async function getDiscoveryCandidates(
  input: IntentInput,
  options: GetDiscoveryCandidatesOptions = {},
): Promise<DiscoveryDirection[]> {
  ...
  const anchoredHighlightNeighborhood =
    intent.anchor?.role === 'highlight'
      ? rolePools.peak.find((candidate) => candidate.isAnchor)?.venue.neighborhood
      : undefined
  const defaultNeighborhood =
    intent.neighborhood ?? anchoredHighlightNeighborhood ?? directionNeighborhoods[0]?.neighborhood
  const alternateNeighborhoods = directionNeighborhoods.filter(
    (entry) => entry.neighborhood !== defaultNeighborhood,
  )
  ...
  const usageState: DiscoveryUsageState = {
    venueIds: new Set<string>(),
    categoryCounts: new Map<DiscoveryCoarseCategory, number>(),
    laneCounts: new Map<DiscoveryExperienceLane, number>(),
  }
