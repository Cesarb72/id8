...
type PlanStoryProjection = Pick<
  Itinerary,
  'city' | 'neighborhood' | 'stops' | 'estimatedTotalLabel' | 'routeFeelLabel' | 'totalRouteFriction'
>

export interface PlanStoryContext {
  descriptor: string
  outingLabel: string
  areaPhrase: string
  routeShape: 'tight' | 'balanced' | 'wandering'
  estimatedTotalLabel: string
  routeFeelLabel: string
  hasSurprise: boolean
  startStop?: ItineraryStop
  highlightStop?: ItineraryStop
  windDownStop?: ItineraryStop
}

function getRouteShape(
  stops: ItineraryStop[],
  totalRouteFriction: number,
): PlanStoryContext['routeShape'] {
...
function getDescriptor(intent: IntentProfile): string {
  const primaryMap: Record<IntentProfile['primaryAnchor'], string> = {
    cozy: 'cozy',
    lively: 'lively',
    playful: 'playful',
    cultured: 'cultured',
...
function getOutingLabel(intent: IntentProfile, stops: ItineraryStop[]): string {
  const nightlifeCount = stops.filter((stop) =>
    ['bar', 'live_music', 'event'].includes(stop.category),
  ).length
  const afternoonCount = stops.filter((stop) =>
    ['cafe', 'park', 'museum'].includes(stop.category),
  ).length
...
function getAreaPhrase(itinerary: PlanStoryProjection, routeShape: PlanStoryContext['routeShape']): string {
...
export function buildPlanStoryContext(
  intent: IntentProfile,
  itinerary: PlanStoryProjection,
): PlanStoryContext {
...
export function buildPlanNarrative(intent: IntentProfile, itinerary: PlanStoryProjection): string {
