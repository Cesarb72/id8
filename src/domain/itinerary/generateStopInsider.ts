import type { RouteContinuity, RouteMovementMode } from '../types/pacing'
import type { StopInsider, UserStopRole } from '../types/itinerary'
import type { VenueCategory } from '../types/venue'

interface StopInsiderRouteContext {
  localTravelMinutes: number
  localContinuity: RouteContinuity
  localMovementMode: RouteMovementMode
  compactness: number
  nearbyCategoryMix: Record<VenueCategory, number>
  nearbyActivation: number
}

export interface StopInsiderContext {
  role: UserStopRole
  ...
  route: StopInsiderRouteContext
}

function hasCategory(
  mix: StopInsiderRouteContext['nearbyCategoryMix'],
  category: VenueCategory,
  threshold = 0.18,
): boolean {
...
function buildRoleReason(context: StopInsiderContext): string {
...
function buildLocalSignal(context: StopInsiderContext): string {
  const nightlifeMix =
    context.route.nearbyCategoryMix.bar +
    context.route.nearbyCategoryMix.live_music +
    context.route.nearbyCategoryMix.event

  if (
    context.socialDensity >= 0.66 &&
  ...
  if (
    context.route.compactness >= 0.64 &&
    context.route.localContinuity === 'same-neighborhood'
  ) {
  ...
  if (
    hasCategory(context.route.nearbyCategoryMix, 'restaurant') &&
...
function buildSelectionReason(context: StopInsiderContext): string {
  const scoreGap =
    typeof context.selectedScore === 'number' && typeof context.runnerUpScore === 'number'
...
export function generateStopInsider(context: StopInsiderContext): StopInsider {
  return {
