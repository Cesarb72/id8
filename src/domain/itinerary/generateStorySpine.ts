import type { CrewProfile, PersonaMode, VibeAnchor } from '../types/intent'
import type { ItineraryStop, StorySpine } from '../types/itinerary'
import type { RouteContinuity } from '../types/pacing'

interface StorySpineRouteInput {
  stops: ItineraryStop[]
  transitions: Array<{
...
interface StorySpineContext {
  vibe: VibeAnchor
  crew: CrewProfile
  persona?: PersonaMode | null
}

type RouteCompactness = 'compact' | 'mixed' | 'roaming'
type EnergyBand = 'calm' | 'balanced' | 'energetic'

const calmCategories = new Set(['dessert', 'cafe', 'park', 'museum'])
const energeticCategories = new Set(['bar', 'live_music', 'event', 'activity'])

function getCompactness(route: StorySpineRouteInput): RouteCompactness {
  if (route.transitions.length === 0) {
    return 'compact'
  }

  const maxTransition = Math.max(
    ...route.transitions.map((transition) => transition.estimatedTransitionMinutes),
  )
  const spreadTransitions = route.transitions.filter(
    (transition) => transition.neighborhoodContinuity === 'spread',
  ).length
...
function getEnergyBand(stop: ItineraryStop | undefined, vibe: VibeAnchor): EnergyBand {
  if (stop) {
...
function resolvePhaseStop(
  route: StorySpineRouteInput,
...
function getTitle(context: StorySpineContext, compactness: RouteCompactness): string {
...
function getStartLabel(context: StorySpineContext, startEnergy: EnergyBand): string {
  if (
    startEnergy === 'calm' ||
...
function getHighlightLabel(context: StorySpineContext, highlightEnergy: EnergyBand): string {
  if (
    highlightEnergy === 'energetic' ||
    context.crew === 'socialite' ||
    context.vibe === 'lively' ||
    context.vibe === 'playful'
  ) {
...
function getWinddownLabel(context: StorySpineContext, finishEnergy: EnergyBand): string {
  if (
    finishEnergy === 'calm' ||
    context.crew === 'romantic' ||
    context.vibe === 'cozy' ||
...
function buildStartSummary(
  compactness: RouteCompactness,
  startEnergy: EnergyBand,
): string {
...
function buildWinddownSummary(
  compactness: RouteCompactness,
  finishEnergy: EnergyBand,
): string {
...
export function generateStorySpine(
  ...
  const phases: StorySpine['phases'] = []
  if (startStop) {
    phases.push({
      role: 'start',
