...
import type {
  ExperienceLens,
  PersonaContractHighlightType,
} from '../types/experienceLens'

export interface RomanticHighlightQualificationAssessment {
  qualifies: boolean
  highlightType?: PersonaContractHighlightType
  expanded: boolean
  reason: string
}

export interface RomanticMomentTierScope {
  candidates: ScoredVenue[]
  appliedTier: 'anchor' | 'builder' | 'support' | 'none'
  fallbackUsed: boolean
}

interface CenterpieceConvictionRead {
  score: number
  qualifies: boolean
  qualifiers: string[]
  missing: string[]
...
function computeRomanticCenterpieceConviction(
  ...
  const chefOrTastingSignal =
    candidate.venue.category === 'restaurant' &&
    (candidate.venue.tags.some((tag) =>
      ['chef-led', 'tasting-menu', 'wine-pairing', 'reservation', 'omakase'].includes(
        tag.toLowerCase(),
      ),
    ) ||
...
export function isRomanticPersonaContractActive(lens: ExperienceLens): boolean {
  return lens.resolvedContract?.persona === 'romantic'
}

export function requiresRomanticPersonaMoment(lens: ExperienceLens): boolean {
  return Boolean(
...
export function getRomanticPersonaHighlightType(
  candidate: ScoredVenue,
  lens?: ExperienceLens,
): PersonaContractHighlightType | undefined {
  return assessRomanticPersonaHighlightQualification(candidate, lens).highlightType
}

export function scopeRomanticHighlightCandidatesByMomentTier(
  candidates: ScoredVenue[],
  lens?: ExperienceLens,
): RomanticMomentTierScope {
...
export function assessRomanticPersonaHighlightQualification(
  candidate: ScoredVenue,
  lens?: ExperienceLens,
): RomanticHighlightQualificationAssessment {
  ...
  const cozyRomanticHighlightMode =
    lens?.resolvedContract?.persona === 'romantic' &&
  ...
  const groupDominant =
    candidate.taste.signals.socialDensity >= 0.74 &&
  ...
  const intimateAtmosphere =
    ...
    (atmosphericStrength >= 0.5 &&
      ...
      candidate.taste.signals.experientialFactor >= 0.66)
  const ambientCulturalCompatible =
...
export function isExpandedRomanticHighlightCandidate(candidate: ScoredVenue): boolean {
  return assessRomanticPersonaHighlightQualification(candidate).expanded
}

export function satisfiesRomanticPersonaHighlightContract(
  candidate: ScoredVenue,
  lens: ExperienceLens,
): boolean {
  ...
  return Boolean(
    isRomanticPersonaContractActive(lens) &&
      ...
      lens.resolvedContract?.highlight.preferredHighlightTypes.includes(
        assessment.highlightType,
      ),
  )
}
