...
import type { HighlightValidityEvaluation } from '../types/highlightValidity'
import type { IntentProfile } from '../types/intent'
import type { StarterPack } from '../types/starterPack'
import type { Venue } from '../types/venue'

interface EvaluateHighlightValidityInput {
  venue: Venue
...
function hasCategory(venue: Venue, categories: Venue['category'][]): boolean {
  return categories.includes(venue.category)
}

function isMusicCapableVenue(venue: Venue, tags: Set<string>): boolean {
  return (
...
function isOutdoorAnchorVenue(venue: Venue, tags: Set<string>): boolean {
  return (
    venue.settings.setting === 'outdoor' ||
...
function isFamilyCodedVenue(venue: Venue, tags: Set<string>): boolean {
  return (
    venue.settings.familyFriendly ||
...
function isKidFocusedVenue(tags: Set<string>): boolean {
...
function isAdultSocialVenue(venue: Venue, tags: Set<string>): boolean {
  return (
...
function isDateToneVenue(venue: Venue, tags: Set<string>): boolean {
  return (
...
function isQuietLingeringVenue(venue: Venue, tags: Set<string>): boolean {
  return (
    venue.energyLevel <= 2 ||
...
function isThoughtfulVenue(venue: Venue, tags: Set<string>): boolean {
  return (
...
function isIndoorCulturalFamilyVenue(venue: Venue, tags: Set<string>): boolean {
  return (
    hasCategory(venue, ['museum', 'activity']) &&
...
function computeCandidateTier(
  ...
  tags: Set<string>,
): HighlightValidityEvaluation['candidateTier'] {
...
function literalRequirementSatisfied(
  requirementType: ReturnType<typeof getHighlightValidityRules>['literalRequirementType'],
  venue: Venue,
  tags: Set<string>,
): boolean {
  switch (requirementType) {
    ...
    case 'conversation-dessert':
      return (
        hasCategory(venue, ['dessert', 'cafe', 'bar']) &&
        isQuietLingeringVenue(venue, tags) &&
        venue.energyLevel <= 3
      )
    case 'wine-evening':
      return (
        hasCategory(venue, ['restaurant', 'bar']) &&
    ...
    case 'thoughtful-date':
      return (
        hasCategory(venue, ['cafe', 'museum', 'dessert']) &&
...
export function evaluateHighlightValidity({
  venue,
  intent,
  starterPack,
}: EvaluateHighlightValidityInput): HighlightValidityEvaluation {
  ...
  const personaVetoes: string[] = []
  const contextVetoes: string[] = []
