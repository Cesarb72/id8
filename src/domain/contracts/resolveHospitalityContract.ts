import type { RoleContractRule } from '../types/roleContract'
import type {
  LensEnergy,
  LensPersonaContract,
  LensStopRole,
  ResolvedHospitalityContract,
  StopShapeProfile,
...
interface ResolvedHospitalityContractOutput {
  resolvedContract: ResolvedHospitalityContract
  roleRulePatches: Partial<Record<LensStopRole, Partial<RoleContractRule>>>
...
function emptyShape(): StopShapePatch {
  return {
    preferredCategories: [],
...
function mergeShape(base: StopShapePatch, patch: StopShapePatch): StopShapePatch {
  return {
...
function roleRulePatch(
  label: string,
  strength: RoleContractRule['strength'],
  shape: StopShapePatch,
  maxEnergyLevel?: number,
): Partial<RoleContractRule> {
  return {
    label,
    strength,
...
function getBlendMode(
  ...
  ResolvedHospitalityContract,
  'blendMode' | 'compatibility' | 'resolutionSummary' | 'priority'
> {
  if (!persona) {
    return {
      blendMode: 'vibe_only',
  ...
  if (persona === 'romantic') {
    ...
    return {
      ...
      resolutionSummary:
        'Vibe supplies movement and selective energy, while romantic caps chaos and keeps the highlight date-shaped.',
      priority: {
        highlightStructure: 'persona',
        pacingEnergy: 'balanced',
        rolePreferences: 'balanced',
  ...
  if (persona === 'friends') {
    if (vibe === 'cozy' || vibe === 'chill' || vibe === 'cultured') {
      return {
        ...
        compatibility: 'tension',
        resolutionSummary:
          'Vibe keeps the route calmer, while friends preserves one social focal point without flattening into a generic middle.',
        priority: {
    ...
    return {
      ...
      compatibility: 'reinforcing',
      resolutionSummary:
        'Social persona and energetic vibe reinforce each other around movement, interaction, and a louder highlight.',
      priority: {
        highlightStructure: 'balanced',
        pacingEnergy: 'balanced',
        rolePreferences: 'balanced',
  ...
  if (vibe === 'lively' || vibe === 'playful' || vibe === 'adventurous-urban') {
    return {
      ...
      compatibility: 'tension',
      resolutionSummary:
        'Vibe supplies activity, while family caps adult-nightlife pressure and keeps the route accessible.',
      priority: {
        highlightStructure: 'persona',
        pacingEnergy: 'balanced',
  ...
  return {
    ...
    compatibility: 'reinforcing',
    resolutionSummary:
      'Vibe and family persona both support calmer pacing, accessibility, and lower-noise highlights.',
    priority: {
      highlightStructure: 'persona',
      pacingEnergy: 'balanced',
      rolePreferences: 'persona',
...
function buildRomanticContract(vibe: VibeAnchor): ResolvedHospitalityContractOutput {
  ...
  let energyBandRemovals: LensEnergy[] = []
  let movementToleranceCap: ResolvedHospitalityContract['movementToleranceCap'] = 'medium'
  let toneOverride: ResolvedHospitalityContract['toneOverride'] = 'intimate'
  let windDownExpectation: ResolvedHospitalityContract['windDownExpectation'] = {
    closeToBase: true,
  }
  let highlightEnergyPreference: LensEnergy[] = ['low', 'medium']

  if (vibe === 'lively' || vibe === 'playful' || vibe === 'adventurous-urban') {
    highlight = mergeShape(highlight, {
      preferredCategories:
        vibe === 'playful' ? ['activity', 'dessert'] : ['live_music', 'activity', 'restaurant'],
      preferredTags:
        vibe === 'lively'
  ...
  const resolvedContract: ResolvedHospitalityContract = {
    primaryVibe: vibe,
    persona: 'romantic',
...
function buildFriendsContract(vibe: VibeAnchor): ResolvedHospitalityContractOutput {
  const highlight = mergeShape(emptyShape(), {
    preferredCategories:
      vibe === 'cozy' || vibe === 'chill' || vibe === 'cultured'
    ...
    energyPreference:
      vibe === 'cozy' || vibe === 'chill' || vibe === 'cultured' ? ['medium'] : ['medium', 'high'],
  })
  ...
  const resolvedContract: ResolvedHospitalityContract = {
    ...
    movementToleranceOverride:
      vibe === 'cozy' || vibe === 'chill' || vibe === 'cultured' ? undefined : 'high',
...
function buildFamilyContract(vibe: VibeAnchor): ResolvedHospitalityContractOutput {
  const highlight = mergeShape(emptyShape(), {
    preferredCategories:
      vibe === 'lively' || vibe === 'playful' || vibe === 'adventurous-urban'
...
function buildVibeOnlyContract(vibe: VibeAnchor): ResolvedHospitalityContractOutput {
  return {
    resolvedContract: {
      primaryVibe: vibe,
...
export function resolveHospitalityContract(
  intent: IntentProfile,
): ResolvedHospitalityContractOutput {
