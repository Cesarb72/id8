...
function main(): void {
  ...
  assert(passing.reviewEligible, 'Build Review eligibility must pass when all gates pass.')
  ...
  assert(scattered.reviewEligible, 'Build scattered geography warning alone must not block Review truth.')
  assert(
  ...
  assert(
    ...
    'Mode-aware Build truth must preserve Review eligibility when gates pass.',
  )
